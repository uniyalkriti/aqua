...
class DMSQuickOrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $div_code_entry = DB::table('dealer')->where('id',$this->dealer_id)->first();
        ...
        $records_data = DB::table('demand_order')
                ...
                if($this->role_id == 37 || $this->role_id == 823)
                {
                    ...
                    {
                        $records_data->select('demand_order.*','demand_order.id as order_id','dealer.*',DB::raw("(SUM((total_rs-t1_rate-atd_rate)+atd_rate)) as total_value"))->join('dealer','dealer.id','=','demand_order.dealer_id')
                                // ->join('dealer','dealer.id','=','dealer.id')
        ...
        $records = $records_data->orderBy('demand_order.order_date','DESC')->get()->toArray();
        ...
        if($this->dealer_id != '1170')
        {
            if($this->role_id == '37')
            {
                ...
                $records_depo = $records_data->orderBy('demand_order.order_date','DESC')->get()->toArray();
    ...
    public function create()
    {
        ...
        $catalog_product_details = DB::table('catalog_product')->select(DB::raw("CONCAT(name,' (',itemcode,')') AS ITEM_NAME"))->whereNotIn('itemcode',$record_product_id)->where('status','=','1')->orderBy('itemcode','ASC')->get();
        $mktg_cat_array = DB::table('MKTG_CATG_MAST')->orderBy('MKTG_CATG','ASC')->get();
        $dealer_array = DB::table('dealer')->where('company_id',$this->company_id)->orderBy('id','ASC')->get();
    ...
    public function store(Request $request)
    {
        ...
        else
        {
            ...
            $div_code_entry = DB::table('dealer_person_login')->select('div_code_main as DIV_CODE')->where('dealer_id',$request->dealer_id_cus)->first();
            // dd($div_code_entry);
            if(empty($div_code_entry->DIV_CODE))
            {
                $div_code_entry = DB::table('dealer')->where('id',$request->dealer_id_cus)->first();
            }
        ...
        $check = DemandOrderCart::where('dealer_id',$request->dealer_id_cus)->first();
        ...
        else
        {
            $myorderArr = [
                'order_id' => $order_id, 
                'dealer_id' => $request->dealer_id_cus, 
        ...
        if(isset($item_code))
        {
            ...
            if(!empty($total_rs) && !empty($qty) && isset($total_rs) && isset($qty)  && count($total_rs)>0 &&  count($qty)>0 && count($total_rs)>0 && count($qty)>0  )
            {
                ...
                foreach ($item_code as $key => $value) {
                    # code...
                    if(!empty($total_rs[$key]) && !empty($qty[$key]) && isset($total_rs[$key]) && isset($qty[$key])  && isset($total_rs[$key])>0 &&  isset($qty[$key])>0 && isset($total_rs[$key])>0 && isset($qty[$key])>0  && $qty[$key] != ''  && $qty[$key] != '0' && $total_rs[$key] != ''  && $total_rs[$key] != '0' )
                    {
                        if(!empty($value))
                        {
                            $mrp_details = DB::table('product_rate_list')
                            ...
                            $ins_qty = !empty($qty[$key])?(int)$qty[$key]:'0';
                            if($ins_qty != 0 || $ins_qty != '0')
                            {
                                $myArrDetails[] = [
                                        'order_id'=>$order_id,
                                        'product_id' => $value,
        ...
        $mktg_cat_array = DB::table('catalog_2')
                            ->orderBy('id','ASC')
                            ->pluck('name as v', 'id as k');
        ...
        $dealer_details = DB::table('dealer')->where('id',$order_details->id)->first();
        $final_out = array();

        foreach ($mktg_cat_array as $key => $value) {
            $record = DB::table('demand_order_cart')
                    ->join('demand_order_details_cart','demand_order_details_cart.order_id','=','demand_order_cart.order_id')  
                    ->join('catalog_product','catalog_product.id','=','demand_order_details_cart.product_id')  
                    ->select('demand_order_cart.*','demand_order_details_cart.*','demand_order_details_cart.id as pid','catalog_product.*')
                    ->where('catalog_product.catalog_id', $key)
                    ->where('demand_order_cart.dealer_id',$request->dealer_id_cus) 
            ...
            foreach ($record as $k => $v) {
                ...
                $out_4s['ITEM_CODE'] = $v->id;
                $out_4s['ITEM_NAME'] = $v->name;
                $out_4s['rate'] = $v->rate;
    ...
    {
        $dealer_id_cus = $request->dealer_id_cus;
    	$mktg_cat_array = DB::table('catalog_2')
                        ->orderBy('id','ASC')
                        ->pluck('catalog_2.name as v', 'catalog_2.id as k');
        $converstion_unit_item_code = DB::table('ITEM_AUM_MAST')->pluck('UMTOAUM','ITEM_CODE');

        $order_details = DB::table('demand_order_cart')
                        ->join('dealer','dealer.id','=','demand_order_cart.dealer_id')
                        ->select('dealer.*','demand_order_cart.id as ppid')
                    	->where('demand_order_cart.dealer_id',$dealer_id_cus) 
                        ->first();
        // dd($order_details);
        // if($order_details)
        ...
        if(!empty($uid))
        {
        	$dealer_details = DB::table('dealer')->where('id',$order_details->dealer_code)->first();
        ...
        return view('DMS/QuickOrder.cartcommonmodal',[
            ...
            'order_date'=> $order_date,
            'mktg_cat_array'=>$mktg_cat_array,
            'converstion_unit_item_code'=>!empty($converstion_unit_item_code)?$converstion_unit_item_code:array(),
            'final_out'=>$final_out,
            'item_aum_mast_array' => !empty($item_aum_mast_array)?$item_aum_mast_array:array(),
        ]);
    }
    public function store_final(Request $request)
    {
        ...
        $dealer_id = $data_use_step1->dealer_id;
        // $dealer_details = 
        if($this->role_id == '37'){
            $depo_dealer_status = 'DO';
            // $sale_div_code = 'JH';
    ...
    public function edit($id)
    {
        ...
        foreach ($mktg_cat_array as $key => $value) {
            $record_data = DB::table('demand_order')
                    ->join('demand_order_details','demand_order_details.order_id','=','demand_order.order_id')  
                    ->join('catalog_product','catalog_product.id','=','demand_order_details.product_id')  
                    ->where('catalog_product.catalog_id', $key)
                    // ->where('demand_order.dealer_id',$this->dealer_id) 
        ...
        return view('DMS/QuickOrder.perfoma',[
            'current_menu'=>$this->current_menu,
            'auth_id'=>$this->dealer_id,
            'party_name'=> !empty($dealer_details->name)?$dealer_details->name:'',
...
// 
    ...
    public function dms_return_rate_on_the_behalf_of_product(Request $request)
    {
        ...
        $check_product_not_allow = DemandOrderCart::join('demand_order_details_cart','demand_order_details_cart.order_id','=','demand_order_cart.order_id')
                ->where('dealer_id',$dealer_id_cus)->groupBy('product_id')->pluck('product_id');

        $return_rate_details = DB::table('product_rate_list')
                            ->where('product_id',$item_code)
                            ->where('state_id',39)
                            ->where('company_id',$this->company_id)
                            ->first();
// dd($return_rate_details);
        ...
        $return_size_um = DB::table('catalog_product')
                            ->where('company_id',$this->company_id)
                            ->where('id',$item_code)
                            // ->whereRaw("TO_DATE is NULL")
                            ->first();

        $item_aum_mast_array_data = DB::table('catalog_product')
                        ->where('company_id',$this->company_id)
                        ->whereNotIn('id', $item_codes_str);
                        if(!empty($catg))
                        {
                            $item_aum_mast_array_data->where('catalog_id',$catg);
                        }
                        if(!empty($check_product_not_allow))
                        {
                            $item_aum_mast_array_data->whereNotIn('id',$check_product_not_allow);
                        }
                        
        $item_aum_mast_array = $item_aum_mast_array_data->groupBy('id')->orderBy('id','ASC')->pluck(DB::raw("CONCAT(itemcode,' - ',name) AS ITEM_NAME"), 'id as k');
        // dd($item_aum_mast_array);
        $item_mast_name = DB::table('catalog_product')
                        ->where('company_id',$this->company_id)
                        ->where('id', $item_code)
        ...
        if (!empty($return_rate_details)) {
            $data['code'] = 200;
            $data['result'] = $return_rate_details->dealer_rate;
            $data['item_mast_name'] = $item_mast_name->name;
    ...
    public function send_order_to_erp(Request $request)
    {
    	...
    	// dd($request->order_no);
        $order_no = $request->order_no;
        $checkbox_uncheck = $request->checkbox_uncheck;
        if(!empty($checkbox_uncheck))
        {
            $update_order = DB::table('demand_order')
                        ->whereIn('id',$checkbox_uncheck)
                        ->update([
                            'send_order_erp_status'=>2, 
    ...
    public function autocomplete_search_url_dms(Request $request)
    {
        // $qty = $request->get('term', '');
        // // dd($url_name);
        // $out = array();
        ...
        $scheme_details_1_step = DB::table('CIRCULAR_MAST')
                ...
                ->first();
        // dd($scheme_details_1_step);
        if(!empty($scheme_details_1_step) && isset($scheme_details_1_step))
        {
            $final_scheme_details_1_step_array = DB::table('SCHEME_SLAB')
                                ->where('SCHEME_SLAB.SCHEME_NO',$scheme_details_1_step->SCHEME_NO)
                                ->get();
    ...
    public function otc(Request $request){
        ...
        $curr_date = date('Y-m');
        $quater_date = DB::table('_quater_master')
                    ->where('details_month',$curr_date)
                    ->first();
        $quater_from_date = $quater_date->quater_from_date;
        $quater_to_date = $quater_date->quater_to_date;
        ...
        if($request->check_status == '1')
        {
            ...
            $dealer_id_cus = '';
            $div_code_cus = '';
            if(!empty($request->dealer_id_cus))
            {
                $dealer_id_cus = explode(',',$request->dealer_id_cus);
            }
            if(!empty($request->terr_id))
            {
                $terr_id = explode(',',$request->terr_id);
            }
            if(!empty($request->div_code_cus))
            {
                $div_code_cus = explode(',',$request->div_code_cus);
            }
            ...
            $deiv_code_login_data = DB::table('dealer_person_login')
                        ->whereIn('dealer_id',$dealer_id);
                        if(!empty($div_code_cus))
                        {
                            $deiv_code_login_data->whereIn('div_code_main',$div_code_cus);
                        }
            $deiv_code_login = $deiv_code_login_data->pluck('div_code_main')->toArray(); 
            ...
            $dealer_code_data = DB::table('dealer')
                        ...
                        if(!empty($div_code_cus))
                        {
                            $dealer_code_data->join('dealer_person_login','dealer_person_login.dealer_id','=','dealer.id')->whereIn('div_code_main',$div_code_cus);
                        }
        ...
        $month_prod_wise_sales_mast_data_qty = DB::table('ITEMTRAN_BODY')
         ...
         $quat_prod_wise_sales_mast_data_qty = DB::table('ITEMTRAN_BODY')
            ->join('ITEMTRAN_HEAD','ITEMTRAN_HEAD.VRNO','=','ITEMTRAN_BODY.VRNO')
            ->join('ITEM_MAST','ITEM_MAST.ITEM_CODE','=','ITEMTRAN_BODY.ITEM_CODE')
            ->join('ITEM_AUM_MAST','ITEM_AUM_MAST.ITEM_CODE','=','ITEM_MAST.ITEM_CODE')
            ->join('PROD_CATG_MAST','PROD_CATG_MAST.PROD_CATG','=','ITEM_MAST.PROD_CATG')
            ->whereRaw("date_format(ITEMTRAN_BODY.VRDATE_FILTER,'%Y-%m-%d')>='$quater_from_date' and date_format(ITEMTRAN_BODY.VRDATE_FILTER,'%Y-%m-%d')<='$quater_to_date'") 
            ->groupBy('PROD_CATG_MAST.PROD_CATG')
            ->orderBy('PROD_CATG_MAST.PROD_CATG', 'ASC')
            ->whereIn('ITEMTRAN_HEAD.DIV_CODE',$div_code)
            ->whereIn('ITEMTRAN_BODY.DIV_CODE',$div_code)
        ...
        // dd($prod_wise_sales_mast_data_val);
            ...
            'quat_prod_wise_sales_mast_data_qty'=>$quat_prod_wise_sales_mast_data_qty
        ]);
    }
    ...
    public function ot2(Request $request){
        ...
        $ot2_wise_target_data_1 = DB::table('DEALER_TARGET_MAST')
            ->join('MKTG_CATG_MAST','MKTG_CATG_MAST.MKTG_CATG','=','DEALER_TARGET_MAST.MKTG_CATG')  
            ->select(DB::raw("SUM(TARGET_AMT) as TARGET_AMT"))
        ...
        $quad_prod_wise_sales_mast_data_qty = DB::table('ITEMTRAN_BODY')
            ->join('ITEMTRAN_HEAD','ITEMTRAN_HEAD.VRNO','=','ITEMTRAN_BODY.VRNO')
            ->join('ITEM_MAST','ITEM_MAST.ITEM_CODE','=','ITEMTRAN_BODY.ITEM_CODE')
            ->join('ITEM_AUM_MAST','ITEM_AUM_MAST.ITEM_CODE','=','ITEM_MAST.ITEM_CODE')
            ->join('PROD_CATG_MAST','PROD_CATG_MAST.PROD_CATG','=','ITEM_MAST.PROD_CATG')
            ...
            ->orderBy('PROD_CATG_MAST.PROD_CATG', 'ASC')
            ->pluck(DB::raw('SUM(ITEMTRAN_BODY.QTYISSUED/ITEM_AUM_MAST.UMTOAUM) as QTYISSUED'), 'PROD_CATG_MAST.PROD_CATG as PROD_CATG');
        // dd($quad_prod_wise_sales_mast_data_qty);
        $quad_prod_wise_sales_mast_data_val = DB::table('ITEMTRAN_BODY')
            ->join('ITEMTRAN_HEAD','ITEMTRAN_HEAD.VRNO','=','ITEMTRAN_BODY.VRNO')
            ->join('ITEM_MAST','ITEM_MAST.ITEM_CODE','=','ITEMTRAN_BODY.ITEM_CODE')
            ->join('PROD_CATG_MAST','PROD_CATG_MAST.PROD_CATG','=','ITEM_MAST.PROD_CATG')
            ->whereRaw("date_format(ITEMTRAN_BODY.VRDATE_FILTER,'%Y-%m-%d')>='$quater_from_date' and date_format(ITEMTRAN_BODY.VRDATE_FILTER,'%Y-%m-%d')<='$quater_to_date'") 
        ...
        return view('DMS/QuickOrder.ot2',[
            ...
            'quad_prod_wise_sales_mast_data_qty'=>$quad_prod_wise_sales_mast_data_qty,
            'quad_prod_wise_sales_mast_data_val'=>$quad_prod_wise_sales_mast_data_val,
        ]);
    }
    public function ethical(Request $request){
        ...
        $FromTargetDate = date('Y-m-d',strtotime($from_target_date));
        $ToTargetDate = date('Y-m-d',strtotime($to_target_date));
        ...
        $prod_wise_target_mast_data = DB::table('DEALER_TARGET_MAST')
                ...
                // ->where('DEALER_TARGET_MAST.FROM_DATE', '=', $from_target_date)
                // ->where('DEALER_TARGET_MAST.TO_DATE', '=', $to_target_date)
                ->whereRaw("DATE_FORMAT(DEALER_TARGET_MAST.FROM_DATE_FILTER,'%Y-%m-%d') = '$FromTargetDate'") 
                ->whereRaw("DATE_FORMAT(DEALER_TARGET_MAST.TO_DATE_FILTER,'%Y-%m-%d') = '$ToTargetDate'") 
                ->whereIn('DEALER_TARGET_MAST.ACC_CODE', $dealer_code)
                ->groupBy('DEALER_TARGET_MAST.PROD_CATG')
                ->orderBy('DEALER_TARGET_MAST.PROD_CATG', 'ASC')
    ...
    public function credit_debit_notes_ajax(Request $request){
        ...
        $accnote_tran_data_DATA = DB::table('ACCNOTE_TRAN')
                ...
                if($roleId != '1'){
                    $accnote_tran_data_DATA->where('ACCNOTE_TRAN.ACC_CODE', $this->dealer_code);
                }
                if(!empty($order_no))
                {
      ...
      public function takeActionForCreditDebitNotes(Request $request)
    {
        $primary_id = $request->primary_id; //order_id
        $VRNO = $request->VRNO; //order_id
        $VRDATE = $request->VRDATE;
        $table = $request->tab;
        $act = $request->act;

       
        if ($request->ajax() && !empty($primary_id)) {
            // DB::beginTransaction();
       $select = DB::table('ACCNOTE_TRAN')->where('id',$primary_id)->where('VRNO',$VRNO)->first();

       // dd($select);

        $log_insertion = DB::table('ACCNOTE_TRAN_TEMP')
                        ->insert(
                            [   
                                'id' => $select->id,
                                'ENTITY_CODE' => $select->ENTITY_CODE,
                                'DIV_CODE' => $select->DIV_CODE,
                                'TCODE' => $select->TCODE,
                                'VRNO' => $select->VRNO,
                                'VRDATE' => $select->VRDATE,
                                'VRDATE_FILTER' => $select->VRDATE_FILTER,
                                'NOTE_TYPE' => $select->NOTE_TYPE,
                                'ADJUST_AT_FLAG' => $select->ADJUST_AT_FLAG,
                                'ACC_CODE' => $select->ACC_CODE,
                                'DRAMT' => $select->DRAMT,
                                'CRAMT' => $select->CRAMT,
                                'PARTICULAR' => $select->PARTICULAR,
                                'BILL_CATG' => $select->BILL_CATG,
                                'STAX_RATE' => $select->STAX_RATE,
                                'EVRDATE' => $select->EVRDATE,
                                'EVRDATE_FILTER' => $select->EVRDATE_FILTER,
                                'DUEDATE' => $select->DUEDATE,
                                'DUEDATE_FILTER' => $select->DUEDATE_FILTER,
                                'QTYISSUED' => $select->QTYISSUED,
                                'QTYRECD' => $select->QTYRECD,
                                'OLD_VRNO' => $select->OLD_VRNO,
                                'EMP_CODE' => $select->EMP_CODE,
                                'APPROVED' => $select->APPROVED,
                                'APPROVEDBY' => $select->APPROVEDBY,
                                'APPROVEDDATE' => $select->APPROVEDDATE,
                                'APPROVEDDATE_FILTER' => $select->APPROVEDDATE_FILTER,
                                'APPROVEDREMARK' => $select->APPROVEDREMARK,
                                'USER_CODE' => $select->USER_CODE,
                                'LASTUPDATE' => $select->LASTUPDATE,
                                'LASTUPDATE_FILTER' => $select->LASTUPDATE_FILTER,
                                'FLAG' => $select->FLAG,
                                'SCHEME_NO' => $select->SCHEME_NO,
                                'ITEM_VRNO' => $select->ITEM_VRNO,
                                'ITEM_TCODE' => $select->ITEM_TCODE,
                                'RETURN_VRNO' => $select->RETURN_VRNO,
                                'RETURN_TCODE' => $select->RETURN_TCODE,
                                'server_date_time' => $select->server_date_time,
                            ]
                        );
       ...
       $delete_sale_order = DB::table('ACCNOTE_TRAN')->where('id',$primary_id)->delete();
    ...
    {
        // dd('1');
        $dms_division = DB::table('MKTG_CATG_MAST')
                        ->where('flag','=','Y')
                        ->pluck('MKTG_CATG_NAME','MKTG_CATG');

        $location_2_arr_cus_data = Location2::where('status',1);
            // ->where('location_2.id',$dealer_report_section_data)
            if($this->role_id != 1){
            $location_2_arr_cus_data->join('dealer_report_section_data','dealer_report_section_data.location_2','=','location_2.id')->where('dealer_id',$this->dealer_id);
            }
            
        $location_2_arr_cus = $location_2_arr_cus_data->groupBy('location_2.id')->pluck('location_2.name as name','location_2.id as id'); 
        ...
        return view('DMS/saleStatement.index',[
                    'dms_division'=> $dms_division,
                    'location_2_arr_cus'=> $location_2_arr_cus,
                ]);
        
    }


     public function saleStatementEthical(Request $request)
    {
        // dd('1');
        ...
        return view('DMS/saleStatementEthical.index',[
                    'dms_division'=> $dms_division,
                    'location_2_arr_cus'=> $location_2_arr_cus,
                ]);
    ...
    public function dms_dealer_dashboard(Request $request)
    {
        ...
        $mktg_catg_array = array("ASV", "CLA", "GEN", "GLD", "OTC", "OT2", "JPS", "FMC","JP2");
        // $mktg_catg_array = array("JP2");
        for ($i=0; $i <= 8; $i++) { 
            # code...
            $mktg_catg_wise_target = DB::table('DEALER_TARGET_MAST')
                ->join('MKTG_CATG_MAST','MKTG_CATG_MAST.MKTG_CATG','=','DEALER_TARGET_MAST.MKTG_CATG')  
                ->where('DEALER_TARGET_MAST.MKTG_CATG','=',"$mktg_catg_array[$i]")
                ->where('DEALER_TARGET_MAST.PROD_CATG','='," ")
                ->where('DEALER_TARGET_MAST.FROM_DATE_FILTER', '=', "$from_sale_date")
                ->where('DEALER_TARGET_MAST.TO_DATE_FILTER', '=', "$to_sale_date")
                ->where('DEALER_TARGET_MAST.ACC_CODE', "$this->dealer_code");
                $mktg_catg_wise_targets[$mktg_catg_array[$i]] = !empty($mktg_catg_wise_target->first()->TARGET_AMT)?$mktg_catg_wise_target->first()->TARGET_AMT:0;
        }
        // dd($mktg_catg_wise_targets);
    ...
    public function dms_order_delete_function(Request $request)
    {
     ...
     public function JPS(Request $request){
        $from_target_date = "01-APR-" .(date('y'));
        $to_target_date = "31-MAR-" .(date('y')+1);
        $from_sale_date = (date('Y'))."-04-01";
        $to_sale_date = (date('Y')+1)."-03-31";
      ...
      $prod_wise_target_um = DB::table('DEALER_TARGET_MAST')
            ->join('PROD_CATG_MAST','PROD_CATG_MAST.PROD_CATG','=','DEALER_TARGET_MAST.PROD_CATG')
            // ->where('DEALER_TARGET_MAST.FROM_DATE', '=', $from_target_date)
            // ->where('DEALER_TARGET_MAST.TO_DATE', '=', $to_target_date)
            ->whereRaw("DATE_FORMAT(DEALER_TARGET_MAST.FROM_DATE_FILTER,'%Y-%m-%d') = '$FromTargetDate'") 
            ...
            ->groupBy('DEALER_TARGET_MAST.PROD_CATG')
            ->orderBy('DEALER_TARGET_MAST.PROD_CATG', 'ASC')
            ->pluck('DEALER_TARGET_MAST.UM as UM', 'DEALER_TARGET_MAST.PROD_CATG as PROD_CATG');
        ...
        $quad_prod_wise_sales_mast_data_val = DB::table('ITEMTRAN_BODY')
            ...
            return view('DMS/QuickOrder.JPS',[
            'current_menu'=>$this->current_menu,
            'prod_catg_mast_array' => $prod_catg_mast_array,
            'prod_wise_target_mast'=>$prod_wise_target_mast_data,
            'um_array'=>$um_array,
            ...
            'prod_wise_target_um'=>$prod_wise_target_um,
      ...
      public function JP2(Request $request){
        $from_target_date = "01-APR-" .(date('y'));
        $to_target_date = "31-MAR-" .(date('y')+1);
        $from_sale_date = (date('Y'))."-04-01";
        $to_sale_date = (date('Y')+1)."-03-31";
        ...
        $prod_catg_mast_array = DB::table('PROD_CATG_MAST')  
                ->where('PROD_CATG_MAST.MKTG_CATG','JP2')
                ->orderBy('PROD_CATG_MAST.USER_SEQ', 'ASC')
                // ->orderBy('PROD_CATG_MAST.PROD_CATG_NAME', 'ASC')
                ->get();
        // dd($prod_catg_mast_array);
        ...
        $jp2_wise_target_data_1 = DB::table('DEALER_TARGET_MAST')
            ->join('MKTG_CATG_MAST','MKTG_CATG_MAST.MKTG_CATG','=','DEALER_TARGET_MAST.MKTG_CATG')  
            ->where('DEALER_TARGET_MAST.MKTG_CATG','JP2')
            ->where('DEALER_TARGET_MAST.PROD_CATG','')
            // ->where('DEALER_TARGET_MAST.FROM_DATE', '=', $from_target_date)
            // ->where('DEALER_TARGET_MAST.TO_DATE', '=', $to_target_date)
            ->where('DEALER_TARGET_MAST.FROM_DATE_FILTER', '=', $from_sale_date)
        ...
        $jp2_wise_target_data = !empty($jp2_wise_target_data_1->TARGET_AMT)?$jp2_wise_target_data_1->TARGET_AMT:0;

            // dd($jp2_wise_target_data);


            return view('DMS/QuickOrder.JP2',[
            'current_menu'=>$this->current_menu,
            'prod_catg_mast_array' => $prod_catg_mast_array,
            'prod_wise_target_mast'=>$prod_wise_target_mast_data,
            'um_array'=>$um_array,
            ...
            'jp2_wise_target_data'=>$jp2_wise_target_data,
