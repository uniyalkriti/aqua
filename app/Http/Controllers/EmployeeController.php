...
class EmployeeController extends Controller
{
    ...
    {   
        ...
        $leave_count_data = DB::table('leaves')
                    ->where('company_id',$company_id);
                    if(!empty($junior_data))
                    {
                        $leave_count_data->whereIn('employee_id',$junior_data);

                    }
        $leave_count = $leave_count_data->distinct('id')->count('id');

        $leave_count_data_a = DB::table('leaves')
                    ->where('company_id',$company_id)
                    ->where('is_approved',1);
                    if(!empty($junior_data))
                    {
                        $leave_count_data_a->whereIn('employee_id',$junior_data);

                    }
        $leave_count_a = $leave_count_data_a->distinct('id')->count('id');


        $expense_details_data = DB::table('user_expense_details')
                    ->where('company_id',$company_id);
                    // ->where('pa',1)
                    if(!empty($junior_data))
                    {
                        $expense_details_data->whereIn('user_id',$junior_data);

                    }
        $expense_details = $expense_details_data->distinct('id')->count('id');
        # for filter 
        ...
        $totalDistributorSaleData=DB::table('user_sales_order')
                            ...
                            if(!empty($location_3_filter))
                            {
                                $totalDistributorSaleData->join('location_view','location_view.l7_id','=','user_sales_order.location_id')->whereIn('l3_id',$location_3_filter);
                            }
        ...
        $totalOutletSaleData=DB::table('user_sales_order')
                        ...
                        if(!empty($location_3_filter))
                        {
                            $totalOutletSaleData->join('location_view','location_view.l7_id','=','user_sales_order.location_id')->whereIn('l3_id',$location_3_filter);
                        }
        ...
        $totalBeatSaleData=DB::table('user_sales_order')
                    ...
                    if(!empty($location_3_filter))
                    {
                        $totalBeatSaleData->join('location_view','location_view.l7_id','=','user_sales_order.location_id')->whereIn('l3_id',$location_3_filter);
                    }
       ...
       // END OF BEAT DETAILS
        ...
        $totalCallData=DB::table('user_sales_order')
                    ...
                    if(!empty($location_3_filter))
                    {
                        $totalCallData->join('location_view','location_view.l7_id','=','user_sales_order.location_id')->whereIn('l3_id',$location_3_filter);
                    }
        ...
        $productiveCallData=DB::table('user_sales_order')
                        ...
                        if(!empty($location_3_filter))
                        {
                            $productiveCallData->join('location_view','location_view.l7_id','=','user_sales_order.location_id')->whereIn('l3_id',$location_3_filter);
                        }
        ...
        $array_v = array('01','02','03','04','05','06','07','08','09','10','11','12');
        ...
        foreach ($array_v as $key => $value) {
            # code...
            $datesArr_new[] = date('Y-').$value;
            $datesDisplayArr[] = date('Y-').$array[$key];
        }
        // dd($datesArr);
        if(empty($check)){
        $saleOrderValueData = DB::table('user_sales_order')
                        ...
                        if(!empty($location_3_filter))
                        {
                            $saleOrderValueData->join('location_view','location_view.l7_id','=','user_sales_order.location_id')->whereIn('l3_id',$location_3_filter);
                        }    
        $saleOrderValue = $saleOrderValueData->pluck(DB::raw("SUM(rate*quantity)"),'user_sales_order.date as date');
            ...
            $saleOrderValue = $saleOrderValueData->pluck(DB::raw("SUM(final_secondary_rate*final_secondary_qty)"),'user_sales_order.date as date');   
        }
        ...
        $attendance_data_inner = DB::table('user_daily_attendance')
                    ->where('company_id',$company_id);
                    if(!empty($junior_data))
                    {
                        $attendance_data_inner->whereIn('user_id',$junior_data);
                    }
                    $attendance_data_i = $attendance_data_inner->groupBy(DB::raw("date_format(work_date,'%Y-%m')"))
                    ->pluck(DB::raw("COUNT(user_id)"),DB::raw("date_format(work_date,'%Y-%m') as dae"));
                    // dd($attendance_data_i);
        foreach ($datesArr_new as $key_new => $value_new)
        {
       
            $out[$value_new]=!empty($attendance_data_i[$value_new])?$attendance_data_i[$value_new]:'0';
        }
        // dd($out);
        ...
        $sumaary_part2_retailer_data = DB::table('user_sales_order')
                                ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(date,'%Y-%m-%d')<='$to_date'")
                                ->where('company_id',$company_id);
                                if(!empty($junior_data))
                                {
        ...
        $sumaary_part2_retailer_id = DB::table('user_sales_order')
                                ->whereRaw("DATE_FORMAT(date,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(date,'%Y-%m-%d')<='$to_date'")
                                ->where('company_id',$company_id)
                                ->groupBy('retailer_id');
                                if(!empty($junior_data))
    ...
    public function getTotalSalesTeamHome(Request $request)
    {
        ...
        $user_details_data = DB::table('person')
                    ...
                    ->select('person.id as user_id','person_id_senior',DB::raw("CONCAT_WS(' ',first_name,middle_name,last_name) as user_name"),'location_3.name as state','_role.rolename as role','mobile',DB::raw("(SELECT CONCAT_WS(' ',first_name,middle_name,last_name) from person where person.id = person_id_senior LIMIT 1) as senior_name"))
                    ->where('person.company_id',$company_id)
                    ->where('person_login.company_id',$company_id)
                    ->where('_role.company_id',$company_id)
                    ->where('is_admin','!=',1)
    ...
    public function total_beat_coverage_details(Request $request)
    {
        ...
        $beat_coverage_detailsData = DB::table('user_sales_order')
                                ...
                                if(!empty($location_3_filter))
                                {
                                    $beat_coverage_detailsData->join('location_view','location_view.l7_id','=','user_sales_order.location_id')->whereIn('l3_id',$location_3_filter);
                                }
    ...
    public function total_productive_coverage_details(Request $request)
    {
        ...
        $productve_call_details_data = DB::table('user_sales_order')
                                ...
                                if(!empty($location_3_filter))
                                {
                                    $productve_call_details_data->join('location_view','location_view.l7_id','=','user_sales_order.location_id')->whereIn('l3_id',$location_3_filter);
                                }
        ...
        if(!empty($productve_call_details))
        {
            $call_status_count_data = DB::table('user_sales_order')
                            ...
                            if(!empty($location_3_filter))
                            {
                                $call_status_count_data->join('location_view','location_view.l7_id','=','user_sales_order.location_id')->whereIn('l3_id',$location_3_filter);
                            }
    ...
    public function get_dealer_details_home(Request $request)
    {
        ...
        $dealer_sale_data = DB::table('user_sales_order')
                    ...
                    if(!empty($location_3_filter))
                    {
                        $dealer_sale_data->join('location_view','location_view.l7_id','=','user_sales_order.location_id');
    ...
    public function get_total_call_details_home(Request $request)
    {
        ...
        if(empty($check)){
            $data_query_data = DB::table('location_view')
                    ...
                    if(!empty($junior_data))
                    {
                        $data_query_data->join('location_view','location_view.l7_id','=','user_sales_order.location_id');
