...
class scheme_sale extends myfilter
{
	public $poid = NULL;
    ...
    public function scheme_save()
	{
		...
		$q = "INSERT INTO `scheme_on_sale` (`scheme_id`, `scheme_name`, `created`, `intype`,`start_date`,`end_date`) VALUES
                    (NULL , '$d1[scheme_name]', NOW(), '$d1[intype]','$start_date','$end_date')";
            //   h1($q);
		...
		$rId = mysqli_insert_id($dbc);
                ...
                $str = implode(',' , $str);
                $q = "INSERT INTO `scheme_on_sale_details`(`scheme_id`, `value`,`value_to`, `scheme_gift`,`remark`) VALUES $str";
               // h1($q);
                $r = mysqli_query($dbc,$q) ;
                if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'Scheme could not be saved, some error occurred');}
                mysqli_commit($dbc);
	...
	public function scheme_edit($id)
	{
		...
		mysqli_query($dbc, "START TRANSACTION");
		// query to update 		
		$q = "UPDATE `scheme_on_sale` SET scheme_name = '$d1[scheme_name]', created = NOW(),start_date='$start_date',end_date='$end_date', intype = '$d1[intype]' WHERE scheme_id = '$id'";
                     $r = mysqli_query($dbc,$q);
		if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'Scheme could not be updated some error occurred.');}
		mysqli_commit($dbc);
		$rId = $id;
                     $q = "DELETE FROM scheme_on_sale_details WHERE scheme_id = '$id'";
                     $r = mysqli_query($dbc, $q);
                     if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'Scheme could not be updated some error occurred.');}
                $str = array();
                if(!empty($d1['value']))
...
//                        $end_date =  !empty($d1['end_date'][$key]) ?  get_mysql_date($d1['end_date'][$key]) : '';
                ...
                $str = implode(',' , $str);
                $q = "INSERT INTO `scheme_on_sale_details` (`scheme_id`, `value`, `scheme_gift`, `value_to`, `remark`) VALUES $str";
                $r = mysqli_query($dbc,$q) ;
		if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'Scheme could not be saved, some error occurred');}
                mysqli_commit($dbc);
	...
	public function get_scheme_list($filter='',  $records = '', $orderby='')
	{
		...
		$q = "SELECT * FROM scheme_on_sale $filterstr";
                //h1($q);
		...
		if(!$opt) return $out; // if no order placed send blank array
                   ...
                   //  $state_map=  get_my_reference_array('location_2','id','name');
                    while($row = mysqli_fetch_assoc($rs))
                    {
                           $id = $row['scheme_id'];
                           $out[$id] = $row;
                           $q="SELECT * FROM `scheme_on_sale_details` WHERE scheme_id = $id";
        ...
        public function scheme_delete($id, $filter='', $records='', $orderby='')
	{
		...
		$delquery = array();
		$delquery['scheme'] = "DELETE FROM scheme_on_sale WHERE id = '$id'";
