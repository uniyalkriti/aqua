<?php
class performance extends myfilter {
    ...
    $end = get_mysql_date($_POST['end'],'/',$time = false, $mysqlsearch = true);
     ...
     $filteruson = $this->oo_filter($filteruso, $records, $orderby);  
            ...
            $q="SELECT person.id as pid,uso.dealer_id,d.name as dealer_name,d.dealer_code,lv.l3_id as stateid,lv.l3_name AS state,lv.l4_name AS city,uso.retailer_id AS rid,retailer.name AS ret_name,
                rot.outlet_type,CONCAT_WS(' ',first_name,middle_name,last_name) AS person_fullname,uso.date,person.role_id,person.emp_code,
                sum(rate*quantity) as order_value_booked,
                count(distinct c1_id) AS categories,count(distinct c2_id) AS variants
                from user_sales_order uso INNER JOIN user_sales_order_details usod using(order_id) "
                . " INNER JOIN catalog_view cv ON cv.product_id=usod.product_id "
                . " INNER JOIN dealer d ON d.id=uso.dealer_id "
                . " INNER JOIN retailer ON retailer.id=uso.retailer_id "
                . " INNER JOIN _retailer_outlet_type rot ON rot.id= retailer.outlet_type_id "
                . " INNER JOIN location_view AS lv ON lv.l5_id = uso.location_id "                
                . " INNER JOIN person ON person.id=uso.user_id "          
                . " $filterstr AND person.role_id in (2,3,10,14,15,21) GROUP BY uso.dealer_id,uso.retailer_id,person.id";
    ...
    $end = get_mysql_date($_POST['end'],'/',$time = false, $mysqlsearch = true);
     ...
     $filteruson = $this->oo_filter($filteruso, $records, $orderby);  
            ...
            // $q="SELECT person.id as pid,uso.dealer_id,d.name as dealer_name,d.dealer_code,state.stateid,state.statename AS state,lv.l4_name AS city,uso.retailer_id AS rid,retailer.name AS ret_name,
            //     rot.outlet_type,CONCAT_WS(' ',first_name,middle_name,last_name) AS person_fullname,uso.date,person.role_id,person.emp_code,
            //     sum(rate*quantity) as order_value_booked,
            //     count(distinct c1_id) AS categories,count(distinct c2_id) AS variants,retailer.retailer_status
            //     from user_sales_order uso INNER JOIN user_sales_order_details usod using(order_id) "
            //     . " INNER JOIN catalog_view cv ON cv.product_id=usod.product_id "
            //     . " INNER JOIN dealer d ON d.id=uso.dealer_id "
            //     . " INNER JOIN retailer ON retailer.id=uso.retailer_id "
            ...
            $q="SELECT person.id as pid,retailer.dealer_id,d.name as dealer_name,d.dealer_code,lv.l2_id as stateid,lv.l2_name AS state,lv.l4_name AS city,retailer.id AS rid,retailer.name AS ret_name,CONCAT_WS(' ',first_name,middle_name,last_name) AS person_fullname,person.role_id,person.emp_code,retailer.retailer_status
                ...
                . " $filterstr AND person.role_id in (3,10,14,15,21) GROUP BY lv.l2_id,rid,retailer.dealer_id,person.id";
           //h1($q); //exit;
    $rs = mysqli_query($dbc, $q);
    $i =1;     
    //if(!$opt) return $out; // if no order placed send blank array
    while ($row = mysqli_fetch_assoc($rs)) {
        ...
        $where1 = "user_id=".$row['pid']." AND DATE_FORMAT(date,'%Y%m%d')>='$start' AND DATE_FORMAT(date,'%Y%m%d')<='$end' AND retailer_id=".$row['rid']." AND dealer_id=".$row['dealer_id']." and call_status=1";
        $out[$i]['booked']= myrowval('user_sales_order','count(order_id)',$where1);


        $where2 = "ch_user_id=".$row['pid']." AND DATE_FORMAT(ch_date,'%Y%m%d')>='$start' AND DATE_FORMAT(ch_date,'%Y%m%d')<='$end' AND ch_retailer_id=".$row['rid']." AND ch_dealer_id=".$row['dealer_id'];
        $out[$i]['billed']= myrowval('challan_order','count(ch_no)',$where2);
        ...
        $out[$i]['booked_value']= myrowval('user_sales_order','SUM(amount)',$where1);
        $out[$i]['billed_value']= myrowval('challan_order','SUM(amount)',$where2);
       $i++;
    }  
    //pre($out);
    return $out;
    ...
    public function get_given_value_till_booked($id,$enddate,$pid) {
        ...
        $q="SELECT sum(rate*quantity) AS giving_value "
                   . "FROM user_sales_order AS uso INNER JOIN user_sales_order_details usod using(order_id) "
                   . " $enddate AND uso.user_id='$pid' AND uso.retailer_id='$id' "
                  . " GROUP BY uso.retailer_id,uso.user_id";                
       // h1($q);
        list($opt, $rs) = run_query($dbc, $q, $mode = 'single', $msg = '');
        if (!$opt)
      ...
      public function get_productive_outlet_tracking_list($filter = '', $records = '', $orderby = '',$date_filter ='') {
        ...
        $rdel = mysqli_query($dbc, $del);
         ...
         $insert = "insert into _temp_outlet_repeat_order(user_id,pid,retailer_id,total_call)"
                 . "(select $uid,person.id,udr.retailer_id,(select count(uso.order_id) from user_sales_order uso 
                     $datefilter AND uso.user_id=person.id and uso.retailer_id=udr.retailer_id and call_status='1') as total_call FROM person 
        ...
        $q="SELECT person.id AS pid,CONCAT_WS(' ',first_name, middle_name, last_name) AS person_fullname, 
                    ...
                    person.state_id,lv.l3_name AS state FROM person  
                     INNER JOIN person_login ON person_login.person_id = person.id AND person_status ='1'   
                     INNER JOIN location_view as lv  ON lv.l3_id=person.state_id 
                     $filterstr AND person.id != 1 order by lv.l3_id,person.person_id_senior,pid";
    ...
    public function get_total_outlet_repeat_orders($pid,$uid) {
     ...
     public function get_total_outlet_visit($user_id,$datefilter) {
        ...
        $q = "SELECT count(distinct retailer_id) as total_visit FROM user_sales_order uso $datefilter AND user_id='$user_id'";
      // h1($q);
         list($opt, $rs) = run_query($dbc, $q, $mode = 'single', $msg = '');
        if (!$opt)
            return $out;
     ...
     public function get_total_outlet_pro($user_id,$datefilter) {
        ...
        $q = "SELECT count(retailer_id) as total_billed FROM user_sales_order uso $datefilter AND user_id='$user_id' AND call_status='1'";
       //h1($q);
         list($opt, $rs) = run_query($dbc, $q, $mode = 'single', $msg = '');
        if (!$opt)
            return $out;
      ...
      public function get_total_outlet_visit_pro($user_id,$datefilter) {
        ...
        $q = "SELECT count(distinct retailer_id) as total_visit_pro FROM user_sales_order uso $datefilter AND user_id='$user_id' AND call_status='1'";
      // h1($q);
         list($opt, $rs) = run_query($dbc, $q, $mode = 'single', $msg = '');
        if (!$opt)
            return $out;
     ...
     $q="select uda.order_id AS idd,person.id as user_id,person_id_senior as senior_id,date,date_format(date,'%d-%m-%Y') as att_date,SUM(rate*quantity) as order_value,lv.l3_name as state,
    ...
    public function get_product_user_performance_list($filter = '', $records = '', $orderby = '',$filterdate ='') {
        ...
        $out = array();
         ...
         $filterstr = $this->oo_filter($filter, $records, $orderby);  
     $q="select dealer.dealer_code,uda.order_id AS idd,person.id as user_id,person_id_senior as senior_id,date,date_format(date,'%d-%m-%Y') as att_date,SUM(rate*quantity) as order_value,lv.l3_name as state,
                   COUNT(distinct order_id) as total_call, CONCAT_WS(' ',first_name,middle_name,last_name) AS person_name,person.emp_code,
                    dealer.name as dealer_name,dealer.id as dealer_id,  
                    lv.l4_name as town,_role.rolename as designation from user_sales_order uda 
                    INNER JOIN user_sales_order_details USING(order_id)
  ...
  $out[$inc]['pro_call']= $this->get_user_product_call($row['date'],$row['user_id'],$row['dealer_id'],$product_id);
        ...
        {
          $wherepro = "call_status = 1 AND date = '$row[date]' AND dealer_id = '$row[dealer_id]' AND user_id = '$row[user_id]'";
       $out[$inc]['pro_call']= myrowval('user_sales_order','count(order_id)',$wherepro);
        }
            $out[$inc]['attendance']= $this->get_attendance_details($row['date'],$row['user_id']);
    ...
    public function get_pulse_product_user_performance_list($filter = '', $records = '', $orderby = '',$filterdate ='') {
        global $dbc;
        $out = array();
              
         $filterstr = $this->oo_filter($filter, $records, $orderby);  
     $q="select person.product_division,dealer.dealer_code,uda.order_id AS idd,person.id as user_id,person_id_senior as senior_id,date,date_format(date,'%d-%m-%Y') as att_date,SUM(rate*quantity) as order_value,sum(user_sales_order_details.quantity) as order_qty,lv.l2_name as state,
                   COUNT(distinct order_id) as total_call, CONCAT_WS(' ',first_name,middle_name,last_name) AS person_name,person.emp_code,
                    dealer.name as dealer_name,dealer.id as dealer_id,  
                    lv.l4_name as town,_role.rolename as designation from user_sales_order uda 
                    INNER JOIN user_sales_order_details USING(order_id)
                    ...
                    INNER JOIN _role ON _role.role_id = person.role_id
                    INNER JOIN location_view lv ON lv.l5_id=uda.location_id
                    INNER JOIN dealer ON dealer.id=uda.dealer_id $filterstr AND person.role_id in (2,3,10,14,15,21,30,31,32,33) 
                    group by date,user_id,dealer_id order by user_id,date asc";                
      //h1($q); //exit;
        $inc = 1;
        $rs = mysqli_query($dbc, $q); 
        $product_id='164'; 
       // $product_id=implode(',',$product_id);         
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $out[$inc]['attendance']= $this->get_attendance_details($row['date'],$row['user_id']);
            $out[$inc]['pro_call']= $this->get_user_product_call($row['date'],$row['user_id'],$row['dealer_id'],$product_id);
            $where_ctc="date='$row[date]' AND user_id='$row[user_id]' AND dealer_id='$row[dealer_id]'";
            $out[$inc]['catch_tc']= myrowval('user_sales_order','count(distinct user_sales_order.order_id)',$where_ctc);
            $out[$inc]['new_outlet_visit'] = $this->get_user_new_outlet_visit($row['date'],$row['user_id'],$row['dealer_id']);
            $out[$inc]['new_outlet_details'] = $this->get_product_user_new_outlet_billed($row['date'],$row['user_id'],$row['dealer_id'],$product_id);
            $out[$inc]['senior'] = myrowval('person',"CONCAT_WS(' ',first_name,middle_name,last_name)",'id='.$row['senior_id']); 
    ...
    public function get_product_user_new_outlet_billed($date,$user_id,$dealer_id,$product_id) {
          ...
          //  $out = NULL;
        $q = "SELECT count(distinct retailer_id) as new_outlet_billed,SUM(rate*quantity) as sale_value,SUM(quantity) AS sale_qty FROM retailer "
    ...
    ."AND date_format(retailer.created_on,'%Y-%m-%d')='$date' and uso.call_status='1'";
      // h1($q); die;
        ...
        if (!$opt)
            return $out;
        return $rs['new_outlet_billed'].'||'.$rs['sale_value'].'||'.$rs['sale_qty'];
    }

    public function get_merchandise_report_list($filter = '', $records = '', $orderby = '') {
    global $dbc;
    $out = array();
    $start = get_mysql_date($_POST['start'],'/',$time = false, $mysqlsearch = true);
    $end = get_mysql_date($_POST['end'],'/',$time = false, $mysqlsearch = true);     
     $filterstr = $this->oo_filter($filter, $records, $orderby);   
            $q="SELECT uso.id AS mid,uso.merchandise_id AS meid,retailer.lat_long AS gps1,uso.image,uso.qty AS mer_qty,uso.merchandise_name AS mer_name,person.person_id_senior AS pis,uso.time,person.id as pid,d.id,d.name as dealer_name,d.dealer_code,l2_name AS state,lv.l4_name AS town,uso.retailer_id AS rid,retailer.name AS ret_name,CONCAT_WS(' ',first_name,middle_name,last_name) AS person_fullname,uso.date,person.role_id,person.emp_code 
                from merchandise uso "
                . " INNER JOIN retailer ON retailer.id=uso.retailer_id "
                . " INNER JOIN dealer d ON d.id=retailer.dealer_id "
                . " INNER JOIN location_view AS lv ON lv.l5_id = retailer.location_id "
                . " INNER JOIN dealer_location_rate_list AS udr ON udr.dealer_id=d.id " 
                . " INNER JOIN person ON person.id=udr.user_id "
                . " $filterstr GROUP BY date,rid,meid";
           //h1($q); //exit;
    $rs = mysqli_query($dbc, $q);   
    $inc=1;   
    while ($row = mysqli_fetch_assoc($rs)) {
        ...
        $id=$inc;
        $out[$id] = $row;
        $where="retailer_id='$row[rid]' AND date='$row[date]' ";
        $out[$id]['designation']= myrowval('_role','rolename','role_id='.$row['role_id']);
        $out[$id]['senior']= myrowval('person',"CONCAT_WS(' ',first_name,middle_name,last_name)",'id='.$row['pis']);
        $senior_role_id= myrowval('person','role_id','id='.$row['pis']);
        $out[$id]['senior_role']= myrowval('_role','rolename','role_id='.$senior_role_id);
        $out[$id]['gps']= myrowval('user_sales_order','lat_lng',$where);
        $out[$id]['remarks']= myrowval('user_sales_order','remarks',$where);
        $inc++;
    }  
    //pre($out);
    return $out;
