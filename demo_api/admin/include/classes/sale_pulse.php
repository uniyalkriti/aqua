<?php
class sale_pulse extends myfilter {
    ...
    public function get_sale_month_summary_list($filter, $start,$end) {
        ...
        $q = "SELECT count(person.id) AS total_users,stateid,statename 
            FROM person INNER JOIN person_login ON person_login.person_id=person.id 
            AND person_login.person_status='1' INNER JOIN state ON state.stateid=person.state_id
            INNER JOIN _role ON _role.role_id= person .role_id  $filterstr  GROUP BY stateid ORDER BY
                statename ASC";
      // h1($q);
       //die;
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
...
//static $data;
    ...
    public function get_time_report_list($filter = '', $records = '', $orderby = '', $date_filter = '',$filter2) {
        ...
        public function get_time_report_details($filterstr,$start,$end) {
            //echo $filterstr;die;
            global $dbc;
            $out = array();
            $q = "SELECT emp_code,person.role_id as role,CONCAT_WS(' ',person.first_name,person.middle_name,person.last_name)as user_name,person.id as p_id,statename
    ...
    public function get_statelevel_performance_list($start,$end, $records = '', $orderby = '') {
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
           // $abc++;
            ...
            $out[$id]['so_data']= $this->get_so_details($id,$start,$end,$pulse);
            $out[$id]['outlet_value']= $this->get_newoutlet_value($id,$start,$end,$pulse);
            $out[$id]['so_sale_data']= $this->get_pro_order_cov($id,$start,$end,$pulse);
        }

        return $out;
    } 
    public function get_so_details($id,$start,$end,$pulse) {
        global $dbc;
        $out = NULL;
        if(!empty($pulse)){
        $filterstr= " AND person.product_division IN ($_POST[product_division])";
      }
...
(SELECT count(uso.retailer_id) AS so_out FROM user_sales_order AS uso INNER JOIN person ON person.id=uso.user_id WHERE state_id='$id' AND person.role_id in (3,14,15,21) AND DATE_FORMAT(`date`,'%Y%m%d')>='$start' AND DATE_FORMAT(`date`,'%Y%m%d')<='$end') AS outlet_visited
    ...
    public function get_pro_order_cov($id,$start,$end,$pulse) {
        global $dbc;
        $out = NULL;
        if(!empty($pulse)){
        $filterstr= " AND person.product_division IN ($_POST[product_division])";
        ...
        (SELECT sum(mtp.total_sales) AS mtarget FROM monthly_tour_program AS mtp INNER JOIN person ON person.id=mtp.person_id  where person.state_id='$id' AND DATE_FORMAT(`working_date`,'%Y%m%d')>='$start' AND DATE_FORMAT(`working_date`,'%Y%m%d')<='$end' AND person.role_id in (3,14,15,21)) AS mtarget
         FROM user_sales_order AS uso INNER JOIN user_sales_order_details USING(order_id) INNER JOIN person ON person.id=uso.user_id WHERE person.state_id='$id' AND person.role_id in (3,14,15,21) AND uso.call_status='1' AND DATE_FORMAT(`date`,'%Y%m%d')>='$start' AND DATE_FORMAT(`date`,'%Y%m%d')<='$end' AND user_sales_order_details.product_id=164";
    ...
    public function get_newoutlet_value($id,$start,$end,$pulse) {
        global $dbc;
        $out = NULL;
        if(!empty($pulse)){
        $filterstr= " AND person.product_division IN ($_POST[product_division])";
        ...
        $q="SELECT sum(usod.rate*usod.quantity) AS outlet_value,count(DISTINCT retailer.id) AS no FROM retailer "
                ...
                . "INNER JOIN person ON person.id=uso.user_id where DATE_FORMAT(`date`,'%Y%m%d')>='$start' AND DATE_FORMAT(`date`,'%Y%m%d')<='$end' AND person.state_id='$id' AND DATE_FORMAT(`created_on`,'%Y%m%d')>='$start' AND DATE_FORMAT(`created_on`,'%Y%m%d')<='$end' AND person.role_id in (3,14,15,21)  AND usod.product_id=164";
    ...
    public function get_dropped_sku_list($filter = '', $records = '', $orderby = '',$lastedate_filter = '',$rdate_filter = '',$usodate ='',$filterlast='',$smonth) {
         ...
         $uid = $_SESSION[SESS . 'data']['id'];
           ...
           $filter_last = $this->oo_filter($filterlast, $records, $orderby);
            ...
            $insertc = "insert into _temp_dropped_sku_outlet(user_id,pid,retailer_id,product_id,month)(select $uid,uso.user_id,uso.retailer_id,usod.product_id,date_format(uso.date,'%m') as month FROM user_sales_order AS uso "
                    ...
                    . "INNER JOIN person ON person.id = uso.user_id "
                    . "INNER JOIN person_login ON person_login.person_id = person.id AND person_status ='1' "
                   . "$filterstr AND uso.call_status='1' AND usod.product_id=164 AND uso.retailer_id not in (select id from retailer where $rdate_filter))";
          // h1($insertc);
            $insertcrun = mysqli_query($dbc, $insertc);
            ...
            $insertl = "insert into _temp_dropped_sku_outlet(user_id,pid,retailer_id,product_id,month)(select $uid,uso.user_id,uso.retailer_id,usod.product_id,date_format(uso.date,'%m') as month FROM user_sales_order AS uso "
                   . "INNER JOIN user_sales_order_details AS usod ON usod.order_id=uso.order_id "
                    . "INNER JOIN person ON person.id=uso.user_id "
                    . "INNER JOIN person_login ON person_login.person_id = person.id AND person_status ='1' "
                   . "$filter_last AND uso.call_status='1' AND usod.product_id=164)";
           ...
           $q="SELECT id AS pid,name AS product_name FROM catalog_product WHERE id=164";
          // h1($q);
        $rs = mysqli_query($dbc, $q);
        $abc = 0;
        //if(!$opt) return $out; // if no order placed send blank array
      ...
      public function get_person_name($id,$filterstr,$lastedate_filter,$rdatefilter,$usodate,$filter_last) {
        ...
        $q="SELECT DISTINCT(CONCAT_WS(' ',first_name, middle_name, last_name)) AS person_fullname,person.id as pid,usod.product_id AS id
            ,state.statename AS state,person.product_division FROM user_sales_order AS uso      
            INNER JOIN person ON person.id = uso.user_id
            INNER JOIN person_login ON person_login.person_id = person.id AND person_status ='1'   
            INNER JOIN user_sales_order_details AS usod ON usod.order_id=uso.order_id 
            INNER JOIN state ON state.stateid=person.state_id
       ...
       $rs = mysqli_query($dbc,$q);
        ...
        while($row=  mysqli_fetch_assoc($rs)){
            ...
            $out[$id]['state']=$row['state'];
            $out[$id]['product_division']=$row['product_division']; 
            ...
            $qdr = "INSERT INTO _temp_manpower_performance(user_id,pid,person_fullname,designation,emp_code,
                 ...
                 (SELECT $uid,person.id AS pid,CONCAT_WS(' ',first_name, middle_name, last_name) AS person_fullname,_role.rolename, 
                    person.emp_code,person.person_id_senior,(select CONCAT_WS(' ', p.first_name,p.middle_name,p.last_name) from person p where p.id=person.person_id_senior) as senior_person_name,
                     state.stateid,state.statename AS state "                 
                   . " FROM person 
                     INNER JOIN person_login ON person_login.person_id = person.id AND person_status ='1'                     
                     INNER JOIN state ON state.stateid = person.state_id AND person_status ='1'       
...
return $out;
  ...
  function get_oneDayReport_list($filter = '', $records = '', $orderby = '')
    ...
    public function get_stock_detail_summary_list($filter='', $records = '', $orderby='')    
    {
       ...
       $filterstr = $this->oo_filter($filter, $records, $orderby);
       //print_r($filterstr);
       $q="SELECT s.id AS sid,qty,mrp,s.product_id AS product_id,ROUND(dealer_rate*qty,2) as dealerrate,catalog_view.product_name AS product_name,dealer.id AS did,dealer.name AS dname,lv.l2_id,lv.l2_name AS state,lv.l4_name as town FROM dealer 
    ...
    function get_twoDayReport_list($filter = '', $records = '', $orderby = '')
    {
        ...
        // h1(Helllooo);
         ...
         $sid = implode(',',$_POST['id']);
                ...
                $q = "SELECT usod.product_id,cv.product_name as sku,cv.c2_name as vname,uso.order_id AS order_id,user_id,retailer_id,l2_name as state, l2_id, l5_name as beat,DATE_FORMAT(date,'%d/%m/%Y') AS sale_date,DATE_FORMAT(time,'%d/%m/%Y') AS sale_time,DATE_FORMAT(date,'%d/%b/%Y') AS fdated,dealer.name AS name,dealer.csa_id 
         FROM user_sales_order uso 
         INNER JOIN user_sales_order_details usod ON uso.order_id=usod.order_id 
         INNER JOIN catalog_view cv ON usod.product_id=cv.product_id 
         INNER JOIN dealer ON dealer.id=uso.dealer_id  
         INNER JOIN location_view ON uso.location_id=l5_id "
                 . "$cond1  GROUP BY dealer.id,retailer_id ORDER BY state ASC";
         ...
         while ($row = mysqli_fetch_assoc($rs)) {
           ...
           //  $out[$id]['name'] = $dealer_map[$row['dealer_id']];
             ...
             $cond2="retailer_id=".$row['retailer_id']." AND DATE_FORMAT(`date`,'%Y%m%d')>='".$start."' AND DATE_FORMAT(`date`,'%Y%m%d')<='".$end."' AND product_id=".$product_id;
             $out[$id][sale_order_value]= myrowvaljoin('user_sales_order_details','sum(rate*quantity)','user_sales_order','user_sales_order.order_id=user_sales_order_details.order_id',$cond2);
         ...
         INNER JOIN location_view lv ON lv.l5_id=dlrl.location_id " 
                 . " $cond3 AND cod.order_id='0' GROUP BY dealer.id,ch_retailer_id,cod.product_id ";
                 //h1($q);
            $rs = mysqli_query($dbc,$q);
                 while ($row = mysqli_fetch_assoc($rs)) {
             $id = $row['order_id'];
