<?php
class tally_report extends myfilter {
...
public function get_ss_bill_list($filter = '', $records = '', $orderby = '') {
    ...
    $end = get_mysql_date($_POST['end'],'/',$time = false, $mysqlsearch = true);
     ...
     $filterstr = $this->oo_filter($filter, $records, $orderby);  
            ...
            $q="SELECT `tally_demo`.`id`, `ch_no`, `first_ch_no`, `ch_date`, `vocher_id`, `ss_voucher_id`, `tally_demo`.`csa_id`,`csa`.`csa_name`, `dealer_id`, `dealer_name`,`dealer`.`name` AS `dname`,`invoice_type`, `amount`, `amount_round`, `cancel_date`, `server_date_time` FROM `tally_demo` INNER JOIN `csa` ON `csa`.`c_id`=`tally_demo`.`csa_id` INNER JOIN `dealer` ON `dealer`.`id`=`tally_demo`.`dealer_id` INNER JOIN `location_view` ON `location_view`.`l3_id`=`csa`.`state_id` $filterstr GROUP BY `tally_demo`.`id` ORDER BY `tally_demo`.`ch_date`,`tally_demo`.`ch_no` DESC";
          // h1($q); //exit;
    $rs = mysqli_query($dbc, $q);
    $i =1;     
    while ($row = mysqli_fetch_assoc($rs)) {
        $id = $row['id'];
        $out[$id] = $row; 
        $qcd="SELECT tally_demo_details.`id` AS `id`, `td_id`, `tally_demo_details`.`product_id`, `qty`, `rate`, `gst`, `gst_amt`, `dis_per`, `dis_amt`, `taxable_amt`, `item_for`, `tally_catalog_product`.`name` AS `pname` FROM `tally_demo_details` INNER JOIN `tally_catalog_product` ON `tally_catalog_product`.`id`=`tally_demo_details`.`product_id` WHERE td_id='$id'";
        //h1($qcd);
    ...
    while ($row = mysqli_fetch_assoc($rs)) {
       ...
       $cond2="`tally_demo`.`csa_id`=".$row['csa_id']." AND tally_demo_details.product_id=".$row['product_id']." AND date_format(ch_date,'%Y%m%d')>='".$odate."' AND date_format(ch_date,'%Y%m%d')<='".$prev_date."' AND `tally_demo`.`drcr`='CR' AND `tally_demo`.`invoice_type` IN (4,5,6,7)";
        ...
        $cond8="`tally_demo`.`csa_id`=".$row['csa_id']." AND tally_demo_details.product_id=".$row['product_id']." AND date_format(ch_date,'%Y%m%d')>='".$odate."' AND date_format(ch_date,'%Y%m%d')<='".$prev_date."' AND `tally_demo_details`.`cr_dr`='CR' AND `tally_demo`.`invoice_type` IN (8,9)";
                $opurchase3 = $this->get_dinward_stock($cond8);            
        ...
        $cond9="`tally_demo`.`csa_id`=".$row['csa_id']." AND tally_demo_details.product_id=".$row['product_id']." AND date_format(ch_date,'%Y%m%d')>='".$odate."' AND date_format(ch_date,'%Y%m%d')<='".$prev_date."' AND `tally_demo_details`.`cr_dr`='DR' AND `tally_demo`.`invoice_type` IN (8,9)";
                $obilled3=$this->get_coutward_stock($cond9);    
       ...
       $rate=myrowval('tally_product_rate_list','dealer_pcs_rate','product_id='.$row['product_id'].' AND state_id='.$row['cstate_id']);         
