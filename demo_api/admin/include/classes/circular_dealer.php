...
class circular_dealer extends myfilter
{
	public $poid = NULL;
 ...
 //        return array('status' => true, 'myreason' => $d1['what'] . ' Successfully Saved', 'rId' => $rId);
	...
	public function circular_save()
    {
        ...
        // query to save
        $q = "INSERT INTO `circular_dealer` (`circular_type`,`title`, `content`, `issued_by_person_id`,`issued_time`,`circular_for_persons`,`image`) VALUES ('$d1[circulartype]','$d1[title]', '$d1[contenttxt]', '$d1[uid]',NOW(),'$person[$c]','$browse_file')";
               // h1($q);
	...
	public function get_person_circular_list($filter='',  $records = '', $orderby='')
	{
		...
		$q = " SELECT *,dealer.id AS rId,dealer_id,name FROM dealer "
                           . " INNER JOIN dealer_person_login ON dealer_person_login.dealer_id = dealer.id $filterstr ";
              // h1($q);
