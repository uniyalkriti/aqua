...
class catalog_product extends myfilter
{
	...
	public function catalog_product_save()
	{
		...
		// query to save
	$q = "INSERT INTO `catalog_product` (`itemcode`, `name`,`image_name`, `company_id`, `catalog_id`) VALUES ('$d1[item_code]', '$d1[item_name]','$filename', '1', '$d1[catalog_2_id]')"; 
        h1($q);
		$r = mysqli_query($dbc,$q) ;
		if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'Catalog Product table error');}
...
//                $exdate = !empty($d1['expiry_date']) ? get_mysql_date($d1['expiry_date']) : '';
		...
		$q="UPDATE catalog_product SET `itemcode` = '$d1[item_code]', `name` = '$d1[item_name]', `image_name` = '$filename',  company_id = '$d1[company_id]',  catalog_id = '$d1[catalog_2_id]'  WHERE id='$id'";
                //h1($q);
		$r=mysqli_query($dbc, $q);
		if(!$r){mysqli_rollback($dbc); return array ('status'=>false, 'myreason'=>'Product Catalog Table error') ;} 
		$rId = $id;	
		mysqli_commit($dbc);
...
//		$q .= " $filterstr";
		...
		list($opt, $rs) = run_query($dbc, $q, $mode='multi', $msg='');
                ...
                //$packname = get_my_reference_array('_packname', 'id', 'name');
		if(!$opt) return $out; // if no order placed send blank array
		while($row = mysqli_fetch_assoc($rs))
		{       
			$id = $row['id'];
			$out[$id] = $row;
                        //$out[$id]['company_name'] = $company_name[$row['company_id']];
                       // $out[$id]['prod_category'] = $prod_category[$row['catalog_1_id']];
                       // $out[$id]['Prod_flavour'] = $Prod_flavour[$row['catalog_2_id']];
                        //$out[$id]['packname'] = $packname[$row['packname']];
		}
               // pre($out);
		return $out;
               // pre($out);
        ...
        public function packname_save()
	{
		...
		if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'packname table error');}
		 $rId = $id;
                //h1($rId);//exit;
		mysqli_commit($dbc);
		//Logging the history		
	...
	public function packname_edit($id)
	{
		...
		$q="UPDATE _packname SET `name` = '$d1[name]' WHERE id='$id'";
                //h1($q);
		$r=mysqli_query($dbc, $q);
		if(!$r){mysqli_rollback($dbc); return array ('status'=>false, 'myreason'=>'Pack Name Table error') ;} 
		$rId = $id;	
		mysqli_commit($dbc);
        ...
        public function product_company_save()
	{
		...
		if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'Product Company table error');}
		 $rId = $id;
                //h1($rId);//exit;
		mysqli_commit($dbc);
		//Logging the history		
