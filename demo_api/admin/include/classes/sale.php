...
// tour and expense
    ...
    public function get_user_plant_list($filter = '', $records = '', $orderby = '') {
        global $dbc;
        ...
        $q = "SELECT uda.*,_role.rolename,uda.id AS uid, DATE_FORMAT(uda.from_date,'%e/%b/%Y') 
        AS from_date,uda.server_date_time as server_date, DATE_FORMAT(uda.cur_date,'%Y%m%d') AS wdatess, DATE_FORMAT(uda.to_date,'%e/%b/%Y') AS to_date,
         CONCAT_WS(' ',first_name,middle_name,last_name) AS name FROM user_plan_request uda 
                INNER JOIN person ON person.id = uda.user_id 
                INNER JOIN person_login ON person.id=person_login.person_id AND person_login.person_status='1' INNER JOIN _role ON person.role_id = _role.role_id $filterstr ";
//        h1($q);
...
//        h1($q);
    ...
    public function get_sale_dynamic_list($filter = '', $records = '', $orderby = '') {
     ...
     public function get_retailer_winback_30($filter = '', $records = '', $orderby = '') {
        global $dbc;
        $out = array();
      // pre($filter);
        $date1 = date("Y-m-d");
        ...
        $filterstr = $this->oo_filter($filter, $records, $orderby);
       // echo $filterstr;die;
        $q = "SELECT retailer.id as rid ,name FROM retailer WHERE retailer.$filter[0] AND retailer.id 
            NOT IN (SELECT retailer_id  FROM user_sales_order
            WHERE user_sales_order.$filter[0] AND date BETWEEN '$day30' AND '$date1')";
       // h1($q);
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
        if (!$opt)
            return $out;
    ...
    public function get_sale_month_report_list($filter = '', $records = '', $orderby = '', $date_filter = '',$filter2) {
        global $dbc;
        $out = array();
        $month = $filter['month'];
     ...
     // h1($filterstr);die;
        ...
        $q = "SELECT id as stateid,name as statename from location_3 $filterstr limit 1 ";
        //h1($q);
       // echo $q;die;
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $out[$id]['details'] = $this->get_sale_month_report_details($filterstr, $start,$end);
        }
      //pre($out);
        return $out;
    }
    public function get_sale_month_report_details($filterstr, $start,$end) {
        ...
        $q = "SELECT emp_code,person.role_id as role,CONCAT_WS(' ',person.first_name,person.middle_name,
            person.last_name)as user_name,person.id as p_id,person.person_id_senior as senior_id,location_3.name as statename
            FROM person INNER JOIN person_login ON person_login.person_id=person.id 
            AND person_login.person_status='1' INNER JOIN location_3 ON location_3.id=person.state_id
    ...
    public function get_sale_month_summary_list($filter, $start,$end) {
        //echo $filterstr;die;
        global $dbc;
        $out = array();
        $filterstr = $this->oo_filter($filter, $records, $orderby);
        $start = get_mysql_date($_POST['start'],'/',$time = false, $mysqlsearch = true);
        $end = get_mysql_date($_POST['end'],'/',$time = false, $mysqlsearch = true);
        $role_id= implode(',',$_POST[role_id]);
        $q = "SELECT count(person.id) AS total_users,l3_id as stateid,l3_name as statename 
            FROM person INNER JOIN person_login ON person_login.person_id=person.id 
            AND person_login.person_status='1' INNER JOIN location_view lv  ON lv.l3_id=person.state_id
            INNER JOIN _role ON _role.role_id= person .role_id  $filterstr  GROUP BY person.id, lv.l3_id ORDER BY
                l3_name ASC";
    //  h1($q);
       //die;
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
           ...
           // $out[$id]['p_id'] = $row['p_id'];
            $out[$id]['state'] = $row['statename'];
            $out[$id]['total_users'] = $row['total_users'];
            $out[$id]['days'] = $this->get_total_dates($month);
            $out[$id]['monthly_total'] = $this->get_grand_total_sale_month_summary($row['stateid'], $start,$end);
            $out[$id]['month_tc'] = $this->get_working_month_tc_summary($row['stateid'],$start,$end);
            $out[$id]['month_pc'] = $this->get_working_month_pc_summary($row['stateid'],$start,$end); 
            $out[$id]['no_usage'] = $this->get_no_usage_month_summary($row['stateid'],$start,$end,$role_id); 
            $out[$id]['regular_usage'] = $this->get_regular_usage_month_summary($row['stateid'],$start,$end,$role_id); 
        }
    // pre($out);
        return $out;
    }
    ...
    public function get_working_month_tc($person, $start,$end) {
       ...
       $q="SELECT DISTINCT  date, (`retailer_id`) as month_tc FROM  `user_sales_order` INNER JOIN user_sales_order_details usod using (order_id)
        WHERE date_format(`date`,'%Y%m%d') >= '$start' AND date_format(`date`,'%Y%m%d') <= '$end' AND user_id='$person' GROUP BY date,retailer_id";
        // h1($q);
        // die;
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
        if (!$opt)
...
public function get_user_expanse_list($filter = '', $records = '', $orderby = '') {
        ...
        $filterstr = $this->oo_filter($filter, $records, $orderby);
        //echo $filterstr;die;
        $q = "SELECT user_expense_report.*,_role.rolename AS role,lv.l3_name as statename ,lv.l3_id,DATE_FORMAT(submit_date,'%e/%b/%Y') AS fdated FROM user_expense_report INNER JOIN person ON person.id=user_expense_report.person_id INNER JOIN location_view as lv ON person.state_id=lv.l3_id INNER JOIN _role ON _role.role_id=person.role_id  $filterstr";
     // h1($q);
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
        if (!$opt)
            return $out;
...
//$opt1 end here
    ...
    public function get_sale_order_list($filter = '', $records = '', $orderby = '') {
        ...
        $q = "SELECT uso.call_status,uso.order_id,lv.l3_name as state,uso.user_id,lv.l5_name as beat,DATE_FORMAT(date,'%d/%m/%Y') AS sale_date,
        DATE_FORMAT(time,'%d/%m/%Y') AS sale_time,DATE_FORMAT(date,'%d/%b/%Y') AS fdated, d.name , r.name as firm_name,
        uso.track_address,uso.image_name FROM user_sales_order uso"
                . " INNER JOIN location_view lv ON uso.location_id=lv.l5_id "
    ...
    public function get_user_tracking_distance_list($filter = '', $records = '', $orderby = '') {
        ...
        $filterstr = $this->oo_filter($filter, $records, $orderby);
        $limit = $_SESSION[SESS . 'constant']['tracking_count'];
        $q = "SELECT *,DATE_FORMAT(track_date,'%Y%m%d') AS tdate FROM 
        user_daily_tracking INNER JOIN person ON person.id=user_daily_tracking.user_id $filterstr";
        //h1($q);
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
        if (!$opt)
            return $out;
...
//static $data;
    ...
    public function get_time_report_list($filter,$records = '', $orderby = '', $date_filter = '') {
     ...
     global $dbc;
        ...
        $start = get_mysql_date($_POST['start'],'/',$time = false, $mysqlsearch = true);
        $end = get_mysql_date($_POST['end'],'/',$time = false, $mysqlsearch = true);
            $q = "SELECT emp_code,person.role_id as role,_role.rolename AS rolename,CONCAT_WS(' ',person.first_name,person.middle_name,person.last_name)as user_name,person.id as p_id,location_3.name as statename,person.joining_date as join_date,person_id_senior as pis 
                FROM person INNER JOIN person_login ON person_login.person_id=person.id  
                INNER JOIN location_3 ON location_3.id=person.state_id
                 INNER JOIN _role ON _role.role_id= person.role_id  $filterstr $person_status  GROUP BY person.id ORDER BY statename ASC";
            // h1($q);//die;
            list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
            if (!$opt)
                return $out;
            ...
            while ($row = mysqli_fetch_assoc($rs)) {
                ...
                $out[$id]['state'] = $row['statename'];
                $out[$id]['emp_code'] = $row['emp_code'];
                $out[$id]['join_date'] = $row['join_date'];
                $out[$id]['name'] = $row['user_name'];
                $out[$id]['role'] =$row['rolename'];
                 $out[$id]['senior'] = myrowval($table = "person", $col = "CONCAT_WS(' ',person.first_name,person.middle_name,person.last_name)", 
                 $where = "id = '$row[pis]'");
                $out[$id]['weekly_off'] = '9';
	...
	// 				$person_status= "'0','9'" ;
	// 					 }
    //             $person_status= " AND person_status IN($person_status)";
    ...
    //    //  h1($filterstr);//die;
    //         // $date_filterstr = $this->oo_filter($date_filter, $records='', $orderby='');     
    //         $q = "SELECT location_3.id as  stateid , location_3.name as statename from location_3 $filterstr limit 1 ";
    //     //  echo $role_id ; //die;
    //        // $filterstr=$filterstr.' AND '.$rol_fltr." AND ".$isr_fltr." AND ".$role_id;
    ...
    //             $id = $row['stateid'];
    //             $out[$id]['State'] = $row['statename'];
    //              $out[$id]['details'] = $this->get_time_report_details($filterstr,$start,$end ,$person_status,$role_id);
    //            // $out[$id]['details'] = $this->get_time_report_details($filterstr,$start,$end);
    //         }
    //       //pre($out);
    //         return $out;
        }
        ...
        public function get_time_report_details($filterstr,$start,$end,$person_status,$role_id) {
           // echo $filterstr;//die;
            global $dbc;
            $out = array();
            $q = "SELECT emp_code,person.role_id as role,CONCAT_WS(' ',person.first_name,person.middle_name,person.last_name)as user_name,person.id as p_id,location_3.name as statename,person_id_senior as pis 
                FROM person INNER JOIN person_login ON person_login.person_id=person.id  
                INNER JOIN location_3 ON location_3.id=person.state_id
                 INNER JOIN _role ON _role.role_id= person.role_id  $filterstr $person_status AND  $role_id GROUP BY person.id ORDER BY statename ASC";
             //h1($q);//die;
            list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
            if (!$opt)
                return $out;
        ...
        public function get_total_attendance($person,$start,$end) {
            ...
            $q = "select (SELECT COUNT(id) from user_daily_attendance where user_id = '$person' AND DATE_FORMAT(work_date,'%Y%m%d') >= '$start' AND DATE_FORMAT(work_date,'%Y%m%d') <= '$end') as attendance, "
                    ...
                    . "(SELECT COUNT(id) from user_daily_attendance where user_id = '$person' AND DATE_FORMAT(work_date,'%Y%m%d') >= '$start' AND DATE_FORMAT(work_date,'%Y%m%d') <= '$end' AND work_status IN(1,19,9)) as working,"//RETAILING ,PROSPECTING
                    ...
                    . "(SELECT COUNT(id) from user_daily_attendance where user_id = '$person' AND DATE_FORMAT(work_date,'%Y%m%d') >= '$start' AND DATE_FORMAT(work_date,'%Y%m%d') <= '$end' AND work_status=15) as training,"//INTERVIEW TRAINING
                    . "(SELECT COUNT(id) from user_daily_attendance where user_id = '$person' AND DATE_FORMAT(work_date,'%Y%m%d') >= '$start' AND DATE_FORMAT(work_date,'%Y%m%d') <= '$end' AND work_status=5) as meeting,"//MEETING 
                    . "(SELECT COUNT(id) from user_daily_attendance where user_id = '$person' AND DATE_FORMAT(work_date,'%Y%m%d') >= '$start' AND DATE_FORMAT(work_date,'%Y%m%d') <= '$end' AND work_status=13) as woff,"//WEEKLY OFF
                    . "(SELECT COUNT(id) from user_daily_attendance where user_id = '$person' AND DATE_FORMAT(work_date,'%Y%m%d') >= '$start' AND DATE_FORMAT(work_date,'%Y%m%d') <= '$end' AND work_status=17) as holiday";//HOLIDAY
            //h1($q);
            $q_res = mysqli_query($dbc, $q);
            while ($q_row = mysqli_fetch_array($q_res)) {
                //pre($q_row);
...
################################ sales man secondary###############################################
    ...
    public function get_sale_summary_report_list($filter = '', $records = '', $orderby = '', $date_filter = '') {
        $holder = array();
        global $dbc;
        $out = array();
        $filterstr = $this->oo_filter($filter, $records, $orderby);
        ...
        $q="SELECT user_sales_order.user_id, _role.rolename,emp_code AS ecod,
            l5_name AS name5,
            l5_id AS location_5_id,
            SUM(rate * quantity)as total_sale,sum(quantity) quantity,
            l4_name as name4,
            l3_name as name3,
            l2_name as name2,
            l1_name as name1,
            c1_name,c2_name,product_name,weight,unit,
            DATE_FORMAT(date,'%d-%m-%Y') as dates,
            DATE_FORMAT(date,'%Y-%m-%d') as `date`,user_sales_order.time,
            user_sales_order.id AS uniq, 
            user_sales_order.dealer_id AS dealer_id,person_id_senior,
            CONCAT_WS(' ',first_name, middle_name, last_name) AS person_name,
            dealer.name AS dealer_name,dealer_code,user_sales_order.location_id AS usolid,
            user_sales_order.dealer_id AS did,retailer.id AS retailer_code,retailer.name AS retailer_name,retailer.outlet_type_id  
            FROM user_sales_order 
            ...
            INNER JOIN dealer ON dealer.id = user_sales_order.dealer_id
            LEFT JOIN retailer ON user_sales_order.retailer_id = retailer.id             
            LEFT JOIN location_view lv ON lv.l5_id=user_sales_order.location_id  $filterstr  GROUP BY user_sales_order.`date`,user_sales_order.user_id,user_sales_order.dealer_id,user_sales_order.retailer_id,user_sales_order.location_id  
                ORDER BY user_sales_order.`date` ASC,l1_name ASC,l5_name ASC,person_name ASC" ;
       //h1($q);
        list($opt, $rs) = run_query($dbc, $q, 'multi');
        if (!$opt)
            return $out;
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $out[$id]['senior_person_name'] = myrowval($table = "person", $col = "CONCAT_WS(' ', first_name,middle_name,last_name)", $where = "id = '$row[person_id_senior]'");
            $out[$id]['outlet_name'] = myrowval($table = "_retailer_outlet_type", $col = "outlet_type", $where = "id = '$row[outlet_type_id]'");
            $weights=ROUND(($row[quantity]*$row[weight])/1000,2);
            $out[$id]['weights'] =$weights;
            $units=ROUND(($row[quantity]/$row[unit]),2);
            $out[$id]['case_qty'] =$units;
            //$out[$id]['total_sale'] = $this->get_total_sale($row['user_id'],$row['date'], $row['did']);
        }
        //  pre($out);
        return $out;
...
//        }
    ...
    public function get_rds_report_list($filter = '', $records = '', $orderby = '', $date_filter = '') {
        ...
        $q = "SELECT DATE_FORMAT(`date`,'%Y-%m-%d') as `date`,DATE_FORMAT(`date`,'%d-%m-%Y') as `show_date`,
            user_sales_order.id AS uniq,lv.l3_name as state,lv.l4_name as city,l4_id,
    ...
    public function get_user_monthlysale_ord_list($filter = '', $records = '', $orderby = '') {
        ...
        $q = "SELECT uso.* , lv.l5_name as beat ,lv.l4_name as town , lv.l3_name as city ,lv.l2_name as state , CONCAT_WS(' ',name, other_numbers) AS dealer_name FROM user_sales_order as uso INNER JOIN person ON person.id=uso.user_id INNER JOIN location_view lv ON lv.l5_id=uso.location_id INNER JOIN dealer ON dealer.id=uso.dealer_id"        
        . " $filterstr";
 //  h1($q);//exit;
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
        if (!$opt)
...
//
         ...
         $q = " SELECT damage_replace.id AS did,
            ...
            extra_amt,
            CONCAT_WS(' ', person.first_name,person.middle_name,person.last_name) as person_fullname,
            dealer.name  FROM person 
            INNER JOIN damage_replace ON person.id = damage_replace.user_id 
            INNER JOIN dealer ON damage_replace.dis_code=dealer.id 
            INNER JOIN location_view ON location_view.l3_id=person.state_id 
...
public function get_damage_replace_retailer_report_list($filter = '', $records = '', $orderby = '', $last_level_loc = '', $lpc_filter = '', $date_filter = '') {
        global $dbc;
        $out = array();
        $filterstr = $this->oo_filter($filter, $records, $orderby);
        // $date_filterstr = $this->oo_filter($date_filter, $records='');
...
//
         ...
         $q = " SELECT damage_replace_retailer.id AS did,
            damage_replace_retailer.replaceid AS s_no,
            damage_replace_retailer.user_id, prod_code,
            replaceid, prod_qty, prod_value, 
            DATE_FORMAT(date_time,'%Y-%m-%d')as date_time, 
            task, 
            mrp, 
            ...
            CONCAT_WS(' ', person.first_name,person.middle_name,person.last_name) as person_fullname,
            dealer.name  FROM person 
            INNER JOIN damage_replace_retailer ON person.id = damage_replace_retailer.user_id 
            INNER JOIN dealer ON damage_replace_retailer.dis_code=dealer.id 
            INNER JOIN location_view ON location_view.l3_id=person.state_id 
 ...
 public function get_dealer_payment_report_list($filter = '', $records = '', $orderby = '', $last_level_loc = '', $lpc_filter = '', $date_filter = '') {
        global $dbc;
        $out = array();
        $filterstr = $this->oo_filter($filter, $records, $orderby);
        // $date_filterstr = $this->oo_filter($date_filter, $records='');
//                h1($filterstr);
         ...
         $q = " SELECT date_format(payment_recevied_date,'%d-%m-%Y') as payment_date, dealer.name as dealername, CONCAT_WS('',first_name,middle_name,last_name) as username ,dealer_payments.* 
         FROM `dealer_payments` 
INNER JOIN dealer ON dealer.id=dealer_payments.dealer_id
INNER JOIN dealer_location_rate_list as dlrl ON dlrl.dealer_id=dealer.id
INNER JOIN person ON person.id=dealer_payments.user_id 
INNER JOIN location_view as lv on lv.l5_id=dlrl.location_id  $filterstr";
       ...
       //h1($q);
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $id = $row['id'];
            $out[$id] = $row;
            $out[$id]['payment_modes']=myrowval('_payment_modes','mode','id='.$row['payment_mode']);
...
#################retailer Reports ##############
 ...
 public function get_retailer_payment_report_list($filter = '', $records = '', $orderby = '', $last_level_loc = '', $lpc_filter = '', $date_filter = '') {
        global $dbc;
        $out = array();
        $filterstr = $this->oo_filter($filter, $records, $orderby);

         $q = " SELECT date_format(payment_date,'%d-%m-%Y') as payment_date, dealer.name as dealername,retailer.name as retailername ,CONCAT_WS('',first_name,middle_name,last_name) as username ,payment_collect_retailer.* FROM `payment_collect_retailer` 
INNER JOIN dealer ON dealer.id=payment_collect_retailer.dealer_id
INNER JOIN retailer ON retailer.id=payment_collect_retailer.tr_code
INNER JOIN person ON person.id=payment_collect_retailer.user_id 
INNER JOIN location_view as lv on lv.l5_id=retailer.location_id  $filterstr";
...
//                . " INNER JOIN location_view AS lv ON lv.l5_id $filterstr GROUP BY date,product_id" ;
        $q="SELECT usod.id as id,usod.rate as rate,SUM(usod.quantity) as quantity,usod.product_id as product_id,uso.location_id as location_id,cv.product_name AS sku,"
                . " cv.c1_name AS category,lv.l4_name AS region,lv.l3_name AS state"
...
///////////////////////////////////////CLOSE RETAILER CAIM////////////////////////////////////////////////
  public function get_dropped_outlet_list($filter = '', $records = '', $orderby = '', $date_filter = '',$lastedate_filter ='',$rdate_filter='',$usodate='',$ldate = '',$smonth='') {
            ...
            //  h1($up);
             $r11 = mysqli_query($dbc, $up);
                ...
                $q="SELECT person.id AS pid,CONCAT_WS(' ',first_name, middle_name, last_name) AS person_fullname, 
                    person.emp_code,person.person_id_senior,person.state_id,lv.l3_name AS state FROM person  
                     INNER JOIN person_login ON person_login.person_id = person.id AND person_status ='1'   
                     INNER JOIN location_view as lv ON lv.l3_id=person.state_id 
                     $filternew";
    ...
    public function get_dropped_sku_list($filter = '', $records = '', $orderby = '',$lastedate_filter = '',$rdate_filter = '',$usodate ='',$filterlast='',$smonth) {
      ...
      public function get_person_name($id,$filterstr,$lastedate_filter,$rdatefilter,$usodate,$filter_last) {
        global $dbc; 
          $out =array();
        //$pre_mth = Date("Y-m", strtotime($date . " last month"));
        $pid=$id;
        $q="SELECT DISTINCT(CONCAT_WS(' ',first_name, middle_name, last_name)) AS person_fullname,person.id as pid,usod.product_id AS id
            ,lv.l3_name AS state FROM user_sales_order AS uso      
            INNER JOIN person ON person.id = uso.user_id
            INNER JOIN person_login ON person_login.person_id = person.id AND person_status ='1'   
            INNER JOIN user_sales_order_details AS usod ON usod.order_id=uso.order_id 
            INNER JOIN location_view ON lv.l3_id=person.state_id
            $filterstr AND usod.product_id='$id' 
                
            UNION 
            
            SELECT DISTINCT(CONCAT_WS(' ',first_name, middle_name, last_name)) AS person_fullname,person.id as pid,usod.product_id AS id
            ,state.statename AS state FROM user_sales_order AS uso      
            INNER JOIN person ON person.id = uso.user_id
            INNER JOIN person_login ON person_login.person_id = person.id AND person_status ='1'   
            INNER JOIN user_sales_order_details AS usod ON usod.order_id=uso.order_id 
            INNER JOIN state ON state.stateid=person.state_id
            $filter_last AND usod.product_id='$id'";
       ...
       $rs = mysqli_query($dbc,$q);
        ...
        while($row=  mysqli_fetch_assoc($rs)){
            $id=$row['pid'];
            $out[$id]['pname']=$row['person_fullname'];         
            $out[$id]['state']=$row['state']; 
            $out[$id]['pre_outlet_billed']= $this->get_sku_pre_outlet_billed($row[pid],$lastedate_filter,$pid);
            $out[$id]['new_outlet_billed']= $this->get_sku_new_outlet_billed($row[pid],$rdatefilter,$pid,$usodate);
            $out[$id]['total_outlet_billed']= $this->get_sku_total_outlet_billed($row[pid],$usodate,$pid,$rdatefilter);
            $out[$id]['sku_dropped_outlet']= $this->get_sku_dropped_outlet($row[pid],$pid);
        }
   ...
   //     pre($out);
      ...
      public function get_sku_dropped_outlet($userid,$pid) {
        global $dbc;
        $out = NULL;
         $uid = $_SESSION[SESS . 'data']['id'];
        //$pre_mth = Date("Y-m", strtotime($date . " last month"));
        
       $q="select count(distinct retailer_id) AS sku_dropped_outlet FROM _temp_dropped_sku_outlet "
               . " where user_id='$uid' AND product_id='$pid' AND pid='$userid' AND dropped=1";
      // h1($q); //exit;
        list($opt, $rs) = run_query($dbc, $q, $mode = 'single', $msg = '');
        if (!$opt)
            return $out;
        return $rs['sku_dropped_outlet'];
    }
    
     public function get_sku_pre_outlet_billed($id,$lastedate_filter,$pid) {
        global $dbc;
        $out = NULL;
        //$pre_mth = Date("Y-m", strtotime($date . " last month"));
        
       $q="select count(distinct uso.retailer_id) AS pre_outlet_billed FROM user_sales_order AS uso "
               . "INNER JOIN user_sales_order_details  AS usod ON usod.order_id= uso.order_id $lastedate_filter AND uso.user_id=$id "
               . "AND usod.product_id='$pid'";
      // h1($q); //exit;
        list($opt, $rs) = run_query($dbc, $q, $mode = 'single', $msg = '');
        if (!$opt)
            return $out;
    ...
    public function get_sku_new_outlet_billed($id,$rdatefilter,$pid,$usodate) {
        global $dbc;
        $out = NULL;
       ...
       $q="SELECT count(distinct uso.retailer_id) AS new_outlet_billed FROM retailer INNER JOIN user_sales_order AS uso ON uso.retailer_id=retailer.id "
               . "INNER JOIN user_sales_order_details AS usod ON usod.order_id= uso.order_id $usodate  "
               . "AND uso.user_id=$id AND usod.product_id='$pid' AND $rdatefilter";
      // h1($q); exit;
        list($opt, $rs) = run_query($dbc, $q, $mode = 'single', $msg = '');
        if (!$opt)
            return $out;
    ...
    public function get_sku_total_outlet_billed($id,$filter_date,$pid,$rdatefilter) {
        global $dbc;
        $out = NULL;
       ...
       $q="SELECT count(distinct uso.retailer_id) AS total_outlet_billed FROM user_sales_order AS uso "
               . "INNER JOIN user_sales_order_details AS usod ON usod.order_id= uso.order_id $filter_date AND uso.user_id=$id "
               . "AND usod.product_id='$pid' and uso.retailer_id not in (select id from retailer where $rdatefilter)";
      // h1($q); exit;
        list($opt, $rs) = run_query($dbc, $q, $mode = 'single', $msg = '');
        if (!$opt)
            return $out;
 ...
 /*************manpower_performance*******Start*******New REport 1st Aug 2017************************/
     public function get_manpower_performance_list($filter = '', $records = '', $orderby = '', $uso_filter = '', $mtp_filter = '',$outlet_filter = '',$att_filter = '',$serdt = '',$chdt = '') {
        global $dbc;
         $uid = $_SESSION[SESS . 'data']['id'];
        $out = array();
        $holder = array();  
        $filterstr = $this->oo_filter($filter, $records, $orderby);
        $filteruso = $this->oo_filter($uso_filter, $records, $orderby);
        $filtermtp = $this->oo_filter($mtp_filter, $records, $orderby);
        $filteroutlet = $this->oo_filter($outlet_filter, $records, $orderby);
        $filteratt = $this->oo_filter($att_filter, $records, $orderby);       
        
        
        
       // print_r($filterstr); die;
        $state=$filter[0];
        $del="DELETE FROM `_temp_manpower_performance` WHERE user_id='$uid'";
        $rdel = mysqli_query($dbc, $del);
        
//         $del1="DELETE FROM `_temp_manpower_performance_details` WHERE user_id='$uid'";
//        $rdel1 = mysqli_query($dbc, $del1);
        
        if($rdel){
            $qdr = "INSERT INTO _temp_manpower_performance(user_id,pid,person_fullname,designation,emp_code,
                 person_id_senior,senior_person_name,state_id,state) 
                 (SELECT $uid,person.id AS pid,CONCAT_WS(' ',first_name, middle_name, last_name) AS person_fullname,_role.rolename, 
                    person.emp_code,person.person_id_senior,(select CONCAT_WS(' ', p.first_name,p.middle_name,p.last_name) from person p where p.id=person.person_id_senior) as senior_person_name,
                     location_view.l3_id ,location_view.l3_name AS state "                 
                   . " FROM person 
                     INNER JOIN person_login ON person_login.person_id = person.id AND person_status ='1'                     
                     INNER JOIN location_view ON location_view.l3_id = person.state_id AND person_status ='1'       
                    INNER JOIN _role ON _role.role_id = person.role_id  $filterstr)";
       // h1($qdr);
            $rdr = mysqli_query($dbc, $qdr);
         // echo "<br/>";
         // "<br/>"; 
//            if($rdel1){
//        echo $inqry = "Insert into _temp_manpower_performance_details(user_id,pid,achieved,total_call,productive,opv,line)"
//                 . "(select $uid,tmp.pid,(select sum(uso.total_sale_value) AS achieved from user_sales_order AS uso "
//                 . "  $filteruso AND uso.user_id=tmp.pid) as achieved,(SELECT count(call_status) as count_tot "
//                   . "FROM user_sales_order uso $filteruso AND uso.user_id = tmp.pid) as count_tot,(SELECT count(call_status) as count_tot "
//                   . "FROM user_sales_order uso $filteruso AND uso.user_id = tmp.pid  "
//                   . " AND uso.call_status ='1') as p_total,(SELECT count(uso.id) AS opv "
//                   . "FROM user_sales_order AS uso $filteruso AND uso.user_id=tmp.pid ) as opv,"
//                   . "(SELECT count(usod.id) AS line FROM user_sales_order AS uso "
//                   . "INNER JOIN user_sales_order_details AS usod ON uso.order_id=usod.order_id  "
//                   . "$filteruso AND uso.user_id=tmp.pid ) as line"
//                   . " from _temp_manpower_performance tmp where tmp.user_id='$uid')";
//            $rdr1 = mysqli_query($dbc, $inqry); 
//            }
     //    echo "<br/>";
     //    echo "<br/>"; 
        // echo "'-------'.achieved.'----------'";
           $upac = "update _temp_manpower_performance tmp set tmp.achieved=(SELECT sum(rate*quantity) AS achieved "
                   . "FROM user_sales_order AS uso INNER JOIN user_sales_order_details usod using(order_id) $filteruso AND uso.user_id=tmp.pid "
                   . "AND tmp.user_id='$uid' GROUP BY uso.user_id)";
           $rupac = mysqli_query($dbc, $upac); 
           
       //  echo "<br/>"; 
       //   echo "<br/>"; 
       //  echo "'-------'.target.'----------'";
           $upmt = "update _temp_manpower_performance tmp set tmp.target=(SELECT sum(mtp.total_sales) AS target "
                   . "FROM monthly_tour_program AS mtp $filtermtp AND mtp.person_id=tmp.pid "
                   . "AND tmp.user_id='$uid' GROUP BY mtp.person_id)";
           $rupmt = mysqli_query($dbc, $upmt); 
           
        // echo "<br/>"; 
      //    echo "<br/>"; 
      //   echo "'-------'.total_call.'----------'";
           $uptot = "update _temp_manpower_performance tmp set tmp.total_call=(SELECT count(call_status) as count_tot "
                   . "FROM user_sales_order uso $filteruso AND uso.user_id = tmp.pid  "
                   . "AND tmp.user_id='$uid')";
           $ruptot = mysqli_query($dbc, $uptot); 
           
      //   echo "<br/>"; 
     //     echo "<br/>"; 
    //   echo "'-------'.productive.'----------'";
           $uppr = "update _temp_manpower_performance tmp set tmp.productive=(SELECT count(call_status) as count_tot "
                   . "FROM user_sales_order uso $filteruso AND uso.user_id = tmp.pid  "
                   . "AND tmp.user_id='$uid' AND uso.call_status ='1')";
           $ruppr = mysqli_query($dbc, $uppr); 
           
           
        //  echo "<br/>";
      //     echo "<br/>"; 
      //    echo "'-------'.outlet_value.'----------'";
           $upov = "update _temp_manpower_performance tmp set tmp.outlet_value=(SELECT sum(usod.rate*usod.quantity - uso.discount) AS outlet_value "
                   . "FROM retailer LEFT JOIN user_sales_order AS uso ON uso.retailer_id=retailer.id "
                   . "LEFT JOIN user_sales_order_details usod using (order_id) $filteroutlet $serdt AND uso.user_id=tmp.pid "               
                   . "AND tmp.user_id='$uid' GROUP BY uso.user_id)";
          $rupov = mysqli_query($dbc, $upov);  
           
      //   echo "<br/>";  
     //     echo "<br/>"; 
      //   echo "'-------'.outlet_billed.'----------'";
           $upob = "update _temp_manpower_performance tmp set tmp.outlet_billed=(SELECT count(id) AS outlet_billed "
                   . "FROM retailer $filteroutlet AND retailer.created_by_person_id =tmp.pid "
                   . " AND tmp.user_id='$uid' GROUP BY retailer.created_by_person_id)";
           $rupob = mysqli_query($dbc, $upob); 
           
       //   echo "<br/>";  
       //    echo "<br/>"; 
       //   echo "'-------'.opv.'----------'";
           $upopv = "update _temp_manpower_performance tmp set tmp.opv=(SELECT count(uso.id) AS opv "
                   . "FROM user_sales_order AS uso $filteruso AND uso.user_id=tmp.pid "
                   . "AND tmp.user_id='$uid')";
           $rupopv = mysqli_query($dbc, $upopv);  
           
      //   echo "<br/>"; 
     //     echo "<br/>"; 
     //    echo "'-------'.line.'----------'";
           $upline = "update _temp_manpower_performance tmp set tmp.line=(SELECT count(usod.id) AS line FROM user_sales_order AS uso "
                   . "INNER JOIN user_sales_order_details AS usod ON uso.order_id=usod.order_id  $filteruso AND uso.user_id=tmp.pid "
                   . " AND tmp.user_id='$uid')";
           $rupline = mysqli_query($dbc, $upline); 
           
       //   echo "<br/>";  
       //    echo "<br/>"; 
      //    echo "'-------'.man_attd.'----------'";
           $upman = "update _temp_manpower_performance tmp set tmp.man_attd=(SELECT count(user_daily_attendance.id) AS man_attd "
                   . "FROM user_daily_attendance $filteratt AND work_status IN (1,9,12,13,14) AND user_daily_attendance.user_id=tmp.pid "
                   . "AND tmp.user_id='$uid')";
           $rupman = mysqli_query($dbc, $upman); 
           
           
           $upman = "update _temp_manpower_performance tmp set tmp.total_leave=(SELECT count(user_daily_attendance.id) AS total_leave "
                   . "FROM user_daily_attendance $filteratt AND work_status IN (1,9,13,14) AND user_daily_attendance.user_id=tmp.pid "
                   . "AND tmp.user_id='$uid')";
           $rupman = mysqli_query($dbc, $upman);

           $upsku = "update _temp_manpower_performance tmp set tmp.sku_booked=(SELECT count(usod.product_id) AS sku_booked FROM user_sales_order AS uso "
                              . "INNER JOIN user_sales_order_details AS usod ON uso.order_id=usod.order_id  $filteruso AND uso.user_id=tmp.pid "
                              . " AND tmp.user_id='$uid')";
                      //h1($upsku);exit;
                      $rupsku = mysqli_query($dbc, $upsku);
                       
                      $upvar = "update _temp_manpower_performance tmp set tmp.varient_booked=(SELECT count(cv.c2_id) AS varient_booked FROM user_sales_order AS uso "
                              . "INNER JOIN user_sales_order_details AS usod ON uso.order_id=usod.order_id INNER JOIN catalog_view AS cv ON usod.product_id=cv.product_id  $filteruso AND uso.user_id=tmp.pid "
                              . " AND tmp.user_id='$uid')";
                      $rupvar = mysqli_query($dbc, $upvar);
           
            
        } 
        
        $q="SELECT * FROM _temp_manpower_performance WHERE user_id='$uid'";                 
      //  h1($q);       
        $rs = mysqli_query($dbc, $q);  
    ...
    /*************manpower_performance*******End*******New REport 1st Aug 2017************************/
    
    ###################MTP Details######################
    public function get_monthly_tour_plan($to_date,$val)
    {
          global $dbc;
          $out = array();
         $query = "SELECT monthly_tour_program.id AS id,monthly_tour_program.*, CONCAT_WS(' ',person.first_name, person.middle_name, person.last_name) AS personname, 
             ...
             DATE_FORMAT(working_date, '%d/%m/%Y') AS working_date FROM `monthly_tour_program` 
 		     INNER JOIN person ON person.id=monthly_tour_program.person_id WHERE person_id=$val AND date_format(working_date,'%Y-%m') = '$to_date' ";
             //h1($query);
...
/*****************************************05-10-2017**************************************************/
        ...
        public function get_sku_details_report_list($filter = '', $records = '', $orderby = '') {
        $holder = array();
        global $dbc;
        $out = array();
        $filterstr = $this->oo_filter($filter, $records, $orderby);       

        $q="SELECT cod.product_id as product_id,cv.product_name as sku,SUM(cod.taxable_amt) as amount
                FROM challan_order_details AS cod 
                INNER JOIN challan_order AS co ON co.id = cod.ch_id 
                INNER JOIN catalog_view AS cv ON cv.product_id  = cod.product_id "
                . "$filterstr GROUP BY cod.product_id" ;
   // h1($q);
        list($opt, $rs) = run_query($dbc, $q, 'multi');
        if (!$opt)
            return $out;
       ...
       public function get_retailer_details_report_list($filter = '', $records = '', $orderby = '') {
        $holder = array();
        global $dbc;
        $out = array();
        $filterstr = $this->oo_filter($filter, $records, $orderby);       

        $q="SELECT co.ch_retailer_id as retailer_id,retailer.name as rname,SUM(co.amount) as amount 
            FROM challan_order AS co 
            INNER JOIN retailer ON retailer.id = co.ch_retailer_id "
                . "$filterstr GROUP BY co.ch_retailer_id" ;
    h1($q);
        list($opt, $rs) = run_query($dbc, $q, 'multi');
        if (!$opt)
            return $out;
  ...
  ###################### Dealer billing report class################################

public function get_dealer_billing_report_list($filter = '', $records = '', $orderby = '') {
global $dbc;
$filterstr = $this->oo_filter($filter, $records);
$out = NULL;

$q = "SELECT d.id as dealer_id,d.name AS rds_name,dealer_code,csa_id,csa_name AS csa, lv.l2_name AS state, lv.l4_name as town from dealer d INNER JOIN dealer_location_rate_list dlrl ON dlrl.dealer_id=d.id 
INNER JOIN location_view lv ON dlrl.location_id=lv.l5_id LEFT JOIN csa ON csa.c_id=d.csa_id LEFT JOIN  dealer_person_login AS dpr ON dpr.dealer_id=d.id 
$filterstr AND dpr.lastvisit!='NULL'  GROUP BY d.id order by lv.l2_name";
// h1($q);
list($opt, $rs) = run_query($dbc, $q, 'multi');
if (!$opt)
return $out;
...
$out[$id][bill_cut]= myrowval('challan_order','count(ch_no)', $cond);
$out[$id][billing_amount]= myrowval('challan_order','sum(amount)', $cond);
//$out[$id]['dealer_code']=$row['dealer_code']; 
$cond1="dealer_id=".$dealer_id." AND DATE_FORMAT(`date`,'%Y%m%d')>='".$start."' AND DATE_FORMAT(`date`,'%Y%m%d')<='".$end."'";

$out[$id][sale_order_value]= myrowval('user_sales_order','sum(amount)', $cond1);
$cond2="dealer_id=".$dealer_id." AND DATE_FORMAT(`date`,'%Y%m%d')>='".$start."' AND DATE_FORMAT(`date`,'%Y%m%d')<='".$end."' AND product_id='122'";
...
public function get_dms_lite_report_list($filter = '', $records = '', $orderby = '') {
global $dbc;
$filterstr = $this->oo_filter($filter, $records);
$out = NULL;

$q = "SELECT dpl.uname,d.id as dealer_id,d.name AS rds_name,dealer_code,csa_id,csa_name AS csa, lv.l2_name AS state, lv.l4_name as town,dlrl.user_id,DATE_FORMAT(dpl.lastvisit,'%d-%m-%Y') as web from dealer d 
INNER JOIN dealer_location_rate_list dlrl ON dlrl.dealer_id=d.id 
INNER JOIN dealer_person_login dpl ON dpl.dealer_id =d.id
INNER JOIN location_view lv ON dlrl.location_id=lv.l5_id 
LEFT JOIN csa ON csa.c_id=d.csa_id 
...
$uname=$row[uname];
$out[$id] = $row;
$cond4="person_username='".$uname."'";
$user_id=myrowval('person_login','person_id',$cond4);
$cond3="person_id=".$user_id;
$out[$id][lite]= myrowval('person_login',"DATE_FORMAT(last_mobile_access_on,'%d-%m-%Y')", $cond3);
$cond="ch_dealer_id=".$dealer_id." AND DATE_FORMAT(`ch_date`,'%Y%m%d')>='".$start."' AND DATE_FORMAT(`ch_date`,'%Y%m%d')<='".$end."' AND ch_user_id='".$user_id."'";
//$out[$id][bill_cut]= myrowval('challan_order','count(ch_no)', $cond);
$out[$id][billing_amount]= myrowval('challan_order','sum(amount)', $cond);
//$out[$id]['dealer_code']=$row['dealer_code']; 
$cond1="dealer_id=".$dealer_id." AND DATE_FORMAT(`date`,'%Y%m%d')>='".$start."' AND DATE_FORMAT(`date`,'%Y%m%d')<='".$end."'  AND user_id='".$user_id."'";
...
return $out;
  ...
  ###########################################  
    public function get_sale_order_list_report($filter = '', $records = '', $orderby = '') {
            ...
            $q = "SELECT *,location_2.name as state,location_5.name as beat,DATE_FORMAT(date,'%d/%m/%Y') AS sale_date,
            DATE_FORMAT(time,'%d/%m/%Y') AS sale_time,DATE_FORMAT(date,'%d/%b/%Y') AS fdated FROM user_sales_order "
    ...
    public function get_retailer_stock_list($filter = '', $records = '', $orderby = '') {
        ...
        $q = "SELECT retailer_stock.*,l3_name as state,l5_name as beat,DATE_FORMAT(date,'%d/%m/%Y') AS sale_date FROM retailer_stock  "
                . "INNER JOIN location_view ON retailer_stock.location_id=location_view.l5_id "
                . "$filterstr GROUP BY order_id";
        // h1($q); //exit;
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
        if (!$opt)
            return $out;
...
#######################dealer Stock#################
 ##################################################
    public function get_dealer_stocks_list($filter = '', $records = '', $orderby = '') {
        global $dbc;
        $out = array();
        $filterstr = $this->oo_filter($filter, $records, $orderby);
        $mtype = $_SESSION[SESS . 'constant']['retailer_level'];

        $q = "SELECT  dealer_balance_stock.*,l3_name as state,l5_name as beat,DATE_FORMAT(submit_date_time,'%d/%m/%Y') AS sale_date FROM dealer_balance_stock 
              INNER JOIN dealer_location_rate_list as dlrl ON dlrl.dealer_id=dealer_balance_stock.dealer_id
              INNER JOIN location_view as lv  ON dlrl.location_id=lv.l5_id "
                . "$filterstr GROUP BY order_id";
        // h1($q); //exit;
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
        if (!$opt)
       ...
       // $brand_map = get_my_reference_array('catalog_1', 'id', 'name');
        while ($row = mysqli_fetch_assoc($rs)) {
           ...
           // $out[$id]['firm_name'] = $retailer_map[$row['retailer_id']];
            $out[$id]['person_name'] = $this->get_username($row['user_id']);
            $qrd="SELECT dsb.id AS id,cp.product_name AS name, dsb.stock_qty as quantity,dsb.mrp as base_price,dsb.product_id FROM dealer_balance_stock as dsb INNER JOIN catalog_view cp ON dsb.product_id=cp.product_id"
                    . " WHERE user_id ='$row[user_id]' AND dsb.order_id = $row[order_id] ";
        //  h1($qrd);
          $out[$id]['order_item'] = $this->get_my_reference_array_direct($qrd, 'id');
        }// while($row = mysqli_fetch_assoc($rs)){ ends
       //pre($out);
...
return $out;
  ...
  function get_oneDayReport_list($filter = '', $records = '', $orderby = '')
    ...
    public function get_used_function_list($filter = '', $records = '', $orderby = '', $date = '') {
            global $dbc;
            $out = array();
            //$date_filter = join(' AND ', $date);
            $uso_date = "DATE_FORMAT(`date`,'" . MYSQL_DATE_SEARCH . "') = '$date'";
            ...
            // if user has send some filter use them.
            $filterstr = $this->oo_filter($filter, $records, $orderby);
            $q="SELECT person.id AS user_id,CONCAT_WS(' ',first_name, middle_name, last_name) AS person_name,rolename,l2_id,l2_name FROM person INNER JOIN person_login ON person.id=person_login.person_id INNER JOIN _role AS r ON r.role_id=person.role_id INNER JOIN location_view AS lv ON lv.l2_id=person.state_id $filterstr AND person_status='1' GROUP BY person.id ORDER BY l2_name";
             // h1($q);
            $rs = mysqli_query($dbc, $q);
            //if(!$opt) return $out; // if no order placed send blank array
            while ($row = mysqli_fetch_assoc($rs)) {
                ...
                $out[$id] = $row;
                //$out[$id]['senior'] = myrowval('person', 'person_fullname', 'id=' . $row[person_id_senior]);
                $out[$id]['attendance']= myrowval('user_daily_attendance', 'user_id', 'user_id=' . $row['user_id']);
                $out[$id]['check_out']= myrowval('check_out', 'user_id', 'user_id=' . $row['user_id']);
                $out[$id]['create_retailer']= myrowval('retailer', 'id', 'created_by_person_id=' . $row['user_id']);
                $out[$id]['complaint']= myrowval('complaint', 'user_id', 'user_id=' . $row['user_id']);
                $out[$id]['mtp']= myrowval('monthly_tour_program', 'person_id', 'person_id=' . $row['user_id']);
                $out[$id]['payment_collection']= myrowval('payment_enrollment', 'user_id', 'user_id=' . $row['user_id']);
                $out[$id]['retailer_stock']= myrowval('retailer_stock', 'user_id', 'user_id=' . $row['user_id']);
                $out[$id]['expense']= myrowval('user_expense_report', 'person_id', 'person_id=' . $row['user_id']);
                $out[$id]['order_booking']= myrowval('user_sales_order', 'user_id', 'user_id=' . $row['user_id']);
                $out[$id]['merchandise']= myrowvaljoin('merchandise','retailer_id','user_sales_order','user_sales_order.retailer_id=merchandise.retailer_id','user_id=' . $row['user_id']);
                $out[$id]['dealer_stock']= myrowval('dealer_balance_stock', 'user_id', 'user_id=' . $row['user_id']);
                $out[$id]['damage']= myrowval('damage_replace', 'user_id', 'user_id=' . $row['user_id']);
            }
            // pre($out);
            return $out;
        }

        public function get_product_wise_summary_new_list($filter='', $records = '', $orderby='')
                {
                    ...
                    $filterstr = $this->oo_filter($filter, $records, $orderby);
                    //print_r($filterstr);
                    $q="SELECT stock.id AS sid,qty,stock.product_id AS product_id,catalog_view.c2_name AS cat,catalog_view.product_name AS product_name,dealer.id AS did,dealer.dealer_code AS rds_code,dealer.name AS dname,lv.l2_id,lv.l2_name AS state,l4_name AS town "
                            . " FROM dealer LEFT JOIN dealer_location_rate_list AS dlrl ON dealer.id=dlrl.dealer_id LEFT JOIN location_view AS lv ON lv.l5_id=dlrl.location_id LEFT JOIN stock ON stock.dealer_id=dealer.id LEFT JOIN catalog_view ON catalog_view.product_id=stock.product_id $filterstr AND qty>0 GROUP BY l2_id,did,product_id,stock.mrp ORDER BY state,did,cat,dname,product_name";
                   // $q="SELECT stock.id AS id,catalog_product.id as product_id,catalog_product.name AS product_name,stock.qty AS qty,mrp,(qty*dealer_rate) AS value FROM stock INNER JOIN catalog_product ON catalog_product.id=stock.product_id INNER JOIN dealer_person_login ON dealer_person_login.dealer_id=stock.dealer_id  $filterstr ORDER BY name ";
                   ...
                   // $mrp=$row['mrp'];
                    $out[$id] = $row;
                    $cond="dealer_id=".$row['did']." AND product_id=".$row['product_id'];
                    $out[$id]['value']= myrowval('stock','(qty*dealer_rate)',$cond);
                    $cond1="ch_dealer_id=".$row['did']." AND DATE_FORMAT(`ch_date`,'%Y%m')='201712' AND product_id='$product_id'";
                    $cond2="ch_dealer_id=".$row['did']." AND DATE_FORMAT(`ch_date`,'%Y%m')='201801' AND product_id='$product_id'";
                   // $out[$id][billing_amount_dec]= myrowvaljoin('challan_order','sum(taxable_amt)','challan_order_details','challan_order.id=challan_order_details.ch_id', $cond1);
                   // $out[$id][billing_amount_jan]= myrowvaljoin('challan_order','sum(taxable_amt)','challan_order_details','challan_order.id=challan_order_details.ch_id', $cond2);
                    }
                    //pre($out);
                    return $out;
                    }

    public function get_stock_detail_summary_list($filter='', $records = '', $orderby='')    
    {
       global $dbc;
       $out = array(); 
       // if user has send some filter use them.
       $filterstr = $this->oo_filter($filter, $records, $orderby);
       //print_r($filterstr);
       $q="SELECT c2_name,s.id AS sid,qty,mrp,s.product_id AS product_id,ROUND(dealer_rate*qty,2) as dealerrate,catalog_view.product_name AS product_name,dealer.id AS did,dealer.name AS dname,lv.l2_id,lv.l2_name AS state,lv.l4_name as town FROM dealer 
       INNER JOIN dealer_location_rate_list AS dlrl ON dealer.id=dlrl.dealer_id 
       INNER JOIN location_view AS lv ON lv.l5_id=dlrl.location_id 
       INNER JOIN stock s ON s.dealer_id=dealer.id 
       INNER JOIN catalog_view ON catalog_view.product_id=s.product_id $filterstr AND dms_status=1 AND dealer_status=1 GROUP BY l2_id,did,product_id ORDER BY state,dname,product_name";
       //h1($q);
       list($opt, $rs) = run_query($dbc, $q, $mode='multi', $msg='');
       if(!$opt) return $out; // if no order placed send blank array
       $start = get_mysql_date($_POST['start'],'/',$time = false, $mysqlsearch = true);
       ...
       $mrp=$row['mrp'];
       $out[$id] = $row;
       $cond="ch_dealer_id=".$row['did']." AND product_id=".$row['product_id']." AND date_format(ch_date,'%Y%m%d')>='".$start."' AND date_format(ch_date,'%Y%m%d')<='".$end."'";
       
       $cond_dec="ch_dealer_id=".$row['did']." AND product_id=".$row['product_id']." AND date_format(ch_date,'%Y%m')>='201712'";
       $cond_jan="ch_dealer_id=".$row['did']." AND product_id=".$row['product_id']." AND date_format(ch_date,'%Y%m')>='201801'";
       $cond_feb="ch_dealer_id=".$row['did']." AND product_id=".$row['product_id']." AND date_format(ch_date,'%Y%m')>='201802'";

       $out[$id]['value']= myrowvaljoin('challan_order_details','SUM(qty)','challan_order','challan_order_details.ch_id=challan_order.id',$cond);

       //$out[$id]['value_dec']= myrowvaljoin('challan_order_details','SUM(taxable_amt)','challan_order','challan_order_details.ch_id=challan_order.id',$cond_dec);
       //$out[$id]['value_jan']= myrowvaljoin('challan_order_details','SUM(taxable_amt)','challan_order','challan_order_details.ch_id=challan_order.id',$cond_jan);
       //$out[$id]['value_feb']= myrowvaljoin('challan_order_details','SUM(taxable_amt)','challan_order','challan_order_details.ch_id=challan_order.id',$cond_feb);

       }
       return $out;
    }
    function get_twoDayReport_list($filter = '', $records = '', $orderby = '')
    {
         global $dbc;
         ...
         if(!empty($_POST['product_id'])){
          $pro=implode(',',$_POST['product_id']);
          $product_id ="usod.product_id IN($pro)";
          $product_id1 ="cod.product_id IN($pro)";
          }else{
            $product_id ="usod.product_id IN(0)";
            $product_id1 ="cod.product_id IN(0)";
          }
          // $q1="SELECT l2_id,cod.order_id from challan_order_details cod INNER JOIN challan_order co ON co.id=cod.ch_id INNER JOIN dealer_location_rate_list dlrl ON dlrl.dealer_id=co.ch_dealer_id INNER JOIN location_view lv ON lv.l5_id=dlrl.location_id $filterstr group by order_id";
          // h1($q1);
        ...
        // h1(Helllooo);
         ...
         $sid = implode(',',$_POST['id']);
                $cond1="where DATE_FORMAT(`date`,'%Y%m%d')>='".$start."' AND DATE_FORMAT(`date`,'%Y%m%d')<='".$end."' AND $product_id AND l2_id IN($sid)";
                $q = "SELECT uso.order_id,usod.product_id,cv.product_name as sku,cv.c2_name as vname,uso.order_id AS order_id,user_id,retailer_id,l2_name as state, l2_id, l5_name as beat,DATE_FORMAT(date,'%d/%m/%Y') AS sale_date,DATE_FORMAT(time,'%d/%m/%Y') AS sale_time,DATE_FORMAT(date,'%d/%b/%Y') AS fdated,dealer.name AS name,dealer.csa_id 
         FROM user_sales_order uso 
         INNER JOIN user_sales_order_details usod ON uso.order_id=usod.order_id 
         INNER JOIN catalog_view cv ON usod.product_id=cv.product_id 
         INNER JOIN dealer ON dealer.id=uso.dealer_id  
         INNER JOIN location_view ON uso.location_id=l5_id "
                 . "$cond1  GROUP BY sale_date,dealer.id,retailer_id,usod.product_id ORDER BY state ASC";
         //h1($q);      
         $rs = mysqli_query($dbc,$q);
         $bi=1;
         while ($row = mysqli_fetch_assoc($rs)) {
           ...
           //  $out[$id]['name'] = $dealer_map[$row['dealer_id']];
             $out[$id]['firm_name'] = myrowval('retailer','name',"id=".$row['retailer_id']);
             //$out[$id]['person_name'] = myrowval('person',"CONCAT_WS(' ',first_name,middle_name,last_name)","id=".$row['user_id']);
             $out[$id]['person_name'] = $this->get_username($row['user_id']);
             $sdate=$row['sale_date'];
             $cond2="retailer_id=".$row['retailer_id']." AND DATE_FORMAT(`date`,'%d/%m/%Y')='".$sdate."' AND product_id=".$product_id;
             $out[$id][sale_order_value]= myrowvaljoin('user_sales_order_details','sum(rate*quantity)','user_sales_order','user_sales_order.order_id=user_sales_order_details.order_id',$cond2);
            
             $out[$id][sale_order_qty]= myrowvaljoin('user_sales_order_details','sum(quantity)','user_sales_order','user_sales_order.order_id=user_sales_order_details.order_id',$cond2);
             $cond="ch_retailer_id=".$row['retailer_id']." AND DATE_FORMAT(`ch_date`,'%Y%m%d')>='".$start."' AND DATE_FORMAT(`ch_date`,'%Y%m%d')<='".$end."' AND product_id=".$product_id;
            // $out[$id][bill_cut]= myrowvaljoin('challan_order','count(ch_no)','challan_order_details','challan_order.id=challan_order_details.ch_id', $cond);
             $out[$id][bill_qty]= myrowvaljoin('challan_order','sum(qty)','challan_order_details','challan_order.id=challan_order_details.ch_id', $cond);
            $out[$id][billing_amount]= myrowvaljoin('challan_order','sum(taxable_amt)','challan_order_details','challan_order.id=challan_order_details.ch_id', $cond);
            $bi++; 
         }
         $cond3=" where DATE_FORMAT(`ch_date`,'%Y%m%d')>='".$start."' AND DATE_FORMAT(`ch_date`,'%Y%m%d')<='".$end."' AND $product_id1 AND l2_id IN($sid)";
                $q = "SELECT cod.product_id,cv.product_name as sku,cv.c2_name as vname,cod.id AS order_id,ch_user_id as user_id,ch_retailer_id as retailer_id,l2_name as state, l2_id, l5_name as beat,DATE_FORMAT(ch_date,'%d/%m/%Y') AS sale_date,DATE_FORMAT(ch_date,'%d/%b/%Y') AS fdated,dealer.name AS name,dealer.csa_id 
         FROM challan_order co 
         INNER JOIN challan_order_details cod ON co.id=cod.ch_id 
         INNER JOIN catalog_view cv ON cod.product_id=cv.product_id 
         INNER JOIN dealer ON dealer.id=co.ch_dealer_id 
         INNER JOIN dealer_location_rate_list dlrl  ON dealer.id=dlrl.dealer_id
         INNER JOIN location_view lv ON lv.l5_id=dlrl.location_id " 
                 . " $cond3 AND cod.order_id='0' GROUP BY sale_date,dealer.id,ch_retailer_id,cod.product_id ";
            // h1($q);
            $rs = mysqli_query($dbc,$q);
            $ci=1;
                 while ($row = mysqli_fetch_assoc($rs)) {
           ...
           //  $out[$id]['name'] = $dealer_map[$row['dealer_id']];
            ...
            // $cond2="retailer_id=".$row['retailer_id']." AND DATE_FORMAT(`date`,'%Y%m%d')>='".$cdate."' AND DATE_FORMAT(`date`,'%Y%m%d')<='".$end."' AND product_id=".$product_id;
           //  $out[$id][sale_order_value]= myrowvaljoin('user_sales_order_details','sum(rate*quantity)','user_sales_order','user_sales_order.order_id=user_sales_order_details.order_id',$cond2);
            
           //  $out[$id][sale_order_qty]= myrowvaljoin('user_sales_order_details','sum(quantity)','user_sales_order','user_sales_order.order_id=user_sales_order_details.order_id',$cond2);
             $cond="ch_retailer_id=".$row['retailer_id']." AND DATE_FORMAT(`ch_date`,'%d/%m/%Y')='".$cdate."' AND product_id=".$product_id;
            // $out[$id][bill_cut]= myrowvaljoin('challan_order','count(ch_no)','challan_order_details','challan_order.id=challan_order_details.ch_id', $cond);
             $out[$id][bill_qty]= myrowvaljoin('challan_order','sum(qty)','challan_order_details','challan_order.id=challan_order_details.ch_id', $cond);
            $out[$id][billing_amount]= myrowvaljoin('challan_order','sum(taxable_amt)','challan_order_details','challan_order.id=challan_order_details.ch_id', $cond);
             $ci++;
    ...
    public function get_challan_report_list($filter ='', $records ='', $orderby ='', $ifilter='')
        {
            global $dbc;
            $out = array();
  ...
  public function get_product_wise_summary_pulse_list($filter='', $records = '', $orderby='')
                {
                    ...
                    //print_r($filterstr);
                    $q="SELECT stock.id AS sid,qty,stock.product_id AS product_id,catalog_view.c2_name AS cat,catalog_view.product_name AS product_name,dealer.id AS did,dealer.dealer_code AS rds_code,dealer.name AS dname,lv.l2_id,lv.l2_name AS state,l4_name AS town "
                            . " FROM dealer LEFT JOIN dealer_location_rate_list AS dlrl ON dealer.id=dlrl.dealer_id LEFT JOIN location_view AS lv ON lv.l5_id=dlrl.location_id LEFT JOIN stock ON stock.dealer_id=dealer.id LEFT JOIN catalog_view ON catalog_view.product_id=stock.product_id $filterstr AND qty>0 AND stock.product_id='164' AND mrp='15.00' GROUP BY l2_id,did,product_id,stock.mrp ORDER BY state,cat,dname,product_name";
                   // $q="SELECT stock.id AS id,catalog_product.id as product_id,catalog_product.name AS product_name,stock.qty AS qty,mrp,(qty*dealer_rate) AS value FROM stock INNER JOIN catalog_product ON catalog_product.id=stock.product_id INNER JOIN dealer_person_login ON dealer_person_login.dealer_id=stock.dealer_id  $filterstr ORDER BY name ";
                   ...
                   // $mrp=$row['mrp'];
                    ...
                    $cond1="ch_dealer_id=".$row['did']." AND DATE_FORMAT(`ch_date`,'%Y%m')='201712' AND product_id='$product_id'";
                    $cond2="ch_dealer_id=".$row['did']." AND DATE_FORMAT(`ch_date`,'%Y%m')='201801' AND product_id='$product_id'";
                    $rwhere="dealer_id=".$row['did']." AND retailer_status=1";
                   // $out[$id][billing_amount_dec]= myrowvaljoin('challan_order','sum(taxable_amt)','challan_order_details','challan_order.id=challan_order_details.ch_id', $cond1);
                   // $out[$id][billing_amount_jan]= myrowvaljoin('challan_order','sum(taxable_amt)','challan_order_details','challan_order.id=challan_order_details.ch_id', $cond2);
                    $out[$id]['retailer_count']=myrowval('retailer','count(id)',$rwhere);
                    }
                    //pre($out);
                    return $out;
                    }
     ...
     public function get_user_yearly_report_list($filter = '', $records = '', $orderby = '') {
        global $dbc;
        $out = array();

        $filterstr = $this->oo_filter($filter, $records, $orderby);
       // echo $filterstr;die;
        $q = "SELECT person.id AS id,CONCAT_WS(' ',first_name,middle_name,last_name) AS pname,_role.rolename AS role,location_view.l3_name AS state,person.emp_code,(SELECT CONCAT_WS( ' ',  `first_name`, `middle_name`, `last_name` )  FROM person AS p1 WHERE p1.id=person.person_id_senior) As senior_name FROM  person   INNER JOIN location_view ON person.state_id=location_view.l3_id INNER JOIN _role ON _role.role_id=person.role_id INNER JOIN person_login ON person_login.person_id=person.id $filterstr AND person_status='1' AND person.role_id IN(3,4,10,14,15,21,30,31,32,33) GROUP BY id ORDER BY state";
    //  h1($q);
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
        if (!$opt)
            return $out;
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $out[$id] = $row; // storing the item id
            //$out[$id]['pname'] = $this->get_username($row['person_id']);
            $out[$id]['apr'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201704'");
            $out[$id]['may'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201705'");
            $out[$id]['june'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201706'");
            $out[$id]['july'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201707'");
            $out[$id]['aug'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201708'");
            $out[$id]['sep'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201709'");
            $out[$id]['oct'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201710'");
            $out[$id]['nov'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201711'");
            $out[$id]['dec'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201712'");
            $out[$id]['jan'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201801'");
            $out[$id]['feb'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201802'");
            $out[$id]['mar'] = myrowvaljoin('user_sales_order','sum(quantity*rate)','user_sales_order_details','user_sales_order.order_id=user_sales_order_details.order_id',"user_id='$row[id]' AND DATE_FORMAT(`date`,'%Y%m')='201803'");
        }
        return $out;
    }

    public function get_user_work_tracking_list($filter = '', $records = '', $orderby = '') {
        global $dbc;
        $out = array();
        $pkeyid = array();
        $filterstr = $this->oo_filter($filter, $records, $orderby);
        ...
        $limit = 50;
          ...
          $q = "SELECT user_work_tracking.*,DATE_FORMAT(track_date,'%Y%m%d') AS tdate,CONCAT_WS(' ',first_name,middle_name,last_name) AS user_name "
                  . "FROM user_work_tracking "
            . " INNER JOIN person ON person.id=user_work_tracking.user_id"
            . " $filterstr";
       //h1($q);
        ...
        while ($rows = mysqli_fetch_assoc($rs)) {
           //pre($rows);
            ...
            $fromTime = '09:00:00';
            $toTime = '19:00:00';
         $qrt=   "SELECT * FROM user_work_tracking WHERE user_id = '$user_id' AND track_date = '$rows[track_date]' AND track_time >='$fromTime' AND track_time <= '$toTime'  LIMIT $limit";
            $out[$id]['track_details'] = $this->get_my_reference_array_direct($qrt,'id');
        }
        return $out;
    }
