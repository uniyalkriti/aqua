...
//        $CometitorNewLaunchProductReport = $data->response->CometitorNewLaunchProductReport;
        ...
        if(!empty($Visit))
        {
            ...
            {
                $visit_query = "INSERT INTO `daily_visit`(`user_id`, `colleague_id`, `location_address`, `dealer_id`, `date`, `time`, `latitude`, `longitude`, `mcc_mnc_lac_cellId`, `created_at`,`check_out_time`) VALUES ('$user_id','$visit_data->colleague_id','$visit_data->location','$visit_data->dealer_id','$visit_data->date','$visit_data->time','$visit_data->latitude','$visit_data->longitude','$visit_data->mcc_mnc_lac_cellId',NOW(),'$visit_data->check_out_time')";
        ...
        if(!empty($CreateDealer))
        {
            foreach ($CreateDealer as $dealer_key => $dealer_value) 
            {
                ...
                $state_id = "SELECT  `location_3_id` from `location_4` where `id` = '$town_id' LIMIT 1";
                $state_id_run = mysqli_query($dbc, $state_id);
                $state_data = mysqli_fetch_row($state_id_run);

                  $create_dealer_query = "INSERT INTO `dealer`(`id`,`name`,`address`,`lat`,`lng`,`mcc_mnc_lac_cellId`,`date`,`time`,`state_id`,`town_id`,`created_at`,`created_by`) VALUES ('$dealer_value->dealer_id','$dealer_value->dealer_name','$dealer_value->location','$dealer_value->latitude','$dealer_value->longitude','$dealer_value->mcc_mnc_lac_cellid','$dealer_value->date','$dealer_value->time','$state_data[0]','$dealer_value->town_id',NOW(),'$user_id')";
...
//                echo $cmp_query;die;
        ...
        if (!empty($dailyReporting)) {
            foreach ($dailyReporting as $newda) {
                $dr_query = "INSERT INTO `daily_reporting`(`user_id`, `work_date`, `server_date_time`, `work_status`, `working_with`, `user_location`, `mnc_mcc_lat_cellid`, `lat_lng`, `remarks`, `attn_address`, `order_id`, `dealer_id`, `location_id`) 
                VALUES ('$user_id','$newda->date_time',NOW(),'$newda->status','$newda->working_with','$newda->location','$newda->check_mcc_mnc_lac_cellId_final','$newda->latlng','$newda->remark','$newda->location','$newda->orderid','$newda->dealerid','$newda->LocId')";
...
//echo $qbr;
        ...
        if (isset($attendance) && !empty($attendance)) {
            ...
            while ($k < $attcount) {
                ...
                $num = mysqli_num_rows($sql);
                if ($num < 1) {
                    $q = "INSERT INTO `user_daily_attendance`(`user_id`, `order_id`, `work_date`,`working_with`, `work_status`,`mnc_mcc_lat_cellid`, `lat_lng`, `track_addrs`, `remarks`,`server_date`,`battery_status`,`gps_status`)VALUES
                ('$user_id','$order_id$user_id','$date_time','$working_with','$status','$mnc_mcc_lat_cellid','$latlng','$user_location','$remark',CURRENT_TIMESTAMP,'$battery_status','$gps_status')";
                    $run = mysqli_query($dbc, $q);

                $qd="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`,`battery_status`,`gps_status`) VALUES ('$user_id','$new_work_date','$new_work_time','$mnc_mcc_lat_cellid','$latlng','$user_location','Attendance',NOW(),'$battery_status','$gps_status')";
        ...
        if (!empty($Checkoutlocation)) {
            ...
            while ($c < $checkcount) {
                ...
                $num = mysqli_num_rows($sql);
                if ($num < 1) {
                     $q = "INSERT INTO check_out(`user_id`,`lat_lng`,`mnc_mcc_lat_cellid`,`work_date`,`server_date_time`,`attn_address`,`order_id`,`remarks`,`total_call`,`total_pc`,`total_sale_value`,`battery_status`,`gps_status`)
                ...
                $qd="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`,`battery_status`,`gps_status`) VALUES ('$user_id','$new_work_date','$new_work_time','$mcc_mnc_lac_cellId_final','$latlng','$user_location','CheckOut',NOW(),'$battery_status','$gps_status')";
                $rd = mysqli_query($dbc, $qd);
                }
                $c++;
            }
...
///////////////////////////////////////////////////////////////////////////////
        ...
        if (!empty($callwisereporting)) {
            ...
            while ($l < $phonestatus) {
                ...
                if ($num_uso < 1) {
                  echo  $quso = "INSERT INTO `user_sales_order`(`order_id`, `company_id`, `user_id`, `dealer_id`, `location_id`, `retailer_id`,`call_status`, `total_sale_value`,`discount`,`amount`, `total_sale_qty`,`lat_lng`, `mccmnclatcellid`,`geo_status`,`track_address`, `date`, `time`, `image_name`, `override_status`,`remarks`,`battery_status`,`gps_status`,`reason`)"
                        . " VALUES('$order_id$user_id','1','$user_id','$dealer_id','$beat_id','$retailer_id','$call_status','$total_sale_value','$discount','$discount_before','$total_sale_qty','$lat_lng','$mcc_mnc','$geo_status','$user_location','$date','$time','.jpg','$override_status','$remarks','$battery_status','$gps_status','$non_productive_id')";
                    //      echo $q;
              echo   $qd="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`,`battery_status`,`gps_status`) VALUES ('$user_id','$date','$time','$mcc_mnc','$lat_lng','$user_location','Order Booking',NOW(),'$battery_status','$gps_status')";
                $rd = mysqli_query($dbc, $qd);
                }
...
//                }
        ...
        if (!empty($tracking)) {
            ...
            while ($tr < $track) {
                ...
                $num = $sql['num'];
                if ($num < 1) {
                    $q = "INSERT INTO `user_daily_tracking`(`user_id`, `track_date`, `track_time`,`mnc_mcc_lat_cellid`, `lat_lng`, `track_address`,`battery_status`,`gps_status`)"
                        . " VALUES('$user_id','$track_date','$track_time','$mnc_mcc_lat_cellid','$lat_lng','$user_location','$battery_status','$gps_status')";
                    $result = mysqli_query($dbc, $q);

                $qd="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`,`battery_status`,`gps_status`) VALUES ('$user_id','$track_date','$track_time','$mnc_mcc_lat_cellid','$lat_lng','$user_location','Tracking',NOW(),'$battery_status','$gps_status')";
                $rd = mysqli_query($dbc, $qd);
        ...
        if (!empty($damage_detail)) {
            ...
            while ($dd < $damage_con) {
                ...
                $q = "INSERT INTO `damage_replace`(`replaceid`, `return_reason_id`,`user_id`, `dis_code`, `prod_code`, `ret_code`, `prod_qty`, `prod_value`, `date_time`, `location`, `reason`, `mrp`, `task`, `extra_amt`) "
                    . " VALUES('$replaceid','$reason_type_id','$user_id','$dis_code','$prod_code','$ret_code','$prod_qty','$prod_value','$date_time','$location','$reason','$mrp','$task','$extra_amt')";
                $result = mysqli_query($dbc, $q);
                $dd++;
            }
        }
        if (!empty($damage_detail_retailer)) {
            ...
            while ($ddr < $damage_con) {
                ...
                $q = "INSERT INTO `damage_replace_retailer`(`replaceid`,`return_reason_id`, `user_id`, `dis_code`, `prod_code`, `ret_code`, `prod_qty`, `prod_value`, `date_time`, `location`, `reason`, `mrp`, `task`, `extra_amt`) "
                    . " VALUES('$replaceid','$reason_type_id','$user_id','$dis_code','$prod_code','$ret_code','$prod_qty','$prod_value','$date_time','$location','$reason','$mrp','$task','$extra_amt')";
                $result = mysqli_query($dbc, $q);
                $ddr++;
            }
