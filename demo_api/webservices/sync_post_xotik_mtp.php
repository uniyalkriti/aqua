    ...
    if ($status == '1') {   
        ...
        if (!empty($mtp)) {
            ...
            while ($m < $mtp_con) {
                ...
                $not_interested_id = $mtp[$m]->not_interested_id;
                $new_search_status_id = $mtp[$m]->new_search_status_id;
                $distributor_lead_name = $mtp[$m]->distributor_lead_name;
                $owner_name = $mtp[$m]->owner_name;
                $contact_no = $mtp[$m]->contact_no_name;
                $lead_town_name = $mtp[$m]->lead_town_name;
                $others_product_handled = $mtp[$m]->others_product_handled;
                $close_date = $mtp[$m]->close_date;


                $q = "INSERT INTO `monthly_tour_program_xotik`(`company_id`,`person_id`, `working_date`, `dayname`, `working_status_id`, `dealer_id`,`town`, `locations`, `total_calls`, `total_sales`, `ss_id`, `travel_mode`, `from`, `to`, `travel_distance`, `category_wise`, `mobile_save_date_time`,`upload_date_time`,`pc`,`rd`,`collection`,`primary_ord`,`new_outlet`,`any_other_task`,`not_interested_id`,`new_search_status_id`,`distributor_lead_name`,`owner_name`,`contact_no`,`lead_town_name`,`others_product_handled`,`close_date`)"
                    . " VALUES('$company_id','$user_id','$working_date','$dayname','$working_status_id','$dealer_id','$town_id','$locations','$total_calls','$total_sales','$ss_id','$travel_mode','$from','$to','$travel_distance','$category_wise','$mobile_save_date_time',NOW(),'$pc','$rd','$collection','$primary_ord','$new_outlet','$any_other_task','$not_interested_id','$new_search_status_id','$distributor_lead_name','$owner_name','$contact_no','$lead_town_name','$others_product_handled','$close_date')";
...
FROM monthly_tour_program_xotik
WHERE 
