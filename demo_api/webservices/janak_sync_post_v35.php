...
///////////////////////////////////////////////////////////////////////////////
            ...
            while ($p < $callwisereport) {
                ...
                if ($num_data < 1) {
                    ...
                    $qusod = "INSERT INTO `user_sales_order_details`(`order_id`, `product_id`, `rate`,`case_rate`, `quantity`, `scheme_qty`,`company_id`,`piece_quantity`,`case_quantity`,`secondary_quantity`,`secondary_rate`,`final_secondary_qty`,`final_secondary_rate`,`quantity_per_other_type_cus`)"
                        . " VALUES('$orderid$user_id','$product_id','$rate','$case_rate','$prod_qty','$scheme_qty','$company_id','$piece_quantity','$case_quantity','$secondary_quantity','$secondary_rate','$final_secondary_quantity','$final_secondary_rate','$quantity_per_other_type')"; 
                    }
        ...
        if (!empty($callwisereporting)) {
            ...
            while ($l < $phonestatus) {
                ...
                $discount = !empty($callwisereporting[$l]->Discount)?$callwisereporting[$l]->Discount:'0';
                ...
                if ($num_uso < 1) {
                  $quso = "INSERT INTO `user_sales_order`(`order_id`, `user_id`, `dealer_id`, `location_id`, `retailer_id`,`call_status`, `total_sale_value`,`discount`,`discount_type`,`amount`, `total_sale_qty`,`lat_lng`, `mccmnclatcellid`,`geo_status`,`track_address`, `date`, `time`, `image_name`, `override_status`,`remarks`,`battery_status`,`gps_status`,`reason`,`company_id`,`total_dispatch_qty`,`order_status`)"
                        . " VALUES('$order_id$user_id','$user_id','$dealer_id','$beat_id','$retailer_id','$call_status','$total_sale_value','$discount','$discount_status','$discount_before','$total_sale_qty','$lat_lng','$mcc_mnc','$geo_status','$user_location','$date','$time','.jpg','$override_status','$remarks','$battery_status','$gps_status','$non_productive_id','$company_id','0','0')";
                    //      echo $q;
        ...
        if (!empty($Primarysaledetail)) {
            ...
            while ($pd < $psalesum) {
                ...
                if($company_id == 50){
                $chk_uso = "SELECT janak_order_sequence FROM user_primary_sales_order where company_id = '$company_id' order by janak_order_sequence DESC limit 1";
                $run_uso = mysqli_query($dbc, $chk_uso);
                $num_uso = mysqli_fetch_assoc($run_uso);

                    if(empty($num_uso['janak_order_sequence'])){
                        $sequence = '1';
                    }else{
                        $sequence = $num_uso['janak_order_sequence']+1;
                    }
                ...
                $q = "INSERT INTO `user_primary_sales_order`(`id`,`order_id`, `dealer_id`, `created_date`, `created_person_id`, `sale_date`, `receive_date`, `date_time`,`lat`,`lng`,`address`,`company_id`,`dispatch_through`,`destination`,`comment`,`amount_before_discount`,`discount_type`,`discount_value`,`amount_after_discount`,`janak_order_sequence`)"
                    . " VALUES('$order_id$user_id','$order_id$user_id','$dealer_id','$created_date','$user_id','$sale_date',NOW(),'$date_time','$lat','$lng','$address','$company_id','$dispatch_through','$destination','$comment','$amount_before_discount','$discount_status','$discount_amount','$amount_after_discount','$sequence')";
            //    echo $q;die;
