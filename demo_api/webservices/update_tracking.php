...
echo $q = "select * from user_daily_tracking where DATE_FORMAT(track_date,'%Y-%m-%d') >= '$predate' AND DATE_FORMAT(track_date,'%Y-%m-%d') <= '$date' AND user_id=703";
//echo $q = "select * from user_daily_tracking where DATE_FORMAT(track_date,'%Y-%m-%d') = '2017-05-04' ";
...
while ($row = mysqli_fetch_array($res)) 
{
	 $lat_lng1 = explode(',', $row['lat_lng']);
         $lat = $lat_lng1[0];
         $lng = $lat_lng1[1];
    if (empty($row['track_address']) || $row['track_address'] == ", , , " ) 
    {
            ...
            if ($row['mnc_mcc_lat_cellid'] != "0:0:0:0" || $row['mnc_mcc_lat_cellid'] != "false") 
            {
                ...
                echo $q = "update user_daily_tracking set track_address = '$address',lat_lng = '$final' where id = '$row[id]' ";
                echo '<br/>';
           ...
           $address = getLocationByLatLng($lat, $lng);
            echo $q = "update user_daily_tracking set track_address = '$address' where id = '$row[id]' ";
            echo '<br/>';
            $res2 = mysqli_query($dbc, $q);
            $count++;
        }
