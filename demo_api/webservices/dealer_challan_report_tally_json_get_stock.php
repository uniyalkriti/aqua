	...
	foreach($stock_list as $co=>$coo)
	{
		...
		$id=date('YmdHis').$dealer_id;
	     	...
	     	$closing=str_replace("(-)","-",$closing);
	     	    ...
	     	    $qdet="INSERT IGNORE `tally_stock`(`csa_id`, `product_id`, `product_name`, `from_date`, `to_date`, `opening`, `inward`, `outward`, `closing`, `server_date_time`) VALUES ('$dealer_id','$product_id','$product_name','$from_date','$to_date','$opening','$inward','$outward','$closing',NOW())";	
