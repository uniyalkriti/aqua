...
//                echo $cmp_query;die;
        ...
        if (!empty($DailyProspectWorking)) {
            foreach ($DailyProspectWorking as $newd) {
                $dp_query = "INSERT INTO `daily_prospecting_working` (`user_id`,`party_name`, `party_address`, `phone_no`, `residence_phone`, `mobile_no`, `email_id`, `person_met_and_status`, `established_since`, `annual_turn_over`, `reputation_trade_relation`, `financial_position`, `level_ofinterst`, `from_time`, `to_time`, `units_availble_and_qty`, `gst_no`, `gst_registrtion_date`, `pan_card_no`, `pan_card_date`, `godown_size`, `no_of_employee`, `terms_condition`, `assured_investment`, `stockiest_from_filled`, `comments`, `orderid`, `latitude`, `longitude`, `mcc_mnc_lac_cellid`, `cur_date_time`) VALUES ('$newd->user_id','$newd->party_name','$newd->party_address','$newd->phone_no','$newd->residence_phone','$newd->mobile_no','$newd->email_id','$newd->person_met_and_status','$newd->established_since','$newd->annual_turn_over','$newd->reputation_trade_relation','$newd->financial_position','$newd->level_ofinterst','$newd->from_time','$newd->to_time','$newd->units_availble_and_qty','$newd->gst_no','$newd->gst_registrtion_date','$newd->pan_card_no','$newd->pan_card_date','$newd->godown_size','$newd->no_of_employee','$newd->terms_condition','$newd->assured_investment','$newd->stockiest_from_filled','$newd->comments','$newd->orderid','$newd->latitude','$newd->longitude','$newd->mcc_mnc_lac_cellid','$newd->cur_date_time')";
...
//echo $q;
        ...
        if (!empty($BalanceStock)) {
            ...
            while ($bre < $count_BalanceStock) {
                ...
                $qbr = "INSERT INTO `dealer_balance_stock`(`order_id`, `dealer_id`, `user_id`, `product_id`, `stock_qty`, `mfg_date`, `exp_date`, `cases`, `submit_date_time`, `server_date_time`,`mrp`,`pcs_mrp`,sstatus) VALUES ('$balance_order_id$user_id','$dealer_id','$user_id','$balance_product_code','$balance_pieces','$mfg_date','$exp_date','$balance_cases','$mobile_datetime',NOW(),'$bsmrp','$balance_pcs_mrp','0')";
...
//echo $qbr;
        ...
        if (isset($attendance) && !empty($attendance)) {
            ...
            while ($k < $attcount) {
				...
				('$user_id','$order_id$user_id','$date_time','$status','$mnc_mcc_lat_cellid','$latlng','$user_location','$remark',CURRENT_TIMESTAMP)";
                    $run = mysqli_query($dbc, $q);
                }
                $k++;
            }
