...
if ($data) {
    ...
    $q = "SELECT person_login.*,person.* From person_login INNER JOIN person ON person.id=person_login.person_id WHERE person_id='$user_id'";

    $user_res = mysqli_query($dbc, $q);
    $q_person = mysqli_fetch_assoc($user_res);
    $person_id = $q_person['person_id'];
    $status = $q_person['person_status'];
    $person_id_senior = $q_person['person_id_senior'];
...
//        $CometitorNewLaunchProductReport = $data->response->CometitorNewLaunchProductReport;
        ...
        $leaveRequest = $data->response->leaveRequest;
        $PlanRequest = $data->response->tourAdvanceExpense;
...
//        echo '<pre>';print_r($leaveRequest);die;
        ...
        if (isset($leaveRequest) && !empty($leaveRequest)) {
            $lv = 0;
            $lvcount = count($leaveRequest);
            while ($lv < $lvcount) {
                $lv_from_date = $data->response->leaveRequest[$lv]->from_date;
                $lv_to_date = $data->response->leaveRequest[$lv]->to_date;
                $lv_leave_id = $data->response->leaveRequest[$lv]->leave_id;
                $lv_remark = str_replace(',','',$data->response->leaveRequest[$lv]->remarks);
                $lv_leave_date = $data->response->leaveRequest[$lv]->leave_date;
                $lv_leave_time = $data->response->leaveRequest[$lv]->leave_time;
                $lv_latlng = str_replace(',',':',$data->response->leaveRequest[$lv]->lat_lng);
                $lv_geo_address = str_replace(',',':',$data->response->leaveRequest[$lv]->geo_address);
                $lv_order_id = $data->response->leaveRequest[$lv]->orderid;
                $lv_mnc_mcc_lat_cellid = $data->response->leaveRequest[$lv]->mnc_mcc_lat_cellid;
                $lv_leave_category_id = $data->response->leaveRequest[$lv]->leave_category_id;
                $lv_leave_category_option_id = $data->response->leaveRequest[$lv]->leave_category_option_id;
                $uuid = $data->response->leaveRequest[$lv]->user_id;
                $lvp="INSERT INTO `user_leave_request`( `user_id`, `order_id`, `leave_date`, `leave_time`, `leave_id`, `leave_category_id`, `leave_category_option_id`, `from_date`, `to_date`, `remarks`, `attn_address`, `mnc_mcc_lat_cellid`, `lat_lng`, `server_date_time`,`approved_for`) VALUES ('$uuid','$lv_order_id','$lv_leave_date','$lv_leave_time','$lv_leave_id','$lv_leave_category_id','$lv_leave_category_option_id','$lv_from_date','$lv_to_date','$lv_remark','$lv_geo_address','$lv_mnc_mcc_lat_cellid','$lv_latlng',NOW(),'$person_id_senior')";
                $runlv = mysqli_query($dbc, $lvp);
                $lv++;
            }
        ...
        if (isset($PlanRequest) && !empty($PlanRequest)) {
            $pr = 0;
            $prcount = count($PlanRequest);
            while ($pr < $prcount) {
                $from_date = $data->response->tourAdvanceExpense[$pr]->from_date;
                $to_date = $data->response->tourAdvanceExpense[$pr]->to_date;
                $place_of_visit = $data->response->tourAdvanceExpense[$pr]->place_of_visit;
                $fare = $data->response->tourAdvanceExpense[$pr]->fare;
                $da = $data->response->tourAdvanceExpense[$pr]->da;
                $fooding = $data->response->tourAdvanceExpense[$pr]->fooding;
                $lodging = $data->response->tourAdvanceExpense[$pr]->lodging;
                $local_conveyance = str_replace(',','',$data->response->tourAdvanceExpense[$pr]->local_conveyance);
                $miscelleanous = $data->response->tourAdvanceExpense[$pr]->miscelleanous;
                $pr_remarks = $data->response->tourAdvanceExpense[$pr]->remarks;
                $cur_date = $data->response->tourAdvanceExpense[$pr]->cur_date;
                $cur_time = $data->response->tourAdvanceExpense[$pr]->cur_time;
                $prlat = $data->response->tourAdvanceExpense[$pr]->lat;
                $prlng = $data->response->tourAdvanceExpense[$pr]->lng;
                $pr_latlng = $prlat.','.$prlng;
                $pr_order_id = $data->response->tourAdvanceExpense[$pr]->order_id;
                $pr_track_address = str_replace(',','',$data->response->tourAdvanceExpense[$pr]->track_address);
                $pr_mcc_mnc_lac_cellId = $data->response->tourAdvanceExpense[$pr]->mcc_mnc_lac_cellId;
                $pr_imei = $data->response->tourAdvanceExpense[$pr]->imei;
                $pr_total_duration = $data->response->tourAdvanceExpense[$pr]->total_duration;
                $pr_purpose = $data->response->tourAdvanceExpense[$pr]->purpose;
                $pr_approval_remarks = $data->response->tourAdvanceExpense[$pr]->approval_remarks;
                $pr_total_amount = $data->response->tourAdvanceExpense[$pr]->total_amount;
                $uuid = $data->response->tourAdvanceExpense[$pr]->user_id;

                $prp="INSERT INTO `user_plan_request`(`user_id`, `from_date`, `to_date`, `place_of_visit`, `fare`, `da`, `fooding`,`lodging`,`local_conveyance`, `miscelleanous`, `remarks`, `cur_date`, `cur_time`, `lat`, `lng`, `lat_lng`, `order_id`, `track_address`, `mcc_mnc_lac_cellId`, `imei`, `total_duration`, `purpose`, `approval_remarks`, `total_amount`, `server_date_time`,`approved_for`) VALUES ('$uuid','$from_date','$to_date','$place_of_visit','$fare','$da','$fooding','$lodging','$local_conveyance','$miscelleanous','$pr_remarks','$cur_date','$cur_time','$prlat','$prlng','$pr_latlng','$pr_order_id','$pr_track_address','$pr_mcc_mnc_lac_cellId','$pr_imei','$pr_total_duration','$pr_purpose','$pr_approval_remarks','$pr_total_amount',NOW(),'$person_id_senior')";
                $runpr = mysqli_query($dbc, $prp);
                $pr++;
            }
