        ...
        if(!empty($modernTradeMeeting))
        {
            foreach($modernTradeMeeting as $mm_key => $mm_value)
            {
             ...
             $morder_trade_meeting_query = "INSERT INTO `mordern_trade_meeting`(`start_time`, `duration`, `isr_name`, `so_name`, `asm_name`, `rsm_name`, `sales_head`, `so_target`, `is_sales_performance`, `is_primary_secondary_scheme`, `is_stock_shortage`, `so_one_travelling`, `so_two_travelling`, `latitude`, `longitude`, `geo_address`, `mcc_mnc`, `unique_id`, `user_id`, `user_role`, `cur_date`, `cur_time`, `image`, `server_date_time`,`company_id`) VALUES('$start_time','$duration','$isr_name','$so_name','$asm_name','$rsm_name','$sales_head','$so_target','$is_sales_performance','$is_primary_secondary_scheme','$is_stock_shortage','$so_one_travelling','$so_two_travelling','$latitude','$longitude','$geo_address','$mcc_mnc','$unique_id','$user_id','$user_role','$cur_date','$cur_time','$image',NOW(),'$company_id')";
                ...
                $modern_track="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`,`battery_status`,`gps_status`,`company_id`) VALUES ('$user_id','$date','$time','$mcc_mnc','$latlng','$geo_address','Modern Trade',NOW(),'$battery_status','$gps_status','$company_id')";
...
///////////////////////////////////////////////////////////////////////////////
        ...
        if (!empty($callwisereportingstatus)) {
            // print_r(());
            // echo $data_test = "INSERT INTO `data_test`(`test`) VALUES ()";
            // // "INSERT INTO text (data) VALUES "
            // $results_test = mysqli_query($dbc, $data_test);
            // die;
            ...
            while ($p < $callwisereport) {
                ...
                $prod_qty = !empty($callwisereportingstatus[$p]->quantity)?$callwisereportingstatus[$p]->quantity:'0';
                $scheme_qty = $callwisereportingstatus[$p]->scheme_qty;
                $rate = !empty($callwisereportingstatus[$p]->rate)?$callwisereportingstatus[$p]->rate:'0';
                ...
                $quantity_per_case = !empty($callwisereportingstatus[$p]->quantity_per_case)?$callwisereportingstatus[$p]->quantity_per_case:'0';
                $quantity_per_other_type = !empty($callwisereportingstatus[$p]->quantity_per_other)?$callwisereportingstatus[$p]->quantity_per_other:'0';
                $quantity_per_other_type_cus = !empty($callwisereportingstatus[$p]->quantity_per_other)?$callwisereportingstatus[$p]->quantity_per_other:'0';
                $product_type = !empty($callwisereportingstatus[$p]->product_type)?$callwisereportingstatus[$p]->product_type:'0';
                ...
                if ($num_data < 1) {
                    ...
                    if(!empty($case_quantity) || !empty($piece_quantity) || !empty($secondary_quantity)){

                        ////////////////////// for secondary quantity ////////////////////

                        $cases_converted_pcs_for_final = ($case_quantity*$quantity_per_case);

                        $pcs_for_final = $piece_quantity;
                       ...
                       $calculated_secondary_qty = 0;
                        ...
                        $final_secondary_quantity = ROUND(($calculated_secondary_qty+$secondary_quantity),3);

                        ////////////////////// for secondary rate ////////////////////
                        $cases_secondary_sale  = ($case_rate*$case_quantity);
                        $pcs_secondary_sale  = ($rate*$piece_quantity);
                        $secondary_secondary_sale  = ($secondary_rate*$secondary_quantity);
                    ...
                    $qusod = "INSERT INTO `user_sales_order_details`(`order_id`, `product_id`, `rate`,`case_rate`, `quantity`, `scheme_qty`,`company_id`,`piece_quantity`,`case_quantity`,`secondary_quantity`,`secondary_rate`,`final_secondary_qty`,`final_secondary_rate`,`quantity_per_other_type_cus`)"
                        . " VALUES('$orderid$user_id','$product_id','$rate','$case_rate','$prod_qty','$scheme_qty','$company_id','$piece_quantity','$case_quantity','$secondary_quantity','$secondary_rate','$final_secondary_quantity','$final_secondary_rate','$quantity_per_other_type_cus')"; 
        ...
        if (!empty($callwisereporting)) {
            ...
            while ($l < $phonestatus) {
                ...
                $Discount_type = $callwisereporting[$l]->Discount_type;

                if($Discount_type == "%"){
                    $discount_status = "1";
                }elseif($Discount_type == "value"){
                    $discount_status = "2";
                }else{
                    $discount_status = "0";
                }
                ...
                if ($num_uso < 1) {
                    $quso = "INSERT INTO `user_sales_order`(`order_id`, `user_id`, `dealer_id`, `location_id`, `retailer_id`,`call_status`, `total_sale_value`,`discount`,`discount_type`,`amount`, `total_sale_qty`,`lat_lng`, `mccmnclatcellid`,`geo_status`,`track_address`, `date`, `time`, `image_name`, `override_status`,`remarks`,`battery_status`,`gps_status`,`reason`,`company_id`)"
                        . " VALUES('$order_id$user_id','$user_id','$dealer_id','$beat_id','$retailer_id','$call_status','$total_sale_value','$discount','$discount_status','$discount_before','$total_sale_qty','$lat_lng','$mcc_mnc','$geo_status','$user_location','$date','$time','.jpg','$override_status','$remarks','$battery_status','$gps_status','$non_productive_id','$company_id')";
                    //      echo $q;
        ...
        if (!empty($PrimarySaleSummary)) {
            ...
            while ($ps < $psale) {
                ...
                $secondary_rate = !empty($PrimarySaleSummary[$ps]->secondary_rate)?$PrimarySaleSummary[$ps]->secondary_rate:'0';


                $quantity_per_case = !empty($PrimarySaleSummary[$ps]->quantity_per_case)?$PrimarySaleSummary[$ps]->quantity_per_case:'0';
                $quantity_per_other_type = !empty($PrimarySaleSummary[$ps]->quantity_per_other)?$PrimarySaleSummary[$ps]->quantity_per_other:'0';
                $product_type = !empty($PrimarySaleSummary[$ps]->secondary_type)?$PrimarySaleSummary[$ps]->secondary_type:'0';

                if(!empty($case) || !empty($pcs) || !empty($secondary_qty)){

                    ////////////////////// for secondary quantity ////////////////////

                    $cases_converted_pcs_for_final = ($case*$quantity_per_case);
                    ...
                    $final_secondary_quantity = ROUND(($calculated_secondary_qty+$secondary_qty),3);

                    ////////////////////// for secondary rate ////////////////////
                    $cases_secondary_sale  = ($case_rate*$case);
                    $pcs_secondary_sale  = ($rate*$pcs);
                    $secondary_secondary_sale  = ($secondary_rate*$secondary_qty);
                 ...
                 $q = "INSERT INTO `user_primary_sales_order_details`(`id`, `order_id`, `product_id`, `rate`, `quantity`, `scheme_qty`,`cases`,`pcs`,`pr_rate`,`company_id`,`secondary_qty`,`secondary_rate`,`final_secondary_qty`,`final_secondary_rate`) "
                    . " VALUES('$order_id$ps','$order_id$user_id','$product_id','$rate','$quantity','$scheme_qty','$case','$pcs','$case_rate','$company_id','$secondary_qty','$secondary_rate','$final_secondary_quantity','$final_secondary_rate')";
                    // die;
                $result = mysqli_query($dbc, $q);
        ...
        if (!empty($Primarysaledetail)) {
            ...
            while ($pd < $psalesum) {
                ...
                $amount_before_discount = $Primarysaledetail[$pd]->total_amount;
                $discount_type = $Primarysaledetail[$pd]->discount_type;
                $discount_amount = $Primarysaledetail[$pd]->discount_amount;
                $amount_after_discount = $Primarysaledetail[$pd]->final_amount;
                ...
                $q = "INSERT INTO `user_primary_sales_order`(`id`,`order_id`, `dealer_id`, `created_date`, `created_person_id`, `sale_date`, `receive_date`, `date_time`,`lat`,`lng`,`address`,`company_id`,`dispatch_through`,`destination`,`comment`,`amount_before_discount`,`discount_type`,`discount_value`,`amount_after_discount`)"
                    . " VALUES('$order_id$user_id','$order_id$user_id','$dealer_id','$created_date','$user_id','$sale_date',NOW(),'$date_time','$lat','$lng','$address','$company_id','$dispatch_through','$destination','comment','$amount_before_discount','$discount_status','$discount_amount','$amount_after_discount')";
            //    echo $q;die;
                $primary_order_track="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`,`battery_status`,`gps_status`,`company_id`) VALUES ('$user_id','$sale_date','$date_time','$mcc_mnc_lac_cellid','$lat_lng','$address','Primary Booking',NOW(),'$battery_status','$gps_status','$company_id')";
