...
else $str='{"response":{"Attandance":[],"Tracking":[],"MobileDetails":[{"deviceName":"Moto E (4) Plus","deviceMan":"motorola","deviceAndroidVersion":"25"}],"RetailerReshuffle":[],"CreateCustomer":[],"CallWiseReporting":[],"CallWiseReportingStatus":[],"Expense":[],"MergeRetailer":[],"RetailerSchemeStatusOtherFocusState":[],"RetailerSchemeStatusDiscoveryoutlet":[],"RetailerLatAndLong":[],"ISRCallWiseReportingStatus":[],"RetailerStock":[],"RetailerStockStatus":[],"PrimarySaleSummary":[],"PrimarySaleDetail":[],"RetailerSchemeStatus":[],"DamageArray":[],"JuniorCheckIn":[],"MTPApprovalReport":[],"JuniorCheckOut":[],"ISRAttandance":[],"MERCHANDISE":[],"RetailerDeleteStatus":[],"Mtp":[],"Checkoutlocation":[],"Complaint":[],"UserEditInFormation":[{"email_id":"","mobile_no":"97","profile_image_path":"http:\/\/162.213.190.125\/msell-gopal\/webservices\/mobile_images\/profile\/","date":"2019-04-01","time":"06:33:36","user_id":"990"}],"CallWiseReason":[],"paymentReceivedDetail":[],"paymentReceivedRetailer":[],"TotalCounterSale":[],"PaymentCollectionForDealer":[],"PaymentCollection":[],"getRetailerLocation":[],"MERCHANDISE_REQUIREMENT":[],"newUserInformation":[{"user_id":"990","new_email":"","new_mobile":"97"}],"leaveUpdate":[{"user_id":"990","leave_id":"1","leave_value":"0"},{"user_id":"990","leave_id":"2","leave_value":"0"},{"user_id":"990","leave_id":"3","leave_value":"0"}],"BalanceStock":[],"CometitorNewLaunchProductReport":[],"DailyProspectWorking":[{"party_name":"uooyb","party_address":"fgggg","phone_no":"5885585555","residence_phone":"5585588885","mobile_no":"8885555555","email_id":"tfytfgg@yhg.uj","person_met_and_status":"ggg","established_since":"2019-04-02","annual_turn_over":"88555","reputation_trade_relation":"yygg","financial_position":"ggt","level_ofinterst":"ggg","from_time":"16:22","to_time":"18:22","units_availble_and_qty":"vggg","gst_no":"fffggty","gst_registrtion_date":"2019-04-02","pan_card_no":"tyfgbfguhg","pan_card_date":"2019-04-02","godown_size":"66","no_of_employee":"88","terms_condition":"YES","assured_investment":"YES","stockiest_from_filled":"YES","comments":"yftt","orderid":"20190402042425","latitude":"28.5574607","longitude":"77.2530409","location":"324-A, Garhi, Amrit Puri, New Delhi, Delhi 110065, India,New Delhi,Delhi,India,110065","mcc_mnc_lac_cellid":"404:11:51074:205121800","cur_date":"2019-04-02","cur_time":"16:24:25","cur_date_time":"2019-04-02 16:24:25","user_id":"990","town":"tgh","district":"tggg","state":"tfffy"}],"retailerComment":[],"dailyReporting":[],"competitivePriceIntelligence":[],"feedbackSuggestion":[],"pendingClaim":[],"productInvestigationReport":[],"ComplaintReport":[],"TravellingExpenseBill":[],"Count":[],"imei":"355632085256072","user_id":"990"}}';
$check=cleanSpecialChar($str);
...
//echo $qbr;
        ...
        if (isset($attendance) && !empty($attendance)) {
            ...
            while ($k < $attcount) {
                ...
                $qd="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`) VALUES ('$user_id','$new_work_date','$new_work_time','$mnc_mcc_lat_cellid','$latlng','$user_location','Attendance',NOW())";
        ...
        if (!empty($Checkoutlocation)) {
            ...
            while ($c < $checkcount) {
                ...
                $num = mysqli_num_rows($sql);
                if ($num < 1) {
                    $q = "INSERT INTO check_out(`user_id`,`lat_lng`,`mnc_mcc_lat_cellid`,`work_date`,`server_date_time`,`attn_address`,`order_id`,`remarks`)
VALUES('$user_id','$latlng','$mcc_mnc_lac_cellId_final','$date_time','$current_date_time','$user_location','$order_id$user_id','$remarks')";
...
WHERE `user_daily_attendance`.`user_id` = '$user_id' and DATE_FORMAT(`user_daily_attendance`.`work_date`,'Y-m-d') = DATE_FORMAT($date_time,'Y-m-d')";
                ...
                $qd="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`) VALUES ('$user_id','$new_work_date','$new_work_time','$mcc_mnc_lac_cellId_final','$latlng','$user_location','CheckOut',NOW())";
                $rd = mysqli_query($dbc, $qd);
                }
                $c++;
            }
        ...
        if (!empty($callwisereporting)) {
            ...
            while ($l < $phonestatus) {
                ...
                if ($num_uso < 1) {
                    ...
                    //      echo $q;
                $qd="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lngs`, `track_address`, `status`, `server_date_time`) VALUES ('$user_id','$date','$time','$mcc_mnc','$lat_lng','$user_location','Order Booking',NOW())";
                $rd = mysqli_query($dbc, $qd);
                }
...
//                }
        ...
        if (!empty($tracking)) {
            ...
            while ($tr < $track) {
                ...
                $qd="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`) VALUES ('$user_id','$track_date','$track_time','$mnc_mcc_lat_cellid','$lat_lng','$user_location','Tracking',NOW())";
                $rd = mysqli_query($dbc, $qd);
