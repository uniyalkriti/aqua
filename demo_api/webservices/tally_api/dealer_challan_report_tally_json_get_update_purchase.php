   ...
   // print_r($voucher_list);
	...
	foreach($voucher_list as $co=>$coo)
	{
	   ...
	   // print_r($start);
		...
		if($ncheck<=0){	
		$q="INSERT IGNORE `tally_demo`( `ch_no`,`first_ch_no`,`ch_date`, `vocher_id`,`ss_voucher_id`,`csa_code`, `csa_id`, `dealer_code`,`dealer_id`, `dealer_name`,`invoice_type`,`drcr`, `amount`, `amount_round`,`server_date_time`) VALUES ('$voucher_no','$first_voucher_no','$ch_date','$voucher_id','$ss_voucher_id','$csa_code','$csa_id','$dealer_code','$dealer_id','$dealer_name','$invoice_type','CR','$amount','$amount',NOW())";
			}else{
		$q="UPDATE `tally_demo` SET `ch_no`='$voucher_no',`first_ch_no`='$first_voucher_no',`ch_date`='$ch_date',`dealer_code`='$dealer_code',`dealer_id`='$dealer_id',`dealer_name`='$dealer_name',`invoice_type`='$invoice_type',`drcr`='CR',`amount`='$amount',`amount_round`='$amount',`server_date_time`=NOW() WHERE ss_voucher_id='$ss_voucher_id'";		
			}
		...
		if(!$result){
            ...
            $str=array();
	     	...
	     	if($ncheck<=0){	
	     	$str[]="('$rId','$product_id','$product_name','$qty','$unit','$rate','$gst','$gst_amt','$dis_per','$dis_amt','$taxable_amt','$item_for')";
	     	//print_r($str);
	     }else{
	     	$str[]="('$check_id','$product_id','$product_name','$qty','$unit','$rate','$gst','$gst_amt','$dis_per','$dis_amt','$taxable_amt','$item_for')";
	     	//print_r($str);
	     }
		}
			$strs='';
