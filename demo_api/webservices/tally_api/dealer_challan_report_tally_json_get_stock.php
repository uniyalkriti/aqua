...
if(!empty($stock_list))
{
	...
	$qcheck="DELETE FROM `tally_stock` WHERE `from_date`='$from_date' AND `csa_id`='$dealer_id'";
	...
	foreach($stock_list as $co=>$coo)
	{
		...
		$id=date('YmdHis').$dealer_id;
	     	...
	     	$alt_unit=$coo['ALTUNIT']?$coo['ALTUNIT']:NA;
	     	...
	     	$inward=$coo['INWARD'];
	     	$inward=str_replace("(-)","-",$inward);
	     	$outward=$coo['OUTWARD'];
	     	$outward=str_replace("(-)","-",$outward);
	     	$closing=$coo['CLOSING'];
	     	$closing=str_replace("(-)","-",$closing);


	     	
	     	    $qdet="INSERT IGNORE `tally_stock`(`csa_id`, `product_id`, `product_name`, `from_date`, `to_date`, `opening`, `inward`, `outward`, `closing`,`unit`, `server_date_time`) 
	     	    VALUES ('$dealer_id','$product_id','$product_name','$from_date','$to_date','$opening','$inward','$outward','$closing','$alt_unit',NOW())";	
