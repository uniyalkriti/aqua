...
if(!empty($stock_list))
{
	...
	foreach($stock_list as $co=>$coo)
	{
		...
		$id=date('YmdHis').$dealer_id;
	     	...
	     	$qdet="INSERT IGNORE `tally_opening_stock`(`csa_id`, `product_id`, `product_name`, `from_date`, `to_date`, `opening`, `unit`,`server_date_time`) 
	     	VALUES ('$dealer_id','$product_id','$product_name','$from_date','$to_date','$opening','$alt_unit',NOW())";	
