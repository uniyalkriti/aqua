   ...
   // print_r($voucher_list);
	...
	foreach($voucher_list as $co=>$coo)
	{
	   ...
	   // print_r($start);
		...
		//if($ncheck>0){	
		$q="UPDATE `tally_demo` SET `ch_date`='$ch_date',`invoice_type`='$invoice_type',`drcr`='$drcr',`amount`='$amount',`amount_round`='$amount',`cancel_date`='$cancel_date',`server_date_time`=NOW() WHERE ss_voucher_id='$ss_voucher_id'";		
			//}
