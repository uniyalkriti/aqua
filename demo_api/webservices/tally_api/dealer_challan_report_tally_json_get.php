   ...
   // print_r($voucher_list);
	...
	foreach($voucher_list as $co=>$coo)
	{
	   ...
	   // print_r($start);
		...
		$q="UPDATE `tally_demo` SET `ch_no`='$voucher_no',`first_ch_no`='$first_voucher_no',`ch_date`='$ch_date',`dealer_code`='$dealer_code',`dealer_id`='$dealer_id',`dealer_name`='$dealer_name',`invoice_type`='$invoice_type',`drcr`='$drcr',`amount`='$amount',`amount_round`='$amount',`server_date_time`=NOW() WHERE ss_voucher_id='$ss_voucher_id'";		
			}
		...
		if(!$result){
            ...
            $str=array();
	     	...
	     	if($ncheck<=0){	
	     	$str[]="('$rId','$product_id','$product_name','$qty','$alt_unit','$rate','$gst','$gst_amt','$dis_per','$dis_amt','$taxable_amt','$item_for')";
	     	//print_r($str);
	     }else{
	     	$str[]="('$check_id','$product_id','$product_name','$qty','$alt_unit','$rate','$gst','$gst_amt','$dis_per','$dis_amt','$taxable_amt','$item_for')";
	     	//print_r($str);
	     }
		}
			$strs='';
	        $strs = implode(',', $str);
	     	...
	     	if(!empty($strs)){
	     	$qdet="INSERT IGNORE `tally_demo_details`(`td_id`, `product_id`, `product_name`, `qty`, `unit`,`rate`, `gst`, `gst_amt`, `dis_per`, `dis_amt`,`taxable_amt`,`item_for`) VALUES $strs";
