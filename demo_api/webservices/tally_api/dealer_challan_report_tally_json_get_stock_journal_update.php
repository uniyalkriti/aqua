   ...
   // print_r($voucher_list);
	...
	foreach($voucher_list as $co=>$coo)
	{
	   ...
	   // print_r($start);
		...
		$product_list1=$voucher_list[$co]['ALLINVENTORYENTRIESIN'];
		$product_list2=$voucher_list[$co]['ALLINVENTORYENTRIESOUT'];
		...
		if($ncheck<=0){	
		$q="INSERT IGNORE `tally_demo`( `ch_no`,`first_ch_no`,`ch_date`, `vocher_id`,`ss_voucher_id`,`csa_code`, `csa_id`,`dealer_code`, `dealer_id`, `dealer_name`,`invoice_type`,`drcr`, `amount`, `amount_round`,`server_date_time`) VALUES ('$voucher_no','$first_voucher_no','$ch_date','$voucher_id','$ss_voucher_id',$csa_code,'$csa_id','$dealer_id','$dealer_code','$dealer_name','$invoice_type','CR/DR','$amount','$amount',NOW())";
			}else{
		$q="UPDATE `tally_demo` SET `ch_no`='$voucher_no',`first_ch_no`='$first_voucher_no',`ch_date`='$ch_date',`dealer_code`='$dealer_code',`dealer_id`='$dealer_id',`dealer_name`='$dealer_name',`invoice_type`='$invoice_type',`amount`='$amount',`amount_round`='$amount',`server_date_time`=NOW() WHERE ss_voucher_id='$ss_voucher_id'";		
			}
		...
		if(!$result){
            ...
            if(!empty($product_list1)){
	     	...
	     	if($ncheck<=0){	
	     	$str[]="('$rId','$product_id','$product_name','$qty','$unit','$rate','$gst','$gst_amt','$dis_per','$dis_amt','$taxable_amt','$item_for','CR')";
	     	//print_r($str);
	     }else{
	     	$str[]="('$check_id','$product_id','$product_name','$qty','$unit','$rate','$gst','$gst_amt','$dis_per','$dis_amt','$taxable_amt','$item_for','CR')";
	     	//print_r($str);
	     }
		}
	  }
	  if(!empty($product_list2)){
             foreach($product_list2 as $cod1=>$poo1)
	     		{ 
	     	$product_name=$poo1['STOCKITEMNAME'];
	     	$product_code=$poo1['STOCKITEMID'];
	     	$qpd="SELECT id FROM tally_catalog_product WHERE itemcode='$product_code' LIMIT 1";
	     	$rpd=mysqli_query($dbc,$qpd);
	     	$rowpd=mysqli_fetch_assoc($rpd);
	     	$product_id=$rowpd['id'];
	     	...
	     	$unit=$poo['ALTUNIT'];
	     	$rate=$poo1['RATE'];
	     	$gst=$poo1['STOCKITEMGSTRATE'];
	     	$gst_amt=$poo1['TAX'];
	     	$gst_amt=str_replace(",","",$gst_amt);
	     	$dis_per=$poo1['DISCOUNT'];
	     	$dis_amt=$poo1['DISCOUNTVALUE'];
	     	$item_for=$poo1['ITEMFOR'];
	     	$dis_amt=str_replace(",","",$dis_amt);  
	     	$taxable_amt=$poo1['AMOUNT'];
	     	$taxable_amt=str_replace(",","",$taxable_amt);
	     	if($ncheck<=0){	
	     	$str[]="('$rId','$product_id','$product_name','$qty','$unit','$rate','$gst','$gst_amt','$dis_per','$dis_amt','$taxable_amt','$item_for','DR')";
	     	//print_r($str);
	     }else{
	     	$str[]="('$check_id','$product_id','$product_name','$qty','$unit','$rate','$gst','$gst_amt','$dis_per','$dis_amt','$taxable_amt','$item_for','DR')";
	     	//print_r($str);
	     }
		}
	  }
	        ...
	        $strs = implode(',', $str);
	     	...
	     	if(!empty($strs)){
	     	$qdet="INSERT IGNORE `tally_demo_details`(`td_id`, `product_id`, `product_name`, `qty`,`unit`, `rate`, `gst`, `gst_amt`, `dis_per`, `dis_amt`,`taxable_amt`,`item_for`,`cr_dr`) VALUES $strs";
