...
$out = array();
$expense = array();
          $q = "SELECT dealer_id,name, sum(amount) as amount FROM  `dealer_payments` INNER JOIN dealer ON dealer.id = dealer_payments.dealer_id WHERE DATE_FORMAT(`payment_recevied_date`,'%Y-%m-%d')>='$fromdate' AND DATE_FORMAT(`payment_recevied_date`,'%Y-%m-%d')<='$todate' AND `user_id`='$user_id' group by `dealer_id`";
        //h1($q);
