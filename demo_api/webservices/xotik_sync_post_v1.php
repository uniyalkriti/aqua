...
//        $CometitorNewLaunchProductReport = $data->response->CometitorNewLaunchProductReport;
        ...
        if(!empty($CounterSaleSummary))
        {
            foreach ($CounterSaleSummary as $counter_key => $counter_value) 
            {
                ...
                $counter_details_sale_data = "INSERT INTO `counter_sale_details`(`order_id`, `product_id`, `rate`, `quantity`, `barcode`, `cases`, `pcs`, `value`, `case_rate`, `pcs_rate`, `company_id`,`created_by`,`server_date_time`) VALUES ('$order_id','$product_id','$rate','$quantity','$barcode','$case','$pcs','$value','$case_rate','$pcs_rate','$company_id','$user_id',NOW())";
...
//echo $qbr;
        ...
        if (isset($attendance) && !empty($attendance)) {
            ...
            while ($k < $attcount) {
                ...
                $area_of_word_id = $data->response->Attandance[$k]->area_of_work_id;
                $working_with_type_id = $data->response->Attandance[$k]->working_with_type_id;
                $working_activity_id = $data->response->Attandance[$k]->working_activity_id;
                $distributor_id = $data->response->Attandance[$k]->distributor_id;
                $new_area_name = $data->response->Attandance[$k]->new_area_name;
                $leave_from_date = $data->response->Attandance[$k]->leave_from_date;
                $leave_to_date = $data->response->Attandance[$k]->leave_to_date;
                $leave_id = $data->response->Attandance[$k]->leave_id;
                ...
                $num = mysqli_num_rows($sql);
                if ($num < 1) {
                    $q = "INSERT INTO `user_daily_attendance`(`user_id`, `order_id`, `work_date`,`working_with`, `work_status`,`mnc_mcc_lat_cellid`, `lat_lng`, `track_addrs`, `remarks`,`server_date`,`battery_status`,`gps_status`,`company_id`,`area_of_work_id`,`working_with_type_id`,`working_activity_id`,`distributor_id`,`new_area_name`,`leave_from_date`,`leave_to_date`,`leave_id`)VALUES
                ('$user_id','$order_id$user_id','$date_time','$working_with','$status','$mnc_mcc_lat_cellid','$latlng','$user_location','$remark',CURRENT_TIMESTAMP,'$battery_status','$gps_status','$company_id','$area_of_word_id','$working_with_type_id','$working_activity_id','$distributor_id','$new_area_name','$leave_from_date','$leave_to_date','$leave_id')";
                    $run = mysqli_query($dbc, $q);
                    if ($run) {
                        $unique_id_array[] = $order_id;
