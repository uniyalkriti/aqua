...
//   $approved_by= $_GET['user_id']; die;
  ...
  foreach($_POST['seniorapproval'] as $key=>$v)
    ...
    $qcheck="SELECT status FROM user_plan_request WHERE order_id=  '$mtp_id'";
 ...
 if(!empty($workdate)){
		 $qupdate = "UPDATE user_plan_request SET status='$v',approved_by='$user_id',sr_remarks='$admin_remark',approved_date=NOW()  WHERE order_id=  '$mtp_id'";
                   // h1($qupdate); exit;
                    $run = mysqli_query($dbc,$qupdate);
 if($run)
 {
  $eq="SELECT CONCAT_WS(' ',person.first_name,person.middle_name,person.last_name) AS jname,email AS jemail,emp_code AS je FROM person WHERE person.id='$person_id'";
...
$jcode=$roweq['je'];
$jemail=$roweq['jemail'];
 $sq="SELECT CONCAT_WS(' ',person.first_name,person.middle_name,person.last_name) AS sname,email AS semail,emp_code AS se FROM person WHERE person.id='$suser_id'";
...
$data.="Your request for dates from $cdate1 to $cdate2 of advance expense amount INR $total has been $estatus .";
...
<?php
	...
	$inc=1;
              ...
              //pre($value);
             $query = "SELECT user_plan_request.id AS id,user_plan_request.*,CONCAT_WS(' ',person.first_name,person.middle_name,person.last_name) as person_fullname,DATE_FORMAT(`from_date`,'%d-%m-%Y') AS wdate,DATE_FORMAT(`to_date`,'%d-%m-%Y') AS todate,remarks FROM user_plan_request INNER JOIN person ON person.id=user_plan_request.user_id WHERE DATE_FORMAT(`server_date_time`,'%Y-%m-%d')>='$from_date' AND DATE_FORMAT(`server_date_time`,'%Y-%m-%d')<='$to_date' AND approved_for='$user_id' AND status IN (0,2)";
   //h1($query);
...
$total=$rows['fare']+$rows['da']+$rows['fooding']+$rows['local_conveyance']+$rows['miscelleanous']+$rows['lodging'];
...
?>
 ...
 <p style="color: black; background-color:#50beed; font-size:100%; ">
  <b> S.No :</b> <?php echo $inc; ?> </br>
   ...
   <b>Place Of Visit :</b><?php echo $rows['place_of_visit'] ;?></br>
   <b>Approval Remarks :</b><?php echo $rows['approval_remarks'] ;?></br>
   <b>Fare :</b><?php echo $rows['fare'] ;?></br>
   <b>DA :</b><?php echo $rows['da'] ;?></br>
   <b>Fooding :</b><?php echo $rows['fooding'] ;?></br>
   <b>Lodging :</b><?php echo $rows['lodging'] ;?></br>
   <b>Local Conveyance :</b><?php echo $rows['local_conveyance'] ;?></br>
   <b>Miscelleanous :</b><?php echo $rows['miscelleanous'] ;?></br>
   <b>Total Expense :</b> <?php echo $total; ?></br>
...
<b> Hold :</b> <input type="radio" class="checkb" name="seniorapproval[<?=$mtpid?>]" id="seniorapproval<?=$inc?>" value="2"<?=$checked?>>
  </br>
