...
//        $CometitorNewLaunchProductReport = $data->response->CometitorNewLaunchProductReport;
        ...
        if (isset($UserEditInFormation) && !empty($UserEditInFormation)) {
            ...
            {
                $update_user = "UPDATE `person` SET `email`='$udata->email_id',``='$user_id',`mobile`='$udata->email_id' WHERE `id` = '$udata->user_id'";
...
//                echo $cmp_query;die;
        ...
        if (!empty($DailyProspectWorking)) {
            foreach ($DailyProspectWorking as $newd) {
                $dp_query = "INSERT INTO `daily_prospecting_working` (`party_name`, `party_address`, `phone_no`, `residence_phone`, `mobile_no`, `email_id`, `person_met_and_status`, `established_since`, `annual_turn_over`, `reputation_trade_relation`, `financial_position`, `level_ofinterst`, `from_time`, `to_time`, `units_availble_and_qty`, `gst_no`, `gst_registrtion_date`, `pan_card_no`, `pan_card_date`, `godown_size`, `no_of_employee`, `terms_condition`, `assured_investment`, `stockiest_from_filled`, `comments`, `orderid`, `latitude`, `longitude`, `mcc_mnc_lac_cellid`, `cur_date_time`) VALUES ('$newd->party_name','$newd->party_address','$newd->phone_no','$newd->residence_phone','$newd->mobile_no','$newd->email_id','$newd->person_met_and_status','$newd->established_since','$newd->annual_turn_over','$newd->reputation_trade_relation','$newd->financial_position','$newd->level_ofinterst','$newd->from_time','$newd->to_time','$newd->units_availble_and_qty','$newd->gst_no','$newd->gst_registrtion_date','$newd->pan_card_no','$newd->pan_card_date','$newd->godown_size','$newd->no_of_employee','$newd->terms_condition','$newd->assured_investment','$newd->stockiest_from_filled','$newd->comments','$newd->orderid','$newd->latitude','$newd->longitude','$newd->mcc_mnc_lac_cellid','$newd->cur_date_time')";
        ...
        if (!empty($paymentReceviedDetails)) {
            foreach ($paymentReceviedDetails as $k => $paymentData) {
                $addDealerPayment = "INSERT INTO dealer_payments( `dealer_id`, `zone_id`, `emp_id`, `user_designation`, `user_hq`, `town`,`payment_mode`, `accout_number`, `amount`, `drawn_from_bank`,`deposited_bank`,`payment_recevied_date`,`deposited_date`,`lat`,`lng`,`geo_address`,`cur_datetime`,`unique_id`,`order_id`, `mcc_mnc`) "
                    . "VALUES('$paymentData->distributor_id', '$paymentData->zone_id', '$paymentData->emp_id', '$paymentData->user_designation', '$paymentData->user_hq', '$paymentData->town', '$paymentData->payment_mode_id','$paymentData->number','$paymentData->amount','$paymentData->drawn_from_bank','$paymentData->deposited_bank','$paymentData->payment_received_date','$paymentData->deposited_date','$paymentData->latitude','$paymentData->longitude','$paymentData->geo_address','$paymentData->cur_datetime','$paymentData->unique_id','$paymentData->order_id', '$paymentData->mcc_mnc')";
...
///////////////////////////////////////////////////////////////////////////////
        ...
        if (!empty($PrimarySaleSummary)) {
            ...
            while ($ps < $psale) {
                ...
                $q = "INSERT INTO `user_primary_sales_order_details`(`id`, `order_id`, `product_id`, `rate`, `quantity`, `scheme_qty`,`cases`,`pcs`) "
                    . " VALUES('$order_id$ps','$order_id$user_id','$product_id','$rate','$quantity','$scheme_qty','$case','$pcs')";
                $result = mysqli_query($dbc, $q);
                $ps++;
            }
        }
...
//                }
        ...
        if (!empty($mtp)) {
            ...
            while ($m < $mtp_con) {
                ...
                $q = "INSERT INTO `monthly_tour_program`(`person_id`, `working_date`, `dayname`, `working_status_id`, `dealer_id`, `locations`, `total_calls`, `total_sales`, `ss_id`, `travel_mode`, `from`, `to`, `travel_distance`, `category_wise`, `mobile_save_date_time`,`upload_date_time`,`pc`,`rd`,`collection`,`primary_ord`,`new_outlet`,`any_other_task`)"
                    . " VALUES('$user_id','$working_date','$dayname','$working_status_id','$dealer_id','$locations','$total_calls','$total_sales','$ss_id','$travel_mode','$from','$to','$travel_distance','$category_wise','$mobile_save_date_time',NOW(),'$pc','$rd','$collection','$primary_ord','$new_outlet','$any_other_task')";
                $result = mysqli_query($dbc, $q);
                $m++;
            }
        }
