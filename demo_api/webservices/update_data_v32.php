...
//        $final_dealer_details = array();
        ...
        $leave_type=array();
        $leave_option=array();
        $return_type_reason = array();
        $non_productive_reason = array();
...
//        $webview=$webdata;
        // is _junior true or false starts here
            $jd = recursivejuniors($get_user_id);
            $temp = $_SESSION['resursivedata'];
            if(!empty($temp))
            {
                $is_junior = True;
            }
        ...
        //  ends here
        // module is for sync image start here
        $app_module_query  = "SELECT id,name FROM image_module WHERE status = 1"; 
        $app_module_run = mysqli_query($dbc, $app_module_query); 
        if ($app_module_run) {
            while ($row = mysqli_fetch_assoc($app_module_run)) {
                $module_data['id'] = $row['id'];
                $module_data['name'] = $row['name'];
                $app_module[] = $module_data;
            }
        }
        // module is for sync image ends here 
        // non_productive_reason starts here 
        $no_sale_reason_query = "SELECT id,name FROM _no_sale_reason WHERE status = 1";
        $no_sale_reason_run = mysqli_query($dbc, $no_sale_reason_query); 
        if ($no_sale_reason_run) {
            while ($row = mysqli_fetch_assoc($no_sale_reason_run)) {
                $nosale_data['id'] = $row['id'];
                $nosale_data['name'] = $row['name'];
                $non_productive_reason[] = $nosale_data;
            }
        }

        // return_type_damage starts here 
        $return_type_damage_query = "SELECT id,name FROM _return_type_damage WHERE status = 1";
        $return_type_damage_run = mysqli_query($dbc, $return_type_damage_query); 
        if ($return_type_damage_run) {
            while ($row = mysqli_fetch_assoc($return_type_damage_run)) {
                $return_data['id'] = $row['id'];
                $return_data['name'] = $row['name'];
                $return_type_reason[] = $return_data;
            }
        }

        // return_type_damage ends here 
        //Leave code start
        $qlt = "SELECT id, name FROM _leave_type";
        ...
        $qlc = "SELECT id, name FROM _leave_type_category";
        ...
        $qlo = "SELECT id, name,leave_cat_id FROM _leave_type_option";
...
//This query is used to fetch dealer information
            if ($role_id != 5) {
                $query_dealer = "SELECT d.id,d.name,csa_id,lv.l4_name,lv.l4_id FROM dealer AS d  INNER JOIN dealer_location_rate_list as dlrl ON dlrl.dealer_id=d.id INNER JOIN location_view as lv ON lv.l5_id=dlrl.location_id AND dlrl.user_id = $get_user_id GROUP BY id DESC"; //user_id
//h1($query_dealer);die;
                ...
                if (mysqli_num_rows($run_dealer) > 0) {
                    ...
                    while ($dealer_fetch = mysqli_fetch_assoc($run_dealer)) {
                        ...
                        $dealer_info['name'] = $dealer_fetch['name']." - ".$dealer_fetch['l4_name'];
...
// . " INNER JOIN catalog_2 ON catalog_2.id=catalog_product.catalog_id INNER JOIN catalog_1 ON catalog_1.id=catalog_2.catalog_1_id WHERE stateId='$state_id' ORDER BY name ASC";
                ...
                $query_catalog_product = "SELECT catalog_product.id,catalog_product.weight as weight,catalog_1.id as classification_id,catalog_1.name as classification_name,catalog_id,unit,catalog_product.name,cprl.retailer_pcs_rate as base_price,cprl.mrp_pcs as mrp, cprl.mrp_pcs,hsn_code,catalog_2.name as cname, cprl.dealer_rate,cprl.dealer_pcs_rate FROM catalog_product "
                    . " INNER JOIN product_rate_list cprl ON catalog_product.id = cprl.product_id"
                    . " INNER JOIN catalog_2 ON catalog_2.id=catalog_product.catalog_id INNER JOIN catalog_1 ON catalog_1.id=catalog_2.catalog_1_id WHERE state_id='$state_id'  ORDER BY catalog_product.id ASC";

                    //AND catalog_product.division IN ($product_division) this clause line is commented  
// h1($query_catalog_product);die;
...
//$catalog_product_info['state'] = $state_id;
                ...
                $query_dealer = "SELECT d.id,d.name,csa_id FROM dealer AS d INNER JOIN dealer_location_rate_list AS dlrl ON dlrl.dealer_id = d.id AND dlrl.user_id = $get_user_id GROUP BY id DESC"; //user_id
               // h1($query_dealer);die;
                $run_dealer = mysqli_query($dbc, $query_dealer);
                if (mysqli_num_rows($run_dealer) > 0) 
                {
...
//                echo $state_id;die;
                $query_catalogproduct = "SELECT catalog_product.id,catalog_1.id as classification_id,catalog_1.name as classification_name,catalog_id,unit,catalog_product.name,cprl.retailer_pcs_rate as base_price,cprl.mrp_pcs as mrp,cprl.mrp_pcs,hsn_code,catalog_2.name as cname, cprl.dealer_rate,cprl.dealer_pcs_rate FROM catalog_product "
                    . " INNER JOIN product_rate_list cprl ON catalog_product.id = cprl.product_id"
                    . " INNER JOIN catalog_2 ON catalog_2.id=catalog_product.catalog_id INNER JOIN catalog_1 ON catalog_1.id=catalog_2.catalog_1_id WHERE state_id='$state_id'  ORDER BY catalog_product.id ASC";
                    // this clause commented by karan AND catalog_product.division IN ($product_division)
...
//here some confusion and i need to ask sujeet sir..
            ...
            $query_retailer_access_on = "SELECT _role.rolename as designation,r.created_on,CONCAT_WS(' ',person.first_name,person.middle_name,person.last_name) as first_name ,udr.seq_id,r.id,r.email as email1,r.landline,r.contact_per_name,r.other_numbers,r.name,location_id,rl.name AS loc_name,r.address,r.tin_no as tin,lat_long FROM retailer r 
                INNER JOIN person on person.id = r.created_by_person_id 
                INNER JOIN _role on _role.role_id = person.role_id
                INNER JOIN user_dealer_retailer udr ON udr.retailer_id = r.id 
                INNER JOIN location_$dconstant_data[retailer_level] AS rl ON rl.id = r.location_id WHERE user_id = '$row[id]' AND r.location_id IN ($reatiler_loc_array_str) AND retailer_status='1' GROUP BY id DESC";
...
//$date_pay = $row_rso['date'];
                    ...
                    $retailer_info['created_by'] = $retailer_fetch['first_name'];
                    $retailer_info['created_by_designation'] = $retailer_fetch['designation'];
                    $retailer_info['created_at'] = $retailer_fetch['created_on'];
                    $retailer_info['last_visit_date'] = "No Oder book Yet";
...
//This query is Used to send catalog Classification name list
            ...
            $query_classification = "SELECT catalog_1.id as id, catalog_1.name as name FROM `catalog_1` INNER JOIN `catalog_view` ON catalog_1.id = c1_id  GROUP BY c1_id";
            // WHERE division IN ($product_division) this clause commented by karan 
...
//This query is used to send top level category list
            $query_category = "SELECT id,name, catalog_view.c1_id as classification_id, catalog_view.c1_name as classification_name FROM catalog_2 INNER JOIN `catalog_view` ON catalog_2.id = c2_id GROUP BY c2_id ORDER BY id ASC";
            $run_category = mysqli_query($dbc, $query_category) or die(mysqli_error($dbc));
            //  WHERE division IN ($product_division) this clause is commented 
...
///////////////////// FOR DEALER USER///////////////////
            if ($role_id == 5) {
                ...
                , "non_productive_reason" => $non_productive_reason
