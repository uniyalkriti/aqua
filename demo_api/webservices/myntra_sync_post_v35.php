...
if(isset($_POST['response']))
{
    // $endData = $_POST['response'];
    // $insertJson = "INSERT INTO `data_test`(`test`) VALUES ('$endData')";
    // $insertJsonData = mysqli_query($dbc, $insertJson);
...
//        $CometitorNewLaunchProductReport = $data->response->CometitorNewLaunchProductReport;
        ...
        $SaleReturnSummary = $data->response->SaleReturnSummary;
        $SaleReturnDetail = $data->response->SaleReturnDetail;



         if (!empty($SaleReturnDetail)) {
            $psalesum = count($SaleReturnDetail);
            $pd = 0;
            while ($pd < $psalesum) {
                $order_id = $SaleReturnDetail[$pd]->order_id;
                $dealer_id = $SaleReturnDetail[$pd]->dealer_id;
                $created_date = $SaleReturnDetail[$pd]->created_date;
                $sale_date = $SaleReturnDetail[$pd]->sale_date;
                $date_time = $SaleReturnDetail[$pd]->date_time;
                $new_work_time = date('H:i:s', strtotime($date_time));
                $lat = $SaleReturnDetail[$pd]->lat;
                $lng = $SaleReturnDetail[$pd]->lng;
                $lat_lng = $lat.','.$lng;
                $address = $SaleReturnDetail[$pd]->address;
                $gps_status = $SaleReturnDetail[$pd]->gps_status;
                $mcc_mnc_lac_cellid = !empty($SaleReturnDetail[$pd]->mcc_mnc_lac_cellid)?$SaleReturnDetail[$pd]->mcc_mnc_lac_cellid:'NA';
                $battery_status = $SaleReturnDetail[$pd]->battery_status;
                $dispatch_through = !empty($SaleReturnDetail[$pd]->dispatch_through)?$SaleReturnDetail[$pd]->dispatch_through:'';
                $destination = !empty($SaleReturnDetail[$pd]->destination)?$SaleReturnDetail[$pd]->destination:'';
                $comment = !empty($SaleReturnDetail[$pd]->comment)?$SaleReturnDetail[$pd]->comment:'';

                $retailer_id = $SaleReturnDetail[$pd]->retailer_id;
                $beat_id = $SaleReturnDetail[$pd]->beat_id;

                //$ch_date=$SaleReturnDetail[$pd]->ch_date;


                $q = "INSERT INTO `sales_return_order`(`id`,`order_id`, `dealer_id`, `created_date`, `created_person_id`, `sale_date`, `receive_date`, `date_time`,`lat`,`lng`,`address`,`company_id`,`dispatch_through`,`destination`,`comment`,`mantra_retailer_id`,`mantra_beat_id`)"
                    . " VALUES('$order_id$user_id','$order_id$user_id','$dealer_id','$created_date','$user_id','$sale_date',NOW(),'$date_time','$lat','$lng','$address','$company_id','$dispatch_through','$destination','$comment','$retailer_id','$beat_id')";
//                echo $q;die;
                $primary_order_track="INSERT INTO `user_work_tracking`(`user_id`, `track_date`, `track_time`, `mnc_mcc_lat_cellid`, `lat_lng`, `track_address`, `status`, `server_date_time`,`battery_status`,`gps_status`,`company_id`) VALUES ('$user_id','$sale_date','$new_work_time','$mcc_mnc_lac_cellid','$lat_lng','$address','Sales Return',NOW(),'$battery_status','$gps_status','$company_id')";
        ...
        if (!empty($SaleReturnSummary)) {
            //print_r($SaleReturnSummary);
            $psale = count($SaleReturnSummary);
            $ps = 0;
            while ($ps < $psale) {
                $order_id = $SaleReturnSummary[$ps]->order_id;
                $product_id = $SaleReturnSummary[$ps]->product_id;
                $rate = $SaleReturnSummary[$ps]->pcs_rate;  //app side sending pcs_rate as pcs rate(dealer rate)
                $quantity = $SaleReturnSummary[$ps]->quantity;
                $scheme_qty = !empty($SaleReturnSummary[$ps]->scheme_qty)?$SaleReturnSummary[$ps]->scheme_qty:'0';
                $case = $SaleReturnSummary[$ps]->case;
                $pcs = $SaleReturnSummary[$ps]->pcs;
                $case_rate = $SaleReturnSummary[$ps]->case_rate;
                $pcs_rate = $SaleReturnSummary[$ps]->pcs_rate;
                $secondary_qty = !empty($SaleReturnSummary[$ps]->secondary_qty)?$SaleReturnSummary[$ps]->secondary_qty:'0';


                $find_dealer_id = "SELECT mantra_retailer_id,sale_date from user_primary_sales_order where order_id='" . $order_id.$user_id . "' LIMIT 1 "; 
                $find_dealer_run = mysqli_query($dbc, $find_dealer_id);
                $find_dealer_array = mysqli_fetch_assoc($find_dealer_run);
                $mantra_retailer_id = $find_dealer_array['mantra_retailer_id'];
                $sale_date = $find_dealer_array['sale_date'];


                $qo = "select * from sales_return_order_details where order_id='" . $order_id.$user_id . "' AND product_id='" . $product_id . "'";
                $sqlo = mysqli_query($dbc, $qo);
                $numo = mysqli_num_rows($sqlo);
                if ($numo < 1) {

                $qunatity_per_case_query = "SELECT quantity_per_case,id FROM  `catalog_product` WHERE company_id = '$company_id'  GROUP BY id";
                $qunatity_per_case_res = mysqli_query($dbc, $qunatity_per_case_query);
                 while ($qunatity_per_case_row = mysqli_fetch_array($qunatity_per_case_res)) {
                       $id = $qunatity_per_case_row['id'];
                       $qunatity_per_case_result[$id] = $qunatity_per_case_row['quantity_per_case'];
                }



                // $qunatity_per_case = !empty($qunatity_per_case_result[$product_id])?$qunatity_per_case_result[$product_id]:'0';
                // $final_qty_conversion = $pcs+($case*$qunatity_per_case);

                // $update_final_stock = "UPDATE `mantra_stock` SET `qty`=`qty`+'$final_qty_conversion' WHERE mantra_retailer_id= $mantra_retailer_id AND product_id=$product_id AND company_id = $company_id";
                // $update_final_stock_run = mysqli_query($dbc, $update_final_stock);



                 $q = "INSERT INTO `sales_return_order_details`(`id`, `order_id`, `product_id`, `rate`, `quantity`, `scheme_qty`,`cases`,`pcs`,`pr_rate`,`company_id`,`secondary_qty`) "
                    . " VALUES('$order_id$ps','$order_id$user_id','$product_id','$rate','$quantity','$scheme_qty','$case','$pcs','$case_rate','$company_id','$secondary_qty')";
                    // die;
                $result = mysqli_query($dbc, $q);


                if ($result) { // stock update
                    $stk_q = "SELECT * FROM mantra_stock WHERE mantra_retailer_id=$mantra_retailer_id AND product_id=$product_id AND company_id = $company_id";
                    $stk_e = mysqli_query($dbc,$stk_q);
                    if(mysqli_num_rows($stk_e)<="0")
                    {
                     $final_stock="INSERT INTO `mantra_stock`(`product_id`, `rate`, `person_id`, `mantra_retailer_id`, `qty`, `date`,`pr_rate`,`company_id`) VALUES ('$product_id','$pcs_rate','$user_id','$mantra_retailer_id','$pcs','$sale_date','$rate','$company_id')"; 
                    $final_stock_run = mysqli_query($dbc, $final_stock);
                    }
                    else{
                      $update_final_stock = "UPDATE `mantra_stock` SET `qty`=`qty`+'$pcs' WHERE mantra_retailer_id=$mantra_retailer_id AND product_id=$product_id AND company_id = $company_id";
                        $update_final_stock_run = mysqli_query($dbc, $update_final_stock);
                    }
        ...
        if(!empty($CounterSaleDetail))
        {
            foreach ($CounterSaleDetail as $key => $c_value_s) 
            {
                ...
                $retailer_id = $c_value_s->retailer_name;
                $beat_id = $c_value_s->beat_id;


                $q2 = "select * from counter_sale_summary where order_id='" . $order_id . "'AND `company_id` = '$company_id'";
                $sql = mysqli_query($dbc, $q2);
                $num = mysqli_num_rows($sql);
                if ($num < 1) {

                $counter_sale_data = "INSERT INTO `counter_sale_summary`(`order_id`, `dealer_id`,`retailer_id`,`mantra_beat_id`, `sale_date`, `created_date`, `date_time`, `battery_status`, `gps_status`, `lat`, `lng`, `mcc_mnc_lac_cellid`, `company_id`, `address`, `created_by_person`,`server_date`) VALUES ('$order_id','$dealer_id','$retailer_id','$beat_id','$sale_date','$created_date','$date_time','$battery_status','$gps_status','$lat','$lng','$mcc_mnc_lac_cellid','$company_id','$address','$user_id',NOW())";
        ...
        if(!empty($CounterSaleSummary))
        {
            foreach ($CounterSaleSummary as $counter_key => $counter_value) 
            {
                ...
                $find_dealer_id = "SELECT retailer_id from counter_sale_summary where order_id='" . $order_id . "' LIMIT 1 "; 
                $find_dealer_run = mysqli_query($dbc, $find_dealer_id);
                $find_dealer_array = mysqli_fetch_assoc($find_dealer_run);
                $retailer_id = $find_dealer_array['retailer_id'];



                $qo = "select * from counter_sale_details where order_id='" . $order_id . "' AND product_id='" . $product_id . "'";
                $sqlo = mysqli_query($dbc, $qo);
                $numo = mysqli_num_rows($sqlo);
                if ($numo < 1) {
                ...
                $find_stock = "SELECT qty from mantra_stock where mantra_retailer_id='" . $retailer_id . "' AND product_id='" . $product_id . "' LIMIT 1 "; 
                $find_stock_run = mysqli_query($dbc, $find_stock);
                $find_stock_array = mysqli_fetch_assoc($find_stock_run);
                $stock_qty = $find_stock_array['qty'];

                if(!empty($stock_qty)){
                    if($stock_qty >= $final_qty_conversion){
                        $stock_qty_for_updation = $stock_qty - $final_qty_conversion;
                        $update_final_stock = "UPDATE `mantra_stock` SET `qty`='$stock_qty_for_updation' WHERE mantra_retailer_id=$retailer_id AND product_id=$product_id AND company_id = $company_id";
                        $update_final_stock_run = mysqli_query($dbc, $update_final_stock);

                    }else{
                        $update_final_stock = "UPDATE `mantra_stock` SET `qty`='0' WHERE mantra_retailer_id=$retailer_id AND product_id=$product_id AND company_id = $company_id";
                        $update_final_stock_run = mysqli_query($dbc, $update_final_stock);
                    }
...
//echo $q;
        ...
        if (!empty($BalanceStock)) {
            ...
            while ($bre < $count_BalanceStock) {
                ...
                $retailer_id = $BalanceStock[$bre]->retailer_id;
                $beat_id = $BalanceStock[$bre]->beat_id;

                $finalOrderId = $balance_order_id.$user_id;

                $qo = "select * from dealer_balance_stock where order_id='" . $finalOrderId . "' AND product_id='" . $balance_product_code . "'";
                $sqlo = mysqli_query($dbc, $qo);
                $numo = mysqli_num_rows($sqlo);
                if($numo < 1) {
                ...
                $qbr = "INSERT INTO `dealer_balance_stock`(`order_id`, `dealer_id`, `user_id`, `product_id`, `stock_qty`, `mfg_date`, `exp_date`, `cases`, `submit_date_time`, `server_date_time`,`mrp`,`pcs_mrp`,sstatus,`lat`,`lng`,`address`,`company_id`,`balance_secondary_qty`,`mantra_retailer_id`,`mantra_beat_id`) VALUES ('$balance_order_id$user_id','$dealer_id','$user_id','$balance_product_code','$balance_pieces','$mfg_date','$exp_date','$balance_cases','$mobile_datetime',NOW(),'$bsmrp','$balance_pcs_mrp','0','$lat','$lng','$address','$company_id','$balance_secondary_qty','$retailer_id','$beat_id')";
                ...
                // $qunatity_per_case = !empty($qunatity_per_case_result[$balance_product_code])?$qunatity_per_case_result[$balance_product_code]:'0';
                // $final_qty_conversion = $balance_pieces+($balance_cases*$qunatity_per_case);
                // $final_qty_conversion = $balance_pieces;




                $stk_q = "SELECT * FROM mantra_stock WHERE mantra_retailer_id=$retailer_id AND product_id=$balance_product_code AND company_id = $company_id";
                $stk_e = mysqli_query($dbc,$stk_q);
                if(mysqli_num_rows($stk_e)<="0")
                {
                 $final_stock="INSERT INTO `mantra_stock`(`product_id`, `rate`, `person_id`, `mantra_retailer_id`, `qty`, `date`,`pr_rate`,`company_id`) VALUES ('$balance_product_code','$balance_pcs_mrp','$user_id','$retailer_id','$balance_pieces','$date','$balance_pcs_mrp','$company_id')"; 
                $final_stock_run = mysqli_query($dbc, $final_stock);
                }
                else{
                  $update_final_stock = "UPDATE `mantra_stock` SET `qty`=`qty`+'$balance_pieces' WHERE mantra_retailer_id=$retailer_id AND product_id=$balance_product_code AND company_id = $company_id";
                    $update_final_stock_run = mysqli_query($dbc, $update_final_stock);
                }
                ...
                if ($result_br) {
                    $unique_id_array[] = $update_final_stock;
...
///////////////////////////////////////////////////////////////////////////////
        if (!empty($callwisereportingstatus)) {
            ...
            while ($p < $callwisereport) {
                ...
                if ($num_data < 1) {
                    $qusod = "INSERT INTO `user_sales_order_details`(`order_id`, `product_id`, `rate`,`case_rate`, `quantity`, `scheme_qty`,`company_id`,`piece_quantity`,`case_quantity`,`secondary_quantity`,`secondary_rate`)"
                        . " VALUES('$orderid$user_id','$product_id','$rate','$case_rate','$prod_qty','$scheme_qty','$company_id','$piece_quantity','$case_quantity','$secondary_quantity','$secondary_rate')";
        ...
        if (!empty($callwisereporting)) {
         ...
         if (!empty($Primarysaledetail)) {
            ...
            while ($pd < $psalesum) {
                ...
                $retailer_id = $Primarysaledetail[$pd]->retailer_id;
                $beat_id = $Primarysaledetail[$pd]->beat_id;

                //$ch_date=$Primarysaledetail[$pd]->ch_date;


                $q = "INSERT INTO `user_primary_sales_order`(`id`,`order_id`, `dealer_id`, `created_date`, `created_person_id`, `sale_date`, `receive_date`, `date_time`,`lat`,`lng`,`address`,`company_id`,`dispatch_through`,`destination`,`comment`,`mantra_retailer_id`,`mantra_beat_id`)"
                    . " VALUES('$order_id$user_id','$order_id$user_id','$dealer_id','$created_date','$user_id','$sale_date',NOW(),'$date_time','$lat','$lng','$address','$company_id','$dispatch_through','$destination','$comment','$retailer_id','$beat_id')";
//                echo $q;die;
        ...
        if (!empty($PrimarySaleSummary)) {
            ...
            while ($ps < $psale) {
                ...
                $qo = "select * from user_primary_sales_order_details where order_id='" . $order_id.$user_id . "' AND product_id='" . $product_id . "'";
                $sqlo = mysqli_query($dbc, $qo);
                $numo = mysqli_num_rows($sqlo);
                if ($numo < 1) {
                ...
                // $update_final_stock_run = mysqli_query($dbc, $update_final_stock);
                 ...
                 $q = "INSERT INTO `user_primary_sales_order_details`(`id`, `order_id`, `product_id`, `rate`, `quantity`, `scheme_qty`,`cases`,`pcs`,`pr_rate`,`company_id`,`secondary_qty`) "
                    . " VALUES('$order_id$ps','$order_id$user_id','$product_id','$rate','$quantity','$scheme_qty','$case','$pcs','$case_rate','$company_id','$secondary_qty')";
                    // die;
                $result = mysqli_query($dbc, $q);
