...
$q1 = "SELECT product_id,catalog_product.name,sum(quantity) AS stock,sum(quantity*rate) AS stock_value FROM user_sales_order INNER JOIN user_sales_order_details ON user_sales_order.order_id=user_sales_order_details.order_id INNER JOIN catalog_product ON  user_sales_order_details.product_id=catalog_product.id WHERE user_id='$user_id' AND DATE_FORMAT(`date`,'%Y-%m-%d')>='$s_date' AND DATE_FORMAT(`date`,'%Y-%m-%d')<='$e_date' AND dealer_id='$dealer_id' GROUP BY user_id,dealer_id,product_id";
//h1($q1);
