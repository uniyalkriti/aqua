<?php
error_reporting(1);
// include "../client/include/generate_data.php";
...
$token = mysqli_real_escape_string($dbc, trim(stripslashes($_GET['token'])));
...
AND imei_number = '$imei' AND person_status = '1' AND status = '1' ORDER BY person_username ASC";
...
$role_id_person =  $user_res['role_id'];
$company_id =  $user_res['company_id'];
// echo $company_id; die;

$qtoken="UPDATE `users` SET `fcm_token`='$token' WHERE user_id='$get_user_id'";
$rtoken=mysqli_query($dbc,$qtoken);

$loggedInTownArr=[];
if (!empty($user_res['state_id']))
{
    $logged_query="SELECT * FROM location_4 WHERE location_3_id='$user_res[state_id]' AND company_id='$company_id' ORDER BY location_4.name ASC";
...
AND DAY(`date`)>='1' AND DAY(`date`)<='$day' AND user_id='$id' WHERE monthly_tour_program.company_id='$company_id'";
// h1($ach);
    ...
    #MTD End
    $rest = substr($imei, -5) * 1;
    $query_person_login = "SELECT *,person.state_id,person.mobile,CONCAT_WS(' ',first_name,middle_name,last_name) as full_person_name FROM `person_login` 
    INNER JOIN person ON person_login.person_id = person.id AND person.id='$id' AND person.company_id='$company_id' AND person_status = '1' ORDER BY person_username ASC";
...
//        $webview=$webdata;
        //Leave code start
        $qlt = "SELECT id, name FROM _leave_type WHERE company_id='$company_id'";
        $rlt = mysqli_query($dbc, $qlt);
        if ($rlt) {
            while ($rowlt = mysqli_fetch_assoc($rlt)) {
                $ltisit['id'] = $rowlt['id'];
                $ltisit['name'] = $rowlt['name'];
                $leave_type[] = $ltisit;
            }
        }

        $qlc = "SELECT id, name FROM _leave_type_category WHERE company_id='$company_id'";
        $rlc = mysqli_query($dbc, $qlc);
        if ($rlc) {
            while ($rowlct = mysqli_fetch_assoc($rlc)) {
                $lctisit['id'] = $rowlct['id'];
                $lctisit['name'] = $rowlct['name'];
                $leave_cat[] = $lctisit;
            }
        }

        $qlo = "SELECT id, name,leave_cat_id FROM _leave_type_option WHERE company_id='$company_id'";
        $rlo = mysqli_query($dbc, $qlo);
        if ($rlo) {
            while ($rowlot = mysqli_fetch_assoc($rlo)) {
                $lotisit['id'] = $rowlot['id'];
                $lotisit['name'] = $rowlot['name'];
                $lotisit['leave_cat_id'] = $rowlot['leave_cat_id'];
                $leave_option[] = $lotisit;
            }
        }

        #working status
        $nonq = "SELECT * FROM `_working_with` WHERE status=1 AND company_id='$company_id'";
        ...
        function getSeniorList($id,$dbc,$j=0)
        {
            ...
            $query = "SELECT a.id as id,CONCAT_WS(' ',a.first_name,a.middle_name,a.last_name) as name from person a INNER JOIN  person b ON a.id=b.person_id_senior WHERE b.id=$id AND b.company_id='$company_id'";
        ...
        function getJuniorList($id,$dbc,$j=0)
        {
            ...
            $query = "SELECT b.id as id,CONCAT_WS(' ',b.first_name,b.middle_name,b.last_name) as name from person b INNER JOIN  person a ON a.id=b.person_id_senior WHERE b.person_id_senior=$id AND b.company_id='$company_id'";
            $qr = mysqli_query($dbc, $query) or die(mysqli_error($dbc));
            if (mysqli_num_rows($qr) > 0) {
                while ($row = mysqli_fetch_assoc($qr)) {
        ...
        $role_query="SELECT role_id,rolename from _role where rolename!='ISR' AND company_id='$company_id'";
        ...
        #Tracking constants
        $tracq = "SELECT * FROM `_tracking` WHERE company_id='$company_id'";
...
//pre($constant_data);
        ...
        $q_sch = "SELECT * FROM _daily_schedule WHERE company_id='$company_id' ORDER BY id ASC";
        ...
        $qoutlet = "SELECT id, outlet_type FROM _retailer_outlet_type WHERE company_id='$company_id'";
        ...
        #outlet category
        $dt=[];
        $query = "SELECT id, outlet_category FROM _retailer_outlet_category where company_id='$company_id' AND status=1";
...
// here we get data from ownership type
        $qowner = "SELECT id, ownership_type FROM _dealer_ownership_type WHERE company_id='$company_id'";
        ...
        $qexperiance = "SELECT id, experience FROM _field_experience WHERE company_id='$company_id'";
        ...
        $qgift = "SELECT id, gift_name FROM _retailer_mkt_gift WHERE company_id='$company_id'";
...
//*******************  NEW product_recommended_details *************************
        $qpr = "SELECT * FROM product_recommended_details WHERE company_id='$company_id' AND view_status=1";
        ...
        #Payment mode
        $pq = "SELECT mode,id FROM  _payment_modes WHERE status=1 AND company_id='$company_id'";
        ...
        $qtravel = "SELECT id, mode FROM _travelling_mode WHERE company_id='$company_id'";
        ...
        $qworking = "SELECT id,name,parent_id FROM _working_status where id !='9' AND company_id='$company_id' order by sequence asc";
//echo $qworking;die;
...
//pre($final_working_deatails);
        ...
        $qrole = "SELECT role_id, rolename FROM _role WHERE company_id='$company_id'";
...
//getting bank branch list
        ...
        $qbank = "SELECT id,branch_name FROM _bank_branch WHERE company_id='$company_id'";
        ...
        $urincreid_qry = "SELECT max(id) as ret_id FROM retailer WHERE id LIKE '$rest%' AND company_id='$company_id' ";
        ...
        $sss=$row['state_id'];
        // h1("SELECT l1_id,l2_id,l1_name,l2_name from location_view where l3_id='$sss' limit 0,1 ");
        $person_location = mysqli_query($dbc, "SELECT l1_id,l2_id,l1_name,l2_name from location_view where l3_id='$sss' AND l3_company_id='$company_id' limit 0,1 ");
...
LEFT JOIN _role on person.role_id=_role.role_id WHERE `person_id` = '$person_id' AND person_details.company_id='$company_id'");
...
LEFT JOIN `location_view` on location_view.l7_id=dealer_location_rate_list.location_id 
WHERE dealer_location_rate_list.user_id = '$person_id' AND dealer_location_rate_list.company_id='$company_id'");
        while ($dlrl_data = mysqli_fetch_assoc($dlrl)) {
            $loc5[$inc]['l4_id'] = $dlrl_data['l6_id'];
            $loc5[$inc]['l4_name'] = $dlrl_data['l6_name'];
            $loc5[$inc]['id'] = $dlrl_data['l7_id'];
            $loc5[$inc]['name'] = $dlrl_data['l7_name'];
            $inc++;
        }
        $inc = 0;
        $dlrl = mysqli_query($dbc, "SELECT location_view.* FROM `dealer_location_rate_list` 
LEFT JOIN `location_view` on location_view.l7_id=dealer_location_rate_list.location_id 
WHERE dealer_location_rate_list.user_id = '$person_id' AND dealer_location_rate_list.company_id='$company_id' GROUP BY l4_id ");
        while ($dlrl_data = mysqli_fetch_assoc($dlrl)) {
            $loc4[$inc]['l3_id'] = $dlrl_data['l5_id'];
            $loc4[$inc]['l3_name'] = $dlrl_data['l5_name'];
            $loc4[$inc]['id'] = $dlrl_data['l6_id'];
            $loc4[$inc]['name'] = $dlrl_data['l6_name'];
            $inc++;
        }
        $inc = 0;
        $dlrl = mysqli_query($dbc, "SELECT location_view.* FROM `dealer_location_rate_list` 
LEFT JOIN `location_view` on location_view.l7_id=dealer_location_rate_list.location_id 
WHERE dealer_location_rate_list.user_id = '$person_id' AND dealer_location_rate_list.company_id='$company_id' GROUP BY l3_id ");
        while ($dlrl_data = mysqli_fetch_assoc($dlrl)) {
        ...
        $dlrl = mysqli_query($dbc, "SELECT location_view.* FROM `dealer_location_rate_list` 
LEFT JOIN `location_view` on location_view.l7_id=dealer_location_rate_list.location_id 
WHERE dealer_location_rate_list.user_id = '$person_id' AND dealer_location_rate_list.company_id='$company_id' GROUP BY l2_id ");
        while ($dlrl_data = mysqli_fetch_assoc($dlrl)) {
        ...
        $dlrl = mysqli_query($dbc, "SELECT location_view.* FROM `dealer_location_rate_list` 
LEFT JOIN `location_view` on location_view.l7_id=dealer_location_rate_list.location_id 
WHERE dealer_location_rate_list.user_id = '$person_id' AND dealer_location_rate_list.company_id='$company_id' GROUP BY l1_id ");
        while ($dlrl_data = mysqli_fetch_assoc($dlrl)) {
...
//here we get user person details
        $query_person = "SELECT p.id AS rId, CONCAT_WS(' ',first_name,middle_name,last_name) AS full_name,mobile,address,dob FROM person p INNER JOIN person_details pd ON pd.person_id = p.id WHERE p.id = $row[id] AND p.company_id='$company_id'";
...
// user person details end here
                $query_dealer = "SELECT d.id,d.name,csa_id FROM dealer AS d 
                INNER JOIN dealer_location_rate_list as dlrl ON dlrl.dealer_id=d.id 
                WHERE dlrl.user_id = '$row[id]' AND dlrl.company_id='$company_id' GROUP BY id DESC"; 
                 // h1($query_dealer);
                ...
                if (mysqli_num_rows($run_dealer) > 0) {
                    ...
                    while ($dealer_fetch = mysqli_fetch_assoc($run_dealer)) {
                        ...
                        $dealer_info['lid'] = '0';
                        $dealer_info['lname'] = 'NA';
                        $dealer_info['rate_lists_id'] = "0";
                ...
                if (!empty($d_arr)) {
                    ...
                    $myq = "SELECT l3_name,l3_id,l4_name,l4_id from dealer_location_rate_list LEFT JOIN location_view ON dealer_location_rate_list.location_id=location_view.l5_id where dealer_id IN ($d_string) AND dealer_location_rate_list.company_id='$company_id' GROUP BY l4_id";
                ...
                if($role_id_person == 1)
                {
                    ...
                    . " INNER JOIN catalog_2 ON catalog_2.id=catalog_product.catalog_id"
                    . " INNER JOIN catalog_1 ON catalog_1.id=catalog_2.catalog_1_id WHERE catalog_product.company_id='$company_id' ORDER BY catalog_product.id ASC";
                }
                else
                {
                    $query_catalogproduct = "SELECT catalog_product.id,catalog_1.id as classification_id,catalog_1.name as classification_name,catalog_id,unit,catalog_product.name,cprl.retailer_pcs_rate as base_price,cprl.mrp_pcs as mrp,cprl.mrp_pcs,hsn_code,catalog_2.name as cname, cprl.dealer_rate,cprl.dealer_pcs_rate FROM catalog_product "
                ...
                if (mysqli_num_rows($run_catalogproduct) > 0) {
                    ...
                    while ($catalog_productfetch = mysqli_fetch_assoc($run_catalogproduct)) {
                        ...
                        $fq = "select `product_id` from `focus` where `product_id`=$pid AND company_id='$company_id'";
...
//Tax
                        $querytax = "SELECT igst as tax FROM `_gst` where `hsn_code`='$hsn' AND company_id='$company_id'";
                        $taxx = mysqli_query($dbc, $querytax);
                        $row_tax = mysqli_fetch_assoc($taxx);

//FOCUS TARGET ADDED 04-12-2017
                        $now_date = date('Y-m-d');
                        $focust = mysqli_query($dbc, "SELECT `target_value` FROM `focus_product_users_target` WHERE company_id='$company_id' AND `product_id`=$pid AND user_id=$id AND '$now_date' BETWEEN `start_date` AND `end_date`");
                        ...
                        $catalog_productinfo['dealer_id'] = "0";
            ...
            if (!empty($dealer_array)) {
                ...
                $query_locality = "SELECT l.name AS locname,l.id AS lid,dealer_id "
                    ...
                    . "WHERE dl.user_id=$id AND dl.dealer_id in ($dealerarray) AND dl.company_id=$company_id group by dealer_id,lid"; //here we get beat
            ...
            if (!empty($beat_array)) {
                ...
                $querylocation .= " INNER JOIN dealer_location_rate_list ON location_$dlevel.id = dealer_location_rate_list.location_id WHERE location_$dlevel.id IN ($locationarray) AND dealer_id IN ($dealerarray) AND dealer_location_rate_list.company_id=$company_id GROUP BY location DESC";
                // h1($querylocation);die;
            ...
            WHERE user_id = '$row[id]' AND r.location_id IN ($reatiler_loc_array_str) AND retailer_status='1' AND r.company_id='$company_id' GROUP BY id DESC";
            ...
            if (mysqli_num_rows($run_retailer) > 0) {
                while ($retailer_fetch = mysqli_fetch_assoc($run_retailer)) {
                    ...
                    $rso = mysqli_query($dbc, "SELECT sum(total_amount) as paid FROM payment_collection WHERE retailer_id = '$retailer_info[id]' AND company_id='$company_id'");
                    $row_rso = mysqli_fetch_assoc($rso);
                    $paid = $row_rso['paid'];
                    $ccso = mysqli_query($dbc, "SELECT SUM(`amount`) AS ch_amt FROM `challan_order` WHERE ch_retailer_id = '$retailer_info[id]' AND company_id='$company_id' ");

                    $row_cso = mysqli_fetch_assoc($ccso);
                    $ch_amt = $row_cso['ch_amt'];
                    $outstanding = $ch_amt - $paid;

                    $rso1 = mysqli_query($dbc, "SELECT total_amount FROM payment_collection WHERE retailer_id = '$retailer_info[id]' AND company_id='$company_id' Order By pay_date_time limit 0,1");
            ...
            $query_classification = "SELECT catalog_1.id as id, catalog_1.name as name FROM `catalog_1` 
            INNER JOIN `catalog_view` ON catalog_1.id = c1_id WHERE catalog_1.company_id='$company_id'  GROUP BY c1_id";
            ...
            $query_damage_product = "SELECT id,`name` as catalog_product,base_price as rate,'1' as state_id FROM catalog_product WHERE catalog_product.company_id='$company_id'";
            ...
            $idealer_data = "SELECT GROUP_CONCAT(DISTINCT dealer_id ORDER BY dealer_id ASC SEPARATOR ',') as dealer_id FROM user_dealer_retailer where user_id='$person_id' AND user_dealer_retailer.company_id='$company_id' ";
   ...
   INNER JOIN dealer ON udr.dealer_id=dealer.id where person_id_senior =$person_id and (person.role_id='14' OR person.role_id='15') AND person.company_id='$company_id' group by dealer_id,person.id";
// h1($isr_data);
...
//This query is used to send top level category list
            $query_category = "SELECT c1_id,c1_name,c2_id as id,c2_name as name FROM catalog_view WHERE c0_company_id='$company_id'   GROUP BY c2_id ORDER BY c2_name ASC";
            // h1($query_category);
            ...
            if (mysqli_num_rows($run_category) > 0) {
                while ($category_fetch = mysqli_fetch_assoc($run_category)) {
                    ...
                    $category_info['classification_id'] = $category_fetch['c1_id'];
                    $category_info['classification_name'] = $category_fetch['c1_name'];
...
//LEAVE
            $query_l = "SELECT id,name FROM `leave_type` WHERE company_id='$company_id' ORDER BY id ASC";
...
//Stock
            $query_s = "SELECT product_id,rate,dealer_id,remaining FROM `stock` where dealer_id IN($dealerarray) AND company_id='$company_id'";
...
//GIFT//
            $query_g = "SELECT * FROM `_retailer_mkt_gift` WHERE company_id='$company_id' ORDER BY id ASC";
...
//TARGET ACHIEVED
            $query_ta = "SELECT svp.id as id,svp.value as value,svp.value_to as value_to,svp.scheme_gift as scheme_gift,svpd.start_date as start_date,svpd.end_date as end_date from scheme_value_product_details svp INNER JOIN scheme_value svpd ON svpd.scheme_id = svp.scheme_id where NOW() BETWEEN svpd.start_date AND svpd.end_date AND user = 2 AND state_id = $state_id AND svp.company_id='$company_id'";
            ...
            if (mysqli_num_rows($run_ta) > 0) {
                while ($ta_fetch = mysqli_fetch_assoc($run_ta)) {
                    ...
                    $q = "SELECT SUM(rate*(quantity+scheme_qty)) as achieved FROM `user_primary_sales_order` upso INNER JOIN user_primary_sales_order_details upsod ON upsod.order_id = upso.order_id where is_claim=0 AND action=1 AND ch_date >='$ta_info[start_date]' AND ch_date<='$ta_info[end_date]' AND upso.company_id='$company_id'";
// h1($q);
...
//Retailer TARGET//
            $query_ta = "SELECT svp.id as id,svp.value as value,svp.value_to as value_to,svp.scheme_gift as scheme_gift,svpd.start_date as start_date,svpd.end_date as end_date from scheme_value_product_details svp INNER JOIN scheme_value svpd ON svpd.scheme_id = svp.scheme_id where NOW() BETWEEN svpd.start_date AND svpd.end_date AND user = 3 AND state_id = $state_id AND svp.company_id='$company_id'";
            $run_ta = mysqli_query($dbc, $query_ta) or die(mysqli_error($dbc));
            if (mysqli_num_rows($run_ta) > 0) {
                while ($ta_fetch = mysqli_fetch_assoc($run_ta)) {
                    $ta_info['id'] = $ta_fetch['id'];
...
//Scheme
            $query_sc = "SELECT * FROM `scheme_product_details` WHERE scheme_quantity>=1 AND company_id='$company_id' ORDER BY id ASC";
            ...
            $cdata = "SELECT id, name FROM complaint_type WHERE company_id='$company_id'";
            $cquery = mysqli_query($dbc, $cdata);
            while ($complain = mysqli_fetch_assoc($cquery)) {
...
//MTP
            $cdate = date('Y-m-d');
            $query_mtp = "SELECT rd,total_sales,working_date,locations,task_of_the_day FROM `monthly_tour_program` WHERE date_format(`working_date`,'%Y-%m')= date_format('$cdate','%Y-%m') AND person_id='$person_id' AND company_id='$company_id' ORDER BY id ASC";
            ...
            if (mysqli_num_rows($run_mtp) > 0) {
                while ($sc_mtp = mysqli_fetch_assoc($run_mtp)) {
                    ...
                    $loc = mysqli_query($dbc, "SELECT name FROM `location_5` WHERE `id` = $loc_mt AND company_id='$company_id'");
...
//                    $mtp_info['task_of_the_day'] = $sc_mtp['task_of_the_day'];
            ...
            #Task Of the Day
            $task = [];
            $q = "SELECT task, id from _task_of_the_day WHERE company_id='$company_id'";
...
//SCHEME BUILT VALUE
            $sosdate = date('Y-m-d');
            $query_sos = "SELECT sos.scheme_id,scheme_name,start_date,end_date,value,value_to,scheme_gift FROM `scheme_on_sale` sos INNER JOIN scheme_on_sale_details sosd ON sos.`scheme_id`=sosd.scheme_id WHERE '$sosdate' BETWEEN `start_date` AND `end_date` AND intype='1' AND sos.company_id='$company_id'";
