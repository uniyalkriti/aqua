...
function breadcumMenu($in, $str = '')
{
	...
	foreach($in as $key=>$value){
		if(!$match){
			...
			if($match)	
				$out .= '<li><a style="background-color:#3d8225;" href="'.$str.'?option='.$key.'">'.$value.'</a></li>';
			else
...
function pagination($table, $per_page = 10,$page = 1, $url = '?', $page_filter){    
	global $dbc; 
        $out = array('totrecords'=>'', 'pagination_link'=>'');
    	$query = "SELECT COUNT(*) as `num` FROM {$table} {$page_filter}";
...
function get_avlb_stock_by_prodid_mrp($prodid,$mrp,$batch_no,$mfg){

    global $dbc;
    /*$q1 = "SELECT SUM(qty) AS issue FROM challan_order INNER JOIN `challan_order_details` ON challan_order.id = challan_order_details.ch_id WHERE challan_order_details.product_id = '$prodid' AND  challan_order_details.mrp='$mrp'  AND ch_dealer_id = '".$_SESSION[SESS.'data']['dealer_id']."' ";
    //echo '<$>'.$q1;
    ...
    $q2 = "SELECT SUM(quantity)as quantity FROM user_primary_sales_order_details  usod INNER JOIN user_primary_sales_order uso ON uso.order_id = usod.order_id WHERE usod.product_id = '$prodid' AND usod.rate='$mrp' AND uso.dealer_id = '".$_SESSION[SESS.'data']['dealer_id']."'  ";
    //echo '<$>'.$q2;
    ...
    $stock_q = "SELECT qty FROM `stock` WHERE `product_id`='$prodid' AND `dealer_id`='".$_SESSION[SESS.'data']['dealer_id']."' AND `mrp`='$mrp' AND `batch_no`='$batch_no' AND `mfg`='$mfg'";
   //h1($stock_q);
    $stock_e = mysqli_query($dbc,$stock_q);
    $stock 	 = mysqli_fetch_assoc($stock_e);
...
function get_trade_disc_calculate($quantity,$rate,$tds_amt,$tds_type,$tds_val)
{

    if($tds_type == '1'){
        $res = $rate * ($tds_val/100);
        $tds_amt = my2digit($res);
        $ttl_amt = ( $rate - $tds_amt) * $quantity;
    }else{
        $res = $tds_val;
        $tds_amt = $res;
        $ttl_amt = ($rate - $tds_amt) * $quantity;
