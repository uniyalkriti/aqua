...
function db_pulldown170($dbc, $name, $q, $arrkey = false, $inioption = true, $jsfunction='',$firstoptiontext = '== Please select ==', $valuetoset='')
{
	...
	if($r)
	{
		if(mysqli_num_rows($r)>0)
		{
			echo'<select name="'.$name.'" '.$jsfunction.' style="width: 300px">';
			$optarray = array();
			if($arrkey)
			{
				while($row = mysqli_fetch_row($r))
		...
		{
			echo'<select name="'.$name.'" '.$jsfunction.' style="width: 270px">';
			echo'<option value="">N.A.</option>';
			echo'</select>';			
		}
	}
...
function get_product_multiple_mrp($dbc,$pid,$did,$type=false)
{
	...
	// echo "SELECT mrp,CONCAT_WS('/',CAST(s.mrp as decimal(10,2)),s.qty,s.batch_no,DATE_FORMAT(s.mfg,'%b-%Y')) as mrp_disp FROM stock s WHERE product_id=$pid AND dealer_id = $did AND qty!='0' ORDER BY s.mrp ASC,s.mfg ASC";die;
	$mrps = mysqli_query($dbc,"SELECT mrp,CONCAT_WS('/',CAST(s.mrp as decimal(10,2)),s.qty,s.batch_no,DATE_FORMAT(s.mfg,'%b-%Y')) as mrp_disp FROM stock s WHERE product_id=$pid AND dealer_id = $did AND qty!='0' ORDER BY s.mrp ASC,s.mfg ASC");
	$mrp_cnt = mysqli_num_rows($mrps);
	// $out = ''; // commented by me
	$out = array(); //addedd by me
	...
	{ 
		...
		if($type)
		{
			$out[] = array('mrp'=>$mrp_row['mrp'],'mrp_disp'=>$mrp_row['mrp_disp']);
		}else{
			$out .= "<option value=".$mrp_row['mrp']." $selected>".$mrp_row['mrp_disp']."</option>";
...
function myrowval_assoc($table, $col,$key, $where)
{
global $dbc;
$out = array();
//list($opt, $rs) = run_query($dbc, "SELECT $col FROM $table WHERE $where");
...
function getDDList($data,$id,$attr)
{
	$attr_data = '';
	...
	$out .= "<select $attr_data>";
	$out .= '<option value="">== Please Select ==</option>';
	foreach($data as $uid=>$name)
	{
		$selected = ($uid==$id) ? 'selected' : '';
		$out .= '<option value="'.$uid.'" '.$selected.'>'.$name.'</option>';
	}
	$out .= '</select>';
