...
class dispatch extends myfilter
{
	...
	public function dispatch_save()
	{
		...
		$q = "INSERT INTO `daily_dispatch` (`dispatch_id`, `dealer_id`, `van_no`, `dispatch_date`, `created_by`, `dispatch_no`) VALUES ('$id' , '$d1[dealer_id]', '$d1[van_no]', '$dispatch', '$d1[uid]', '$dis_num')";
		//h1($q);exit;
		...
		if($actiontype == 'Update') mysqli_query($dbc, "DELETE FROM daily_dispatch_details WHERE dispatch_id = '$rId'");	
                ...
                foreach ($d1['chk'] as $key=>$value){
                    ...
                    $str[] = '(\''.$rId.'\', \''.$value.'\', \''.$j.'\')';
                       ...
                       $query1 = "select sum(qty) as sqty,sum(free_qty) as frqty from `challan_order_details` WHERE `ch_id` = '$value'"; 
                   // h1($query1);
                       $result1 = mysqli_query($dbc,$query1);
                      $row1 = mysqli_fetch_assoc($result1);
                     $qtyy = $row1['sqty'];
                     $fqtyy = $row1['frqty'];
                     $quan = ($qtyy+$fqtyy)+$quan;
