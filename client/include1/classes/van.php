...
class van extends myfilter
{
	...
	public function van_edit($id)
	{
		...
		mysqli_query($dbc, "START TRANSACTION");
		// query to update 
		$q="UPDATE van SET capacity = '$d1[capacity]',`van_no` = '$d1[van_no]', company_id = '$d1[company_id]',mobile = '$d1[mobile]', license_no = '$d1[license_no]', address = '$d1[address]', driver_name = '$d1[driver_name]',`dealer_id` = '$dealer_id' WHERE vanId='$id'";
		$r=mysqli_query($dbc, $q);
		if(!$r){mysqli_rollback($dbc); return array ('status'=>false, 'myreason'=>$d1['what'].'Van Table error') ;} 
