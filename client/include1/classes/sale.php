...
//
        ...
        if (!$opt)
          ...
          while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $out[$id]['did'] = myrowval('dealer', 'name', $row['did']);
            $out[$id]['retailer'] = myrowval('dealer', 'name', $row['retailer_id']);
    ...
    public function sale_challan_item($order_id)
    {
       ...
       /*$q = "SELECT usod.product_id as productid, usod.quantity as sale_qty, cod.qty as ch_qty
        ...
        $q = "SELECT usod.product_id as productid, usod.quantity as sale_qty, cod.qty as ch_qty
          FROM user_sales_order_details as usod INNER JOIN challan_order_details AS cod ON usod.order_id=cod.order_id
          where usod.order_id = '$order_id'";          
