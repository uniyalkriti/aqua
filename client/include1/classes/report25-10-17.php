...
class report extends myfilter
{
    ...
    public function get_balance_stock_report($filter='',  $records = '', $orderby='',$ch_filter)
	{
		...
		$filterstr = $this->oo_filter($filter, $records, $orderby);
                ...
                $q1 = "SELECT c.product_id as pid,c.c1_id, s.rate ,c.product_name, s.qty,s.nonsalable_damage as nonsale, s.mrp, s.dealer_rate FROM `catalog_view` c LEFT JOIN `stock` s ON c.product_id=s.product_id AND s.dealer_id=$dealer_id $filterstr ORDER BY c.c1_id, pid ASC";
               	// h1($q1);
