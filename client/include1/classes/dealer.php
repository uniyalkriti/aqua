...
class dealer extends myfilter {
    ...
    public function dealer_save() {
        ...
        $q = "INSERT INTO `dealer` (`id`, `name`, `dealer_code`, `address`, `email`, `landline`, `other_numbers`, `tin_no`, `pin_no`,`ownership_type_id`,`avg_per_month_pur`) 
			VALUES (NULL, '$d1[name]', '$d1[dealer_code]', '$d1[address]', '$d1[email]', '$d1[landline]', '$d1[other_numbers]','$d1[tin_no]','$d1[pin_no]','$d1[ownership_type_id]','$d1[avg_per_month_pur]');";
        ...
        $extrawork = $this->dealer_location_extra('save', $rId, $_POST['location_id'], $_POST['rate_list_id']);
        if (!$extrawork['status']) {
            mysqli_rollback($dbc);
    ...
    public function dealer_location_extra($actiontype, $rId, $location, $rate_list_id) {
        global $dbc;
        $uncode = '';
        $str = array();
    ...
    public function dealer_edit($id) {
        ...
        mysqli_query($dbc, "START TRANSACTION");
        // query to update
        $q = "UPDATE dealer SET name = '$d1[name]', address = '$d1[address]', email = '$d1[email]', landline = '$d1[landline]', other_numbers = '$d1[other_numbers]', tin_no = '$d1[tin_no]', pin_no = '$d1[pin_no]', ownership_type_id='$d1[ownership_type_id]',avg_per_month_pur='$d1[avg_per_month_pur]', dealer_code = '$d1[dealer_code]' WHERE id = '$id'";
        $r = mysqli_query($dbc, $q);
        if (!$r) {
            mysqli_rollback($dbc);
        ...
        $extrawork = $this->dealer_location_extra('update', $id, $_POST['location_id'], $_POST['rate_list_id']);
    ...
    public function get_dealer_list($filter = '', $records = '', $orderby = '') {
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $out[$id]['dealer_state_id'] = $this->get_dealer_rate_list($row['id']);
    ...
    public function get_rate_list_id($id) {
     ...
     public function get_dealer_rate_list($id) {
        global $dbc;
        $out = NULL;
    ...
    public function person_dealer_save() {
        ...
        //$is_multiple_rate_list = $this->get_user_person_multiple_status($d1['user_id'], $d1['dealer_id']);
        ...
        if (!empty($d1['retailer_id'])) {
            ...
            foreach ($d1['retailer_id'] as $key => $retailer_id) {
                if (!uniqcheck_msg($dbc, $field_arry, 'user_dealer_retailer', false, "retailer_id='$retailer_id'"))
    ...
    public function get_user_dealer_retailer_list($filter = '', $records = '', $orderby = '') {
        ...
        $q = "SELECT r.id, r.name FROM dealer_location_rate_list dlrl INNER JOIN location_$dealer_level l$dealer_level ON l$dealer_level.id = dlrl.location_id";
