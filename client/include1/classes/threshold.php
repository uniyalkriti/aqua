...
class threshold extends myfilter
{
	...
	public function threshold_save()
	{
        ...
        if(!empty($d1['product_id']))
        {
            foreach($d1['product_id'] as $key=>$value)
            {
                ...
                if(!$trs_product)
                {
	                $str_arr[] = '(NULL,\''.$dealer_id.'\',\''.$value.'\',\''.$d1['buy_quantity'][$key].'\')';
                }else{
                	$new_qty = $d1['buy_quantity'][$key];

                	$tp_u_q = "UPDATE `threshold` SET `qty` = '$new_qty' WHERE `product_id` = '$value' AND `dealer_id` = '$dealer_id'";                	
        ...
        {
	        $str = implode(',' , $str_arr);
			$q = "INSERT INTO `threshold` (`id`,`dealer_id`, `product_id`, `qty`) VALUES $str";
	...
	public function get_threshold_list($filter='',  $records = '', $orderby='')
	{
		...
		$q = "SELECT catalog_view.product_id as pid, catalog_view.product_name, s.qty FROM `catalog_view` LEFT JOIN `threshold` s ON catalog_view.product_id=s.product_id AND dealer_id=$dealer_id ORDER BY catalog_view.c1_id,pid ASC";
