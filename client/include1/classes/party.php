...
class party extends myfilter
{
	...
	public function vendor_save()
	{
		...
		// query to save
		 $q = "INSERT INTO `party` (`partyId`, `ptype`, `partycode`, `partyname`, `email`, `mobile`, `phone`, `fax`, `website`, `tinno`, `eccno`, `vatno`, `cstno`, `crange`, `cdivision`, `adr`,`locality`,`landmark`,`city_district`,`state`, `country`, `crId`, `created`, `remark`, `company_id`) VALUES ('$id', 1, '$d1[partycode]', '$d1[partyname]', '$d1[email]', '$d1[mobile]', '$d1[phone]', '$d1[fax]', '$d1[website]', '$d1[tinno]', '$d1[eccno]', '$d1[vatno]', '$d1[cstno]', '$d1[crange]', '$d1[cdivision]', '$d1[adr]','$d1[adr]','$d1[landmark]','$d1[city_district]', '$d1[state]', '$d1[country]', '$d1[uid]', NOW(), '$d1[remark]', '{$_SESSION[SESS.'data']['company_id']}');"; 
		
                $r = mysqli_query($dbc,$q) ;
		if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'party table error');}
		//$rId = mysqli_insert_id($dbc);
		$extrawork = $this->party_extra('save', $id, $_POST['cname'], $_POST['cdesignation'], $_POST['cmobile'], $_POST['cemail'], $_POST['cphone'], $_POST['cremark']); 
	...
	public function vendor_edit($id)
	{
		...
		mysqli_query($dbc, "START TRANSACTION");
		// query to update 		
		$q = "UPDATE `party` SET `partycode` = '$d1[partycode]', `partyname` = '$d1[partyname]', `email` = '$d1[email]', `mobile` = '$d1[mobile]', `phone` = '$d1[phone]', `fax` = '$d1[fax]', `website` = '$d1[website]', `tinno` = '$d1[tinno]', `eccno` = '$d1[eccno]', `vatno` = '$d1[vatno]', `cstno` = '$d1[cstno]', `crange` = '$d1[crange]', `cdivision` = '$d1[cdivision]', `adr` = '$d1[adr]', `locality` = '$d1[locality]', `landmark` = '$d1[landmark]', `city_district` = '$d1[city_district]', `state` = '$d1[state]', `remark` = '$d1[remark]', `mrId` = $d1[uid], `modified` = NOW() WHERE partyId = '$id'";
		$r = mysqli_query($dbc,$q);
		if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'party table error');}
		$rId = $id;
	...
	public function customer_save()
	{
		...
		mysqli_query($dbc, "START TRANSACTION");
		// query to save
		$q = "INSERT INTO `party` (`partyId`, `ptype`, `partycode`, `partyname`, `email`, `mobile`, `phone`, `fax`, `website`, `tinno`, `eccno`, `vatno`, `cstno`, `crange`, `cdivision`, `adr`, `state`,  `country`, `crId`, `created`, `remark`) VALUES (NULL, 2, '$d1[partycode]', '$d1[partyname]', '$d1[email]', '$d1[mobile]', '$d1[phone]', '$d1[fax]', '$d1[website]', '$d1[tinno]', '$d1[eccno]', '$d1[vatno]', '$d1[cstno]', '$d1[crange]', '$d1[cdivision]', '$d1[adr]', '$d1[state]', '$d1[country]', '$d1[uid]', NOW(), '$d1[remark]');";
		$r = mysqli_query($dbc,$q) ;
		if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'party table error');}
		$rId = mysqli_insert_id($dbc);
	...
	public function customer_edit($id)
	{
		...
		mysqli_query($dbc, "START TRANSACTION");
		// query to update 		
		$q = "UPDATE `party` SET `partycode` = '$d1[partycode]', `partyname` = '$d1[partyname]', `email` = '$d1[email]', `mobile` = '$d1[mobile]', `phone` = '$d1[phone]', `fax` = '$d1[fax]', `website` = '$d1[website]', `tinno` = '$d1[tinno]', `eccno` = '$d1[eccno]', `vatno` = '$d1[vatno]', `cstno` = '$d1[cstno]', `crange` = '$d1[crange]', `cdivision` = '$d1[cdivision]', `adr` = '$d1[adr]', `state` = '$d1[state]', `remark` = '$d1[remark]', `mrId` = $d1[uid], `modified` = NOW() WHERE partyId = '$id'";
		$r = mysqli_query($dbc,$q);
		if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'party table error');}
		$rId = $id;
		//Doing some extra work
