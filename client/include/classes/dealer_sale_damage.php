...
class dealer_sale_damage extends myfilter {
 ...
 public function challan_extra_edit($actiontype, $rId, $d1,$company_id) {
    ...
    public function get_challan_list($filter ='', $records ='', $orderby ='', $ifilter='')
    {
		...
		$q = "SELECT damage_order.discount_per, damage_order.damage_order_type_id,damage_order.ch_retailer_id,damage_order.ch_user_id,damage_order.date_added,damage_order.ch_no,damage_order.ch_no,damage_order.discount_amt,damage_order.amount,damage_order.id as id, DATE_FORMAT(dispatch_date, '" . MASKDATE . "') AS dispatch_date, DATE_FORMAT(ch_date, '%Y-%m-%d') AS ch_date FROM damage_order INNER JOIN dealer_location_rate_list AS dlrl ON dlrl.dealer_id=ch_dealer_id $filterstr";
        }else{
		$q = "SELECT damage_order.discount_per, damage_order.damage_order_type_id,damage_order.ch_retailer_id,damage_order.ch_user_id,damage_order.date_added,damage_order.ch_no,damage_order.ch_no,damage_order.discount_amt,damage_order.amount,damage_order.id as id, DATE_FORMAT(dispatch_date, '" . MASKDATE . "') AS dispatch_date, DATE_FORMAT(ch_date, '%Y-%m-%d') AS ch_date FROM damage_order $filterstr";	
		}
		//h1($q);
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $q = "SELECT damage_order_details.*,damage_order_details.product_id AS pid, "
                    . "catalog_product.name,"
                    . "catalog_product.hsn_code FROM "
                    . "damage_order_details INNER JOIN catalog_product ON catalog_product.id = damage_order_details.product_id "
                    . " INNER JOIN catalog_3 c3 ON c3.id = catalog_product.catalog_id "
                    . " LEFT JOIN product_rate_list ON damage_order_details.product_id = product_rate_list.product_id "
                    . " AND product_rate_list.state_id ='$state_id' $ifilterstr ch_id = $id GROUP BY damage_order_details.product_id,damage_order_details.ch_id ";
            //h1($q);
...
//    }
    ...
    public function print_looper_challan($multiId, $options = array())
    {
        ...
        foreach ($multiId as $key => $value) {
            ...
            //read the record statistics
            $rcdstat = $this->get_challan_list("damage_order.id = $id");
            if (empty($rcdstat))
                continue;
            $temp = $rcdstat[$id];
    ...
    public function next_challan_num() {
        global $dbc;
        $out = array();
        $q = "SELECT COUNT(ch_no) AS total FROM damage_order";
        list($opt, $rs) = run_query($dbc, $q, $mode = 'single', $msg = '');
        return $rs['total'] + 1;
    }
//    public function next_challan_num() {
                ...
                $qtyy= $qty+$schqty;
                 ...
                 //   h1($update_stock_q);exit;
                    mysqli_query($dbc, $update_stock_q);
						...
						$qst="SELECT nonsalable_damage FROM stock WHERE `product_id`=$prod AND 
         `dealer_id`=$dealer_id AND `mrp`=$mrp AND batch_no='$batch_no' AND mfg='$mfg_date' ";
                     $rst=mysqli_query($dbc, $qst); 
...
//            h1($disc_amt); exit;
        ...
        $qqq = "INSERT INTO `damage_order` (`id`, `ch_no`, `ch_created_by`, `ch_dealer_id`, `ch_retailer_id`,`ch_user_id`, `ch_date`,`date_added`, `company_id`,`invoice_type`
            ,`discount_per`,`discount_amt`,`amount`,`remaining`,`damage_order_type_id`) 
    VALUES ($iddd, '$chlaan_no','$d1[uid]', '$d1[dealer_id]', '$d1[retailer_id]','$d1[user_id]','$ch_date',NOW(), '1','4',
        ...
        if (!$r) {
            ...
            $str = implode(',', $str);
             $q = "INSERT INTO damage_order_details (`id`,`ch_id`,`product_id`,`supply_status`,`qty`,`product_rate`, `free_qty`,`tax`,`mrp`,`cd`,`cd_type`,
