...
class dealer_sale extends myfilter {
    ...
    public function get_dealer_sale_details_list($filter = '', $records = '', $orderby = '', $popup=false)
    {
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            {
                $out[$id]['order_item'] = $pending_items_check['order_item'];
                $out[$id]['sale_value'] = $pending_items_check['sale_value'];
            }else{
    ...
    public function get_dealer_sale_list($filter = '', $records = '', $orderby = '', $popup=false)
    {
        ...
        while ($row = mysqli_fetch_assoc($rs))
        {
            ...
            if($popup)
            {                
                $item_q = "SELECT s.qty as aval_qty,usod.id,cp.name, cp.taxable,s.rate,usod.quantity,usod.scheme_qty,usod.product_id,usod.remaining_qty,s.mrp FROM user_sales_order_details usod 
                    ...
                    WHERE usod.order_id = $row[order_id] AND status!=2 AND s.dealer_id=$dealer_id ORDER BY s.product_id,s.mrp";
                // h1($item_q);
                // die;
...
$uid= $id;
    ...
    if($d1['deleted_order_item'][$k]!='')
    {
        ...
        $del_item_q = "DELETE FROM user_sales_order_details WHERE id IN($deleted_item_id) AND order_id = $k";
