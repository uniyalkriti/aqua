...
//               $q .= "$filterstr";
        ...
        public function get_catalog_rate_list($filter='',  $records = '', $orderby='')
	{
		...
		while($row = mysqli_fetch_assoc($rs))
		{
			$id = $row["stateid"];
			$out[$id] = $row;
                               $q=" SELECT cprl.*,CONCAT_WS(' ',c2.name,cp.name,cp.unit)as name from catalog_product_rate_list cprl LEFT JOIN catalog_product cp ON cp.id = cprl.catalog_product_id INNER JOIN catalog_2 c2 ON c2.id = cp.catalog_id  where stateId = '$id' ";
	...
	public function catalog_rate_save()
	{
		...
		if(!$status) return array('status'=>false, 'myreason'=>$d1['myreason']);		
                ...
                foreach($d1['name'] as $key => $value){
                    $str .= "('{$d1[product_id][$key]}','{$d1[state_id]}','{$d1[rate][$key]}','{$d1[base_price][$key]}','{$d1[tax][$key]}','1'),";
                }
                $str = rtrim($str,',');
               
	       $q = "INSERT INTO catalog_product_rate_list (`catalog_product_id`, `stateId`, `rate`,`base_price`, `tax`,`company_id`) VALUES $str ";
                    $r = mysqli_query($dbc,$q) ;
		if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>$d1['what'].' table error');}
		$rId = $id;	
		mysqli_commit($dbc);
	...
	public function modify_post() {
	    ...
	    $d1['state_id'] = $_SESSION[SESS . 'data']['state_id'];
	    $d1['sesId'] = $_SESSION[SESS . 'csess'];
	    $d1['myreason'] = 'Please fill all the required information';
	    return array(true, $d1);
	}	

	public function get_brand_product_list($filter='',  $records = '', $orderby='')
	{
		global $dbc;
		$out = array();		
		// if user has send some filter use them.
		$filterstr = $this->oo_filter($filter, $records, $orderby);
                $q = "SELECT catalog_product.id,catalog_product.hsn_code,catalog_product.itemcode,cp_name,catalog_product.name FROM catalog_product INNER JOIN other_company ON catalog_product.company_id=other_company.id   $filterstr";
	...
	public function brand_product_save()
	{
		global $dbc;
		$out = array('status'=>'false','myreason'=>'');

		list($status,$d1)=$this->modify_post();
		if(!$status) return array ('staus'=>false,'myreason'=>$d1['myreason']);

		//start the transaction
		mysqli_query($dbc, "START TRANSACTION");

		/* Entry to catalog_product */
		$q = "INSERT INTO catalog_product 
					SET 
					name='".$d1['pname']."'
					,itemcode='".$d1['itemcode']."'
					,hsn_code='".$d1['hsncode']."'
					,catalog_id=8
					,company_id='".$d1['brand_id']."'
					,base_price='".$d1['rrate']."'";

		$r=mysqli_query($dbc, $q);
		if(!$r){mysqli_rollback($dbc); return array ('status'=>false, 'myreason'=>$d1['what'].'catalog_product Table error') ;}
		$p_id = mysqli_insert_id($dbc);


		$gst = $d1['gst']/2;
		$q = "INSERT IGNORE _gst 
					SET 
					hsn_code = '".$d1['hsncode']."'
					,sgst='".$gst."'
					,cgst='".$gst."'
		...
		$r=mysqli_query($dbc, $q);
		if(!$r){mysqli_rollback($dbc); return array ('status'=>false, 'myreason'=>$d1['what'].'_gst Table error') ;}


		$q = "INSERT INTO product_rate_list 
					SET 
					product_id='$p_id'
					,state_id='".$d1['state_id']."'
					,mrp='".$d1['mrp']."'
					,retailer_rate='".$d1['rrate']."'
					,dealer_rate='".$d1['rrate']."'
					,company_id='".$d1['brand_id']."'";

		$r=mysqli_query($dbc, $q);
		if(!$r){mysqli_rollback($dbc); return array ('status'=>false, 'myreason'=>$d1['what'].'product_rate_list Table error') ;}
		...
		$r=mysqli_query($dbc, $q);
		if(!$r){mysqli_rollback($dbc); return array ('status'=>false, 'myreason'=>$d1['what'].'stock Table error') ;}
		...
		$r=mysqli_query($dbc, $q);
		if(!$r){mysqli_rollback($dbc); return array ('status'=>false, 'myreason'=>$d1['what'].'cases Table error') ;}


		mysqli_commit($dbc);

		return array ('status'=>true,'myreason'=>$d1['what'].' successfully Updated', 'p_id'=>$p_id);
	}
