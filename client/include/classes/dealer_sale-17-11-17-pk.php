    ...
    public function get_dealer_sale_list($filter = '', $records = '', $orderby = '', $popup=false)
    {
       ...
       /* h1($q);
        ...
        while ($row = mysqli_fetch_assoc($rs))
        {
            ...
            if($popup)
            {                
                $item_q = "SELECT s.qty as aval_qty,usod.id,cp.name, cp.taxable,s.rate,usod.quantity,usod.scheme_qty,usod.product_id FROM user_sales_order_details usod 
                    ...
                    WHERE usod.order_id = $row[order_id] AND status!=2 AND s.dealer_id=$dealer_id";
                // h1($item_q);
                // die;
...
// print_r($str2);exit;
$query ="INSERT INTO `challan_order`(`id`, `ch_no`, `ch_created_by`,
`ch_dealer_id`, `ch_retailer_id`, `ch_date`, `date_added`, `company_id`, `dispatch_status`,
