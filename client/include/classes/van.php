...
class van extends myfilter
{
	...
	public function van_save()
	{
		...
		// query to save
                ...
                $vstatus=$d1['vstatus'];
		$q = "INSERT INTO `van` (`vanId`, `van_no`, `mobile`, `license_no`, `address`, `driver_name`, `vtype`, `company_id`,`dealer_id`,`capacity`,`status`) VALUES ('$id', '$d1[van_no]', '$d1[mobile]', '$d1[license_no]', '$d1[address]', '$d1[driver_name]', '1', '{$_SESSION[SESS.'data']['company_id']}','$dealer_id','$d1[capacity]','$vstatus')";
                //h1($q);
	...
	public function van_edit($id)
	{
		...
		mysqli_query($dbc, "START TRANSACTION");
		// query to update 
		$q="UPDATE van SET capacity = '$d1[capacity]',`van_no` = '$d1[van_no]', company_id = '$d1[company_id]',mobile = '$d1[mobile]', license_no = '$d1[license_no]', address = '$d1[address]', driver_name = '$d1[driver_name]',`dealer_id` = '$dealer_id',`status`='$vstatus' WHERE vanId='$id'";
               // h1($q);exit;
		$r=mysqli_query($dbc, $q);
		if(!$r){mysqli_rollback($dbc); return array ('status'=>false, 'myreason'=>$d1['what'].'Van Table error') ;} 
