...
class dealer_sale extends myfilter {
    ...
    public function dealer_sale_edit($id) {
        ...
        if (!empty($d1['hid_product'])) {
                    foreach ($d1['hid_product'] as $key1 => $value1) {
                        $hprod = $d1['hid_product'][$key1];
                        $hqty = $d1['hid_quantity'][$key1];
                        $hschqty = $d1['scheme'][$key1];
                        $hmrp = $d1['hid_mrp'][$key1];
                        $hrate = $d1['hid_new_rate'][$key1];
                        $htoal_sale_qty[] = $hqty;
                        $huncode = $key1 + 1;
                        //To save the value of the other columns as some columns are affected by po
                        // $str[] = "('$uncode','$id','$prod','$qty','$schqty')";
                        $hstr[] = "('$id','$hprod','$hmrp','$hrate','$hqty','$hschqty',NOW())"; //puneet
                    }
                    $hstr = implode(', ', $hstr);
                    ...
                    $hq = "INSERT INTO `user_sales_order_details_log` (`order_id`,`product_id`,`mrp`,`rate`,`quantity`,`scheme_qty`,server_date_time) VALUES $hstr"; //puneet
                   // h1($hq);
                    $rh = mysqli_query($dbc, $hq);
                    if (!$rh)
                        return array('status' => false, 'myreason' => 'Sale Order Details LOG could not be saved');
                     if ($d1['company_id'] == 1)
                    write_query($hq);
                }
    ...
    public function get_dealer_sale_list($filter = '', $records = '', $orderby = '', $popup=false)
    {
        ...
        while ($row = mysqli_fetch_assoc($rs))
        {
            ...
            if($popup)
            {
                $out[$id]['order_item'] = $this->get_my_reference_array_direct("SELECT s.qty as aval_qty,usod.id,cp.name, cp.taxable,usod.rate,usod.quantity,usod.scheme_qty,usod.product_id FROM user_sales_order_details usod 
                    ...
                    WHERE usod.order_id = $row[order_id] AND status!=2 AND s.dealer_id=$dealer_id", 'id');                
...
$uid= $id;
    ...
    $amount=0;
        ...
        if($aval_stock>=$qty1)
        {
        $store[] = "('$new_ch_id','$prod','0','','$taxId','$vat_amt','$qty', '$rate','$schqty'
        ,'$uso_order_id','$user_id','$mrp','$cd','$cd_type','$cd_amt','$dis_type',
...
$str1[] = "($new_ch_id,'$ch_no','$user_id','$dealer_id',
'$retailer_id','$ch_date','1','0',
'0','$remark','1','2','0','$discount_per','$discounted_amt','$amount1','$amount1')";
$i++;
}
//***********************INSERT INTO challan_order*********************
$str2 = implode(',', $str1);
...
return array('status' => true, 'myreason' => '<strong>'.$d1['what'] . ' successfully Saved </strong>', 'rId' => $rId);
