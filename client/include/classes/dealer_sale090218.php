            ...
            foreach ($d1['product_id'] as $key => $value) {
               ...
               // $total_qty = $total_qty + $d1['quantity'][$key];
                ...
                $str[] = "(NULL,'$iddd','$prod','$bill_of_supply','$qty', '$rate','$schqty','$taxId','$mrp',
                    '$cd','$cd_type','$cd_amt','$spl_dis_type','$spl_dis_percent','$spl_dis_amt','$batch_no','$dis_type','$dis_amt','$dis_percent','$taxable_amt','$taxable_amt','$vat_amt')";
        ...
        if (!$r) {
            ...
            $str = implode(',', $str);
             $q = "INSERT INTO challan_order_details (`id`,`ch_id`,`product_id`,`supply_status`,`qty`,`product_rate`, `free_qty`,`tax`,`mrp`,`cd`,`cd_type`,
                 `cd_amt`,`spl_disc_type`,`spl_disc_val`,`spl_disc_amt`,`batch_no`,`dis_type`,`dis_amt`,`dis_percent`,`taxable_amt`,`remain_amount`,`vat_amt`)
            VALUES $str";
           /*h1($q);
           die;*/
            $r = mysqli_query($dbc, $q);
