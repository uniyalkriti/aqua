...
class dealer_sale_damage_depot extends myfilter {
...
/******************************************/
    public function get_challan_list($filter ='', $records ='', $orderby ='', $ifilter='')
    {
        ...
        $q = "SELECT damage_order_depot.discount_per,damage_order_depot.ch_csa_id,
        damage_order_depot.ch_user_id,damage_order_depot.date_added,damage_order_depot.damage_order_type_id,
        damage_order_depot.ch_no,damage_order_depot.ch_no,
        damage_order_depot.discount_amt,damage_order_depot.amount,
        damage_order_depot.id as id, DATE_FORMAT(dispatch_date, '" . MASKDATE . "') AS dispatch_date, DATE_FORMAT(ch_date, '%Y-%m-%d') AS ch_date FROM damage_order_depot
         INNER JOIN dealer_location_rate_list AS dlrl ON dlrl.dealer_id=ch_dealer_id $filterstr";
        }else{
		$q = "SELECT damage_order_depot.discount_per,damage_order_depot.damage_order_type_id ,damage_order_depot.ch_csa_id,damage_order_depot.ch_user_id,damage_order_depot.date_added,damage_order_depot.ch_no,damage_order_depot.ch_no,damage_order_depot.discount_amt,damage_order_depot.amount,damage_order_depot.id as id, DATE_FORMAT(dispatch_date, '" . MASKDATE . "') AS dispatch_date, DATE_FORMAT(ch_date, '%Y-%m-%d') AS ch_date FROM damage_order_depot $filterstr";	
		}
		//h1($q);
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
        if (!$opt)
        ...
        $dealer_map = get_my_reference_array('dealer', 'id', 'name');
          ...
          $retailer_map = get_my_reference_array('csa', 'c_id', 'csa_name');
         // pre($retailer_map) ;
        //  echo "ANKUSH PANDEY";
        //  print_r($retailer_map);
        //  exit; 
        while ($row = mysqli_fetch_assoc($rs)) {
            $where = 'c_id = '.$row['ch_csa_id'];
            
            $retailer_map =  myrowval('csa', 'csa_name',$where);
            ...
            $q = "SELECT damage_order_depot_details.*,date_format(mfg_date,'%d-%m-%Y') as mfgdate,damage_order_depot_details.product_id AS pid, "
                    . "catalog_product.name,"
                    . "catalog_product.hsn_code FROM "
                    . "damage_order_depot_details INNER JOIN catalog_product ON catalog_product.id = damage_order_depot_details.product_id "
                    . " INNER JOIN catalog_3 c3 ON c3.id = catalog_product.catalog_id "
                    . " LEFT JOIN product_rate_list ON damage_order_depot_details.product_id = product_rate_list.product_id "
                    . " AND product_rate_list.state_id ='$state_id' $ifilterstr ch_id = $id GROUP BY damage_order_depot_details.product_id,damage_order_depot_details.ch_id ";
            //h1($q);
      ...
      //  $retailer_map = get_my_reference_array('retailer', 'id', 'name');
        while ($row = mysqli_fetch_assoc($rs)){
           // pre($row);
            ...
            $out[$id]['retailer_id'] = $row['ch_retailer_id'];
             $q1 = "SELECT ch_retailer_id, sum(old_payment_dues.remaining) as old_total FROM old_payment_dues WHERE ch_retailer_id=$row[ch_retailer_id]";
             $r1= mysqli_query($dbc, $q1);
...
//    }
    ...
    public function print_looper_challan($multiId, $options = array())
    {
        ...
        foreach ($multiId as $key => $value) {
            ...
            //read the record statistics
            $rcdstat = $this->get_challan_list("damage_order_depot.id = $id");
            if (empty($rcdstat))
                continue;
            $temp = $rcdstat[$id];
            ...
            $out[$id]['adr'] = $party->get_csa_adr($temp['ch_csa_id']);
        }
       // pre($out);
        return $out;
    }
    ...
    public function next_challan_num() {
        global $dbc;
        $out = array();
        $q = "SELECT COUNT(ch_no) AS total FROM damage_order_depot";
        list($opt, $rs) = run_query($dbc, $q, $mode = 'single', $msg = '');
        return $rs['total'] + 1;
    }
//    public function next_challan_num() {
...
//    }
    ...
    public function direct_challan_save() {
       ...
       // $iddd = $d1['dealer_id'].$orderno;
        ...
        $chlaan_no = $d1['ch_no'];
        $damage_cond=$d1['saleable_non_saleable'];
        mysqli_query($dbc, "START TRANSACTION");
        ...
        if (!empty($d1['product_id'])) {
            ...
            foreach ($d1['product_id'] as $key => $value) {
                ...
                $mfg_date = empty($d1['mfg_date'][$key]) ? '' : get_mysql_date($d1['mfg_date'][$key], '/', false, false);
               ...
               // $total_qty = $total_qty + $d1['quantity'][$key];
                ...
                $str[] = "(NULL,'$iddd','$prod','$bill_of_supply','$qty', '$rate','$schqty','$taxId','$mrp',
                    '$cd','$cd_type','$cd_amt','$spl_dis_type','$spl_dis_percent','$spl_dis_amt','$batch_no','$mfg_date','$dis_type','$dis_amt','$dis_percent','$taxable_amt','$taxable_amt','$vat_amt')";
                //$str1[] = "(,'$orderno','$prod','$rate', '$qty','$schqty')";
                $qtyy= $qty+$schqty;
                    if($damage_cond==1){
                    $new_stock_qty = $aval_stock-$qtyy;
                    $qst="SELECT qty FROM stock WHERE `product_id`=$prod AND `dealer_id`=$dealer_id AND `mrp`=$mrp AND batch_no=$batch_no AND mfg=$mfg_date ";
                    $rst=mysqli_query($dbc, $qst); 
                    if(mysqli_num_rows($rst)>0){
                    $update_stock_q="UPDATE `stock` SET qty=$new_stock_qty,remaining=$new_stock_qty,salable_damage=$new_stock_qty WHERE `product_id`=$prod AND `dealer_id`=$dealer_id AND `mrp`=$mrp";
                    }else{
                    $update_stock_q="INSERT INTO `stock`( `product_id`, `batch_no`, `rate`, `dealer_rate`, `mrp`, `person_id`, `csa_id`, `dealer_id`, `qty`, `salable_damage`, `nonsalable_damage`, `remaining`, `mfg`, `expire`, `date`, `pr_rate`, `company_id`, `action`, `sync_status`, `update_date_time`) "
                            . "VALUES ('$prod','$batch_no','$rate','$rate','$mrp','','','$dealer_id','$new_stock_qty','$new_stock_qty','0','$new_stock_qty','$mfg_date','',NOW(),'','1','1','1',NOW())";    
                    }
                    //h1($update_stock_q);exit;
                    mysqli_query($dbc, $update_stock_q);
                    }elseif($damage_cond==2){
						 $qst="SELECT nonsalable_damage FROM stock WHERE `product_id`=$prod AND `dealer_id`=$dealer_id AND `mrp`=$mrp AND batch_no=$batch_no AND mfg=$mfg_date ";
                        $rst=mysqli_query($dbc, $qst); 
                        $rowst = mysqli_fetch_assoc($rst); 
                        $nqty=$rowst['nonsalable_damage'] ;
                     
                        $new_stock_qty = $nqty-$qtyy;
                     //$new_stock_qty = $qtyy;
                     if(mysqli_num_rows($rst)>0){
                    $update_stock_q="UPDATE `stock` SET nonsalable_damage=$new_stock_qty WHERE `product_id`=$prod AND `dealer_id`=$dealer_id AND `mrp`=$mrp AND batch_no=$batch_no AND mfg=$mfg_date";
                    }else{
                    $update_stock_q="INSERT INTO `stock`( `product_id`, `batch_no`, `rate`, `dealer_rate`, `mrp`, `person_id`, `csa_id`, `dealer_id`, `qty`, `salable_damage`, `nonsalable_damage`, `remaining`, `mfg`, `expire`, `date`, `pr_rate`, `company_id`, `action`, `sync_status`, `update_date_time`) "
                            . "VALUES ('$prod','$batch_no','$rate','$rate','$mrp','','','$dealer_id','$new_stock_qty','$new_stock_qty','0','$new_stock_qty',$mfg_date,'',NOW(),'','1','1','1',NOW())";    
                    }
...
//            h1($disc_amt); exit;
        ...
        $qqq = "INSERT INTO `damage_order_depot` (`id`, `ch_no`, `ch_created_by`, `ch_dealer_id`, `ch_csa_id`,`ch_user_id`, `ch_date`,`date_added`, `company_id`,`invoice_type`
            ,`discount_per`,`discount_amt`,`amount`,`remaining`,`damage_order_type_id`) 
    VALUES ($iddd, '$chlaan_no','$d1[uid]', '$d1[dealer_id]', '$d1[csa_id]','$d1[dealer_id]','$ch_date',NOW(), '1','4',
        '$disc','$disc_amt','$amount1','$amount1','$damage_cond')";
          //h1($qqq);//exit;
        $r = mysqli_query($dbc, $qqq);
        
        if (!$r) {
            mysqli_rollback($dbc);
            return array('status' => false, 'myreason' => 'Damage Could Not saved, Some error occurred');
        }
            $str = implode(',', $str);
             $q = "INSERT INTO damage_order_depot_details (`id`,`ch_id`,`product_id`,`supply_status`,`qty`,`product_rate`, `free_qty`,`tax`,`mrp`,`cd`,`cd_type`,
                 `cd_amt`,`spl_disc_type`,`spl_disc_val`,`spl_disc_amt`,`batch_no`,`mfg_date`,`dis_type`,`dis_amt`,`dis_percent`,`taxable_amt`,`remain_amount`,`vat_amt`)
            VALUES $str";
       //   h1($q);
//           die;
            $r = mysqli_query($dbc, $q);
...
$uid= $id;
    ...
    $user_id = $d1['user_id'][$k];
        ...
        $update="UPDATE `user_sales_order_details` SET status=$product_status WHERE `user_sales_order_details`.`order_id` =$uso_order_id";
        ...
        $store[] = "('$new_ch_id','$prod','$bill_of_supply','0','','$taxId','$vat_amt','$qty', '$rate','$free_qty'
        ,'$uso_order_id','$user_id','$mrp','$cd','$cd_type','$cd_amt','$dis_type',
