...
class catalog extends myfilter
{
	...
	public function catalog_category_save()
	{
		...
		mysqli_query($dbc, "START TRANSACTION");
                ...
                $id = $d1['uid'].date('Ymdhis');
		// query to save
	        $q = "INSERT INTO catalog_$mtype (`id`, `name`, `catalog_".$loop."_id`, `company_id`) VALUES ('$id', '$d1[$catname]','$d1[$catalogloopid]', '$company_id')";
        ...
        public function catalog_category_edit($id)
	{
		...
		mysqli_query($dbc, "START TRANSACTION");
		// query to update 
		$q="UPDATE catalog_$mtype SET `name` = '$d1[$catname]',`catalog_".$loop."_id` = '$d1[$catalogloopid]', company_id = '$company_id' WHERE id='$id'";
        ...
        public function get_catalog_category_list($filter='',  $records = '', $orderby='')
	{
		...
		$filterstr = $this->oo_filter($filter, $records, $orderby);
                ...
                for($k = $mtype; $k>=1; $k--)
               {
                ...
                $q = "SELECT * $str FROM catalog_$mtype ";
                for($i = $mtype; $i>1;$i--)
                {
                    $j = $i - 1; 
