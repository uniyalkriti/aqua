...
class retailer extends myfilter {
    ...
    public function retailer_save() {
       ...
       /* if ($d1['chk'] == 'chk') {
        ...
        $q = "INSERT INTO `retailer` (`id`, `name`, `image_name`,`location_id`, `address`,`contact_per_name`, `email`, `landline`, `other_numbers`, `tin_no`,`drug_lic_no`,`pan_no`, `pin_no`,`outlet_type_id`,`avg_per_month_pur`,`created_on`, `created_by_person_id`,`dealer_id`, `company_id`,`retailer_status`) 
            VALUES ($maxid, '$d1[name]','' ,'$d1[$location]', '$d1[address]','$d1[contact_per_name]','$d1[email]', '$d1[landline]', '$d1[other_numbers]','$d1[tin_no]','$d1[drug_lic_no]','$d1[pan_no]','$d1[pin_no]','$d1[outlet_type_id]','$d1[avg_per_month_pur]',NOW(),'$d1[so]','{$_SESSION[SESS . 'data']['dealer_id']}','$company_id','$d1[retailer_status]' );";
        ...
        if (!$r) {
            ...
            if ($company_id == 1)
                write_query($q);
            $q = "INSERT user_dealer_retailer VALUES('$d1[so]','$dealer_id','$maxid',NOW())";
    ...
    public function retailer_edit($id) {
        ...
        $q = "UPDATE retailer SET name = '$d1[name]', address = '$d1[address]', email = '$d1[email]', landline = '$d1[landline]', other_numbers = '$d1[other_numbers]', tin_no = '$d1[tin_no]', drug_lic_no = '$d1[drug_lic_no]', pan_no = '$d1[pan_no]', pin_no = '$d1[pin_no]', `location_id` = '$d1[$location]',`outlet_type_id`='$d1[outlet_type_id]',`avg_per_month_pur`='$d1[avg_per_month_pur]', dealer_id = '$d1[dealer_id]', `company_id` = '$company_id',`contact_per_name`='$d1[contact_per_name]',created_by_person_id='$d1[so]',retailer_status='$d1[retailer_status]' WHERE id = '$id'";
       //h1($q);exit;
        $r = mysqli_query($dbc, $q);
        if (!$r) {
            mysqli_rollback($dbc);
    ...
    public function get_retailer_list($filter = '', $records = '', $orderby = '') {
        ...
        $mtype = $_SESSION[SESS . 'constant']['retailer_level'];
        // $q = "SELECT retailer.*, CONCAT_WS(' ',person.first_name,person.last_name)as pname,created_by_person_id as so FROM retailer LEFT JOIN person ON retailer.created_by_person_id = person.id $filterstr";
        $q = "SELECT location_view.l3_id,location_view.l4_id,location_view.l5_id,l5_name,retailer.*, CONCAT_WS(' ',person.first_name,person.last_name)as pname,created_by_person_id as so 
        FROM retailer LEFT JOIN person ON retailer.created_by_person_id = person.id 
        LEFT JOIN location_view ON location_view.l5_id=retailer.location_id $filterstr";
        // h1($q);
 ...
 // Here we get csa dfetails address
    public function get_csa_adr($id, $seperator = '<br>') {
        global $dbc;
        $out = '';
        $q = "SELECT * FROM csa WHERE csa.c_id = '$id' LIMIT 1";
       //h1($q);
        list($opt, $rs) = run_query($dbc, $q, $mode = 'single', $msg = '');
       if (!$opt)
            return $out; // if no order placed send blank array
        $out = address_representer($rs, array('csa_name','adress','mobile','email','state_id','tin_no'), $seperator);
        return $out;
    }
     // Here we end 
     /**********************/
    ...
    ############# retailer Delete ##########################
    public function retailer_delete($id, $filter='', $records='', $orderby='')
    {
        global $dbc;
        if(empty($filter)) $filter = "retailer.id = '$id'";
        $out = array('status'=>false, 'myreason'=>'');
        $deleteRecord = $this->get_retailer_list($filter, $records, $orderby);
                
        if(empty($deleteRecord)){ $out['myreason'] = 'Retailer not found'; return $out;}
        //start the transaction
        mysqli_query($dbc, "START TRANSACTION");
        
        //Running the deletion queries
        $delquery = array();
        $delquery['retailer_deactivate'] = "UPDATE retailer SET retailer_status = '0' WHERE retailer.id = '$id'";
        ...
        mysqli_commit($dbc);
                return array('status'=>true, 'myreason'=>'Retailer successfully deactivated');
    }
