    ...
    public function get_primary_sale_order_list($filter = '', $records = '', $orderby = '') {
      ...
      // h1($q);
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $order_item = "SELECT cases,usod.id,cp.name,usod.rate,usod.quantity,usod.scheme_qty,pr_rate, batch_no,DATE_FORMAT(mfg_date, '%d/%m/%Y') AS mfg_date,purchase_inv, DATE_FORMAT(expiry_date, '%d/%m/%Y') AS expiry_date,cp.id AS product_id FROM purchase_order_details usod INNER JOIN purchase_order uso ON usod.order_id = uso.order_id AND usod.purchase_inv=uso.challan_no INNER JOIN catalog_product cp ON usod.product_id=cp.id WHERE usod.purchase_inv = '$row[challan_no]' AND uso.csa_id='$row[csa_id]'";
