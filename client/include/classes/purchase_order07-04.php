...
$cr_date=date('Y-m-d',strtotime($d1['receive_date']));
$date_time=date('Y-m-d H:i:s');
mysqli_query($dbc, "START TRANSACTION");
$qps = "INSERT INTO `purchase_order`(`order_id`, `dealer_id`, `created_date`, `created_person_id`, `order_date`, `receive_date`, `date_time`, `company_id`, `ch_date`, `challan_no`, `csa_id`) VALUES ('$orderno', '$dealer_id', '$date_time', '$pid','$receive_date', '1970-01-01', '$date_time', '$d1[company_id]','1970-01-01', '0','$csa_id')";
// h1($qps); 
// die;
...
$extrawork = $this->purchase_order_extra('save', $orderno, $_POST['product_id'], $_POST['batch_no'], $_POST['base_price'], $_POST['quantity'], $_POST['scheme_quantity'],$_POST['purchase_inv'], $_POST['mfg_date'], $_POST['expiry_date'], $_POST['sale_price'],$_POST['company_id'],$_POST['cp'],$_POST['dealer_rate']);
}
if (!$extrawork['status']) {
mysqli_rollback($dbc);
...
public function purchase_order_extra($actiontype, $rId, $productId, $batch_no, $base_price, $quantity, $scheme,$purchase_inv, $mfg_date, $expiry_date, $sale_price,$company_id,$cases,$landing_price) {
	/*pre($base_price); exit();*/
	global $dbc;
	$uncode = '';
	$str = $str_cat = array();
...
$gst = $rowgst['igst']; 
		...
		$str[] = "('$rId', '$value', '{$base_price[$key]}','{$landing_price[$key]}', '{$quantity[$key]}', '$mfdate', '$expdate', '{$purchase_inv[$key]}','{$sale_price[$key]}', '{$cases[$key]}','$gst')";
		...
		$str[] = "('$rId', '$value', '{$base_price[$key]}', '{$landing_price[$key]}', '{$quant[$key]}', '$mfdate', '$expdate', '{$purchase_inv[$key]}','{$sale_price[$key]}', '{$quantity[$key]}','$gst')";
	...
	$q = "INSERT INTO `purchase_order_details` (`order_id`, `product_id`, `mrp`,`rate`, `quantity`,`purchase_inv`, `mfg_date`, `expiry_date`, `pr_rate`,`cases`,`gst`) VALUES $str";
...
public function get_purchase_order_list($filter = '', $records = '', $orderby = '') {
	...
	$q = "SELECT *,csa_name,DATE_FORMAT(created_date,'%d/%m/%Y') AS sale_date,DATE_FORMAT(order_date,'%d/%m/%Y') AS order_date,DATE_FORMAT(receive_date,'%d/%m/%Y') AS receive_date,"
	...
	while ($row = mysqli_fetch_assoc($rs)) {
		...
		if($out[$id]['receive_date']=='01/01/1970')
               {
                $pquery = "SELECT mrp,gst_percentage,cases,usod.id,cp.name,usod.rate,
                    ...
                    INNER JOIN purchase_order uso ON usod.order_id = uso.order_id
                    INNER JOIN catalog_product cp ON usod.product_id=cp.id WHERE
                    usod.order_id = '$row[order_id]'";
                //h1($pquery);
               }
               else
               {
                $pquery = "SELECT gst_percentage,cases,usod.id,cp.name,usod.rate,usod.quantity, pr_rate, 
