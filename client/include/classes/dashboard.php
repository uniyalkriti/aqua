...
class dashboard extends myfilter {
    ...
    public function current_scheme() {
        ...
        global $dbc;   
        $date = date('Y-m-d');
        $q = "SELECT svp.id,svp.value,svp.value_to,svp.scheme_gift,svpd.start_date,svpd.end_date, svpd.scheme_name from scheme_value_product_details svp INNER JOIN scheme_value svpd ON svpd.scheme_id = svp.scheme_id where '$date' BETWEEN svpd.start_date AND svpd.end_date AND user = 2 AND state_id = $state";
        // h1($q);
        $r = mysqli_query($dbc, $q);
        while ($row = mysqli_fetch_array($r)) {
            $id = $row['id'];   
    ...
    public function mtd_received($month,$dealer)
    {
       ...
       //  $q= "SELECT SUM(rod.rate*(rod.quantity)) as amount FROM receive_order_details rod
       // INNER JOIN receive_order ro ON rod.order_id = ro.order_id 
       // where DATE_FORMAT(ro.ch_date,'%Y-%m') ='$month' AND ro.dealer_id = '$dealer'";

             $q= "SELECT SUM(total_amount) as amount FROM purchase_order_details rod
       INNER JOIN purchase_order ro ON rod.order_id = ro.order_id AND ro.challan_no=rod.purchase_inv 
       where DATE_FORMAT(ro.ch_date,'%Y-%m') ='$month' AND ro.dealer_id = '$dealer'";
        ...
        $qp= "SELECT SUM(total_amount) as amountp FROM purchase_order_details rod
       INNER JOIN purchase_order ro ON rod.order_id = ro.order_id AND ro.challan_no=rod.purchase_inv  
       where DATE_FORMAT(ro.ch_date,'%Y-%m') ='$pre_mth' AND ro.dealer_id = '$dealer'"; 
        $rp = mysqli_query($dbc,$qp);
        $rowp = mysqli_fetch_assoc($rp);
  ...
  ///////////////////////////////TOTAL MTD INVOICE//////////////////////////////////////////////////////
    ...
    ///////////////////////////////////GET CHALLAN COUNT///////////////////////////////
     ...
     public function total_retailer()
     {
       ...
       // $state_id = $_SESSION[SESS . 'data']['state_id'];
        ...
        $q = "SELECT count(id) as retailer_count FROM 
            retailer WHERE dealer_id='$dealer_id' AND retailer_status='1'";
       // h1($q);
     ...
     public function pending_invoice()
     {
       ...
       // $state_id = $_SESSION[SESS . 'data']['state_id'];
        $dealer_id = $_SESSION[SESS . 'data']['dealer_id'];
         ...
         $q= "SELECT count(DISTINCT order_id) as count FROM `user_sales_order` INNER JOIN `user_sales_order_details` USING (order_id) where dealer_id ='$dealer_id' AND status!=2 AND order_status!=8 AND call_status=1";
      //  h1($q);
        $r = mysqli_query($dbc,$q);
        $row = mysqli_fetch_assoc($r);
        $count = $row['count'];
     ...
     public function receive_amount()
     {
       ...
       // $q= "SELECT SUM(rod.cases*(rod.pr_rate)) as amount FROM receive_order_details rod
       // INNER JOIN receive_order ro ON rod.order_id = ro.order_id 
       // where DATE_FORMAT(ro.ch_date,'%Y-%m') ='$month' AND ro.dealer_id = '$dealer_id'";  
       $q= "SELECT SUM(rod.quantity*rod.rate) as amount FROM purchase_order_details rod
       INNER JOIN purchase_order ro ON rod.order_id = ro.order_id 
       where DATE_FORMAT(ro.ch_date,'%Y-%m') ='$month' AND ro.dealer_id = '$dealer_id'";   
      // h1($q);
       $r = mysqli_query($dbc,$q);
       $row = mysqli_fetch_assoc($r);

       $unit = ROUND($row['amount'],2);
       // setlocale(LC_MONETARY, 'en_IN');
        //$out['this'] = money_format('%!i', $unit);
       $out['this'] =$unit;
       // pre($out);
...
///////////////////////////??DISTRIBUTOR COMPLAINT/////////////////////////////////////////
  ...
  //  $filterstr=$this->oo_filter($filter, $records, $orderby);
    //$mtype = $_SESSION[SESS.'constant']['retailer_level'];
		$q = "SELECT * FROM `complaint` where dealer_retailer_id='$dealer_id' AND date='$prev_date'";
    // h1($q);
...
$q="SELECT rod.id,ro.challan_no AS ch_no,SUM(rod.rate*(rod.quantity)) as amount FROM receive_order_details rod
       INNER JOIN receive_order ro ON rod.order_id = ro.order_id 
       where DATE_FORMAT(ro.ch_date,'%Y-%m') = '$pmonth' AND ro.dealer_id = '$dealer_id' GROUP BY ro.order_id";
// h1($q);
list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
if (!$opt)
return $out;
...
$pmonth = Date("Y-m", strtotime($month . " last month")); 
$filterstr = $this->oo_filter($filter, $records, $orderby);
$q = "SELECT pc.id,pc.retailer_id AS rid,retailer.name AS retname,pc.total_amount AS amount,pc.pay_mode AS mode,DATE_FORMAT(pc.pay_date_time,'%Y-%m-%d') AS date FROM payment_collection AS pc INNER JOIN retailer ON retailer.id=pc.retailer_id WHERE DATE_FORMAT(pc.pay_date_time,'%Y-%m')='$pmonth' AND pc.dealer_id='$dealer_id'";
//h1($q);
list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
if (!$opt)
return $out;
...
$pmonth = Date("Y-m", strtotime($month . " last month")); 
$filterstr = $this->oo_filter($filter, $records, $orderby);
$q = "SELECT pc.id,pc.retailer_id AS rid,retailer.name AS retname,pc.total_amount AS amount,pc.pay_mode AS mode,DATE_FORMAT(pc.pay_date_time,'%Y-%m-%d') AS date FROM payment_collection AS pc INNER JOIN retailer ON retailer.id=pc.retailer_id WHERE DATE_FORMAT(pc.pay_date_time,'%Y-%m')='$month' AND pc.dealer_id='$dealer_id'";
//h1($q);
list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
if (!$opt)
return $out;
