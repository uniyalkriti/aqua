...
//                $exdate = !empty($d1['expiry_date']) ? get_mysql_date($d1['expiry_date']) : '';
    ...
    public function get_catalog_product_list($filter = '', $records = '', $orderby = '')
    {
        ...
        $q = "SELECT catalog_view.*,catalog_view.product_id as uid,c2_name as c1,product_name AS name 
                FROM catalog_view 
                INNER JOIN product_rate_list ON catalog_view.product_id = product_rate_list.product_id 
                $filterstr ORDER BY c1"; 
