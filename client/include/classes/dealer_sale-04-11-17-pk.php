...
class dealer_sale extends myfilter {
    ...
    public function dealer_sale_edit($id) {
        ...
        if (!empty($d1['product'])) {
            ...
            $q = "INSERT INTO `user_sales_order_details` (`order_id`,`product_id`, `quantity`,`scheme_qty`) VALUES $str"; //puneet
    ...
    public function get_dealer_sale_list($filter = '', $records = '', $orderby = '', $popup=false)
    {
        ...
        $q = "SELECT *,order_id,DATE_FORMAT(date,'%d/%m/%Y') AS sale_date,DATE_FORMAT(time,'%d/%m/%Y') AS sale_time,DATE_FORMAT(date,'%d/%b/%Y') AS fdated FROM user_sales_order INNER JOIN user_sales_order_details USING(order_id) $filterstr AND user_sales_order_details.status !=2 GROUP BY user_sales_order.order_id";
       /* h1($q);
        die;*/
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            if($popup)
            {
                $out[$id]['order_item'] = $this->get_my_reference_array_direct("SELECT usod.id,cp.name, cp.taxable,usod.rate,usod.quantity,usod.scheme_qty,usod.product_id FROM user_sales_order_details usod INNER JOIN user_sales_order uso ON usod.order_id = uso.order_id INNER JOIN catalog_product cp ON usod.product_id=cp.id 
                    WHERE usod.order_id = $row[order_id] AND  status!=2 ", 'id');
                ...
                // $out[$id]['gift_item'] = $this->get_my_reference_array_direct("SELECT gift_id,quantity FROM user_retailer_gift_details WHERE order_id = $row[order_id]", 'gift_id');
                  ...
                  $q = "SELECT count(order_id) as order_item FROM `user_sales_order_details` WHERE `order_id` = $id";
       ...
       // h1($q);
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $q = "SELECT challan_order_details.*,challan_order_details.product_id AS pid, "
                    ...
                    . " INNER JOIN product_rate_list ON challan_order_details.product_id = product_rate_list.product_id "
                    . " AND product_rate_list.state_id ='$state_id' WHERE ch_id = $id GROUP BY challan_order_details.product_id,challan_order_details.ch_id ";
            ...
            $q1 = "SELECT challan_order_details.id,catalog_product.hsn_code,sum(challan_order_details.tax) as gst_tax,"
                    ...
                    . "INNER JOIN catalog_2 c2 ON c2.id = catalog_product.catalog_id "
                    . "INNER JOIN product_rate_list ON challan_order_details.product_id = product_rate_list.catalog_product_id "
       ...
       // $retailer_map = get_my_reference_array('retailer', 'id', 'name');
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
           ...
           // $out[$id]['tin_no'] = $retailer_map1[$row['ch_retailer_id']];
            $q = "SELECT challan_order_details.*,sum(qty*product_rate) as challan_val,sum(cd_amt) as total_cd_amt,challan_order_details.product_id AS pid, catalog_product.name 
            FROM challan_order_details 
            INNER JOIN catalog_product ON catalog_product.id = challan_order_details.product_id  
            INNER JOIN catalog_2 c2 ON c2.id = catalog_product.catalog_id  
            WHERE ch_id =$id AND tax=0 group by ch_id";
            // h1($q);

            $q1 = "SELECT challan_order_details.*,sum(qty*product_rate) as challan_val,sum(cd_amt) as total_cd_amt,challan_order_details.product_id AS pid, catalog_product.name 
            FROM challan_order_details 
            INNER JOIN catalog_product ON catalog_product.id = challan_order_details.product_id  
            INNER JOIN catalog_2 c2 ON c2.id = catalog_product.catalog_id  
            WHERE ch_id =$id AND tax=5 group by ch_id";
           //h1($q1);
            $out[$id]['challan_item'] = $this->get_my_reference_array_direct($q, 'id');
        ...
        if (!empty($d1['product_id'])) {
            ...
            foreach ($d1['product_id'] as $key => $value) {
               ...
               // $total_qty = $total_qty + $d1['quantity'][$key];
                //To save the value of the other columns as some columns are affected by po
                $str[] = "(NULL,'$iddd','$prod','$qty', '$rate','$schqty','$taxId','$mrp',
...
//            h1($disc_amt); exit;
        ...
        $qqq = "INSERT INTO `challan_order` (`id`, `ch_no`, `ch_created_by`, `ch_dealer_id`, `ch_retailer_id`, `ch_date`, `company_id`,`invoice_type`
            ,`discount_per`,`discount_amt`,`amount`,`remaining`) 
    VALUES ($iddd, '$chlaan_no','$d1[uid]', '$d1[dealer_id]', '$d1[retailer_id]', '$ch_date', '$d1[company_id]','1',
        ...
        if (!$r) {
            ...
            $str = implode(',', $str);
             $q = "INSERT INTO challan_order_details (`id`,`ch_id`,`product_id`,`qty`,`product_rate`, `free_qty`,`tax`,`mrp`,`cd`,`cd_type`,
    ...
    public function direct_challan_edit($id)
    {
        ...
        if (!empty($d1['product_id']))
        {
            ...
            foreach ($d1['product_id'] as $key => $value)
            {
                ...
                //To save the value of the other columns as some columns are affected by po
                $str[] = "('$id','$prod','$qty', '$rate','$schqty','$taxId','$mrp','$cd','$cd_type','$cd_amt','$batch_no','$dis_type','$dis_amt','$dis_percent','$amount','$amount','$vat_amt')";
                $nid++;
       ...
       // h1($qup);
        ...
        $nid= $id;
         ...
         $q1 = "INSERT INTO challan_order_details (`ch_id`,`product_id`,`qty`,`product_rate`, `free_qty`,`tax`,`mrp`,`cd`,`cd_type`,`cd_amt`,`batch_no`,`dis_type`,`dis_amt`,`dis_percent`,`taxable_amt`,`remain_amount`,`vat_amt`) VALUES $str";
...
mysqli_query($dbc, $update);
    ...
    $update_stock_q="UPDATE `stock` SET qty=$new_stock_qty WHERE `product_id`=$prod AND `dealer_id`=$dealer_id";
...
if($aval_stock>=$qty1)
{
$store[] = "('$iddd','$prod','0','','$taxId','$vat_amt','$qty', '$rate','$schqty'
,'$uso_order_id','$user_id','$mrp','$cd','$cd_type','$cd_amt','$dis_type',
