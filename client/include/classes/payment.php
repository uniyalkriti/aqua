...
//                $payment_status = $this->payment_status($_POST['challan_no']); 
	...
	function save_payment(){
		...
		// query to save
		  ...
		  $q="INSERT INTO `payment_collection`(`id`, `dealer_id`, `retailer_id`, `challan_id`, `total_amount`,`reciept_no`, `pay_mode`, `bank_name`, `chq_no`, `chq_date`, `Remark`, `pay_date_time`) VALUES ('','$dealer_id','$d1[retailer_id]','$challan_id','$d1[remain_amt]','$d1[reciept_no]','$d1[pay_mode]','$d1[bank_name]','$chq_no','$chq_date','$d1[Remark]',NOW())";
               //h1($q); exit;
		$r = mysqli_query($dbc,$q) ;
		if(!$r){mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'Payment table error');}
		$rId = $id;	
		...
		//mysqli_query($dbc,"UPDATE `challan_order` SET `payment_status`='1' where `id` IN($challan_id)");
                ...
                $size=  sizeof($challan_id1);
                 $oldq =  "SELECT id, sum(old_payment_dues.remaining) as old_total FROM old_payment_dues WHERE id='$challan_id1[0]' ";
                     $oldr = mysqli_query($dbc,$oldq);
                    $numold = mysqli_num_rows($oldr);
                    if($numold>0)
                    {
                     $rowold = mysqli_fetch_assoc($oldr);
                    
                    $remain_amtold = $rowold['old_total'];
                  if($amt>=$remain_amtold)
                           {
                       $amt=$amt-$remain_amtold;
                       $q23="UPDATE `old_payment_dues` SET  `remaining` =  '0',`payment_status`='1' WHERE `id` =$challan_id1[0]";
                      // h1($q23);exit;
                      mysqli_query($dbc,$q23);   
                       }
                else if($amt < $remain_amtold && $amt > 0){
                               $remain=$remain_amtold-$amt; 
                               $amt=0;
                                mysqli_query($dbc,"UPDATE `old_payment_dues` SET  `remaining` =  '$remain' WHERE `id` =$challan_id1[0]");   
                            }
                    }
	...
	public function print_looper_payment($multiId, $options=array())
	{
     ...
     /*     public function get_payment_retailer($dealer_id){
		...
		{                  
		 ...
		 public function get_old_payment_list($filter='',  $records = '', $orderby='')
        {
            global $dbc;
            $out = array();
            $filterstr = $this->oo_filter($filter, $records, $orderby);
            $q = "SELECT old_payment_dues.*,retailer.name FROM old_payment_dues INNER JOIN retailer ON retailer.id = old_payment_dues.ch_retailer_id $filterstr";
           //h1($q);
            list($opt, $rs) = run_query($dbc, $q, 'multi');
            if(!$opt) return $out;
            while($row = mysqli_fetch_assoc($rs))
 ...
 public function get_old_payment_se_data()
	{  
		$d1 = array();
		$d1 = $_POST;
	...
	public function old_payment_save()
	{
		global $dbc;	
		$out = array('status'=>false, 'myreason'=>'');
                $dealer_id = $_SESSION[SESS.'data']['dealer_id'];
		list($status, $d1) = $this->get_old_payment_se_data();
		if(!$status) return array('status'=>false, 'myreason'=>$d1['myreason']);		
		//start the transaction
		mysqli_query($dbc, "START TRANSACTION");
		//pre($d1);
                foreach($d1['retailer_id'] AS $k=>$v){
                $amount=$d1['amount'][$k];
		$q = "INSERT INTO `old_payment_dues`(`ch_dealer_id`, `ch_retailer_id`, `amount`, `remaining`, `payment_status`) "
                        . "VALUES ('$dealer_id','$v','$amount','$amount','0')";
                //h1($q);
                $r = mysqli_query($dbc,$q) ;
                }
    ...
    mysqli_query($dbc, "START TRANSACTION");
    // query to save
    $old_ch_id=$d1[old_ch_id];
    $old_remain_amount=$d1[old_remain_amount];
    $old_recieved_amt=$d1[old_recieved_amt];
    if(!empty($old_recieved_amt)){
    if($old_remain_amount==$old_recieved_amt){
    $q23="UPDATE `old_payment_dues` SET  `remaining` = '0',`payment_status`='1' WHERE `id` ='$old_ch_id'";
    }else{
    $q23="UPDATE `old_payment_dues` SET  `remaining` = (`remaining`-'$old_recieved_amt') WHERE `id` ='$old_ch_id'";  
    }
    // h1($q23);exit;
    mysqli_query($dbc,$q23);   

    $q="INSERT INTO `payment_collection`(`id`, `dealer_id`, `retailer_id`, `challan_id`, `total_amount`,`reciept_no`, `pay_mode`, `bank_name`, `chq_no`, `chq_date`, `Remark`, `pay_date_time`) VALUES ('','$dealer_id','$d1[retailer_id]','$old_ch_id','$old_recieved_amt','$d1[reciept_no]','$d1[pay_mode]','$d1[bank_name]','$chq_no','$chq_date','$d1[Remark]',NOW())";
               //h1($q); exit;
    $r = mysqli_query($dbc,$q) ;
    }

    if(!empty($d1['invoice'])){
      foreach ($d1['invoice'] as $key => $value) {
          $challan_id=$d1['invoice'][$key];
          $remain_amount=$d1['remain_amount'][$key];
          $recieved_amt=$d1['recieved_amt'][$key];

    $q="INSERT INTO `payment_collection`(`id`, `dealer_id`, `retailer_id`, `challan_id`, `total_amount`,`reciept_no`, `pay_mode`, `bank_name`, `chq_no`, `chq_date`, `Remark`, `pay_date_time`) VALUES ('','$dealer_id','$d1[retailer_id]','$challan_id','$recieved_amt','$d1[reciept_no]','$d1[pay_mode]','$d1[bank_name]','$chq_no','$chq_date','$d1[Remark]',NOW())";
               //h1($q); exit;
    $r = mysqli_query($dbc,$q) ;
    if(!$r){
      mysqli_rollback($dbc); return array('status'=>false, 'myreason'=>'Payment table error');
    }
    $rId = $id; 

    if($remain_amount==$recieved_amt){
    $q23="UPDATE `challan_order` SET  `remaining` =  '0',`payment_status`='1' WHERE `challan_order`.`id` =$challan_id";
    }else{
    $q23="UPDATE `challan_order` SET  `remaining` =  (`remaining`-'$recieved_amt'),`payment_status`='2' WHERE `challan_order`.`id` =$challan_id";  
    }
    // h1($q23);exit;
    mysqli_query($dbc,$q23);  
      }
