...
class dealer_sale extends myfilter {
    ...
    order_id,DATE_FORMAT(date,'%d/%m/%Y') AS sale_date,DATE_FORMAT(time,'%d/%m/%Y') AS sale_time,DATE_FORMAT(date,'%d/%b/%Y') AS fdated FROM user_sales_order uso INNER JOIN user_sales_order_details USING(order_id) $filterstr AND user_sales_order_details.status !=2 AND uso.order_status !=8 GROUP BY uso.order_id";
        ...
        public function get_order_against_details_list($filter = '', $records = '', $orderby = '', $popup=false)
        {
            global $dbc;
            $out = array();
            $filterstr = $this->oo_filter($filter, $records, $orderby);
            ...
            $q = "SELECT uso.dealer_id,uso.created_person_id,uso.order_id,uso.order_remark as remarks,count(distinct demand_order_details.id ) as total_qty, SUM(total_rs) as total_sale,
            (SELECT count(id) as order_item FROM `demand_order_details` WHERE `order_id` = uso.order_id ) as rem_qty,(SELECT SUM(total_rs) FROM `demand_order_details` WHERE `order_id` = uso.order_id ) as rem_sale_val,
            order_id,DATE_FORMAT(order_date,'%d/%m/%Y') AS sale_date,DATE_FORMAT(date_time,'%d/%m/%Y') AS sale_time,DATE_FORMAT(order_date,'%d/%b/%Y') AS fdated FROM demand_order uso INNER JOIN demand_order_details USING(order_id) $filterstr  AND uso.status !=2 GROUP BY uso.order_id";
            ...
            while ($row = mysqli_fetch_assoc($rs)) {
                ...
                $retailer_map =  myrowval('dealer', 'name',$where);
                $id = $row['order_id'];
                $out[$id] = $row; // storing the item id
                $out[$id]['dspId'] = $row['created_person_id'];
                $out[$id]['remarks'] = $row['remarks'];
                $out[$id]['name'] = '';//$dealer_map[$row['dealer_id']];
                $out[$id]['firm_name'] = $retailer_map;
    ...
    public function get_dealer_sale_list($filter = '', $records = '', $orderby = '', $popup=false)
    {
        ...
        while ($row = mysqli_fetch_assoc($rs))
        {
            ...
            if($popup)
            {                
                $item_q = "SELECT usod.id,cp.name, cp.taxable,usod.quantity,usod.rate,usod.scheme_qty,usod.product_id,usod.remaining_qty FROM user_sales_order_details usod 
                    INNER JOIN user_sales_order uso ON usod.order_id = uso.order_id 
                    INNER JOIN catalog_product cp ON usod.product_id=cp.id 
                    WHERE usod.order_id = $row[order_id] AND usod.status!=2 AND uso.dealer_id=$dealer_id ORDER BY usod.product_id";
                // h1($item_q);
                // die;
    ...
    public function get_order_against_list($filter = '', $records = '', $orderby = '', $popup=false)
    {
        global $dbc;
        $out = array();
        $filterstr = $this->oo_filter($filter, $records, $orderby);
        ...
        $q = "SELECT *,order_id,DATE_FORMAT(order_date,'%d/%m/%Y') AS sale_date,DATE_FORMAT(order_date,'%d/%m/%Y') AS sale_time,DATE_FORMAT(order_date,'%d/%b/%Y') AS fdated FROM demand_order 
            $filterstr GROUP BY demand_order.order_id";
                // h1($q);exit;
        ...
        while ($row = mysqli_fetch_assoc($rs))
        {
            ...
            $out[$id] = $row; // storing the item id
            $out[$id]['dspId'] = $row['created_person_id'];
            $out[$id]['remarks'] = $row['order_remark'];
            $out[$id]['name'] = '';//$dealer_map[$row['dealer_id']];
            $out[$id]['firm_name'] = $retailer_map;
            $out[$id]['firm_id'] = myrowval('dealer', 'id',$where);
            $out[$id]['person_name'] = $this->get_username($row['created_person_id']);

            if($popup)
            {                
                // $item_q = "SELECT usod.id,cp.name, cp.taxable,usod.quantity,usod.rate,usod.scheme_qty,usod.product_id,usod.remaining_qty FROM demand_order_details usod 
                //     INNER JOIN demand_order uso ON usod.order_id = uso.order_id 
                //     INNER JOIN catalog_product cp ON usod.product_id=cp.id 
                //     WHERE usod.order_id = $row[order_id] AND uso.status!=2 AND uso.dealer_id=$dealer_id ORDER BY usod.product_id";

                $item_q = "SELECT usod.id,cp.name, cp.taxable,usod.quantity,usod.rate,usod.scheme_qty,usod.product_id,usod.free_qty as remaining_qty FROM demand_order_details usod 
                    INNER JOIN demand_order uso ON usod.order_id = uso.order_id 
                    INNER JOIN catalog_product cp ON usod.product_id=cp.id 
                    WHERE usod.order_id = $row[order_id] AND uso.status!=2  ORDER BY usod.product_id";
                // h1($item_q);
                // die;
                $o_item_data = mysqli_query($dbc,$item_q);
 ...
 public function challan_extra_edit($actiontype, $rId, $d1,$company_id) {
    ...
    public function get_challan_list($filter ='', $records ='', $orderby ='', $ifilter='')
    {
		...
		$q = "SELECT challan_order.crdr_status,challan_order.book_status,challan_order.invoice_type,challan_order.discount_per,challan_order.ch_retailer_id,challan_order.ch_user_id,challan_order.date_added,challan_order.ch_no,challan_order.ch_no,challan_order.discount_amt,challan_order.amount,challan_order.amount_round,challan_order.id as id, DATE_FORMAT(dispatch_date, '" . MASKDATE . "') AS dispatch_date, DATE_FORMAT(ch_date, '%Y-%m-%d') AS ch_date FROM challan_order INNER JOIN dealer_location_rate_list AS dlrl ON dlrl.dealer_id=ch_dealer_id $filterstr";
        }else{
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            $q = "SELECT challan_order_details.*,challan_order_details.product_id AS pid, "
                    ...
                    . " INNER JOIN catalog_3 c3 ON c3.id = catalog_product.catalog_id "
                    . " LEFT JOIN product_rate_list ON challan_order_details.product_id = product_rate_list.product_id "
                    . " AND product_rate_list.state_id ='$state_id' $ifilterstr ch_id = $id GROUP BY challan_order_details.id ";
    ...
    public function get_payment_data($filter = '', $records = '', $orderby = '')
     {
      ...
      //  $retailer_map = get_my_reference_array('retailer', 'id', 'name');
        while ($row = mysqli_fetch_assoc($rs)){
           // pre($row);
            ...
            $out[$id]['retailer_id'] = $row['ch_retailer_id'];
             $q1 = "SELECT ch_retailer_id, sum(old_payment_dues.remaining) as old_total FROM old_payment_dues WHERE ch_retailer_id='$row[ch_retailer_id]' AND payment_status='0'";
             $r1= mysqli_query($dbc, $q1);
             $row1= mysqli_fetch_assoc($r1);
             $old_total=$row1['old_total'];
            $out[$id]['total'] = $row['total']+$old_total;
            $where = 'id = '.$row['ch_retailer_id'];
      ...
      $chreta =  implode(',', $retailerarr);
      echo $_POST[ch_retailer_id];  
      if(!empty($_POST[retailer_id])){
        $chretailer = " AND ch_retailer_id='$_POST[retailer_id]'";
      }
      if(!empty($chreta)){
        $chretailer = " AND ch_retailer_id NOT IN ($chreta)";
      }
 $qreta = "SELECT ch_retailer_id, sum(old_payment_dues.remaining) as total FROM 
            old_payment_dues WHERE  ch_dealer_id = $dealer_id  AND payment_status='0' $chretailer group by ch_retailer_id";
           $rreta = mysqli_query($dbc,$qreta);
//h1($qreta);
           while ($rowreta = mysqli_fetch_assoc($rreta)){
           // pre($row);
            $id = $rowreta['ch_retailer_id'];
            $out[$id] = $rowreta; // storing the item id
            $retailer = $rowreta['ch_retailer_id'];
            $out[$id]['retailer_id'] = $rowreta['ch_retailer_id'];
             
            $out[$id]['total'] = $rowreta['total'];
            $where = 'id = '.$rowreta['ch_retailer_id'];
            $out[$id]['retailer_name']=  myrowval('retailer', 'name',$where);
            $out[$id]['last'] = $this->get_last_payment($retailer,$dealer_id);
           // $retailerarr[]  =  $retailer;
            //$out[$id]['retailer_name'] = $retailer_map[$row['ch_retailer_id']];
       ...
       // $retailer_map = get_my_reference_array('retailer', 'id', 'name');
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
           ...
           // $out[$id]['tin_no'] = $retailer_map1[$row['ch_retailer_id']];
            $q = "SELECT challan_order_details.*,sum(qty*product_rate) as challan_val,sum(cd_amt) as total_cd_amt,challan_order_details.product_id AS pid,sum(taxable_amt) AS amt, catalog_product.name,sum(vat_amt) as vat_amt   
            FROM challan_order_details 
            INNER JOIN catalog_product ON catalog_product.id = challan_order_details.product_id  
            INNER JOIN catalog_2 c2 ON c2.id = catalog_product.catalog_id  
...
//    }
    ...
    public function direct_challan_receieve_save(){
        global $dbc;
        $out = array('status' => 'false', 'myreason' => '');
        list($status, $d1) = $this->get_challan_se_data();
        if (!$status)
       ...
       // $iddd = $d1['dealer_id'].$orderno;
        ...
        if(isset($d1['book_status'])){
        $book_status=1;
        }else{
        $book_status=0;
        }
        $ch_no_cus = 'ADL-'.$chlaan_no;
        
        mysqli_query($dbc, "START TRANSACTION");
        if($d1[retailer_id] == '117020210903060555'){
            if (!empty($d1['product_id'])) {
                $str = array();
                $str1 = array();
                $batch_id = array();          
                ...
                foreach ($d1['product_id'] as $key => $value) {
                    ...
                    if(!empty($value)){
                        ...
                        $cash_amt = $d1['cash_amt'][$key];
                        if(empty($cash_amt)){
                           $cash_amt=0; 
                        }
                        $spl_dis_type = $d1['spl_disc_type'][$key];
                        $spl_dis_percent = $d1['spl_disc_val'][$key];
                        $spl_dis_amt = $d1['spl_disc_amt'][$key];

                        $dis_type = $d1['trade_disc_type'][$key];
                        $dis_amt = $d1['trade_disc_amt'][$key];
                        $dis_percent = $d1['trade_disc_val'][$key];
                        $taxable_amt = $d1['amount'][$key];
                        $taxable_amt_cus = $d1['taxable_amt'][$key];
                        $vat_amt = $d1['vat_amt'][$key];
                        $gross_amt = $rate*$qty;
                        $wheres="`product_id`=$prod AND `dealer_id`='$d1[dealer_id]' AND `mrp`=$mrp AND batch_no='$batch_no'";
                        $mfg1=myrowval('stock','mfg',$wheres);
                        if(empty($vat_amt) || $vat_amt==0.0000)
                        {
                           $vat_amt = $d1['cgst_amt'][$key]*2; 
                        }

                        $cgst_amt_cus = $d1['cgst_amt'][$key]; 
                        $sgst_amt_cus = $d1['sgst_amt'][$key]; 
                        $igst_amt_cus = $d1['vat_amt'][$key]; 
                        $net_amount_cus = $d1['amount'][$key];
                        ...
                        $details_order = mysqli_query($dbc, $qqq);

                    }

       
                    $qpp="SELECT itemcode AS itemcode FROM catalog_product WHERE id='$prod' and status = '1' LIMIT 1";
                    $rpp=mysqli_query($dbc,$qpp);
                    $rowpp=mysqli_fetch_assoc($rpp);
                    $itemcode=$rowpp['itemcode'];
                    if(empty($itemcode)){
                        $itemcode=0;
                    }

                   
                    $qqq_details = "INSERT INTO `purchase_order_details`(`order_id`, `product_id`, `item_code`, `mrp`, `rate`, `quantity`, `scheme_qty`, `purchase_inv`, `mfg_date`,`batch_no`, `pr_rate`, `cases`, `gross_amt`, `taxable_amount`, `td_amount`, `sch_amt`, `spl_amt`, `cd_amount`, `atd_amt`, `cgst_amount`, `sgst_amount`, `igst_amount`, `gst_percentage`, `cr_note`, `dr_note`, `total_amount`) VALUES ('$ch_no','$prod',$itemcode,'$mrp','$rate','$qty','$schqty','$ch_no_cus','$mfg1','$batch_no','$rate','0','$gross_amt','0.00','$dis_amt','0','$cash_amt','$spl_dis_amt','$cd_amt','$cgst_amt_cus','$sgst_amt_cus','$igst_amt_cus','0','0','0','$net_amount_cus')";

                      
                    $set_query_details = mysqli_query($dbc,$qqq_details); 
                    if (!$set_query_details) {
                        mysqli_rollback($dbc);
                        return array('status' => false, 'myreason' => 'Purchase Details Table Could Not saved, Some error occurred');
                    }
                }
                

                $qqq = "INSERT INTO `purchase_order`(`order_id`, `dealer_code`, `dealer_id`, `created_date`, `created_person_id`, `order_date`, `date_time`, `company_id`, `ch_date`, `challan_no`, `depo_code`, `csa_id`, `firm_name`) VALUES ('$ch_no','40095','1169',NOW(),$d1[uid],'$ch_date',NOW(),'1','$ch_date','$ch_no_cus','0','0','S.B.A.B.PVT.LTD.(SALES DEPOT),LAJPATNAGAR')";

                    // $rs1=     
                $set_query = mysqli_query($dbc,$qqq); 
                if (!$set_query) {
                    mysqli_rollback($dbc);
                    return array('status' => false, 'myreason' => 'Purchase Table Could Not saved, Some error occurred');
                }
                mysqli_commit($dbc); 

                return array('status' => true, 'myreason' => '<strong> successfully Saved <br/> '.'% &nbsp; '.'<br><br> '.'</strong>','challan_id'=>$iddd);
            }
        }
            ...
            foreach ($d1['product_id'] as $key => $value) {
               ...
               // $total_qty = $total_qty + $d1['quantity'][$key];
                ...
                $str[] = "(NULL,'$iddd','$prod','$bill_of_supply','$qty', '$rate','$schqty','$taxId','$mrp',
                    '$cd','$cd_type','$cd_amt','$spl_dis_type','$spl_dis_percent','$spl_dis_amt','$batch_no','$mfg1','$dis_type','$dis_amt','$dis_percent','$taxable_amt','$taxable_amt','$vat_amt','$cash_amt')";
                //$str1[] = "(,'$orderno','$prod','$rate', '$qty','$schqty')";
                $qtyy= $qty+$schqty;
                $sq = "SELECT `id`,`remaining`,`product_id` FROM `stock` where `product_id`='$prod' AND dealer_id='$dealer_id' AND batch_no='$batch_no' AND `mfg`='$mfg1' order by `mfg` asc"; 
                ...
                while($row = mysqli_fetch_assoc($rs1))
                {
                    ...
                    $update_stock_q="UPDATE `stock` SET qty=$new_stock_qty WHERE `product_id`=$prod AND `dealer_id`=$dealer_id AND `mrp`=$mrp AND batch_no='$batch_no' AND `mfg`='$mfg1'";
                    //h1($update_stock_q);exit;
...
//            h1($disc_amt); exit;
        ...
        $qqq = "INSERT INTO `challan_order` (`id`, `ch_no`, `ch_created_by`, `ch_dealer_id`, `ch_retailer_id`,`ch_user_id`, `ch_date`,`date_added`, `company_id`,`invoice_type`,`book_status`,`crdr_status`
            ,`discount_per`,`discount_amt`,`amount`,`remaining`,`amount_round`,`dispatch_status`,`sesId`,`remark`) 
    VALUES ($iddd, '$chlaan_no','$d1[uid]', '$d1[dealer_id]', '$d1[retailer_id]','$d1[user_id]','$ch_date',NOW(), '1','1','$book_status','$crdr',
        '$disc','$disc_amt','$amount1','$amount1','$amount1','0','0','NULL')";
          // h1($qqq);exit;
        ...
        if (!$r) {
            ...
            $str = implode(',', $str);
             $q = "INSERT INTO challan_order_details (`id`,`ch_id`,`product_id`,`supply_status`,`qty`,`product_rate`, `free_qty`,`tax`,`mrp`,`cd`,`cd_type`,
                 `cd_amt`,`spl_disc_type`,`spl_disc_val`,`spl_disc_amt`,`batch_no`,`mfg_date`,`dis_type`,`dis_amt`,`dis_percent`,`taxable_amt`,`remain_amount`,`vat_amt`,`cash_amt`)
            VALUES $str";
           // h1($q);
           // die;
            $r = mysqli_query($dbc, $q);
    ...
    public function direct_challan_edit($id)
    {
        ...
        $product_batch_str = explode(',',$d1['batchstr']);
        $product_mfg_str = explode(',',$d1['mfgstr']);
        ...
        if (!empty($d1['product_id']))
        {
            ...
            foreach ($d1['product_id'] as $key => $value)
            {
                ...
                $cd_type = $d1['cd_type'][$key];
                $batch_no = $abc[2];
                $mfg_date = $d1['mfg_date'][$key];
                if(empty($mfg_date) || $mfg_date=='0000-00-00'){
                     $wheres="`product_id`=$prod AND `dealer_id`='$d1[dealer_id]' AND `mrp`=$mrp AND batch_no='$batch_no'";
                $mfg_date=myrowval('stock','mfg',$wheres);
                }
                //h1($batch_no);
                $cd_amt = $d1['cd_amt'][$key];
                $cash_amt = $d1['cash_amt'][$key];
                ...
                $spl_disc_type=$d1['spl_disc_type'][$key];
                $spl_disc_val=$d1['spl_disc_val'][$key];
                $spl_disc_amt=$d1['spl_disc_amt'][$key];

                $cgst = $d1['cgst'][$key];
                $cgst_amt = $d1['cgst_amt'][$key];
                $sgst = $d1['sgst'][$key];
                $sgst_amt = $d1['sgst_amt'][$key];
               
                //if($taxId=='0.00' && $vat_amt=='0.00'|| empty($taxId)&& empty($vat_amt))
               // {
                    $taxId=$cgst+$sgst;
                    // h1( $taxId);
                     $vat_amt=$cgst_amt+ $sgst_amt;
                    // h1( $vat_amt);
               // }
                // h1( $taxId);
                // h1( $vat_amt);
                //To save the value of the other columns as some columns are affected by po
                $str[] = "('$id','$prod','$bill_of_supply','$qty1', '$rate','$schqty','$taxId','$mrp','$cd',
                '$cd_type','$cd_amt','$spl_disc_type','$spl_disc_val','$spl_disc_amt','$batch_no','$dis_type','$dis_amt','$dis_percent',
                '$amount','$amount','$vat_amt','$mfg_date','$cash_amt')";
                $nid++;
                ...
                if($qty>$ordered_qty && $qty!=$aval_stock)
                {
                    ...
                    $stock_q = "UPDATE `stock` SET `qty`=(qty-$updated_qty) WHERE `product_id`='$prod' AND `dealer_id`='$d1[dealer_id]' AND `mrp`='$mrp' AND batch_no='$batch_no' AND mfg='$mfg_date'";
                    //h1($stock_q);exit;
                    $stock_e = mysqli_query($dbc,$stock_q);
                }
                ...
                elseif($qty==$aval_stock)
                {
                     ...
                     $stock_q = "UPDATE `stock` SET `qty`= '$updated_qty' WHERE `product_id`='$prod' AND `dealer_id`='$d1[dealer_id]' AND `mrp`='$mrp' AND batch_no='$batch_no' AND mfg='$mfg_date'";
                        //h1($stock_q);
                    $stock_e = mysqli_query($dbc,$stock_q);
                }
            ...
            if(count($d1['product_id'])<count($product_ids_str))
            {
                ...
                foreach($product_ids_str as $k=>$removed_id)
                {
                    $removed_product_qty[$removed_id] = array(
                                                            ...
                                                            'batch'=>$product_batch_str[$k],
                                                            'mfg'=>$product_mfg_str[$k]
                                                        );
                }
                ...
                foreach($new_id_arr as $pid)
                {
                    ...
                    $batch_no2 = $removed_product_qty[$pid]['batch'];
                    $mfg_date2 = $removed_product_qty[$pid]['mfg'];
                    $stock_q = "UPDATE `stock` SET `qty`=(qty+$updated_qty) WHERE `product_id`='$pid' AND `dealer_id`='$d1[dealer_id]' AND `mrp`='$mrp' AND batch_no='$batch_no2' AND mfg='$mfg_date2'";
         ...
         $qup="UPDATE `challan_order` SET `ch_no`='$chlaan_no',`ch_created_by`='$d1[uid]',
             `ch_dealer_id`='$d1[dealer_id]',`ch_retailer_id`='$d1[retailer_id]',
              `ch_date`='$ch_date',`discount_per`='$d1[dis]',`discount_amt`='$discount',`amount`='$final',`remaining`='$final',`amount_round`='$final' WHERE id='$id' ";
      /* h1($qup);
       die;*/
        ...
        $nid= $id;
          ...
          $q1 = "INSERT INTO challan_order_details (`ch_id`,`product_id`,`supply_status`,`qty`,`product_rate`, `free_qty`,`tax`,`mrp`,`cd`,`cd_type`,`cd_amt`, `spl_disc_type`, `spl_disc_val`, `spl_disc_amt`,`batch_no`,`dis_type`,`dis_amt`,`dis_percent`,`taxable_amt`,`remain_amount`,`vat_amt`,`mfg_date`,`cash_amt`)
          VALUES $str";
      //h1($q1);
...
$uid= $id;
    ...
    $firm_id = $d1['firm_id'][$k];
    $firm_name = $d1['firm_name'][$k];
    ...
    $user_id = $d1['user_id'][$k];
        ...
        // If dealer and retailer both are from same state then cgst & sgst is applicable
        if(empty($taxId) || $taxId=0)
        {
            $taxId = $d1['cgst'][$k][$key]*2;
        }
       
        $vat_amt = $d1['vat_amt'][$k][$key];

        // If dealer and retailer both are from same state then cgst & sgst is applicable
        if(empty($vat_amt) || $vat_amt==0)
        {
           $vat_amt = $d1['cgst_amt'][$k][$key]*2; 
        }
        ...
        $free_qty= $d1['freeqty'][$k][$key];

        if(!empty($d1['spl_disc_amt'][$k][$key]) || $d1['spl_disc_amt'][$k][$key] > 0)
        {
            $spl_disc_amt = $d1['spl_disc_amt'][$k][$key];
        }
        else {
            $spl_disc_amt=0.00;
        }
        $cd = !empty($d1['cd'][$k][$key])?$d1['cd'][$k][$key]:0;
        $spl_disc_val=!empty($d1['spl_disc_val'][$k][$key])?$d1['spl_disc_val'][$k][$key]:0.00;
        $spl_disc_type=!empty($d1['spl_disc_type'][$k][$key])?$d1['spl_disc_type'][$k][$key]:0;
// //20180907151358237
//  pre($d1['spl_disc_amt'][$k][$key]); die;
        $mrp = $d1['mrp'][$k][$key];
        $abc=explode('/',$mrp);
        $mrp=$abc[0];
        $batch_no=$abc[2];
        ...
        if($aval_stock<$qty2)
        {
            if($aval_stock>0)
            {
                ...
                $update_stock_q="UPDATE `stock` SET qty=0 WHERE `product_id`=$prod AND `dealer_id`=$firm_id AND mrp=$mrp AND batch_no='$batch_no' AND mfg='$mfg_date'";
        ...
        $update="UPDATE `user_sales_order_details` SET status=$product_status WHERE `user_sales_order_details`.`order_id` =$uso_order_id AND product_id='$prod'";

        mysqli_query($dbc, $update);
        //Updated By Deepak at 31-05-2019
        //Because free qty should be minus from current stock
        if($free_qty>0)
        {
            $qtynew=$qty2+$free_qty;
        }else
        {
           $qtynew= $qty2;
        }
        ...
        $new_stock_qty = $aval_stock-$qtynew;
        $update_stock_q="UPDATE `stock` SET qty=$new_stock_qty WHERE `product_id`=$prod AND `dealer_id`=$firm_id AND mrp=$mrp AND batch_no='$batch_no' AND mfg='$mfg_date'";
        ...
        $store[] = "('$new_ch_id','$prod','$bill_of_supply','0','$batch_no','$taxId','$vat_amt','$qty', '$rate','$free_qty'
        ,'$uso_order_id','$user_id','$mrp','$cd','$cd_type','$cd_amt','$dis_type',
        '$dis_amt','$dis_percent','$taxable_amt','$taxable_amt','$mfg_date','$cash_amt','$spl_disc_val','$spl_disc_amt','$spl_disc_type')";
...
$str1[] = "($new_ch_id,'$ch_no','$user_id','$firm_id','$retailer_id','$ch_uid','$ch_date',NOW(),'1','0','0','$remark','1','2','0','$discount_per','$disc_amount','$amount1','$amount1','$amount1')";
$i++;
}
...
`sesId`, `remark`, `sync_status`, `invoice_type`, `payment_status`,`discount_per`,`discount_amt`,`amount`,`remaining`,`amount_round`) VALUES $str2";
 //h1($query);
$run = mysqli_query($dbc, $query);
if (!$run) {
mysqli_rollback($dbc);
...
`free_qty`, `order_id`, `user_id`, `mrp`, `cd`, `cd_type`, `cd_amt`,
`dis_type`, `dis_amt`, `dis_percent`, `taxable_amt`, `remain_amount`,`mfg_date`,`cash_amt`,`spl_disc_val`,`spl_disc_amt`,`spl_disc_type`) VALUES $stored";
...
//    
       ...
       //$rs = mysqli_query($dbc,$qq);
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
           ...
           // $out[$id]['rid'] = $row['name'];
            ...
            $out[$id]['image'] =  $a[$id]['scheme']['image'];
            }// while($row = mysqli_fetch_assoc($rs)){ ends
     // pre($out);
        return $out;
    }
    ...
    public function get_challan($rid)
    {
     ...
     public function current_scheme($sale) {
        ...
        global $dbc;   
        $date = date('Y-m-d');
        $q = "SELECT svp.id,svp.value,svp.value_to,svp.scheme_gift,svpd.start_date,svpd.end_date,svp.image from scheme_value_product_details svp INNER JOIN scheme_value svpd ON svpd.scheme_id = svp.scheme_id where '$date' BETWEEN svpd.start_date AND svpd.end_date AND user = 3 AND state_id = $state AND '$sale' BETWEEN svp.value AND svp.value_to";
      // h1($q);
        $r = mysqli_query($dbc, $q);
...
//h1($q);
    ...
    {
        global $dbc;
        $qry = "SELECT CONCAT_WS('/',CAST(mrp as decimal(10,2)),qty,batch_no,DATE_FORMAT(mfg,'%b-%Y')) as mrp FROM stock WHERE product_id=$pid AND dealer_id=$did";
...
public function get_tax_inv_gst_list($filter = '', $records = '', $orderby = '') {
       ...
       // h1($q);
        ...
        while ($row = mysqli_fetch_assoc($rs)) {           
            $id = $row['cid'];
            $out['B2C Large'] = $row; // storing the item id
          
        }
        $q = "SELECT count(DISTINCT ch_retailer_id) as retailer_count,co.ch_retailer_id as cid,sum(cod.vat_amt) as total_tax_amount,sum(cod.taxable_amt-cod.vat_amt) as total_taxable_amount FROM challan_order co inner join challan_order_details cod on co.id = cod.ch_id inner join retailer r on co.ch_retailer_id = r.id $filterstr and co.ch_dealer_id=$dealer_id AND co.book_status!=0";
        ...
        while ($row = mysqli_fetch_assoc($rs)) {           
            $id = $row['cid'];
            $out['Exempt'] = $row; // storing the item id
 ...
 public function print_looper_challan_gst($start, $end,$status)
    {  
       ...
       $dealer_id = $_SESSION[SESS . 'data']['dealer_id'];
    if($status == 1){
        $q = "SELECT co.id AS cid,co.ch_retailer_id,r.name as retailer_name,cod.id as id, r.tin_no as gstino,co.ch_no as invoice_no,date_format(co.ch_date,'%d/%m/%Y') as invoice_date,cod.tax as rate,sum(cod.vat_amt) as tax_amount,sum(cod.taxable_amt) as taxable_amt, SUM(cod.taxable_amt-cod.vat_amt) as txbl FROM challan_order co inner join challan_order_details cod on co.id = cod.ch_id inner join retailer r on co.ch_retailer_id = r.id where date_format(`ch_date`,'%Y%m%d')>= $start and date_format(`ch_date`,'%Y%m%d') <= $end and co.ch_dealer_id=$dealer_id and r.tin_no != 0 and co.book_status=0 group by co.ch_no,cod.tax ";
    }
    elseif($status == 2){
        $q = "SELECT co.id AS cid,co.ch_retailer_id,r.name as retailer_name,cod.id as id, r.tin_no as gstino,co.ch_no as invoice_no,date_format(co.ch_date,'%d/%m/%Y') as invoice_date,cod.tax as rate,sum(cod.vat_amt) as tax_amount,sum(cod.taxable_amt) as taxable_amt, SUM(cod.taxable_amt-cod.vat_amt) as txbl FROM challan_order co inner join challan_order_details cod on co.id = cod.ch_id inner join retailer r on co.ch_retailer_id = r.id where date_format(`ch_date`,'%Y%m%d')>= $start and date_format(`ch_date`,'%Y%m%d') <= $end and co.ch_dealer_id=$dealer_id and r.tin_no = 0  and co.amount<=50000 and co.book_status=0 group by co.ch_no,cod.tax ";
    }  elseif($status == 3){
        $q = "SELECT co.id AS cid,co.ch_retailer_id,r.name as retailer_name,cod.id as id, r.tin_no as gstino,co.ch_no as invoice_no,date_format(co.ch_date,'%d/%m/%Y') as invoice_date,cod.tax as rate,sum(cod.vat_amt) as tax_amount,sum(cod.taxable_amt) as taxable_amt, SUM(cod.taxable_amt-cod.vat_amt) as txbl FROM challan_order co inner join challan_order_details cod on co.id = cod.ch_id inner join retailer r on co.ch_retailer_id = r.id where date_format(`ch_date`,'%Y%m%d')>= $start and date_format(`ch_date`,'%Y%m%d') <= $end and co.ch_dealer_id=$dealer_id and r.tin_no = 0 and co.amount>50000 and co.book_status=0 group by co.ch_no,cod.tax ";
    }elseif($status == 4){
        $q = "SELECT co.id AS cid,co.ch_retailer_id,r.name as retailer_name,cod.id as id, r.tin_no as gstino,co.ch_no as invoice_no,date_format(co.ch_date,'%d/%m/%Y') as invoice_date,cod.tax as rate,sum(cod.vat_amt) as tax_amount,sum(cod.taxable_amt) as taxable_amt, SUM(cod.taxable_amt-cod.vat_amt) as txbl FROM challan_order co inner join challan_order_details cod on co.id = cod.ch_id inner join retailer r on co.ch_retailer_id = r.id where date_format(`ch_date`,'%Y%m%d')>= $start and date_format(`ch_date`,'%Y%m%d') <= $end and co.ch_dealer_id=$dealer_id and co.book_status!=0 group by co.ch_no,cod.tax ";
    }
...
public function get_gst_wise_list($filter = '',$key, $records = '', $orderby = '') {
        global $dbc;
        $out = array();
        $dealer_id = $_SESSION[SESS . 'data']['dealer_id'];
        $filterstr = $this->oo_filter($filter, $records, $orderby);
        //h1($key);
            $qd="SELECT l1_name AS state FROM dealer INNER JOIN dealer_location_rate_list AS dlrl ON dlrl.dealer_id=dealer.id INNER JOIN location_view AS lv ON lv.l5_id=dlrl.location_id WHERE dealer.id='$dealer_id' LIMIT 1";
            $rsd = mysqli_query($dbc,$qd);
            $rowd=mysqli_fetch_assoc($rsd);
            $state= $rowd['state'];
            if($key=='B2B'){

        $q = "SELECT cod.id AS id,sum(taxable_amt-vat_amt) AS taxable_amt,round(tax) AS tax,sum(vat_amt) AS vat_amt FROM challan_order_details AS cod INNER JOIN challan_order AS co ON co.id=cod.ch_id INNER JOIN retailer AS r ON r.id=co.ch_retailer_id $filterstr and co.ch_dealer_id=$dealer_id AND co.book_status=0 AND cod.tax!='0.00' AND r.tin_no!=0 GROUP BY cod.tax";
           }elseif($key=='B2C'){
        $q = "SELECT cod.id AS id,sum(taxable_amt-vat_amt) AS taxable_amt,round(tax) AS tax,sum(vat_amt) AS vat_amt FROM challan_order_details AS cod INNER JOIN challan_order AS co ON co.id=cod.ch_id INNER JOIN retailer AS r ON r.id=co.ch_retailer_id $filterstr and co.ch_dealer_id=$dealer_id AND co.book_status=0 AND cod.tax!='0.00' AND r.tin_no=0 AND co.amount<=50000 GROUP BY cod.tax";    
    }elseif($key=='B2C Large'){
       $q = "SELECT cod.id AS id,sum(taxable_amt-vat_amt) AS taxable_amt,round(tax) AS tax,sum(vat_amt) AS vat_amt FROM challan_order_details AS cod INNER JOIN challan_order AS co ON co.id=cod.ch_id INNER JOIN retailer AS r ON r.id=co.ch_retailer_id $filterstr and co.ch_dealer_id=$dealer_id AND co.book_status=0 AND cod.tax!='0.00' AND r.tin_no=0 AND co.amount>50000 GROUP BY cod.tax";
    }elseif($key=='Exempt'){
       $q = "SELECT cod.id AS id,sum(taxable_amt-vat_amt) AS taxable_amt,round(tax) AS tax,sum(vat_amt) AS vat_amt FROM challan_order_details AS cod INNER JOIN challan_order AS co ON co.id=cod.ch_id INNER JOIN retailer AS r ON r.id=co.ch_retailer_id $filterstr and co.ch_dealer_id=$dealer_id AND co.book_status!=0 GROUP BY cod.tax";
    }else{
        $q = "SELECT cod.id AS id,sum(taxable_amt-vat_amt) AS taxable_amt,round(tax) AS tax,sum(vat_amt) AS vat_amt FROM challan_order_details  AS cod INNER JOIN challan_order AS co ON co.id=cod.ch_id $filterstr and co.ch_dealer_id=$dealer_id AND co.book_status=0 AND cod.tax!='0.00' GROUP BY cod.tax";
    }
       // h1($q);
        list($opt, $rs) = run_query($dbc, $q, $mode = 'multi', $msg = '');
        if (!$opt)
        ...
        while ($row = mysqli_fetch_assoc($rs)) {           
            $id = $row['id'];
            $out[$id] = $row;
            $out[$id]['Perticular'] = $state; // storing the item id
    ...
    public function get_hsn_count_list($start, $end) {
        global $dbc;
        //$out = array();
        $dealer_id = $_SESSION[SESS . 'data']['dealer_id'];
        $q="SELECT count(DISTINCT catalog_product.hsn_code) AS count FROM catalog_product INNER JOIN challan_order_details ON catalog_product.id=challan_order_details.product_id INNER JOIN challan_order ON      challan_order.id=challan_order_details.ch_id WHERE ch_dealer_id=".$dealer_id." AND DATE_FORMAT(ch_date,'%Y%m%d')>='".$start."' AND DATE_FORMAT(ch_date,'%Y%m%d')<='".$end."' LIMIT 1";
        //h1($q);
        $r=mysqli_query($dbc,$q);
        $row=mysqli_fetch_assoc($r);
        $hsn_count=$row['count'];
        ...
        public function get_doc_count_list($start, $end) {
        global $dbc;
        //$out = array();
        $dealer_id = $_SESSION[SESS . 'data']['dealer_id'];
        $q="SELECT count(DISTINCT ch_no) AS count FROM challan_order WHERE ch_dealer_id=".$dealer_id." AND DATE_FORMAT(ch_date,'%Y%m%d')>='".$start."' AND DATE_FORMAT(ch_date,'%Y%m%d')<='".$end."' AND book_status='1' LIMIT 1";
        //h1($q);
        $r=mysqli_query($dbc,$q);
        $row=mysqli_fetch_assoc($r);
        $hsn_count=$row['count'];
        return $hsn_count;
            ...
            public function get_hsn_purchase_count_list($start, $end) {
        global $dbc;
        //$out = array();
        $dealer_id = $_SESSION[SESS . 'data']['dealer_id'];
        $q="SELECT count(DISTINCT catalog_product.hsn_code) AS count FROM catalog_product INNER JOIN purchase_order_details ON purchase_order_details.product_id=catalog_product.id INNER JOIN purchase_order ON purchase_order_details.order_id=purchase_order.order_id AND purchase_order_details.purchase_inv=purchase_order.challan_no WHERE dealer_id=".$dealer_id." AND DATE_FORMAT(order_date,'%Y%m%d')>='".$start."' AND DATE_FORMAT(order_date,'%Y%m%d')<='".$end."' LIMIT 1";
        //h1($q);
        $r=mysqli_query($dbc,$q);
        $row=mysqli_fetch_assoc($r);
        $hsn_count=$row['count'];
...
$out = array();
//echo $filterstr;
$q = "SELECT rod.order_id,sum(rod.total_amount) AS total_amount,sum(gross_amt+td_amount+atd_amt+cd_amount+spl_amt+sch_amt) AS gross_amt,sum(cgst_amount+sgst_amount) AS tax1,sum(igst_amount) AS tax2,ro.receive_date,ro.challan_no,csa.csa_name,csa.gst_no FROM purchase_order_details rod JOIN purchase_order ro ON rod.order_id=ro.order_id AND rod.purchase_inv=ro.challan_no JOIN csa ON ro.csa_id=csa.c_id $filterstr AND cgst_amount!='0.00' AND sgst_amount!='0.00' GROUP BY dealer_id,csa_id";
...
public function print_purchase_gst($start, $end,$did,$status)
{
global $dbc;
$out = array();
//echo $start."<br>".$end."<br>".$did;
if($status==1){
$q = "SELECT ro.id AS id,ro.receive_date,ro.order_id as orderid ,sum(gross_amt+td_amount+atd_amt+cd_amount+spl_amt+sch_amt) AS gross_amt,round(SUM(total_amount),2) as total_amount,sum(cgst_amount) AS cgst_amount,sum(sgst_amount) AS sgst_amount,sum(igst_amount) AS igst_amount,ro.challan_no,csa.csa_name,csa.gst_no FROM purchase_order_details rod JOIN purchase_order ro ON rod.order_id=ro.order_id AND rod.purchase_inv=ro.challan_no INNER JOIN csa ON ro.csa_id=csa.c_id WHERE DATE_FORMAT(ro.created_date,'%Y%m%d') >= '$start' AND DATE_FORMAT(ro.created_date,'%Y%m%d') <= '$end' AND ro.dealer_id = '$did' GROUP BY ro.challan_no,rod.igst_amount HAVING cgst_amount!=0 AND sgst_amount!=0";
}elseif($status==2){
$q = "SELECT ro.id AS id,ro.receive_date,ro.order_id as orderid ,sum(gross_amt+td_amount+atd_amt+cd_amount+spl_amt+sch_amt) AS gross_amt,round(SUM(total_amount),2) as total_amount,sum(cgst_amount) AS cgst_amount,sum(sgst_amount) AS sgst_amount,sum(igst_amount) AS igst_amount,ro.challan_no,csa.csa_name,csa.gst_no FROM purchase_order_details rod JOIN purchase_order ro ON rod.order_id=ro.order_id AND rod.purchase_inv=ro.challan_no INNER JOIN csa ON ro.csa_id=csa.c_id WHERE DATE_FORMAT(ro.created_date,'%Y%m%d') >= '$start' AND DATE_FORMAT(ro.created_date,'%Y%m%d') <= '$end' AND ro.dealer_id = '$did' GROUP BY ro.challan_no,rod.igst_amount HAVING cgst_amount=0 AND sgst_amount=0";    
}
 //h1($q);
...
/////////////////////////////////////////END PURCHASE GST REPORT/////////////////////////////
    public function pur_looper_challan_gst($start, $end,$oid)
       ...
       public function print_looper_challan_gst3($start, $end,$status)
    {  
        global $dbc;
        $out = array();
       $dealer_id = $_SESSION[SESS . 'data']['dealer_id'];
...
$q = "SELECT ro.receive_date,ro.order_id as orderid ,sum(gross_amt+td_amount+atd_amt+cd_amount+spl_amt+sch_amt) AS taxable_amt,round(SUM(total_amount),2) as total_amount,sum(cgst_amount) AS cgst_amount,sum(sgst_amount) AS sgst_amount,sum(igst_amount) AS igst_amount,ro.challan_no,csa.csa_name,csa.gst_no FROM purchase_order_details rod JOIN purchase_order ro ON rod.order_id=ro.order_id AND rod.purchase_inv=ro.challan_no INNER JOIN csa ON ro.csa_id=csa.c_id WHERE DATE_FORMAT(ro.order_date,'%Y%m%d') >= '$start' AND DATE_FORMAT(ro.order_date,'%Y%m%d') <= '$end' AND ro.dealer_id = '$did' GROUP BY ro.challan_no,rod.igst_amount HAVING cgst_amount!=0 AND sgst_amount!=0";
}elseif($status==5){
$q = "SELECT ro.receive_date,ro.order_id as orderid ,sum(gross_amt+td_amount+atd_amt+cd_amount+spl_amt+sch_amt) AS gross_amt,round(SUM(total_amount),2) as total_amount,sum(cgst_amount) AS cgst_amount,sum(sgst_amount) AS sgst_amount,sum(igst_amount) AS igst_amount,ro.challan_no,csa.csa_name,csa.gst_no FROM purchase_order_details rod JOIN purchase_order ro ON rod.order_id=ro.order_id AND rod.purchase_inv=ro.challan_no INNER JOIN csa ON ro.csa_id=csa.c_id WHERE DATE_FORMAT(ro.order_date,'%Y%m%d') >= '$start' AND DATE_FORMAT(ro.order_date,'%Y%m%d') <= '$end' AND ro.dealer_id = '$did' GROUP BY ro.challan_no,rod.igst_amount HAVING cgst_amount=0 AND sgst_amount=0";    
}elseif($status==6){
$q = "SELECT ro.receive_date,ro.order_id as orderid ,sum(gross_amt+td_amount+atd_amt+cd_amount+spl_amt+sch_amt) AS gross_amt,round(SUM(total_amount),2) as total_amount,sum(cgst_amount) AS cgst_amount,sum(sgst_amount) AS sgst_amount,sum(igst_amount) AS igst_amount,ro.challan_no,csa.csa_name,csa.gst_no FROM purchase_order_details rod JOIN purchase_order ro ON rod.order_id=ro.order_id AND rod.purchase_inv=ro.challan_no INNER JOIN csa ON ro.csa_id=csa.c_id WHERE DATE_FORMAT(ro.order_date,'%Y%m%d') >= '$start' AND DATE_FORMAT(ro.order_date,'%Y%m%d') <= '$end' AND ro.dealer_id = '$did' GROUP BY ro.challan_no,rod.igst_amount";    
}
