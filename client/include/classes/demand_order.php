...
class demand_order extends myfilter {
...
public function get_demand_order_se_data() {
$d1 = $_POST;
...
public function demand_order_save() {
global $dbc;
...
list($status, $d1) = $this->get_demand_order_se_data();
//pre($d1); exit;
if (!$status)
...
$qps = "INSERT INTO `demand_order`(`order_id`, `dealer_id`, `created_date`, `created_person_id`, `order_date`, `receive_date`, `date_time`, `company_id`, `ch_date`, `challan_no`, `csa_id`,`total_value`) VALUES ('$orderno', '$dealer_id', '$date_time', '$pid','$cr_date', '1970-01-01', '$date_time', '$d1[brand_id]','1970-01-01', '0','$csa_id','$d1[tot_amt]')";
 
$rps = mysqli_query($dbc, $qps);
if (!$rps) {
mysqli_rollback($dbc);
return array('status' => false, 'myreason' => 'Purchase Sales Table error');
}
...
$extrawork = $this->demand_order_extra('save', $orderno, $_POST['product_id'], $_POST['batch_no'], $_POST['base_price'], $_POST['quantity'], $_POST['scheme_quantity'],$_POST['purchase_inv'], $_POST['mfg_date'], $_POST['expiry_date'], $_POST['sale_price'],$_POST['company_id'],$_POST['cp'],$_POST['dealer_rate']);
}
if (!$extrawork['status']) {
mysqli_rollback($dbc);
...
public function demand_order_extra($actiontype, $rId, $productId, $batch_no, $base_price, $quantity, $scheme,$purchase_inv, $mfg_date, $expiry_date, $sale_price,$company_id,$cases,$landing_price) {
	/*pre($base_price); exit();*/
	global $dbc;
	$uncode = '';
	$str = $str_cat = array();
	if ($actiontype == 'Update')
	mysqli_query($dbc, "DELETE FROM demand_order_details WHERE order_id = $rId");
	...
	$q = "INSERT INTO `demand_order_details` (`order_id`, `product_id`, `mrp`,`rate`, `quantity`,`purchase_inv`, `mfg_date`, `expiry_date`, `pr_rate`,`cases`) VALUES $str";
...
public function demand_order_extra_edit($actiontype, $rId, $productId, $batch_no, 
...
public function demand_order_edit($id) {
global $dbc;
$out = array('status' => 'false', 'myreason' => '');
list($status, $d1) = $this->get_demand_order_se_data();
if (!$status)
...
mysqli_query($dbc, "START TRANSACTION");
// query to update
$q = "UPDATE demand_order SET created_date = NOW(), company_id = '$d1[company_id]' ,csa_id = '$d1[csa_id]' WHERE id = '$id'";
// h1($q);
...
$extrawork = $this->demand_order_extra_edit('Update', $orderno, $_POST['product_id'], $_POST['batch_no'], $_POST['base_price'], $_POST['quantity'], $_POST['scheme_quantity'],$_POST['purchase_inv'], $_POST['mfg_date'], $_POST['expiry_date'], $_POST['pr_rate'], $_POST['cases'],$_POST['dealer_rate']);
...
public function get_demand_order_list($filter = '', $records = '', $orderby = '') {
	global $dbc;
	$out = array();
	...
	. "DATE_FORMAT(date_time,'%d/%b/%Y') AS fdated FROM demand_order left join csa c on demand_order.csa_id=c.c_id $filterstr";
	// h1($q);
	...
	while ($row = mysqli_fetch_assoc($rs)) {
		...
		$out[$id]['order_item'] = $this->get_my_reference_array_direct("SELECT cases,usod.mrp,usod.id,cp.name,usod.rate,usod.quantity,usod.scheme_qty,pr_rate, batch_no,DATE_FORMAT(mfg_date, '%d/%m/%Y') AS mfg_date,purchase_inv, DATE_FORMAT(expiry_date, '%d/%m/%Y') AS expiry_date,cp.id AS product_id FROM demand_order_details usod INNER JOIN demand_order uso ON usod.order_id = uso.order_id INNER JOIN catalog_product cp ON usod.product_id=cp.id WHERE usod.order_id = $row[order_id]", 'id');
