...
$qps = "INSERT INTO `purchase_order`(`order_id`, `dealer_id`, `created_date`, `created_person_id`, `order_date`, `receive_date`, `date_time`, `company_id`, `ch_date`, `challan_no`, `csa_id`) VALUES ('$orderno', '$dealer_id', '$date_time', '$pid','$cr_date', '1970-01-01', '$date_time', '$d1[company_id]','1970-01-01', '0','$csa_id')";
...
public function purchase_order_extra($actiontype, $rId, $productId, $batch_no, $base_price, $quantity, $scheme,$purchase_inv, $mfg_date, $expiry_date, $sale_price,$company_id,$cases,$landing_price) {
	...
	foreach ($productId as $key => $value) {
		...
		$str[] = "('$rId', '$value', '{$base_price[$key]}','{$landing_price[$key]}', '{$quantity[$key]}','{$scheme[$key]}', '$mfdate', '$expdate', '{$purchase_inv[$key]}','{$sale_price[$key]}', '{$cases[$key]}')";
		...
		$str[] = "('$rId', '$value', '{$base_price[$key]}', '{$landing_price[$key]}', '{$quant[$key]}','{$schem[$key]}', '$mfdate', '$expdate', '{$purchase_inv[$key]}','{$sale_price[$key]}', '{$quantity[$key]}')";
	...
	$q = "INSERT INTO `purchase_order_details` (`order_id`, `product_id`, `mrp`,`rate`, `quantity`,`scheme_qty`,`purchase_inv`, `mfg_date`, `expiry_date`, `pr_rate`,`cases`) VALUES $str";
	...
	// $r = mysqli_query($dbc, $q);
	if (!$r)
	return array('status' => false, 'myreason' => 'User Purchase Stock details could not be saved some error occurred.');
	if ($company_id == 1){
	write_query($q);
	}
...
$rId = $id;
$extrawork = $this->purchase_order_extra('Update', $orderno, $_POST['product_id'], $_POST['batch_no'], $_POST['base_price'], $_POST['quantity'], $_POST['scheme_quantity'],$_POST['purchase_inv'], $_POST['mfg_date'], $_POST['expiry_date'], $_POST['pr_rate'], $_POST['cases']);
