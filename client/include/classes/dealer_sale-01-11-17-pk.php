...
class dealer_sale extends myfilter {
    ...
    public function get_dealer_sale_list($filter = '', $records = '', $orderby = '', $popup=false) {
        ...
        $q = "SELECT *,DATE_FORMAT(date,'%d/%m/%Y') AS sale_date,DATE_FORMAT(time,'%d/%m/%Y') AS sale_time,DATE_FORMAT(date,'%d/%b/%Y') AS fdated FROM user_sales_order INNER JOIN user_sales_order_details USING(order_id) $filterstr AND user_sales_order_details.status !=2 GROUP BY user_sales_order.order_id";
        ...
        while ($row = mysqli_fetch_assoc($rs)) {
            ...
            if($popup)
            {
            //h1("SELECT usod.id,cp.name, cp.taxable,usod.rate,usod.quantity,usod.scheme_qty,usod.product_id FROM user_sales_order_details usod INNER JOIN user_sales_order uso ON usod.order_id = uso.order_id INNER JOIN catalog_product cp ON usod.product_id=cp.id INNER JOIN catalog_2 c2 ON c2.id = cp.catalog_id WHERE usod.order_id = $row[order_id] AND  status!=2");
            $out[$id]['order_item'] = $this->get_my_reference_array_direct("SELECT usod.id,cp.name, cp.taxable,usod.rate,usod.quantity,usod.scheme_qty,usod.product_id FROM user_sales_order_details usod INNER JOIN user_sales_order uso ON usod.order_id = uso.order_id INNER JOIN catalog_product cp ON usod.product_id=cp.id WHERE usod.order_id = $row[order_id] AND  status!=2 ", 'id');
