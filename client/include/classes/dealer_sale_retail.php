...
class dealer_sale_retail extends myfilter {
    ...
    public function direct_challan_save() {
       ...
       // $iddd = $d1['dealer_id'].$orderno;
        ...
        if (!empty($d1['product_id'])) {
            ...
            $d1['product_id'] = array_values(array_filter($d1['product_id'])); // this line because when db select an empty row then product array increased a blank key , those blank values removed by this line.

            // pre($d1['mrp']);die;

             $d1['quantity'] = array_values(array_filter($d1['quantity'])); 
            
            foreach ($d1['mrp'] as $key => $value) {
...
//            h1($disc_amt); exit;
    ...
    VALUES ($iddd, '$chlaan_no','$d1[uid]', '$d1[dealer_id]', '$d1[retailer_id]','$d1[user_id]','$ch_date',NOW(), '1','10','$book_status','$crdr',
        ...
        if (!$r) {
            mysqli_rollback($dbc);
            return array('status' => false, 'myreason' => 'Challan Could Not saved, Some error occurred1');
        }
            $str = implode(',', $str);
        ...
        if (!empty($d1['product_id']))
        {
            ...
            foreach ($d1['product_id'] as $key => $value)
            {
               ...
               // }
                ...
                // h1( $vat_amt);
                //To save the value of the other columns as some columns are affected by po
                $str[] = "('$id','$prod','$bill_of_supply','$qty', '$rate','$schqty','$taxId','$mrp','$cd',
      ...
      /* h1($qup);
       die;*/
        ...
        $nid= $id;
          ...
          $q1 = "INSERT INTO challan_order_details (`ch_id`,`product_id`,`supply_status`,`qty`,`product_rate`, `free_qty`,`tax`,`mrp`,`cd`,`cd_type`,`cd_amt`, `spl_disc_type`, `spl_disc_val`, `spl_disc_amt`,`batch_no`,`dis_type`,`dis_amt`,`dis_percent`,`taxable_amt`,`remain_amount`,`vat_amt`)
          VALUES $str";
      //h1($q1);
