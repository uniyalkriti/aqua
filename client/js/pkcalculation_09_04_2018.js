$('#mytable').on('keyup change','.calcpk',function(){
  ...
  if(table.attr('class')!='')
  {
    ...
    {*/
      ...
      var product_id  = row.find('.item_details').val();
      ...
      if(!qty || !mrp)
      {
        ...
        trade_amt.val(trd_amt.toFixed(4));
         ...
         spl_disc_amt.val(spl_amt.toFixed(4));
          ...
          cd_amt.val(cdamt.toFixed(4));

          // taxable amount after cash discount
          taxable_sub_total = taxable_sub_total-cdamt;


          taxable.val(taxable_sub_total.toFixed(4));

            // 0 = igst, 1 = cgst,sgst
            if(parseInt(check_gst))
            {
              var tax_amt = taxable_sub_total*sgst/100;
              sgst_amt.val(tax_amt.toFixed(4));
              cgst_amt.val(tax_amt.toFixed(4));
              amount.val((taxable_sub_total+(tax_amt*2)).toFixed(4));
            }else{
              var tax_amt = taxable_sub_total*vat/100;
              vat_amt.val(tax_amt.toFixed(4));
              amount.val((taxable_sub_total+tax_amt).toFixed(4));
            }
            ...
            $($total_td).val(ttd.toFixed(4));
            $($total_cd).val(tcd.toFixed(4));
            $($total_taxable).val(tta.toFixed(4));
            $($total_vat).val(ttxa.toFixed(4));
            $($total).val(ta.toFixed(4));
            ...
            if(discount>0)
            {
              ...
              $($tot_disc).val(total_disc.toFixed(4));

              var total_amount_a = ta-total_disc;
              $($tot_amount_a).val(total_amount_a.toFixed(4));
            }else{
              $($tot_disc).val(0.00);
              $($tot_amount_a).val(ta.toFixed(4));
            }
