...
//		}
            ...
            case 'getItemDetails':
            {
                ...
                $q = "SELECT CONCAT_WS('/',CAST(s.mrp as decimal(10,2)),s.qty,s.batch_no,DATE_FORMAT(s.mfg,'%b-%Y')) as mrp_dis,CAST(s.mrp as decimal(10,2)) as mrp,(CASE WHEN(t.igst IS NULL) THEN 0 ELSE t.igst END) as gst, (CASE WHEN(t.sgst IS NULL) THEN 0 ELSE t.sgst END) as sgst, (CASE WHEN(t.cgst IS NULL) THEN 0 ELSE t.cgst END) as cgst
                    ...
                    WHERE s.dealer_id='$dealer_id' AND p.id =  '$id' AND s.qty>'0' ORDER BY s.mfg ASC";
            ...
            case 'rateNstock':
                {
                    ...
                    LEFT JOIN _gst t ON p.hsn_code=t.hsn_code WHERE s.mrp='$mrp' AND s.product_id=$id AND s.dealer_id=$dealer_id AND s.batch_no='$batch_no' AND DATE_FORMAT(`mfg`,'%b-%Y')='$mfg'";
                    //h1($qry);
            ...
            case 'rateNstockRetail':
                {
                	...
                	$batch_no=$abc[2];
                        $mfg=$abc[3];
                    $qry = "SELECT t.igst,s.qty,s.mrp AS rate,s.product_id FROM stock s
                    INNER JOIN catalog_product p ON s.product_id=p.id 
                    LEFT JOIN _gst t ON p.hsn_code=t.hsn_code WHERE s.mrp='$mrp' AND s.product_id=$id AND s.dealer_id=$dealer_id AND s.batch_no='$batch_no' AND DATE_FORMAT(`mfg`,'%b-%Y')='$mfg'";
				...
				case 'getItemDetailsDamage':
                {
                    ...
                    $q = "SELECT s.mrp as mrp_dis,CAST(s.mrp as decimal(10,2)) as mrp,(CASE WHEN(t.igst IS NULL) THEN 0 ELSE t.igst END) as gst, (CASE WHEN(t.sgst IS NULL) THEN 0 ELSE t.sgst END) as sgst, (CASE WHEN(t.cgst IS NULL) THEN 0 ELSE t.cgst END) as cgst
                        FROM catalog_product p 
                        INNER JOIN stock s ON p.id=s.product_id 
                        LEFT JOIN _gst t ON p.hsn_code=t.hsn_code 
                        WHERE s.dealer_id='$dealer_id' AND p.id =  '$id' LIMIT 1";                    
            ...
            case 'getDealerRate':
            {
                ...
                $q = "SELECT prl.dealer_rate,CAST(prl.mrp as decimal(10,2)) as mrp, (CASE WHEN(t.igst IS NULL) THEN 0 ELSE t.igst END) as gst 
                ...
                {   
                    ...
                    while($row = mysqli_fetch_array($item_data_e))
                    {
                        ...
                        $item_data['gst']    = $row['gst'];
                        $item_data['dealer_rate'] = $row['dealer_rate'];

                        //$where1 = "dealer_id=$dealer_id AND product_id=$product_id";
                        //$item_data['sih'] = myrowval('stock','qty',$where1);
                        //$item_data['min_t'] = myrowval('threshold','qty',$where1);
                    }
