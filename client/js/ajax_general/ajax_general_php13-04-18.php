...
//		}
            ...
            case 'rateNstockRetail':
				...
				case 'getItemDetailsDamage':
                {
                    ...
                    $q = "SELECT mrp as mrp_dis,CAST(s.mrp as decimal(10,2)) as mrp,(CASE WHEN(t.igst IS NULL) THEN 0 ELSE t.igst END) as gst, (CASE WHEN(t.sgst IS NULL) THEN 0 ELSE t.sgst END) as sgst, (CASE WHEN(t.cgst IS NULL) THEN 0 ELSE t.cgst END) as cgst
                        FROM catalog_product p 
                        INNER JOIN stock s ON p.id=s.product_id 
                        LEFT JOIN _gst t ON p.hsn_code=t.hsn_code 
