...
<?php
$forma = 'Purchase Report'; // to indicate what type of form this is
$formaction = $p;
    ...
    if (valid_token($_POST['hf'])) { // checking if post value is same as timestamp stored in session during form load
        ...
        if ($checkpass) {
            ...
            if (!empty($_POST['start'])) {
                ...
                $filter[] = "DATE_FORMAT(ro.receive_date,'" . MYSQL_DATE_SEARCH . "') >= '$start'";
                $filterstr[] = '<b>Start : </b>' . $_POST['start'];
            }
            if (!empty($_POST['end'])) {
                $end = get_mysql_date($_POST['end'], '/', $time = false, $mysqlsearch = true);
                $filter[] = "DATE_FORMAT(ro.receive_date,'" . MYSQL_DATE_SEARCH . "') <= '$end'";
...
<div class="row">
    ...
    color:#000;
                            ...
                            <tr>
                                ...
                                <!-- <th>
                                    <table style="width:100%">
                                        <tr>
                                            <th colspan="2" style="text-align:center">CGST Details</th>
                                        </tr>
                                        <tr>
                                            <td>Rate</td>
                                            <td>Amt</td>
                                ...
                                <!-- <th>
                                    <table style="width:100%">
                                        <tr>
                                            <th colspan="2" style="text-align:center">SGST Details</th>
                                        </tr>
                                        <tr>
                                            <td>Rate</td>
                                            <td>Amt</td>
                ...
                // $total_amount = ($rows['total_tax_amount'] + $rows['total_taxable_amount'] );
                                $oid = $rows['order_id'];
             $printlink = '<a class="iframef" title="print Invoice ' . $uidname . '" href="index.php?option=' . $formaction . '&showmode=1&mode=1&start=' . $start . '&end=' . $end . '&status='.$inc.'&oid='.$oid.'&actiontype=print"><img src="../icon-system/i16X16/print.png"></a>'; 
