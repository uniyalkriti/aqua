...
$cls_func_str = 'dispatch_productwise'; //The name of the function in the class that will do the job
...
if(isset($_POST['filter']) && $_POST['filter'] == 'Filter')
{
	...
	$rs = $mycurobj->$funcname($filter="$myfilter'$ajaxshowid'",  $records = '', $orderby='');
      ...
      $filter[] = "DATE_FORMAT(daily_dispatch.dispatch_date,'".MYSQL_DATE_SEARCH."') <= '$end'";
     // echo $funcname;die;
    $rs = $myobj->$funcname($filter,  $records = '', $orderby=''); //get_dispatch_productwise_list
}
$rs1 = array();
                ...
                <br>
                 ...
                 <div class="table-header">
                   <span style="color:#fff;"> Dispatch List</span>
                    ...
                    <input type="button" onclick="tableToExcel('searchdata', 'W3C Example Table')" value="Export to Excel" class="btn btn-success"></div>
                </div>
                    ...
                    $deletelink = '<a href="javascript:void(0);" onclick="do_delete(\'Dispatch Delete\', \''.$uid.'\',\'Dispatch\',\''.addslashes($uidname).'\');"><img src="./images/b_drop.png"></a>';
                          ...
                          if($auth['del_opt'] !=1) $deletelink = '';
                            ...
                            <tr>
                                ...
                                <td>
                                    <?=$rows['mrp']?>
                                    <?php $total_mrp += $rows['mrp']; ?>
                                </td>
                                 <td>
                                    <?=$rows['qty']?>
                                    <?php $total_qty += $rows['qty']; ?>
                                </td>
                            </tr>                            
                            <?php
                            $inc++;
