...
<?php
$forma = 'GSTR-2 RETURN SUMMARY'; // to indicate what type of form this is
$formaction = $p;
$myobj = new dealer_sale();
$cls_func_str = 'purchase_reg'; //The name of the function in the class that will do the job
    ...
    if (valid_token($_POST['hf'])) { // checking if post value is same as timestamp stored in session during form load
        ...
        if ($checkpass) {
            ...
            if (!empty($_POST['start'])) {
                ...
                $filter[] = "DATE_FORMAT(ro.created_date,'" . MYSQL_DATE_SEARCH . "') >= '$start'";
                $filterstr[] = '<b>Start : </b>' . $_POST['start'];
            }
            if (!empty($_POST['end'])) {
                $end = get_mysql_date($_POST['end'], '/', $time = false, $mysqlsearch = true);
                $filter[] = "DATE_FORMAT(ro.created_date,'" . MYSQL_DATE_SEARCH . "') <= '$end'";
                $filterstr[] = '<b>End : </b>' . $_POST['end'];
            }
             //$filter[] = "DATE_FORMAT(ro.receive_date,'%Y-%m-%d') != '0000-00-00'";
            ...
            $filter[] = "ro.dealer_id = '{$_SESSION[SESS.'data']['dealer_id']}'";
...
elseif (isset($_GET['ajaxshow']) || isset($_GET['ajaxshowblank'])) {
    ...
    $rs = $myobj->$funcname($filter = "$myfilter'$ajaxshowid'", $records = '', $orderby = '');
     ...
     $filter= array();
     $date = date('Y-m-d');
     $filter[] = "DATE_FORMAT(ro.receive_date,'%Y-%m-%d') >= '$date'";
...
if (isset($_GET['actiontype'])) {
    switch ($_GET['actiontype']) {
        case'print':
            require_once('purchase-reg-print.inc.php');
            exit();
            break;
                        case'printbill':
        //echo getcwd();
          require_once('modules/dispatch/purchase_order_print.php');
            exit();
            break;
        default:
                    ...
                    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            foreach ($rs as $key => $rows) {
                                if(!empty($rows[tax2]) || $rows[tax2]==''){
                                    $total_tax=$rows[tax1];
                                }else{
                                    $total_tax=$rows[tax2];
                                }
                                $taxable_amt=$rows['gross_amt'];
                                $total_taxable+=$taxable_amt;
                                $total_taxes+=$total_tax;
                                $total_amt+=$rows['total_amount'];
                            
                                $oid = $rows['order_id'];
             $printlink = '<a class="iframef" title="print Invoice ' . $uidname . '" href="index.php?option=' . $formaction . '&showmode=1&mode=1&start=' . $start . '&end=' . $end . '&status='.$inc.'&did='.$dealer_id.'&actiontype=print"><img src="../icon-system/i16X16/print.png"></a>'; 
...
//                             $printlink = '<span class="seperator"></span> <a class="iframef" target="_blank" title="Edit Invoice ' . $uidname . '" href="index.php?option=' . $formaction . '&showmode=1&mode=1&id=' . $uid . '&actiontype=print"><img src="./images/b_edit.png"></a>';
                            ...
                            <tr>
                                ...
                                <td><?=$_POST['start']?></td>
				                <td><?=$_POST['end']?></td>
                                <td><?=my2digit($taxable_amt)?></td>
                                <td><?=my2digit($total_tax)?></td>
                                <td><?=my2digit($rows['total_amount'])?></td>
                              ...
                              <!--   <td>
                                ...
                                <?php //} ?>
                                <?php /* <td><img src="../icon-system/i16X16/rupee.png"><?=round($amt)?></td> */ ?>
            ...
            <!-- <tr></tr> -->
                        ...
                        <td><b><?=my2digit($total_taxable)?></b></td>
                        <td><b><?=my2digit($total_taxes)?></b></td>
                        <td><b><?=my2digit($total_amt)?></b></td>
                        <td></td>
                        </tr>
                        
                        <tr>
                        <td></td>
                        <td><?php echo '<a target="_blank" href="index.php?option=hsn-purchase-details&start='.$start.'&end='.$end.'&status=2">'; ?><?php echo "HSN/SAC Summary";?></a></td>
                        <td><?php 
                        $hsn_sac=$myobj->get_hsn_purchase_count_list($start, $end);
                        echo $hsn_sac;?></td>
                        <td colspan="6"></td>
                        </tr>
                        </tfoot>
