...
$cls_func_str = 'retailer_ledger_tranc'; //The name of the function in the class that will do the job
...
if(isset($_POST['filter']) && $_POST['filter'] == 'Filter')
{
  ...
  $rs = $mycurobj->$funcname($filter="$myfilter'$ajaxshowid'",  $records = '', $orderby='');
    ...
    //$filter=array();
     ...
     //$cmonth=date('Ym');
     //$filter[] = "DATE_FORMAT(`ch_date`,'%Y%m')>='$cmonth'";
     //echo $funcname;die;
    //$rs = $myobj->$funcname($cdate,$filter,  $records = '', $orderby='');
}
$rs1 = array();
            ...
            <tr>
                <td><strong>Name</strong><br />
                   <!-- <div style="float:left; "><input type="radio" style="float:left" name="search" checked="checked"  onclick="return search_user(1)" /> -->
                        ...
                        <?php db_pulldown($dbc, 'retailer_id', "SELECT id AS retailer_id, name AS retailer_name FROM `retailer` WHERE `dealer_id` = '$dea_id' and retailer_status='1' ", true, true, ''); ?> 
                    </div>
                </td>
                ...
                <!-- div.table-responsive -->
                  ...
                  <!-- div.dataTables_borderWrap -->
                   <div id="divToPrint">
                    ...
                    ?>
                    <h3><strong>Retailer Payment Register From <?=$start1?> To <?=$end1?></strong></h3>
                        <thead>                            
                            <tr> 
                        ...
                        <tbody>
                            ...
                            $inc =1;
                            $cvalue=0;
                $cvalue=$myobj->get_retailer_ledger_open_list("","","");
                  $inc++;
                ?>
                                <tr>
                                <td><?=$inc?></td>  
                                <td><?php echo "01-04-2018";?></td>
                                <td><?php echo "Opening Dues";?></td>
                                <td><?php echo "";?></td>
                                <td><?php echo "";?></td>
                                <td><?=my2digit($cvalue)?></td>
                                <td><?php echo "";?></td>
                                <td><?=my2digit($cvalue)?></td>
                                <tr>
                            ...
                            for ($i = 0; $i < $datediff; $i++) {
                          $cdate=$datearray[$i];
                          $rs= $myobj->get_retailer_ledger_tranc_list($cdate,$filter,  $records = '', $orderby ="");
                            foreach($rs as $key=>$rows){
                              //pre($rows);
                            $uid = $rows['id'];
                            $uidname = $rows['dname'];
                            $value=round($rows['value'],2);
                            $cvalue+=$rows['ovalue']-$rows['ivalue'];
                            $vtype1=explode('/',$rows['ch_no']);
                            if($vtype1[0]=='BDN'){
                            $vtype="Pending";
                            $ch_no='<a target="_blank" href="index.php?option=' . $formaction . '&showmode=1&mode=1&id=' . $uid .'&actiontype=print">'.$rows[ch_no].'</a>'; 
                            }else{
                            $vtype="Recieved"; 
                            $ch_no=$rows[ch_no]; 
                            }
                            ...
                            <tr>
                                <td><?=$inc?></td>  
                                <td><?=$rows['ch_date']?></td>
                                <td><?=strtoupper($rows['pert'])?></td>
                                <td><?=$vtype?></td>
                                <td><?=$ch_no?></td>
                                <td><?=my2digit($rows['ovalue'])?></td>
                                <td><?=my2digit($rows['ivalue'])?></td>
                                <td><?=my2digit($cvalue)?></td>   
                            </tr>                            
                            <?php
                            $inc++;
