...
<?php
$forma = "GSTR-1 ".$_GET['key']." SUMMARY"; // to indicate what type of form this is
$formaction = $p;
...
<div class="row">
    ...
    color:#000;
                    ...
                    <?php 
                      ...
                      $key=$_GET['key'];
                    ?>
                    <h2><strong>GST Wise Report:- For :<?=$key?> From <?=$start1?> To <?=$end1?></strong></h2>
                        <thead>                            
                            <tr>       
                ...
                $total_amount = ($rows['vat_amt'] + $rows['taxable_amt'] );
                            ...
                            $total_value+=round($total_amount);
             $printlink = '<a class="iframef" title="print Invoice ' . $uidname . '" href="index.php?option=' . $formaction . '&showmode=1&mode=1&start=' . $start . '&end=' . $end . '&status='.$inc.'&actiontype=print"><img src="../icon-system/i16X16/print.png"></a>'; 
                            ...
                            ?>
                                ...
                                <td><?=my2digit($rows['taxable_amt'])?></td>
                                  <td>
                                    <?php echo '<a target="_blank" href="index.php?option=bill-detailed-gst&start='.$start.'&end='.$end.'&tax='.$rows['tax'].'&key='.$gst_type.'">'; ?><?php echo $rows['tax'];?></a>
                                    </td>
