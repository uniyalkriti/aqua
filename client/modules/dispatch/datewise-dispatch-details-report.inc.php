...
$cls_func_str = 'dispatch_datewise'; //The name of the function in the class that will do the job
...
if(isset($_POST['filter']) && $_POST['filter'] == 'Filter')
{
	if(valid_token($_POST['hf'])) // checking if post value is same as timestamp stored in session during form load
	{
		...
		if($checkpass)
		{
			...
			if(!empty($_POST['van_no'])){
				$filter[] = "daily_dispatch.van_no = '$_POST[van_no]'";
				$filterstr[] = '<b>Van No : </b>'.$_POST['van_no'];
			}
	...
	$rs = $mycurobj->$funcname($filter="$myfilter'$ajaxshowid'",  $records = '', $orderby='');
      ...
      $filter=array();
            ...
            $filter[] = "DATE_FORMAT(daily_dispatch.dispatch_date,'".MYSQL_DATE_SEARCH."') <= '$end'";
     //echo $funcname;die;
    $rs = $myobj->$funcname($filter,  $records = '', $orderby='');  //get_dispatch_datewise_list
}
$rs1 = array();
...
function updatelink()
{
	var cb = document.getElementsByName('printinvid[]');
	var pt = document.getElementById('print_all');
	var link = 'index.php?option=datewise-dispatch-report&showmode=1&mode=1&actiontype=print&id=';
	var print_link = link;
	var l = [];
	...
	if(cb.length>0)
	{
		if(document.getElementById("checkall").checked == true)
		{
			...
			{
				print_link = print_link+'-'+cb[i].value;
			}
		}else{
			print_link = link;
		}
	}
	pt.setAttribute('href',print_link);
}
    ...
    <div id="workarea">
	  ...
	  if(isset($_GET['actiontype'])){
		switch($_GET['actiontype']){
			case'print':
				require_once('date-dispatch-print.inc.php');
				exit();
				break;	
                                default:
         ...
         if(!empty($rs1)) { ?>
            <tr>
                <td colspan="6">
                    ...
                    <tr class="search1tr">
                        <td class="sno">S.No<input onclick="selectCheckBoxes('checkall','','chk[]');" type="checkbox" id="checkall1"></td>
                      <td>Invoice No</td>
                ...
                <td width="10%"><span class="star">*</span>End<br />	
                  ...
                  <td width="10%">Van<br/>
                        <?php
                            ...
                            $q = "SELECT van_no,van_no from van where dealer_id='$dealer_id' order by van_no asc";

                            db_pulldownsmall($dbc, 'van_no', $q, true, true, 'id="van_no" class="chosen-select"','==Please Select==',$_POST['van_no']);
                        ?>
                    </td>                  
                <td width="61%"><br />
...
<!--                 <div class="table-header">
                    <a class="iframef" title="print All Selected Invoice" id="print_all" href="index.php?option=datewise-dispatch-report&showmode=1&mode=1&actiontype=print&id="><i class="fa fa-print" style="color:#fff;"></i><span style="color:#fff;"> Dispatch List</span></a>
                   
                    <div class="pull-right tableTools-container"></div>
                </div>-->
                <div class="table-header">
                   <span style="color:#fff;"> Dispatch List</span>
                   
                    <div class="pull-right tableTools-container" align="right">
                        <a class="iframef" title="print All Selected Invoice" id="print_all" href="index.php?option=datewise-dispatch-report&showmode=1&mode=1&actiontype=print&id="><i class="fa fa-print" style="color:#fff;"></i><span style="color:#fff;">Print List</span></a>
                       <input type="button" onclick="tableToExcel('searchdata', 'W3C Example Table')" value="Export to Excel" class="btn btn-success">
                   </div>
                </div>                
                   <div>
                       <style> th {
    ...
    color:#000;
                    ...
                    <table id="searchdata" class="table table-striped table-bordered table-hover">
                        <thead>                            
                            <tr> <th class="center">
                                  <input onclick="selectCheckBoxes('checkall', 'printinvid[]');updatelink();" type="checkbox" id="checkall">
                                </th>  
...
<!--                                <th class="options">Options</th>-->
                        ...
                        <tbody>
                            ...
                            $uid = $rows['cid'];
                            $uidname = $rows['dname'];
                            $gtotal=$gtotal+$rows['amount'];
                            $dstatus = $rows['delivery_status'];
                            if($dstatus==0)
                            {
                                $dstatus='On The Way';
                            }
...
<!--                                    <td class="options"><?= $printlink?></td>-->
                           ...
                           <tr>
                                ...
                                <td><strong><i class="fa fa-inr"></i> <?=$gtotal?></strong></td>
<!--                                <td></td>-->
                            </tr>
                        </tbody>
                    </table>
