...
<?php
$forma = 'GSTR-1 RETURN SUMMARY'; // to indicate what type of form this is
$formaction = $p;
$myobj = new dealer_sale();
$cls_func_str = 'tax_inv_gst'; //The name of the function in the class that will do the job
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="table-header">
                   GST Return Summary List
                    <?php echo '<a target="_blank" href="http://162.222.39.114/msell-baidyanath/webservices/dealer_challan_report_json.php?dealer_id='.$dealer_id.'&from_date='.$start.'&to_date='.$end.'">'; 
                   ?><img src="images/json.png" /></a>
                    ...
                    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                        <thead>                            
                            <tr>       
                                ...
                                <th class="options">Print Invoice</th>
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            ...
                            foreach ($rs as $key => $rows) {
                             ...
                             $uid = $rows['id'];
                             $cid = $rows['cid'];
                             if(empty($rows['total_taxable_amount'])){
                               $tta='0.00'; 
                             }else{
                               $tta=$rows['total_taxable_amount']; 
                             }
                             $cgst = ( $rows['total_tax_amount']/2 );
                            $sgst = ($rows['total_tax_amount'] - $cgst );
                            $igst = '0.00';
                $total_amount = ($rows['total_tax_amount'] + $rows['total_taxable_amount'] );
                            $total_ta+=$tta;
                            $total_igst+=$igst;
                            $total_cgst+=$cgst;
                            $total_sgst+=$sgst;
                            $total_tax+=$rows['total_tax_amount'];
                            $total_ba+=$total_amount;
                            $total_rc+=$rows['retailer_count'];
...
//                             $printlink = '<span class="seperator"></span> <a class="iframef" target="_blank" title="Edit Invoice ' . $uidname . '" href="index.php?option=' . $formaction . '&showmode=1&mode=1&id=' . $uid . '&actiontype=print"><img src="./images/b_edit.png"></a>';
                            ...
                            ?>
                                ...
                                <td>
                                    <?php echo '<a target="_blank" href="index.php?option=gst_wise_summary_new&start='.$start.'&end='.$end.'&key='.$key.'">'; ?><?php echo $key;?></a>
                                </td>
                              <!--   <td>
                                   <?=$key?>
                                </td> -->
                                <td><?=$rows['retailer_count']?></td>
                                <td><?php echo my2digit($tta); ?></td>
                                <td><?php echo my2digit($cgst); ?></td>
                                <td><?php echo my2digit($sgst); ?></td>
                                <td><?php echo my2digit($igst); ?></td>
                                <td><?php echo my2digit($rows['total_tax_amount']); ?></td>
                                <td><?php echo my2digit(round($total_amount));?></td>
                                
                <?php //} ?>
        <?php /* <td><img src="../icon-system/i16X16/rupee.png"><?=my2digit($amt)?></td> */ ?>
                        ...
                        <td><b><?=$total_rc?></b></td>
                        <td><b><?=my2digit($total_ta)?></b></td>
                        <td><b><?=my2digit($total_cgst)?></b></td>
                        <td><b><?=my2digit($total_sgst)?></b></td>
                        <td><b><?=my2digit($total_igst)?></b></td>
                        <td><b><?=my2digit($total_tax)?></b></td>
                        <td><b><?=my2digit($total_ba)?></b></td>
                        </tr>
                        <tr>
                        <td></td>
                        <td><?php echo '<a target="_blank" href="index.php?option=hsn-details&start='.$start.'&end='.$end.'&status=1">'; ?><?php echo "HSN/SAC Summary";?></a></td>
                        <td><?php 
                        $hsn_sac=$myobj->get_hsn_count_list($start, $end);
                        echo $hsn_sac;?></td>
                        <td colspan="7"></td>
                        </tr>
                        <tr>
                        <td></td>
                        <td><?php echo '<a target="_blank" href="index.php?option=book-details-doc&start='.$start.'&end='.$end.'&status=1">'; ?><?php echo "Document Summary";?></a></td>
                        <td><?php 
                        $hsn_sac=$myobj->get_doc_count_list($start, $end);
                        echo $hsn_sac;?></td>
                        <td colspan="7"></td>
                        </tr>
                        </tfoot>
