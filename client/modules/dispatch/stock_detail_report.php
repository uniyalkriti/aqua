...
if(isset($_POST['filter']) && $_POST['filter'] == 'Filter')
{
  if(valid_token($_POST['hf'])) // checking if post value is same as timestamp stored in session during form load
  {
    ...
    if($checkpass)
    {
      ...
      $filter[] = "catalog_view.c3_id=".$_POST['division'];
            }
            ...
            <td>
             ...
             <table>
               ...
               <!--  <td width="10%"><span class="star">*</span>Start<br /> 
                ...
                <td>Category<br />
                     <?php $q="SELECT id,name FROM catalog_3";
                     db_pulldownsmall($dbc, 'division', $q, true, true, 'id="division" class="division"','==Please Select==',$_POST['division']);
                     ?>
                </td> 
                ...
                <td width="10%"><strong>Rate Type</strong><br />
                  <select name="wrate" id="wrate
                  ">
                    <option value="1" <?php if($_POST['wrate']==1) echo 'selected'; ?>>WH. Rate</option>
                    <option value="2" <?php if($_POST['wrate']==2) echo 'selected'; ?>>MRP</option>
                  </select>
                </td>
                <td width="10%"><strong>Stock Type</strong><br />
                  <select name="stktyp" id="stktyp">
                  <option value="1" <?php if($_POST['stktyp']==1) echo 'selected'; ?>>ALL</option>
                    <option value="2" <?php if($_POST['stktyp']==2) echo 'selected'; ?>>Slow Moving Stock</option>
                    <option value="3" <?php if($_POST['stktyp']==3) echo 'selected'; ?>>Min Level Stock</option>
                  </select>
                </td>
                <td width="61%"><br />
                ...
                <!-- div.table-responsive -->
                  ...
                  <!-- div.dataTables_borderWrap -->
                   ...
                   <?php if(empty($_POST['division'])){ ?>
                    Stock Details-<?=$dname?>:-FROM <?=$start1?> TO <?=$end1?> 
                  <?php  }else{ 
                    $cat=myrowval('catalog_3','name','id='.$_POST['division']);
                    ?>
                  Stock Details(<?=$cat?>)-<?=$dname?>:-FROM <?=$start1?> TO <?=$end1?> 
                  <?php } ?>
                    </strong></caption>
                        <thead>                            
                            <tr> 
                               <th rowspan="2" class="sno">S.No</th>
                                ...
                                <th colspan="3" style="text-align: center;">Opening Stock</th>
                                <th rowspan="2">Inwards</th>
                                <th rowspan="2">Outwards</th>
                    ...
                    $deletelink = '<a href="javascript:void(0);" onclick="do_delete(\'Dispatch Delete\', \''.$uid.'\',\'Dispatch\',\''.addslashes($uidname).'\');"><img src="./images/b_drop.png"></a>';
                          ...
                          $billed = ($rows['billed'])?ROUND($rows['billed']):0;
                          $ostock = ($rows['opening'])?ROUND($rows['opening']):0;
                          $stock_new = ($rows['stock_new'])?ROUND($rows['stock_new']):0;//mine

                          $cstock=$rows['opening']+$purchase-$rows['billed'];
                          $cstock=round($cstock);
                          if(empty($rows['rate'])){
                            $rows['rate']='0';
                          }
                          $wrate=$_POST['wrate'];
                          if($wrate==1){
                            $orate=$rows['opening_rate'];
                            $crate=$rows['rate'];
                          }elseif($wrate==2){
                            $orate=$rows['opening_mrp'];
                            $crate=$rows['mrp']; 
                          }
                          if($_POST['stktyp']==2){
                            $smstock=round(($billed*90)/365);
                            if($smstock<=$cstock){
                              $sms="1";
                              $oq+=$ostock;
                            $ov+=round(($crate*$ostock),2);
                            $iq+=$purchase;
                            $bq+=$billed;
                            $cq+=$cstock;
                            $cv+=round(($crate*$cstock),2);
                            }else{
                              $sms="2";
                            }
                          }
                          elseif($_POST['stktyp']==3){
                            $lstock=round(($billed*21)/365);
                            if($lstock<=$cstock){
                              $sms="3";
                              $oq+=$ostock;
                            $ov+=round(($crate*$ostock),2);
                            ...
                            $stock_n+=$stock_new;//mine
                            $cv_n+=round(($crate*$stock_new),2);//mimne
                            ...
                            <?php if($sms == "1"){ 
                              ...
                              <tr class="trcolor">
                                ...
                                <td><?=$ostock?></td>
                                <td><?=$crate?></td>
                                <td><?=round(($crate*$ostock),2)?></td>
                                <td><?=$purchase?></td>
                                <td><?=$billed?></td>
                                <td><?=$cstock?></td> 
                                <td><?=$orate?></td>
                                <td><?=round(($crate*$cstock),2)?></td> 
                            </tr> 
                            <?php }elseif($sms == "3"){ 
                           ...
                           <?php }elseif($sms == "5"){ 
                            ...
                            ?>  
                                ...
                                <td><?=$ostock?></td>
                                <td><?=$crate?></td>
                                <td><?=my2digit(round(($crate*$ostock),2))?></td>
                                <td><?=$purchase?></td>
                                <td><?=$billed?></td>
                                <td><?=$cstock?></td> 
                                <!-- <td><?=$stock_new?></td>  -->
                                ...
                                <td><?=my2digit(round(($crate*$cstock),2))?></td> 
                                <!-- <td><?=my2digit(round(($crate*$stock_new),2))?></td>  -->
                        ...
                        <td><b><?=my2digit($cv)?></b></td>
                        <!-- <td><b><?=my2digit($cv_n)?></b></td> -->
