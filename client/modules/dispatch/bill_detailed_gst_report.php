...
<?php 
$forma = 'Bill Summary'; // to indicate what type of form this is
$formaction = $p;
$myobj = new dispatch_report();
$objchallan = new dealer_sale();
$cls_func_str = 'bill_detailed_gst'; //The name of the function in the class that will do the job
...
$dealer_data = $dealer_data[$dea_id];
  ...
  $rs = $mycurobj->$funcname($filter="$myfilter'$ajaxshowid'",  $records = '', $orderby='');
     ...
     //echo $funcname;die;
      ...
      $end=$_GET['end'];
       ...
       $gst_type=$_GET['key'];
      // h1($gst_type);
    $rs = $myobj->$funcname($start,$end,$gst,$filter,  $records = '', $orderby='',$gst_type);
}
$rs1 = array();
            ...
            <div class="col-xs-12">
                ...
                <br>
                 ...
                 <div class="table-header">
                    <span style="color:#fff;">Bill Details</span>
                ...
                <!-- div.table-responsive -->
                  ...
                  <!-- div.dataTables_borderWrap -->
                   <div id="divToPrint">
                    ...
                    ?>
                    <?php if(empty($gst_type)){?>
                    <h2><strong>Bill Wise Summary  From <?=$start1?> To <?=$end1?> Of <?=$gst.'%'?> GST</strong></h2>
                  <?php }else{?>
                    <h2><strong>GSTR-1 (<?=$gst_type?>) Bill Wise Detail For <?=$gst.'%'?> Slab From <?=$start1?> To <?=$end1?></strong></h2>
                   <?php }?>
                            <thead>          
                            <tr> 
                        ...
                        <tbody>
                            ...
                            $uid = $rows['id'];
                            $retailer_state_id = $rows['adr']['state_id'];
                            $uidname = $rows['dname'];
                              if($dealer_state_id == $retailer_state_id){
                            $cgst=round(($rows['vat_amt']/2),2);
                            $sgst=round(($rows['vat_amt']/2),2);
                            $total_cgst+=$cgst;
                            $total_sgst+=$sgst;
                            }else{
                            $igst=$rows['vat_amt'];
                            ...
                            $total_tax+=$rows['vat_amt'];
                            $total_qty+=$rows['qty'];
                            $total_taxable+=$rows['taxable_amt'];
                             $total_value+=$rows['total_amt'];
                    ...
                    $deletelink = '<a href="javascript:void(0);" onclick="do_delete(\'Dispatch Delete\', \''.$uid.'\',\'Dispatch\',\''.addslashes($uidname).'\');"><img src="./images/b_drop.png"></a>';
                          ...
                          if($auth['del_opt'] !=1) $deletelink = '';
                            ...
                            <tr>
                                ...
                                <td><?=strtoupper($rows['tin_no'])?></td>
                                <td>GST Sale</td>
                            <td>    <?php  echo'
                        <a target="_blank" href="index.php?option=' . $formaction . '&showmode=1&mode=1&id=' . $uid .'&actiontype=print">'; ?><?php echo $rows['ch_no'];?></a> 
                        </td>
                                <td>GOODS</td>
                                <td>PCS-PIECES</td>
                                <td><?=$rows['qty']?></td>
                                
                                <td><?=my2digit($rows['taxable_amt'])?></td>
                                <td><?=my2digit($cgst)?></td>
                                <td><?=my2digit($sgst)?></td>
                                <td><?=my2digit($igst)?></td>
                                <td><?=my2digit($rows['vat_amt'])?></td> 
                                <td><?=my2digit(round($rows['total_amt']))?></td>  
                            </tr>                            
                            <?php
                            $inc++;
                            }                            
                        ...
                        <td><b>Grand Total</b></td>
                         <td><?=$total_qty?></td>
                            ...
                            <td><?=my2digit($total_taxable)?></td>
                            <td><?=my2digit($total_cgst)?></td>
                            <td><?=my2digit($total_sgst)?></td>
                            <td><?=my2digit($total_igst)?></td>
                            <td><?=my2digit($total_tax)?></td>
                            <td><?=my2digit(round($total_value))?></td>
                        </tr>
                        <!-- </tfoot> -->
                    </table>
                </div>
