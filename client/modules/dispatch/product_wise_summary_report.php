...
<?php 
$forma = 'Product Wise Summary Report'; // to indicate what type of form this is
$formaction = $p;
$myobj = new dispatch();
$objchallan = new dealer_sale();
$cls_func_str = 'product_wise_summary'; //The name of the function in the class that will do the job
	...
	if(valid_token($_POST['hf'])) // checking if post value is same as timestamp stored in session during form load
	{
		...
		if($checkpass)
		{
			...
			if(!empty($_POST['end'])){
				...
				$filter[] = "catalog_product.id= $_POST[product_id]";
				//$filterstr[] = '<b>End : </b>'.$_POST['end'];
			}
            ...
            <div class="col-xs-12">
                ...
                <br>
                 ...
                 <div class="table-header">
                    <span style="color:#fff;">Product Wise Summary Report</span>
    ...
    color:#000;
                    ...
                    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            $uidname = $rows['dname'];
                            if(!empty($rows['billed_qty'])){
                            $billed_qty=$rows['billed_qty'];
                            }else{
                            $billed_qty=0;    
                            }
                            if(!empty($rows['billed_amt'])){
                            $billed_amt=$rows['billed_amt'];
                            }else{
                            $billed_amt=0.00;    
                            }
