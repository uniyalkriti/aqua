...
<?php 
$forma = 'Stock Ledger'; // to indicate what type of form this is
$formaction = $p;
$myobj = new dispatch_report();
$objchallan = new dealer_sale();
$cls_func_str = 'stock_ledger_tranc'; //The name of the function in the class that will do the job
...
if(isset($_POST['filter']) && $_POST['filter'] == 'Filter')
{
  if(valid_token($_POST['hf'])) // checking if post value is same as timestamp stored in session during form load
  {
    ...
    if($checkpass)
    {
      ...
      if(!empty($_POST['product_id'])){
        //$filter[] = "cod.product_id= $_POST[product_id]";
      }
        //$filter[] = "co.ch_dealer_id= $dea_id";
        $datearray = array();
        $datearray1 = array();
     ...
     new DateTime("$end_date")
           ...
           foreach( $period as $date1) {
               $datearray[] = $date1->format('Ymd'); 
               $datearray1[] = $date1->format('Y-m-d'); 
           }
                     // print_r($datearray);
     ...
     // for ($i = 0; $i < $datediff; $i++) {
        $cdate=$datearray[$i];
       // $rs[] = $myobj->$funcname($cdate,$filter,  $records = '', $orderby ="");
      //} 
      //pre($rs);die;        
  ...
  $rs = $mycurobj->$funcname($filter="$myfilter'$ajaxshowid'",  $records = '', $orderby='');
    ...
    $filter=array();
     $filter[] = "co.ch_dealer_id = '{$_SESSION[SESS.'data']['dealer_id']}'";
   ...
   $rs[] = $myobj->$funcname($product_id1,$cdate,$filter,  $records = '', $orderby='');
}
$rs1 = array();
...
<!--                              <td width="20%"><strong>Product Name</strong><br />
                        <?php db_pulldown($dbc, 'product_id', "SELECT id, name FROM `catalog_product` ORDER BY name", true, true, 'class="chosen-select form-control" lang="product"'); ?> 
                    </div>
                </td>
                <td width="61%"><br />
            ...
            <div class="col-xs-12">
                ...
                <br>
                 ...
                 <div class="table-header">
                    <span style="color:#fff;">Stock Ledger</span>
                   
                    <div class="pull-right tableTools-container" align="right"></div>
                    <!-- <a href="javascript:printing1('dynamic-table');" title="take a printout" style="margin:0 10px;"><img src="./icons/print.png" /></a> -->
                ...
                <!-- div.table-responsive -->
                  ...
                  <!-- div.dataTables_borderWrap -->
                   <div id="divToPrint">
                    ...
                    <table id="dynamic-table" class="table table-striped table-bordered table-hover" border="1" cellpadding="0" cellspacing="0">
                    <?php 
                    $pro_name=myrowval('catalog_product','name',"id=".$product_id1);
                      $start1 = date("d/M/Y", strtotime($start));
                      $end1 = date("d/M/Y", strtotime($end));
                    ?>
                    <caption><strong>
                    Product Name:- <?=$pro_name?></br>
                    Stock Ledger:- <?=$start1?> TO <?=$end1?>	
                    </strong></caption>
                        <thead>                            
                            <tr> 
                        ...
                        <tbody>
                            ...
                            $cstock=0;
                            $cvalue=0;
        $open=$myobj->get_stock_ledger_open_list($start,$end,$product_id1);
        $opens=explode('|',$open);
                $cstock=$opens['0'];
                $cvalue=$opens['1'];
                ?>
                          ...
                          for ($i = 0; $i < $datediff; $i++) {
                          $cdate=$datearray[$i];
                          $rs= $myobj->get_stock_ledger_tranc_list($product_id1,$cdate,$filter,  $records = '', $orderby ="");
                            foreach($rs as $key=>$rows){
                           $uid = $rows['cid'];
                            $chno = $rows['ch_no'];
                           // $uidname = $rows['dname'];
                           $pwhere="dealer_id=".$dea_id." AND product_id=".$rows['product_id'];
                           $prate=myrowval('stock','rate',$pwhere);
                            $value=round($rows['value'],2);
                            $cstock+=$rows['iqty']-$rows['oqty'];
                            //$cvalue+=$rows['ivalue']-$rows['ovalue'];'
                            $cvalue=$cstock*$prate;
                            if($cvalue<0){
                            	$cvalue='0.00';
                            }
                            $vtype1=explode('/',$rows['ch_no']);
                            if($vtype1[0]=='BDN'){
                            $vtype="GST SALE";
                            $ch_no='<a target="_blank" href="index.php?option=' . $formaction . '&showmode=1&mode=1&id=' . $uid .'&actiontype=printbl">'.$rows[ch_no].'</a>'; 
                            }else{
                            $vtype="GST PURCHASE"; 
                            $ch_no='<a target="_blank" href="index.php?option=' . $formaction . '&showmode=1&mode=1&id=' . $uid .'&actiontype=printpl">'.$rows[ch_no].'</a>';   
                            }
                            ?>
                            <tr>
                                <td><?=$inc?></td>  
