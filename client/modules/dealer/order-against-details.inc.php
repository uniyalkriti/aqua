...
$myorderby = 'demand_order.dealer_id DESC'; //The orderby clause for fetching of the
...
elseif (isset($_GET['ajaxshow']) || isset($_GET['ajaxshowblank']))
{
    ...
    $today = date('Y-m-d');
    $rs = $myobj->$funcname_new($filter="uso.`order_date`='$today' ",  $records = '', $orderby='');
        //get_dealer_sale_list
        /*pre($rs);
        die('lfjsd');*/
...
<script type="text/javascript">    
    ...
    function FormSubmit()
    {
        ...
        if (str != '') {
            $.colorbox({href: 'index.php?option=challan-order-wise-modal&showmode=1&mode=1&order_id=' +str+'&fd='+fd+'&td='+td, iframe: true, width: '95%', height: '95%'});
                return true;
        }
        else
            return false;
...
<input type="radio" name="bedStatus" class="reloaded" id="direct"  value="direct">Generate Invoice
            ...
            <input type="radio" name="bedStatus" id="order_against" class="reloaded" checked="checked" value="order_against_challan">Billing Against Order
<hr width="100%">
</div>
                    ...
                    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                        <thead>
                           <tr>
                            ...
                            <!--                                            <th>Dealer Name</th>-->
                            <th style="background-color:#C7CDC8; color:#000;">Distributor Name</th>
                ...
                $editlink = '<a class="iframef" href="index.php?option=' . $formaction . '&showmode=1&mode=1&id=' . $uid . '"><img src="./images/b_edit.png"></a>';               
                 ...
                 $outs="SELECT 
                        sum(`challan_order`.`amount_round`) AS AmountPaid 
                         FROM `challan_order`  WHERE `challan_order`.`ch_retailer_id`='$rows[retailer_id]'";
                 //h1($outs);                
