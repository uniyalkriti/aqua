...
$formaction = $p;
$myobj = new dashboard();
$cls_func_str = 'payment_collection'; //The name of the function in the class that will do the job
...
if (isset($_POST['filter']) && $_POST['filter'] == 'Filter') {
    ...
    $rs = $myobj->$funcname($filter, $records = '', $orderby = "ORDER BY $myorderby",$month);
    $rs1 = $myobj->get_pre_payment_collection_list($filter, $records = '', $orderby = "ORDER BY $myorderby",$month);
}
                ...
                <div>
                    ...
                    <style> th { background-color: #C7CDC8;color:#000; }</style>
                     <table id="dynamic-table" border="2" width='100%' class="table table-striped table-bordered table-hover">
                        ...
                        <thead>
                             <tr><th colspan="8" style="background-color:#307ECC;color:#fff;height:35px;font-size: 20px">Payment Collection</th></tr>
                             
                            <tr>      
                         
                         <th colspan="4" style="text-align:center; ">Payment Collection of <?php if(isset($month)){echo date("F-Y", strtotime($month));}?>(<?=$curm?>)</th>
                           <th colspan="4" style="text-align:center; ">Payment Collection Of <?php if(isset($pmonth)){echo date("F-Y", strtotime($pmonth));}?>(<?=$prem?>)</th>       
                            </tr>
                            <tr valign="top" class="search1tr">
                                
                                <th width="330px">Retailer Name</th>
                                <th width="210px">Payment Date</th>
                                <th width="190px">Amount</th>
                                 <th width="160px">Payment Mode</th>
                                 <th width="330px">Retailer Name</th>
                                <th width="210px"> Payment Date</th>
                                <th width="190px">Amount</th>
                                <th width="160px">Payment Mode</th>
                        ...
                        <tbody>
                            ...
                            // pre($rs);
                             ...
                             ?>
                                       ...
                                       <tr><td>
                                     <td width="250px"style="background-color:<?php echo $col; ?> ">
                                    <?=$rows['rname']?>
                                </td>
                                  <td width="170px" style="text-align:center;background-color:<?php echo $col; ?>;">
                                    <?=$rows['date']?>
                                </td>
                                  <td width="180px" style="text-align:center;background-color:<?php echo $col; ?>;">
                                 &#8377;   <?=$rows['amount']?>
                                </td>
                                <td width="150px" style="text-align:center;background-color:<?php echo $col; ?>;">
                                    <?=$mode?>
                                </td>
                                </td></tr>
                             <?php $inc++; } ?>
