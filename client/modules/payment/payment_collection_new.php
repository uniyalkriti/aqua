...
if(isset($_POST['Submit2']) && $_POST['Submit2']=='Save Payment'){
    ...
    $data = $myobj_pay->save_payment_new();

                ?><script>
                    setTimeout("window.parent.location = 'index.php?option=payment-new'", 500);
                </script>
<?php
}
...
if (isset($_POST['filter']) && $_POST['filter'] == 'Filter') {
    ...
    //echo"hgfjh";
       $filter[] = "ch_dealer_id = '{$_SESSION[SESS.'data']['dealer_id']}'";
       $start=date('Ymd');
        $filter[] = "DATE_FORMAT(`ch_date`,'%Y%m%d') = '$start'";
            $filterused = implode($filterstr, '<span style="margin: 0 10px;">|</span>');
             $filter[] = "payment_status != '1'";
      $rs = $myobj->get_payment_data($filter, $records = '', $orderby = "GROUP BY ch_retailer_id"); // $myobj->get_item_category_list()
        ...
        <table width="100%" border="0" cellspacing="2" cellpadding="2" class="tableform">
         ...
         <!--<td><span class="star">*</span>Total Amount<br>-->
            ...
            <!--<input type="text" name="total_amount_pay" value="<?php echo $_GET['total_val']?>" disabled="disabled">-->
             ...
             <td> <strong>Recieved Amt</strong></td>
         </tr>
            ...
            //  $ch = rtrim($_GET['order_id'],',');
             // $challan_id=explode(',',$ch);?>
              ...
              // for($i=0;$i<$size;$i++){
               ...
               $str= array();
               $total_amount=0;
                $q2 = "SELECT id, sum(old_payment_dues.remaining) as old_total FROM old_payment_dues WHERE ch_retailer_id='$retailer' AND payment_status='0'";
                $r2= mysqli_query($dbc, $q2);
                $row2= mysqli_fetch_assoc($r2);
                $old_remain_amt=$row2[old_total];
				if($old_remain_amt!=0){
                 echo '<tr>'
                 .'<td><input type="checkbox" name="old_invoice" value="'.$row2['id'].'" class="checkboxes"></td>'
                    .'<td>Old Dues</td>'.'<td>'.'₹ '.$old_remain_amt.'</td>
                        <td>₹<input type="text" name="old_remain_amount" value="'.$old_remain_amt.'" readonly /> </td>'
                        .'<td>
                ₹ <input type="text" name="old_recieved_amt" class="recieved_amt" placeholder="Enter Amount" readonly="readonly"/>
             </td>'
                    . '</tr>';
				}
                    echo'<input type="hidden" name="old_ch_id" value="'.$row2['id'].'" >';
			 ...
			 if($old_remain_amt!=0){
                ...
                while ($row1 = mysqli_fetch_assoc($rs1)){
                  ...
                  echo '<tr>'
                  .'<td><input type="checkbox" name="invoice['.$row1[id].']" value="'.$row1['id'].'" class="checkboxes"></td>'
                    .'<td>'.$ch_no.'</td>'.'<td>'.'₹ '.$remain_amt.'</td>
                        <td>₹<input type="text" name="remain_amount['.$row1[id].']" value="'.$remain_amt.'" readonly class="remain_amount" /> </td>'
                        .'<td>
                ₹ <input type="text" name="recieved_amt['.$row1[id].']" class="recieved_amt" placeholder="Enter Amount" readonly="readonly"/>
             </td>'
                    . '</tr>';
                    echo'<input type="hidden" name="ch_id['.$row1[id].']" value="'.$row1['id'].'" >';
//                  $str = $row1['id'];
                }//}
        ...
        <input type="hidden" name="dealer_id" value="<?php echo $dealer_id?>" >
         ...
         <td></td>
            ...
            ?>
             ...
             <td>
                ₹ <input type="text" name="remain_amt" class="remain_amt" placeholder="Enter Amount" style="width:150px" readonly="" />
             </td>
         </tr>
               </table>
         ...
         <tr>
           <td><span class="star">*</span>Payment Type<br />
            ...
            <td><span class="star">*</span>Reciept No.<br />
            <input  name="reciept_no" id="reciept_no"  value="<?php if(isset($_POST['reciept_no'])) echo $_POST['reciept_no']; ?>"  required=""/>
            </td>
         </tr>
...
<?php echo arr_pulldown('datepref', $mymatch['datepref'], '', true, true, '', false, 'Select Pref Date'); ?>
                ...
                <!-- <div style="float:left; "> <input type="radio" style="float:left" name="search"  onclick="return search_user(2)" />  -->
                      ...
                      $dealer_id = $_SESSION[SESS.'data']['dealer_id'];
                       db_pulldown($dbc, 'location_id', "SELECT location_id as beat_id,(select name from location_5 where id=dealer_location_rate_list.location_id) as beat_name FROM `dealer_location_rate_list` WHERE `dealer_id` = '$dealer_id'", true, true, 'id=locid'); ?> 
                    </td>                  
                    <td>Retailer<br/>
                        <?php

                            
                                $r_id = $_POST['retailer_id'];
                                $beat_id = $_POST['location_id'];
                                $q = "SELECT retailer.id as id, CONCAT_WS(' ',retailer.name,'[',retailer.address,']',' [',sum(challan_order.remaining),']') as name FROM retailer LEFT JOIN challan_order ON challan_order.ch_retailer_id=retailer.id where retailer.dealer_id ='" . $_SESSION[SESS . 'data']['dealer_id'] . "' AND retailer_status='1' group by retailer.id ORDER BY retailer.name ASC  ";
                                //h1($q);
                                db_pulldown($dbc, 'retailer_id', $q, true, true, 'id="retailerId" class="chosen-select form-control"','==Please Select==',$r_id);                               
                            ?>
                    </td>
                   <!--  <td>Challan No<br />
...
//$result = $myobj->print_looper_invoice($multiId=1, $options=array());
            ...
            foreach ($rs as $key => $rows) {
                ...
                if(empty($rows['last']['lastdate'])){
				$lastdate="00-00-0000 00:00";	
				}else{
				$lastdate=date('d-m-Y H:i',strtotime($rows['last']['lastdate']));	
				}
                ...
                echo'
                      <tr BGCOLOR="' . $bg . '" id="tr' . $uid . '" class="ihighlight">
                        ...
                        <td>' .$lastdate. '<div style="display:none" id="delDiv' . $uid . '"></div></td>
                         <td>₹ ' . my2digit($rows['last']['lastamt']) . '</td>
                        <td>₹ ' . my2digit($rows['total']) . '</td>
<!--                            <td></td>  -->
                       <td><span class="btn btn-sm btn-warning"><a style="color:#fff;"
                        class="iframef" href="index.php?option=payment-new&showmode=1&mode=1&retailer='.$retailer.'">Collect Payment</a></b></span></td>
                        ';
     ...
     <script src="assets/js/jquery.dataTables.min.js"></script>
        ...
        <script type="text/javascript">
            $('.checkboxes').on('click',function(){
                if($(this).is(':checked')){
                $(this).closest('tr').find('.recieved_amt').removeAttr('readonly');
                }else{
                $(this).closest('tr').find('.recieved_amt').attr('readonly','readonly');    
                }
        ...
        var total=parseInt(0);
            $('.recieved_amt').on('keyup',function(){
                    var sum = 0;
    $(".recieved_amt").each(function(){
        sum += +$(this).val();
    });
    $(".remain_amt").val(sum);
            });
        ...
        <script type="text/javascript">
            $('.recieved_amt').on('change',function(){
            var ret=0;
            var comp=0;
            ret=parseInt($(this).val());
            comp=parseInt($(this).closest('tr').find('.remain_amount').val());
            if(ret>comp){
             alert("Recieved Amount can't be greater than Remaining Amount");
             $(this).val('');  
             var sum = 0;
    $(".recieved_amt").each(function(){
        sum += +$(this).val();
        ...
        $(document).ready(function(){
            $('#locid').change(function(){
               ...
               if(loc_id!=''){
                   $.ajax({
                       type:"POST",
                       url:'js/ajax_general/ajax.php',
                       data:{lid:loc_id,wcase:'getBeatRetailer'},
                       success: function(data) {
                           if(data!=false)
                           {
                             $('#retailerId').html(data);
                           }else{
                ...
                $('#retailerId').html('<option>==Please Select==</option>');
               }
