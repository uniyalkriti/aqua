...
if(isset($_POST['Submit2']) && $_POST['Submit2']=='Save Payment'){
    ...
    $data = $myobj_pay->save_payment();
                ...
                ?><script>
                    setTimeout("window.parent.location = 'index.php?option=payment-collection'", 500);
                </script>
<?php
}
...
if (isset($_POST['filter']) && $_POST['filter'] == 'Filter') {
    if (valid_token($_POST['hf'])) { // checking if post value is same as timestamp stored in session during form load
        ...
        if ($checkpass) {
            ...
            $rs = $myobj->get_payment_list($filter, $records = '', $orderby = "ORDER BY $myorderby"); // $myobj->get_item_category_list()
            if (empty($rs))
      ...
      <fieldset>
        ...
        <table width="100%" border="0" cellspacing="2" cellpadding="2" class="tableform">
         ...
         <!--<td><span class="star">*</span>Total Amount<br>-->
            ...
            <!--<input type="text" name="total_amount_pay" value="<?php echo $_GET['total_val']?>" disabled="disabled">-->
              ...
              <input type="hidden" name="challan_id" value="<?php echo $_GET['order_id']?>" >
               <input type="hidden" name="retailer_id" value="<?php echo $_GET['retailer_id']?>" >
               <table border="3" height="100%" width="180%"> <tr>
             <td>
             ...
             <td><strong>Remaining</strong></td>
              ...
              $challan_id=explode(',',$ch);?>
               ...
               for($i=0;$i<$size;$i++){
               //  pre($challan_id[0]);
               $qwer="SELECT  `challan_order`.`ch_no` ,`challan_order`.`remaining`
                    FROM  `challan_order` 
                    WHERE  `challan_order`.`id`='$challan_id[$i]' order by auto ASC";
                ...
                while ($row1 = mysqli_fetch_assoc($rs1)){
                  ...
                  echo '<tr>'
                    .'<td>'.$ch_no.'</td>'.'<td>'.'₹ '.$remain_amt.'</td>
                        <td> ₹ <input type="text" name="remain_amount[]" value="'.$remain_amt.'" readonly/> </td>'
                    . '</tr>';
                }}
                       ?>
         ...
         <tr><td><strong>Total Amount:</strong></td>
            <?php $total_amt1=number_format((float)$_GET['total_val'], 2, '.', '');
            $total_amt1=round($remain_amt1,2);
            ?>
             <td> <b><input type="text" name="total_amount_pay" value="<?php echo '₹ '.$total_amt1?>" disabled="disabled" style="color:RED"></b></td>
             <td>
                ₹ <input type="text" name="remain_amt" placeholder="Enter Amount" required/>
             </td>
         </tr>
               </table>
         ...
         <tr>
            ...
            <input id="mysave" type="submit" name="Submit2" style="background-color: #438EB9" value="<?php if(isset($heid)) echo'Update'; else echo'Save Payment';?>" />
...
<?php echo arr_pulldown('datepref', $mymatch['datepref'], '', true, true, '', false, 'Select Pref Date'); ?>
                ...
                <!-- <div style="float:left; "> <input type="radio" style="float:left" name="search"  onclick="return search_user(2)" />  -->
                    ...
                    <td><span class="star">*</span>Retailer<br/>
                        <?php
                            ...
                            db_pulldown($dbc, 'retailer_id', $q, true, true, 'id="retailerId" lang="Retailer"','==Please Select==',$_POST['retailer_id']);
                        ?>
                    </td>
                   <!--  <td>Challan No<br />
                    ...
                    <td>
                        <input id="mysave" type="submit" name="filter" value="Filter"  class="btn btn-info"/>
         ...
         <div class="row">
            <div class="col-xs-12">
                ...
                <div class="table-header">
                    Payment Collection &nbsp; &nbsp;&nbsp; <span class="label label-lg label-yellow arrowed-in arrowed-in-right"><a style="color:red" href="javascript:void(0)" onclick="FormSubmit();" >Make Payment</a></b></span> <div class="pull-right tableTools-container"></div>
                </div>
                </div>
        <?php
            ...
            foreach ($rs as $key => $rows) {
                ...
                echo'
                      <tr BGCOLOR="' . $bg . '" id="tr' . $uid . '" class="ihighlight">
                        <td class="myintrow myresultrow">' . $inc . '<input type="checkbox" name="chlan_id[]" id="chk'.$uid.'"  value="'.$uid.'" onclick="print_all_selected(this.id,this.value,this.name)"></td>
                        <td><strong>' . $rows['ch_date'] . '</strong></td>
                ...
                foreach ($rows['challan_item'] as $inkey => $invalue) {
                    ...
                    $total=$invalue['remaining'];
                }
             
                // showing the po item details ends here <img src="../icon-system/i16X16/rupee.png">
                echo'<td><strong><span class="star">₹ </span> <span id="total_'.$uid.'">' . my2digit($total) . '</strong></td>
