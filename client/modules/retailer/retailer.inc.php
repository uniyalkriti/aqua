...
if(isset($_POST['filter']) && $_POST['filter'] == 'Filter')
{
  if(valid_token($_POST['hf'])) // checking if post value is same as timestamp stored in session during form load
  {
    ...
    if($checkpass)
    {
      ...
      if(!empty($_POST['location_5_id'])){
        $filter[] = "location_id='$_POST[location_5_id]'";
        //$filterstr[] = '<b>Name  : </b>'.$_POST['name'];
      }
      
                        $filter[] = "retailer.dealer_id = '{$_SESSION[SESS.'data']['dealer_id']}' ";
                        $filter[]="retailer.retailer_status=1";
  ...
  $rs = $myobj->$funcname($filter="$myfilter'$ajaxshowid'",  $records = '', $orderby='');
            ...
            $rs = $myobj->$funcname($filter=" retailer.dealer_id = '{$_SESSION[SESS.'data']['dealer_id']}' AND retailer_status=1 ",  $records = '', $orderby=" ORDER BY $myorderby");
}
...
<!--             <td><span class="star">*</span>Please tick, if retailer work for D.S. Group.<br>
              ...
              <td><span class="star">*</span>Retailer Name<br>
               <input onChange="this.value = ucwords(trim(this.value));" type="text"  lang="Name" name="name" value="<?php echo (isset($_POST['name'])) ? $_POST['name']:''; ?>" style="text-transform:uppercase">
              </td>
              ...
              if(!isset($heid)){
                    ...
                    db_pulldown($dbc, "so", $q, true,true,'id="so" ');
              }else{
                  $q = "Select person.id,CONCAT_WS(' ',person.first_name,last_name) as name from person INNER JOIN user_dealer_retailer udr ON person.id = udr.user_id  where dealer_id ='$dealer'  ";
                    db_pulldown($dbc, "so", $q, true,true,'id="so" ','',TRUE,TRUE,$_POST['so']);
              }
              
              ?>
              </td>
              <td><span class="star">*</span>Mobile No <Br/>
                <input type="text" onkeypress="return isNumberKey(event);" name="landline" value="<?php if(isset($_POST['landline'])) echo $_POST['landline']; ?>" maxlength="10" required/>
              
            </td>
			<td>
              <span class="star">*</span> Account Active <Br/>
              <?php 
              if(!isset($_POST['retailer_status'])) $_POST['retailer_status'] = 1;
              arr_pulldown('retailer_status', array('1'=>'Active', '0'=>'Deactive'), $msg='', $usearrkey=true, $ini_option=false, $jsfunction='lang="Select Retailer Active"',false); ?>
            </td>
			
		 <!--	<td>PAN Card<Br/>
                <input type="text"  name="pan_card" value="<?php if(isset($_POST['pan_card'])) echo $_POST['pan_card']; ?>" maxlength="10" />
          ...
          <tr>
              ...
              <input type="text" name="contact_per_name" value="<?php if(isset($_POST['contact_per_name'])) echo $_POST['contact_per_name']; ?>" style="text-transform:uppercase"/>
             </td> 
              <td colspan="5">Address<br>
                <textarea name="address" style="text-transform:uppercase"><?php if(isset($_POST['address'])) echo $_POST['address']; ?></textarea>
            </td> 
<!--            <td>Image <?php if(isset($heid) && !empty($_POST['image_name'])) { echo '<img height="20" width="20" src="../myuploads/retailer/create/'.$_POST['image_name'].'" height="30" width="30" >'; } ?><br>
          ...
          <tr>
            <td>
              ...
              <input type="text" name="pin_no" value="<?php if(isset($_POST['pin_no'])) echo $_POST['pin_no']; ?>" maxlength="6" required/>
             </td>
             <td>
              GSTIN No<Br/>
              <input type="text" name="tin_no" value="<?php if(isset($_POST['tin_no'])) echo $_POST['tin_no']; ?>" maxlength="60" style="text-transform:uppercase"/>
             </td>
			 <td>
              Drug Lic No.<Br/>
              <input type="text" name="drug_lic_no" value="<?php if(isset($_POST['drug_lic_no'])) echo $_POST['drug_lic_no']; ?>" maxlength="60" style="text-transform:uppercase"/>
             </td>
			 <td>
              PAN No.<Br/>
              <input type="text" name="pan_no" value="<?php if(isset($_POST['pan_no'])) echo $_POST['pan_no']; ?>" maxlength="60" style="text-transform:uppercase"/>
             </td>
			 </tr>
			 <tr>
...
<!--             <tr>
        ...
        for($i = 3; $i<=$retailer_level; $i++)
        {
         ...
         <?php
                    ...
                    $d_city = "SELECT l3_id as id, l3_name as name FROM location_view INNER JOIN dealer_location_rate_list dl ON location_view.l5_id=dl.location_id WHERE state_id=$state AND dealer_id=$dealer";

                     if($i == 3) db_pulldown($dbc, "location_".$i."_id", $d_city, true, true, $js_attr); 
           			 if($i > 3) {
                       $o = $i-1;
    ...
    ?>
          ...
          <tr>
            <td>
            <span class="star">*</span>City<br>
            <?php
               // $d_city = "SELECT l3_id as id, l3_name as name FROM location_view INNER JOIN dealer_location_rate_list dl ON location_view.l5_id=dl.location_id WHERE state_id=$state AND dealer_id=$dealer";

                 $d_city = "SELECT l3_id as id, l3_name as name FROM location_view INNER JOIN dealer_location_rate_list dl ON location_view.l5_id=dl.location_id WHERE  dealer_id=$dealer group by l3_id";
                db_pulldown($dbc, "l3_id", $d_city, true, true, 'onchange="fetch_location(this.value,\'progress_div\',\'l4_id\',\'location-subcategory-dealer-wise\');"id="l3_id"');  ?></td>

            <td>
            <span class="star">*</span>District<br>
            <?php 
                  $d_dist = "SELECT l4_id as id, l4_name as name FROM location_view INNER JOIN dealer_location_rate_list dl ON location_view.l5_id=dl.location_id WHERE l3_id='$_POST[l3_id]' AND dealer_id=$dealer";
                  db_pulldown($dbc, "l4_id", $d_dist, true, true, 'onchange="fetch_location(this.value,\'progress_div\',\'l5_id\',\'location-locality-dealer-wise\');" id="l4_id"');  ?></td>
            <td>
            <span class="star">*</span>Locality<br>
              <?php 
              $d_loc = "SELECT l5_id as id, l5_name as name FROM location_view INNER JOIN dealer_location_rate_list dl ON location_view.l5_id=dl.location_id WHERE l4_id='$_POST[l4_id]' AND dealer_id=$dealer";
              // db_pulldown($dbc, "l5_id", $d_loc, true, true, 'id="l5_id"') 
                db_pulldown($dbc, "location_5_id", $d_loc, true, true, 'id="l5_id"')
              ?>
            </td>
          </tr>
           <tr>
...
<!--            <input onclick="parent.$.fn.colorbox.close();" type="button" value="Exit" /> <br />-->
    ...
    <td>Name<br />
                <input type="text" placeholder="Enter Retailer Name" class="retailer" name="name" id="name" value="<?php if(isset($_POST['name'])) echo $_POST['name'];?>" /> 
                  ...
                  <td>
            <span class="star">*</span>Beat<br>
              <?php 
              $d_loc = "SELECT l5_id as id, l5_name as name FROM location_view INNER JOIN retailer r ON location_view.l5_id=r.location_id WHERE r.dealer_id='$dealer'";
              // db_pulldown($dbc, "l5_id", $d_loc, true, true, 'id="l5_id"') 
                db_pulldown($dbc, "location_5_id", $d_loc, true, true, 'id="l5_id"')
              ?>
            </td>
...
<!--                                <th class="center">
                            ...
                            foreach($rs as $key=>$rows)
                  {
                      ...
                      $uid = $rows['id'];
                      $uidname = $rows['name'].' ['.$rows['address'].']';
