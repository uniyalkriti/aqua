...
<?php 
$forma = 'Company Import'; // to indicate what type of form this is
$formaction = $p;
$myobj = new company();
$cls_func_str = 'vendor'; //The name of the function in the class that will do the job
...
?>
      <div id="breadcumb"><a href="#">Master</a> &raquo; <a href="#">Party</a> &raquo; <a href="index.php?option=<?php echo $formaction; ?>" style="color:#2dcf5f;"><?php echo $forma;?></a> <span id="ajaxloader"><img src="images/ajaxloader.gif" /> Loading...</span>
...
if(isset($_GET['mode']) && $_GET['mode'] == 1)
{
	if(isset($_GET['id']) && is_numeric($_GET['id']))
	{
		...
		if(!empty($mystat))
		{
			...
			create_multi_post($mystat[$id]['party_contact'], array('cname'=>'cname', 'cdesignation'=>'cdesignation', 'cmobile'=>'cmobile', 'cemail'=>'cemail', 'cphone'=>'cphone', 'cremark'=>'cremark'));
...
function check_selected_value()
{
   ...
   if(str != '')
      getdata_div(str+'<$>'+comp_id, 'progress_div', 'get-company-dynamic-data', 'comp_locality');
     ...
     <form method="post" action="<?php if(isset($eformaction)) echo  $eformaction; ?>" class="iform" name="genform" onsubmit="return checkForm_alert('genform');">
      <fieldset>
        ...
        <table width="100%" border="0" cellspacing="2" cellpadding="2" class="tableform">
         <tr>
             <td>Company<span class="star">*</span><br />
             <?php db_pulldown($dbc, 'company_id', $q="SELECT id, name FROM company ORDER BY name ASC", true, true, $jsfunction='onchange="getdata_div(this.value, \'progress_div\', \'get-company-import-locality\', \'usermodules\');" id="company_id"'); ?>
            </td>           
            <td colspan="5"> 
             <div  id="usermodules">
    ...
    <td><strong>'.$rows['partycode'].'</strong><div style="display:none" id="delDiv'.$uid.'"></div></td>
                            <td>'.$uidname.'</td>						
                            <td>'.$rows['phone'].'</td>
                            <td>'.$rows['email'].'</td>
