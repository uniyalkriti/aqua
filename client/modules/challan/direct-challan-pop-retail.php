    ...
    document.getElementById('total_amount_a').value = totalamt.toFixed(2);
             ...
             <div class="widget-box" >
                 <div class="widget-header widget-header-blue widget-header-flat">
                        <h4 class="widget-title lighter">Direct Invoice Details Retail</h4>
                </div>
                 <form class="form-horizontal enter" method="post" action="<?php if (isset($eformaction)) echo $eformaction; ?>"  name="genform" onsubmit="return checkuniquearray('genform');" enctype="multipart/form-data">
            ...
            <div>
	    <input type="radio" name="bedStatus" class="reloaded" id="direct"  value="direct">Generate Invoice
            <input type="radio" name="bedStatus" id="retail" class="reloaded" checked="checked" value="retail">Generate Retail Invoice
            <input type="radio" name="bedStatus" id="sfa" class="reloaded" value="sfa">Billing Through SFA
            <input type="radio" name="bedStatus" id="order_against" class="reloaded"  value="order_against_challan">Billing Against Order
            </div>
            <div class="rdrt">
            <input type="radio" name="crdr" class="cd" id="credit" value="1">Credit Note
            <input type="radio" name="crdr" class="cd" id="debit"  value="2">Debit Note
            </div>
            <table>
                <tr>
<!--                    <td width="7%">
    ...
    ?>
                        ...
                        <td><strong>Please Tick Here For Document Billing</strong></br>
                        <input type="checkbox" name="book_status"  id="book_status" value="1" />
                        </td>
                ...
                $query = "select `ch_no` from `challan_order` where `ch_dealer_id`=$dealer_id1 AND book_status='0' order by `ch_no` DESC";
          ...
          echo $ch_id ?>"  id="ch_no">
          <input  type="hidden" name="ch_no_hid" style="width:220px" value="<?php
          echo $ch_id ?>"  id="ch_no_hid">
                       
          <input type="hidden" name="ch_no_prifix" value="<?php echo 'BDN/' . $sesId = $_SESSION[SESS . 'data']['dealer_id'] . '/'; ?>">
                    </td>
                    <td> 
...
$qrt = "SELECT retailer.id as id, CONCAT_WS(' ',retailer.name,'[',retailer.address,']','[',sum(challan_order.remaining),']') as name FROM retailer LEFT JOIN challan_order ON challan_order.ch_retailer_id=retailer.id where retailer.dealer_id ='" . $_SESSION[SESS . 'data']['dealer_id'] . "' AND retailer_status='1' group by retailer.id ORDER BY retailer.name ASC  ";
 //h1($qrt);
db_pulldown($dbc, 'retailer_id', $qrt, true, true, 'id="retailer" class="form-control chosen-select retailer" style="margin-left:10px;" lang="Retailer" onchange="getdata(this.value, \'progress_div\', \'get-retailer-location\', \'location_id\');"','=====Please Select=====',$_POST['retailer_id']);
?>
 ...
 <input type="hidden" class="pkigst" value="0">
                    ...
                    <td>
                      <strong><span class="star">*</span>Field Force Name</strong><br>
                      <?php 
        ...
        <td colspan="5">
            <table width="100%" id="mytable" class="">
                <tr class="thead" style="font-weight:bold;">
                    <th style="background-color:#C7CDC8; color:#000; border:1px solid black;"> S.NO</th>
                    <th style="background-color:#C7CDC8; color:#000; width:20%; border:1px solid black;"> &nbsp; Item Name</th> 
                   <!--  <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Bill of Supply</th> -->
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">M.R.P/Cur. Stock/Batch No./Mfg Date</th>
                    <!-- <th style="background-color:#C7CDC8; color:#000;">HSN Code</th> -->
                    <!-- <th style="background-color:#C7CDC8; color:#000;">Com. Code</th> -->
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Curr. Stock</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Qty</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Free Qty</th>
<!--                    <th style="background-color:#C7CDC8; color:#000;">Sch. Quantity</th>-->
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Rate</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Trade Type</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Trade/<br>Sch. Disc.</th>                  
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Trade <br> Amt.</th>

                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">CD Type</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">CD Disc.</th>                  
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">CD Amt.</th>

                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Spl Sch. Amt.</th>


                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">ATD Type</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">ATD</th>                                                
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">ATD Amt</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Taxable Amt.</th>
                    
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">CGST%</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">CGST Amt</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">SGST%</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">SGST Amt</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">IGST%</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">IGST Amt</th>
                    

                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Amount</th>
                    <!--<th style="background-color:#C7CDC8; color:#000;border:1px solid black;">&nbsp;</th>-->
                </tr>
                <?php
                

                $keys = array_keys( $_POST['challan_item']);
                if(isset($_POST['challan_item'])){ $num_rows = count($_POST['challan_item']); }else{ $num_rows=1; }
                for($z=1;$z<=$num_rows;$z++){
                    $avlb_stock = 0;
                    $ttl_amt = 0;
                    $avlb_stock = get_avlb_stock_by_prodid_mrp($_POST['challan_item'][$keys[$z-1]]['product_id'],$_POST['challan_item'][$keys[$z-1]]['mrp'],$_POST['challan_item'][$keys[$z-1]]['batch_no'],$_POST['challan_item'][$keys[$z-1]]['mfg']);
                ...
                <td width="20%">
                 <?php
                   ...
                   // $q = 'SELECT cp.id, CONCAT_WS(" ",cp.name,"[",oc.cp_short_name,"]","[",SUM(s.qty),"]") from catalog_product cp INNER JOIN stock s ON cp.id = s.product_id INNER JOIN other_company oc ON cp.company_id=oc.id AND s.dealer_id='.$_SESSION[SESS.'data']['dealer_id'].' where s.dealer_id = '.$_SESSION[SESS.'data']['dealer_id'].' AND s.qty>0 GROUP BY s.product_id ORDER BY cp.id';

                      $q = 'SELECT cp.id, CONCAT_WS(" ",cp.name,"[BAID]","[",SUM(s.qty),"]") from catalog_product cp INNER JOIN stock s ON cp.id = s.product_id  where s.dealer_id = '.$_SESSION[SESS.'data']['dealer_id'].' AND s.qty>0 GROUP BY s.product_id ORDER BY cp.id';
                    // h1($q);
                    db_pulldown170($dbc,  'product_id[]', $q, TRUE, TRUE,'id="product_id'.$z.'" class="item_details_retail chosen-select" ','',$_POST['challan_item'][$keys[$z-1]]['product_id']);
                    ?>   
                </td>                
                 <!-- <td style="text-align: center;"><input type="checkbox" name="bos[<?php echo ($z-1) ?>]" value="1"></td> -->
                 
                 <td>                     
                     <select  style="width:70px" name="mrp[]" class="mrp mrp_dd_retail calcpk" id="mrp<?=$z?>" placeholder="MRP">
...
<!--                 <td>                    
                 ...
                 <td style="width:50px">
                    ...
                    <td style="width:50px">
                        <input style="width:98%" placeholder="Qty" onkeypress="javascript:return isNumber(event)" type="text"  name="quantity[]" id="quantity" value="<?=$_POST['challan_item'][$keys[$z-1]]['qty']?>" class="quantitycl calcpk"/>
                    </td>
                    <td>
                      <!-- onblur="total_amount_invoice()" -->
                        <input style="width:98%" placeholder="Sch. Qty" type="text" name="scheme[]" id="scheme" class="quantityfr" value="<?=$_POST['challan_item'][$keys[$z-1]]['free_qty']?>" />
                    </td>
                    <td style="width:60px">
                     <input style="width:98%" placeholder="Rate" type="text" name="rate[]" id="rate"  class="rate" value="<?=$_POST['challan_item'][$keys[$z-1]]['product_rate']?>" />
                    </td>
                    <td  style="width:50px">
                    ...
                    <td style="width:50px">
                        <input style="width:98%" placeholder="Trade Amt." type="text" name="trade_disc_amt[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['dis_amt']?>" class="trade_disc_amt" readonly />
                        <input type="hidden" name="ttl_amt[]" value="<?=$ttl_amt?>"   />
                    </td>

                    <td>
                        <select name="spl_disc_type[]" lang="spl_disc" style="width:98%" class="spl_disc_type calcpk">
                            <option value="1">%</option>
                            <option value="2">Amount</option>
                      </select>
                    </td>
                    <td>
                      <input style="width:98%" placeholder="CD" type="text" name="spl_disc_val[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['dis_percent']?>"  class="spl_disc_val calcpk" onkeypress="javascript:return isNumber(event)"/>
                    </td>
                    <td>
                      <input style="width:98%" placeholder="CD Amt." type="text" name="spl_disc_amt[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['dis_amt']?>" class="spl_disc_amt"  />                
                    </td>
                    <td style="width:50px">
                      <input style="width:98%" placeholder="Spl Amt" type="text" name="cash_amt[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['dis_percent']?>"  class="cash_amt calcpk" onkeypress="javascript:return isNumber(event)"/>
                    </td>
                    <td style="width:50px">
...
<!--                            <option value="3"> Kg </option>-->
                    ...
                    <td style="width:50px">
                        <input type="text" style="width:98%" placeholder="ATD amt" name="cd_amt[]" class="cd_amt" value="<?=$_POST['challan_item'][$keys[$z-1]]['cd_amt']?>" readonly/>
                    </td>
                    <td>
                        <input type="text" style="width:100%" placeholder="Taxable" name="taxable_amt[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['taxable_amt']?>" class="taxable_amt" readonly />
                    </td>
                    <td  style="width:40px">
                   ...
                   <input type="text" style="width:100%" placeholder="CGST" name="cgst[]" class="cgst" id="cgst<?=$z?>" readonly/>
                 </td>
                 <td>
                   <input type="text" style="width:100%" placeholder="AMT" name="cgst_amt[]" class="cgst_amt" id="cgst_amt" readonly/>
                 </td>
                 <td>
                   <input type="text" style="width:100%" placeholder="SGST" name="sgst[]" class="sgst" id="sgst<?=$z?>" readonly/>
                 </td>
                 <td>
                   <input type="text" style="width:100%" placeholder="AMT" name="sgst_amt[]" class="sgst_amt" id="sgst_amt" readonly/>
                 </td>

                 <td>
                   <input type="text" style="width:100%" placeholder="IGST" name="vat[]" class="vat" id="vat<?=$z?>" readonly/>
                 </td>                  
                  <td style="width:70px">
                        <input type="text" name="vat_amt[]" style="width:100%" placeholder="Vat" id="vat_amt" class="vat_amt" value="<?=$vat_amt?>" readonly/>
                        <input type="hidden" name="surcharge[]" id="surcharge"  value="<?php echo $surcharge;?>" readonly/>
                    </td>
                    <td>
                        <input type="text" style="width:100%" class="<?php if($z-1>=7){?>addbutton<?php } ?> amount" placeholder="Amount" name="amount[]" id="amount" value="<?=number_format($amount,2,'.',','); ?>"  readonly/>
                    </td>
                ...
                <tr class="addarea">
                  <td colspan="25" style="border-top: #000 1px solid;height: 10px;"></td>
                </tr>
                 ...
                 <td colspan="16" style="border-bottom:1px solid #efefef;">
                   <table width="100%">
                     <tr>
                       ...
                       <td><strong>Total ATD Discount</strong></td>
...
$(document).on('click', '.addrow', function () {
  ...
  var j = $("#mytable").find('.item_details_retail').length;
  j = j*timestamp;

  var tr = $(this).closest('tr');
  var $c_row = tr.clone();

  var pid = tr.find('.item_details_retail').chosen().val();
  var mrp = tr.find('.mrp').val();
  var tds = tr.find('.trade_disc_type').val();
  var ct  = tr.find('.cd_type').val();

  $c_row.find('.item_details_retail').removeClass('chzn-done').css({'display':'block'}).removeAttr('id').val('').next('div').remove();
  $c_row.find('td:last').html('<img title="Less" src="images/less.png" class="removebutton calcpk">');
  $c_row.find('.item_details_retail').attr('id','product_id'+j);

  $c_row.find('.mrp').val(mrp);
  $c_row.find('.trade_disc_type').val(tds);
  $c_row.find('.cd_type').val(ct);

  $('#mytable tr.tdata').each(function(z){
  ...
  tr.find('.item_details_retail').removeClass('chzn-done').css({'display':'block'}).removeAttr('id').val('').next('div').remove();
  tr.find('.item_details_retail').chosen();
...
$(document).on('click', '.removebutton', function () { 
     var tot_row = $('#mytable').find('.item_details_retail').length;
...
$(function() {
    ...
    $('.calcpk').keypress(function (e) {
      var key = e.which;
             if(key == 13)  // the enter key code
             {
              var addrow = $(this).closest('tr').find('.addrow');
...
<script>
  ...
  $('#mytable').on('change','.quantitycl',function()
   ...
   $('#mytable').on('change','.quantityfr',function()
   {
      var frqty1 = parseInt($(this).val());
      var avbl_qty1 = parseInt($(this).closest("tr").find('.avlb_quantity').val());
      var qty1 = parseInt($(this).closest("tr").find('.quantitycl').val());
      var qty2=frqty1+qty1;
      if(avbl_qty1<qty2)
      {
          alert('Available stock must be greater then input quantity');
          $(this).val('0');
...
<script>
       $(".reloaded").change(function () {
		//var isschecked= $(this).is(':checked');
		var r=this.value;
		//alert(r);	
            if (r== 'direct') {
        window.location = "https://demo.msell.in/client/index.php?option=direct-challan";
            }
            if (r== 'retail') {
        window.location = "https://demo.msell.in/client/index.php?option=direct-challan-retail";
            }
            if (r== 'sfa') {
        window.location = "https://demo.msell.in/client/index.php?option=sale-order-detailes";
            }
            if (r== 'order_against_challan') {
        window.location = "https://demo.msell.in/client/index.php?option=challan-against-order";
            }
        });
</script>   
