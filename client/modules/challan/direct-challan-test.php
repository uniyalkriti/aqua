...
function total_amount_with_discount(){
    ...
    document.getElementById('total_amount_a').value = totalamt.toFixed(2);
	    ...
	    <input type="radio" name="bedStatus" class="reloaded" id="direct" checked="checked" value="direct">Generate Invoice
            ...
            <input type="radio" name="bedStatus" id="sfa" class="reloaded" value="sfa">Biiling Through SFA
            <table>
                <tr>
<!--                    <td width="7%">
...
<!--                    <th style="background-color:#C7CDC8; color:#000;">Sch. Quantity</th>-->
                    ...
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Spl Sch Type</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Spl Sch. Disc.</th>                  
                ...
                <td width="20%">
                 <?php
                   ...
                   $q = 'SELECT cp.id, CONCAT_WS(" ",cp.name,"[",oc.cp_short_name,"]","[",SUM(s.qty),"]") from catalog_product cp INNER JOIN stock s ON cp.id = s.product_id INNER JOIN other_company oc ON cp.company_id=oc.id AND oc.dealer_id='.$_SESSION[SESS.'data']['dealer_id'].' where s.dealer_id = '.$_SESSION[SESS.'data']['dealer_id'].' AND s.qty>0 GROUP BY s.product_id ORDER BY cp.id';
                    // h1($q);
...
<!--                 <td>                    
                 ...
                 <td style="width:50px">
                    ...
                    <td>
                      <input style="width:98%" placeholder="Spl" type="text" name="spl_disc_val[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['dis_percent']?>"  class="spl_disc_val calcpk" onkeypress="javascript:return isNumber(event)"/>
                    </td>
                    <td>
                      <input style="width:98%" placeholder="Spl Amt." type="text" name="spl_disc_amt[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['dis_amt']?>" class="spl_disc_amt"  />                
                    </td>
                    <td style="width:50px">
...
<!--                            <option value="3"> Kg </option>-->
                    ...
                    <td style="width:50px">
                        <input type="text" style="width:98%" placeholder="cd amt" name="cd_amt[]" class="cd_amt" value="<?=$_POST['challan_item'][$keys[$z-1]]['cd_amt']?>" readonly/>
                    </td>
                    <td>
...
<script>
       $(".reloaded").change(function () {
		...
		//alert(r);	
            if (r== 'direct') {
			$(".loader").fadeIn();
		window.location = "http://162.222.39.114/msell-baidyanath/client/index.php?option=direct-challan";
            }
            if (r== 'retail') {
		$(".loader").fadeIn();
		window.location = "http://162.222.39.114/msell-baidyanath/client/index.php?option=direct-challan-retail";
            }
            if (r== 'sfa') {
		$(".loader").fadeIn();
		window.location = "http://162.222.39.114/msell-baidyanath/client/index.php?option=sale-order-detailes";
            }
        });
</script>
