...
<!--                    <td width="7%">
    ...
    ?>
                ...
                $query = "select `ch_no` from `damage_order` where `ch_dealer_id`=$dealer_id1 order by `ch_no` DESC";
...
$qrt = "SELECT retailer.id as id, CONCAT(retailer.name,' [',retailer.address,'] ')as name FROM retailer where retailer.dealer_id ='" . $_SESSION[SESS . 'data']['dealer_id'] . "' AND retailer_status='1' group by retailer.id ORDER BY retailer.name ASC  ";
 //h1($qrt);
db_pulldown($dbc, 'retailer_id', $qrt, true, true, 'id="retailer" class="form-control chosen-select retailer"  lang="Retailer" onchange="getdata(this.value, \'progress_div\', \'get-retailer-location\', \'location_id\');"','=====Please Select=====',$_POST['retailer_id']);
?>
