...
<script type="text/javascript">
  ...
  function total_amount_with_discount(disc,total_disc,total_amount_a,total){
...
if (!empty($rs)) {
  ...
  foreach ($rs as $key => $value) { 
          ...
          <table width="100%" border="0" cellspacing="2" cellpadding="2" class="tableform" id="mytable">
            ...
            <tr>
              ...
              <td><strong><span class="star">*</span>Retailer Name</strong><br>
               ...
               <select lang="retailer" name="retailer_id[<?=$value['order_id'];?>]" id="retailer_id">
    ...
    $prod_q = 'SELECT cp.id, cp.name from catalog_product cp INNER JOIN stock s ON cp.id = s.product_id where s.dealer_id = '.$_SESSION[SESS.'data']['dealer_id'].' ORDER BY cp.id';
     ...
     db_pulldown170($dbc, "product_id[$o_id][]", $prod_q, TRUE, TRUE,"id='p$i' class='item_details' ",'',$prod_id);
    ?> 
   <!--  <input type="hidden" id="p<?php echo $i; ?>" name="product_id[<?=$o_id;?>][]" value="<?php echo $prod_id; ?>" class="item_details"> -->
    ...
    <td>
      ...
      <select name="mrp[<?=$value['order_id'];?>][]" class="mrp mrp_dd" lang="mrp" id="mrp<?php echo $i; ?>">
        <?php 
              ...
              {
                 echo "<option value='$m_val'>$m_val</option>";
              }
         ?>
      </select>
    ...
    <td><input id="ch_qty<?php echo $i; ?>" class="ch_qty" onblur="cd_product_calculate(<?=$i;?>);" type="text" lang="quantity" name="quantity[<?=$value['order_id'];?>][]" value="<?php echo $invalue['quantity']; ?>"></td>
    <?php
    ...
    /*if($igst>0)
    {
      ...
      {
        $act_rate = ($row['mrp']*82/105);
        $rates = number_format($act_rate,2);
      }else{
        $act_rate = ($row['mrp']*75/105);
        $rates = number_format($act_rate,2);
      }
    }else{
      $act_rate = ($row['mrp']*82/100);
      $rates = number_format($act_rate,2);
    }*/
    ...
    ?>
    <td>
      <input readonly type="text" class="taxable_amt<?php echo $key; ?>" id="taxable_amt<?php echo $i; ?>" name="taxable_amt[<?=$value['order_id'];?>][]"  value="<?php echo $rates*$invalue['quantity'];?>"   />
    </td>
    <td>
      <input type="hidden" id="state<?php echo $i; ?>" name="state[]" id="state" value="<?=$location_id?>"  />
      <input readonly type="text" id="vat<?php echo $i; ?>" name="vat[<?=$value['order_id'];?>][]" onchange="cd_product_calculate(<?=$i;?>);" value="<?php echo round($igst,2); ?>"></td>
    <td>
    ...
    <td><input readonly type="text" class="amountpk<?php echo $key; ?>" id="amount<?php echo $i; ?>" name="amount[<?=$value['order_id'];?>][]" value="<?php if(!empty($tda) || !empty($cda)) echo round($famt,2); else echo round($f_amt,2);?>"></td>
      ...
      <!--<td style="width:10px"><?php //echo $deletelink1; ?></td>-->
            ...
            <tr>
              ...
              <td style="width:100px"><input type="text" class="totalkp<?php echo $key?>" id="total<?php echo $key?>" name="total" value="<?=round($total,2)?>"></td>
            ...
            <tr>
             ...
             <td colspan="2">
               ...
               <select name="dis[<?=$value['order_id'];?>]" id="dis" class="dis dis<?php echo $key?>" onchange="total_amount_with_discount(this.value,'total_disc<?php echo $i?>','total_amount_a<?php echo $i?>','total<?php echo $key?>')">
              ...
              <strong>Discount Amount</strong>
               <strong>
                <input  style="width:98%" type="text" name="total_disc[<?=$value['order_id'];?>]" class="total_disc<?php echo $key?>" id="total_disc<?php echo $i?>" value="0" readonly>    </strong> 
                <br/>
                <strong>Total Amount</strong>
               <strong>
                <input  style="width:98%" type="text" class="total_amount_a<?php echo $key?>" name="total_amount_a[<?=$value['order_id'];?>]" id="total_amount_a<?php echo $i?>" value="<?=round($total,2)?>" readonly>    </strong> 
            </td>
