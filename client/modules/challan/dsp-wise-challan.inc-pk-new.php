...
<script type="text/javascript">
  function checkuniquearray(name)
  {
    ...
    {
      var m = '';
      for(var s=0;s<low_stock.length;s++)
      {
         m += low_stock[s]+'\n\r';
      }

      alert('Please update the stock of below products :\n\r'+m);
      return false;
    }*/
...
if (!empty($rs)) {
  ...
  /* For salesmen dropdown */
  $u_q = "SELECT d.user_id,CONCAT_WS(' ',p.first_name,p.last_name, '[',r.rolename,']') as sm_name FROM `user_dealer_retailer` d INNER JOIN person p ON d.user_id=p.id INNER JOIN _role r ON p.role_id=r.role_id  WHERE d.dealer_id = '" . $_SESSION[SESS . 'data']['dealer_id'] . "'";
  $sm_q = run_query($dbc,$u_q,'multi');
  $sm_data = array();
  while($sm = mysqli_fetch_assoc($sm_q[1]))
  {
    $sm_data[$sm['user_id']] = $sm['sm_name'];
  }
  ...
  $prod_q = run_query($dbc,$prod_q,'multi');
  $prod_data = array();
  while($pr = mysqli_fetch_assoc($prod_q[1]))
  {
    $prod_data[$pr['id']] = $pr['name'];
  }
  ...
  foreach ($rs as $key => $value) { 
                  ...
                  <tr>
                    <td>
                      ...
                      $u_q = "SELECT d.user_id,CONCAT_WS(' ',p.first_name,p.last_name, '[',r.rolename,']') FROM `user_dealer_retailer` d INNER JOIN person p ON d.user_id=p.id INNER JOIN _role r ON p.role_id=r.role_id  WHERE d.dealer_id = '" . $_SESSION[SESS . 'data']['dealer_id'] . "'";
                        ...
                        echo getDDList($sm_data,$value['user_id'],$attr);
                  ...
                  <tr>
                    <td>
                      ...
                      <?php
                        ...
                        $attr = array(
                                  "name"=>"retailer_id[$order_id]",
                                  "id"=>"retailer_id$order_id",
                                  "class"=>"chosen-select"
                                  );

                        echo getDDList($ret_data,$value['retailer_id'],$attr);
            ...
            <div id="product">
              <table width="100%" id="mytableabc" class="table<?php echo $value['order_id']?>">
                  <tr style="font-weight:bold;">
                    ...
                    <td style="width:10px" class="options">#</td>
                  </tr>
                ...
                if (!empty($value['order_item']))
                {
                  ...
                  /* $q = 'SELECT mrp,retailer_rate as rate from `product_rate_list` cp WHERE  state_id = '.$_SESSION[SESS.'data']['state_id'].' AND cp.product_id = '.$product_id.' Group by rate';
                   ...
                   $avl_item=$invalue['aval_qty']; //$myobj->calculate_available_stock($invalue['product_id']);
                 ...
                 <input type="hidden" name="order_id[<?=$value['order_id'];?>][]" value="<?php echo $value['order_id']; ?>" class="oid"></td>
    ...
    $prod_q = 'SELECT cp.id, cp.name from catalog_product cp INNER JOIN stock s ON cp.id = s.product_id where s.dealer_id = '.$_SESSION[SESS.'data']['dealer_id'].' ORDER BY cp.id';
     ...
     $attr = array(
               "name"=>"product_id[$o_id]",
               "id"=>"id$i",
               "class"=>"item_details chosen-select",
               "style"=>"width:270px;"
               );

     echo getDDList($prod_data,$prod_id,$attr);
    ?> 
   <!--  <input type="hidden" id="p<?php echo $i; ?>" name="product_id[<?=$o_id;?>][]" value="<?php echo $prod_id; ?>" class="item_details"> -->
    ...
    <td>
      ...
      <?php 
        ...
        $mrp = max($mrp_list);
        ?>
      <select style="width:50px;" name="mrp[<?=$value['order_id'];?>][]" class="mrp mrp_dd calcpk" lang="mrp" id="mrp<?php echo $i; ?>">
        <?php 
              foreach($mrp_list as $m_val)
              {
                 $selected = ($m_val==$invalue['mrp']) ? "selected":"";
                 echo "<option value='$m_val' $selected>$m_val</option>";
              }
         ?>
      </select>
    ...
    <td><input id="ch_qty<?php echo $i; ?>" class="ch_qty quantitycl calcpk" type="text" lang="quantity" name="quantity[<?=$value['order_id'];?>][]" value="<?php echo $qty; ?>"></td>
    <?php
    ...
    <td><input readonly id="r<?php echo $i; ?>" type="text" class="rate" name="rate[<?=$value['order_id'];?>][]" value="<?php echo $rates ?>"></td>
    ...
    <td>
      <input type="text" id="trade_disc_val<?php echo $i; ?>" name="trade_disc_val[<?=$value['order_id'];?>][]" value="<?php if(!empty($trade)) echo $trade;  ?>" class="trade_disc_val calcpk"/>
    </td>   
    ...
    <td><input type="text" id="cd<?php echo $i; ?>" name="cd[<?=$value['order_id'];?>][]" value="<?php if(!empty($cd)) echo $cd;  ?>" class="cd calcpk"></td>
              ...
              <td colspan="9">
                <table width="100%">
                 ...
                 <tr>
                  <td align="right">
                   <strong>
                     <input  style="width:98%" type="text" name="total_td[<?php echo $key ?>]" class="total_td_kp<?php echo $key?>" id="total_td<?php echo $key?>" value="<?php  echo $total_td; ?>" readonly></strong>
                   </td>
                   <td align="right"><strong>
                     <input  style="width:98%" type="text" name="total_cd[<?php echo $key ?>]" class="total_cd_kp<?php echo $key?>" id="total_cd<?php echo $key?>" value="<?=$total_cd?>" readonly></strong>
                   </td>
                   <td align="right"><strong>
                     <input  style="width:98%" type="text" name="total_taxable[<?php echo $key ?>]" class="total_tx_kp<?php echo $key?>" id="total_taxable<?php echo $key?>" value="<?=round(($total-$total_vat),2)?>" readonly></strong>
                   </td>
                   <td align="right"><strong>
                     <input  style="width:98%" type="text" name="total_vat[<?php echo $key ?>]" class="total_vat_kp<?php echo $key?>" id="total_vat<?php echo $key?>" value="<?=round($total_vat,2)?>" readonly></strong>
                   </td>
                   <td align="right"><strong>
