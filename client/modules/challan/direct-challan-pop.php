...
$dealer_id_cus =  $_SESSION[SESS.'data']['dealer_id'];
//pre($_SESSION);
//if(empty($dealer_id))
...
// die;
    if (valid_token($_POST['hf'])) { // checking if post value is same as timestamp stored in session during form load
        ...
        if ($checkpass) {
            ...
            if($dealer_id_cus == '1170'){
                $action_status_cus = $myobj->direct_challan_receieve_save(); // $myobj->item_category_save()
            }
...
function total_amount_with_discount(){
    ...
    document.getElementById('total_amount_a').value = totalamt.toFixed(2);
            ...
            <div>
	    <input type="radio" name="bedStatus" class="reloaded" id="direct" checked="checked" value="direct">Generate Invoice
            <input type="radio" name="bedStatus" id="retail" class="reloaded" value="retail">Generate Retail Invoice
...
<!--                    <th style="background-color:#C7CDC8; color:#000;">Sch. Quantity</th>-->
                ...
                <td width="20%">
                 ...
                 <td>                     
                     <select  style="width:70px" name="mrp[]" class="mrp mrp_dd calcpk" id="mrp<?=$z?>" placeholder="MRP">
...
<!--                 <td>                    
                 ...
                 <td style="width:50px">
                    ...
                    <td style="width:50px">
                        <input style="width:98%" placeholder="Qty" onkeypress="javascript:return isNumber(event)" type="text"  name="quantity[]" id="quantity" value="<?=$_POST['challan_item'][$keys[$z-1]]['qty']?>" class="quantitycl calcpk"/ required>
                    </td>
                    <td>
                      <!-- onblur="total_amount_invoice()" -->
                        <input style="width:98%" placeholder="Sch. Qty" type="text" class="quantityfr" name="scheme[]" id="scheme" value="<?=$_POST['challan_item'][$keys[$z-1]]['free_qty']?>" />
                    </td>
                    <td style="width:60px">
                                     ...
                                     <td colspan="16" align="right" style="border-bottom:1px solid #efefef;">
                                      <table>
                                        <tr>
                                          ...
                                          <td><strong>
                                            <input  style="width:98%" type="text" name="final_amount_a" id="final_amount_a" value="0" readonly>    </strong></td>
                                          </tr>
                                        </table>                                       
                                      </td>                     
                                    </tr>               
...
$(document).on('click', '.addrow', function () {
  ...
  var pid = tr.find('.item_details').chosen().val();
  ...
  $c_row.find('.item_details').attr('id','product_id'+j);
  ...
  tr.find('.item_details').removeClass('chzn-done').css({'display':'block'}).removeAttr('id').val('').next('div').remove();
  tr.find('.item_details').chosen();
