    ...
    if (valid_token($_POST['hf'])) { // checking if post value is same as timestamp stored in session during form load
        ...
        if ($checkpass) {
            ...
            if (!empty($_POST['division'])) {
                $ifilter[] = "catalog_product.division=".$_POST['division'];
            }

            $filter[] = "ch_dealer_id = '{$_SESSION[SESS.'data']['dealer_id']}'";
            $filterused = implode($filterstr, '<span style="margin: 0 10px;">|</span>');
            $rs = $myobj->$funcname($filter, $records = '', $orderby = "ORDER BY $myorderby",$ifilter); // get_challan_list
            // pre($rs);
...
<?php echo arr_pulldown('datepref', $mymatch['datepref'], '', true, true, '', false, 'Select Pref Date'); ?>
                    ...
                    <td width="10%">Division<br />
                        <?php 
                        $q = "SELECT id,type from _product_division";
            ...
            <div class="col-xs-12">
                ...
                <div class="table-header">
                    ...
                    <div class="pull-right tableTools-container">
                        <!-- <input type="button" onclick="tableToExcel('dynamic-table', 'Division Wise')" value="Export to Excel" class="btn btn-success" style="height: 38px;"> -->
                    </div>
                </div>
                                    ...
                                    <tr style="font-weight:bold;">
                                     ...
                                     foreach ($rows['challan_item'] as $inkey => $invalue) {
                                        // pre($invalue);
                                         ...
                                         $ta += $invalue['taxable_amt'];
                                         $new_dis += $invalue['cd_amt']+$invalue['dis_amt'];
                                         ?> 
                                         <style>tr.bordered {border-bottom: 1px solid #000;}</style>
                                         <tr class="bordered">
                                           <td style="border:1px;"><strong>
                                            <?=$invalue['name']?></strong></td>  
                            ...
                            <td>
                                <strong>
                                    <?php 
                                        if($_POST['division']!='')
                                        {
                                            echo my2digit($new_dis);
                                        }else{
                                            echo my2digit($rows['discount_amt']);
                                        }
                                     ?>
                                </strong>
                            </td>
