...
$myorderby = 'damage_order_depot.ch_no DESC'; // The orderby clause for fetching of the data
$myfilter = 'damage_order_depot.id = '; //the main key against which we will fetch data in the get_item_category_function
...
if (isset($_POST['filter']) && $_POST['filter'] == 'Filter') {
    if (valid_token($_POST['hf'])) { // checking if post value is same as timestamp stored in session during form load
        ...
        if ($checkpass) {
            ...
            if (!empty($_POST['csa_id'])) {
                $filter[] = "ch_csa_id = '".$_POST['csa_id']."'";
...
if (isset($_GET['actiontype'])) {
    switch ($_GET['actiontype']) {
        case'print':
            require_once('challan-print-damage-depot.inc.php');
            exit();
            break;
        default:
...
<?php echo arr_pulldown('datepref', $mymatch['datepref'], '', true, true, '', false, 'Select Pref Date'); ?>
  ...
  /*$q = "SELECT retailer.id as id ,concat(retailer.name,' [',location_5.name,']') as `name` from location_5 INNER JOIN retailer ON retailer.location_id = location_5.id where dealer_id='$dealer_id' order by retailer.name asc";*/
     $q = "SELECT c_id as csa_id ,csa_name as csaname  FROM `csa` inner join dealer on dealer.csa_id=csa.c_id where dealer.id='$dealer_id1'";
      db_pulldown($dbc, 'csa_id', $q, true, true, 'id="csa_id" class="chosen-select less"','==Please Select==',$_POST['csa_id']);
                        ?>
                    </td>
                    <td width="10%">Invoice No<br />
                        ...
                        <thead>                           
                         ...
                         <tbody>
                            ...
                            foreach ($rs as $key => $rows) {
								 $damage_type=$rows['damage_order_type_id'];
                                if($damage_type==1)
                                {
                                    $damage_cat='Saleable';
                                }
                                ...
                                {
                                 $damage_cat='Non Saleable';
 
                                } 
                                $bg = ($bg == TR_ROW_COLOR1 ? TR_ROW_COLOR2 : TR_ROW_COLOR1); // to provide different row colors(member_contacted table)
                                $uid = $rows['id'];                
                                $uidname = $rows['ch_no'];                              
                                 $frz_date = date("d-m-Y", strtotime($rows['ch_date'].'+2 days'));
                                 $created_date = date("d-m-Y", strtotime($rows['date_added']));
...
//$rows['postat'] == 1 ? '<span class="seperator">|</span> <a class="iframef" title="print PO '.$uidname.'" href="index.php?option='.$formaction.'&showmode=1&mode=1&id='.$uid.'&actiontype=print"><img src="../icon-system/i16X16/print.png"></a>' : '';
                            ...
                            ?>
                                ...
                                <td>
                                 ...
                                 <td>
                                    <?=($created_date!='01-01-1970')?$created_date:'--'?>
                                </td>
                                 <td style="width:15%">
                                ...
                                <td>
                                   <table style="border:1px; border-collapse:collapse">
                                    ...
                                    <td style="border:1px; background-color:#1E82DA; color:#fff; padding:3px;">Mfg Date</td> 
                                     <td style="border:1px; background-color:#1E82DA; color:#fff; padding:3px;">Rate</td>   
                                                                 
                                     <td style="border:1px; background-color:#1E82DA; color:#fff; padding:3px;">Qty</td>
                                      <td style="border:1px; background-color:#1E82DA; color:#fff; padding:3px;">Free</td>                               
                                 <!--    <td style="border:1px; background-color:#1E82DA; color:#fff; padding:3px;"> TD.Amt </td>
									 ...
									 <td style="border:1px; background-color:#1E82DA; color:#fff; padding:3px;">G Value</td>
                                     ...
                                     foreach ($rows['challan_item'] as $inkey => $invalue) {
										...
										if (!empty($invalue['free_qty'])) {
                                             $freeqty = $invalue['free_qty'];
                                         } else{
                                             $freeqty =0;
                                         }
                                      ...
                                      <!--    <td style="border:1px;text-align: center;"><?=my2digit($invalue['dis_amt'])?></td> 
                                           <td style="border:1px;text-align: center;"><?=my2digit($invalue['cd_amt'])?></td>-->
											<td style="border:1px;text-align: center;"><?=my2digit($invalue['product_rate']*$invalue['qty'])?></td>
                            ...
                            <td><strong><?=round($rows['amount'])?></strong></td>
