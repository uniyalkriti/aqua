...
$formaction = $p;
$myobj = new dealer_sale_damage_depot();
...
<!--                    <td width="7%">
    ...
    ?>
                ...
                $query = "select `ch_no` from `damage_order_depot` where `ch_dealer_id`=$dealer_id1 order by `ch_no` DESC";
          ...
          <input type="hidden" name="ch_no_prifix" value="<?php echo 'BDN/' . $sesId = $_SESSION[SESS . 'data']['dealer_id'] . '/'; ?>">
                    ...
                    <td> 
                        <span class="star">*</span><strong>Depot Name</strong><br>
                        
<?php



//$qrt = "SELECT csa.* ,dealer.id as dealer_id, dealer.name as dealer_name FROM `csa` inner join dealer on dealer.csa_id=csa.c_id  ";
 $qrt="SELECT c_id as csa_id ,csa_name as csaname  FROM `csa` inner join dealer on dealer.csa_id=csa.c_id where dealer.id='$dealer_id1'";
//h1($qrt);
db_pulldown($dbc, 'csa_id', $qrt, true, true, 'id="csa" class="form-control chosen-select retailer"  lang="Retailer" ','=====Please Select=====',$_POST['retailer_id']);
?>
 ...
 <input type="hidden" class="pkigst" value="0">
                   ...
                   <td><span class="star">*</span><strong>Saleable/Non-Saleable</strong><br>
                                <?php
            ...
            <table width="100%" id="mytable" class="">
                <tr class="thead" style="font-weight:bold;">
                   ...
                   <!--  <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Bill of Supply</th> -->
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">M.R.P</th>
...
<!--                    <th style="background-color:#C7CDC8; color:#000;">Sch. Quantity</th>-->
                    ...
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Mfg Date</th>
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Batch No.</th>                  
                ...
                <td width="20%">
                 <?php
                   ...
                   $q = 'SELECT cp.id, CONCAT_WS(" ",cp.name,"[",oc.cp_short_name,"]","[",SUM(s.qty),"]") from catalog_product cp INNER JOIN stock s ON cp.id = s.product_id INNER JOIN other_company oc ON cp.company_id=oc.id where s.dealer_id = '.$_SESSION[SESS.'data']['dealer_id'].'  GROUP BY s.product_id ORDER BY cp.id';
                    // h1($q);
                    db_pulldown170($dbc,  'product_id[]', $q, TRUE, TRUE,'id="product_id'.$z.'" class="item_details_damage chosen-select" ','',$_POST['challan_item'][$keys[$z-1]]['product_id']);
                    ?>   
                </td>                
                 <!-- <td style="text-align: center;"><input type="checkbox" name="bos[<?php echo ($z-1) ?>]" value="1"></td> -->
                 
<!--                 <td>                     
                     <select  style="width:70px" name="mrp[]" class="mrp mrp_dd_damage calcpk" id="mrp<?=$z?>" placeholder="MRP">
                   <option value="<?=$_POST['challan_item'][$keys[$z-1]]['mrp']?>" selected="selected"><?=$_POST['challan_item'][$keys[$z-1]]['mrp']?></option>
                    </select>
                 </td>-->
                <td style="width:50px">
                 <input style="width:98%" placeholder="mrp" id="mrp<?=$z?>" type="text" name="mrp[]"  value="<?=$_POST['challan_item'][$keys[$z-1]]['mrp']?>"  class="mrp mrp_dd_damage calcpk"/>    
...
<!--                 <td>                    
                 ...
                 <td style="width:50px">
                    ...
                    <td  style="width:50px" class="hidden">
                        ...
                        <input style="width:98%" placeholder="Trade" type="hidden" name="trade_disc_val[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['dis_percent']?>"  class="trade_disc_val calcpk" onkeypress="javascript:return isNumber(event)"/>

                        <input style="width:98%" placeholder="Trade Amt." type="hidden" name="trade_disc_amt[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['dis_amt']?>" class="trade_disc_amt" readonly />
                        <input type="hidden" name="ttl_amt[]" value="<?=$ttl_amt?>"   />
                    <td style="width:50px">
                        <input style="width:98%" placeholder="Mfg Date" type="text" name="mfg_date[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['mfg_date']?>"  class="datepicker"/>
                        <input type="hidden" name="ttl_amt[]" value="<?=$ttl_amt?>"   />
                    </td>
                    <td style="width:50px">
                        <input style="width:98%" placeholder="Batch No." type="text" name="batch_no[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['batch_no']?>"  />
                        <input type="hidden" name="ttl_amt[]" value="<?=$ttl_amt?>"   />
                    </td>
                    ...
                    <td>
                      <input style="width:98%" placeholder="Spl Amt." type="text" name="spl_disc_amt[]" value="<?=$_POST['challan_item'][$keys[$z-1]]['dis_amt']?>" class="spl_disc_amt" readonly />                
                    </td>
                    <td style="width:50px">
...
$(document).on('click', '.addrow', function () {
  ...
  var j = $("#mytable").find('.item_details_damage').length;
  j = j*timestamp;

  var tr = $(this).closest('tr');
  var $c_row = tr.clone();

  var pid = tr.find('.item_details_damage').chosen().val();
  var mrp = tr.find('.mrp').val();
  var tds = tr.find('.trade_disc_type').val();
  var ct  = tr.find('.cd_type').val();

  $c_row.find('.item_details_damage').removeClass('chzn-done').css({'display':'block'}).removeAttr('id').val('').next('div').remove();
  $c_row.find('td:last').html('<img title="Less" src="images/less.png" class="removebutton calcpk">');
  $c_row.find('.item_details_damage').attr('id','product_id'+j);
  ...
  tr.find('.item_details_damage').removeClass('chzn-done').css({'display':'block'}).removeAttr('id').val('').next('div').remove();
  tr.find('.item_details_damage').chosen();
...
$(document).on('click', '.removebutton', function () { 
     var tot_row = $('#mytable').find('.item_details_damage').length;
...
//                    }
    ...
    function custum_function(pid, pvalue, event) {
        ...
        get_damageer_rate(pid, pvalue, event);
        setTimeout(function() {
...
function get_damageer_rate(pid, pvalue, event){
...
<script>
$('.datepicker').datepicker({ minDate: 0,dateFormat: 'dd/M/yy'});
</script>
