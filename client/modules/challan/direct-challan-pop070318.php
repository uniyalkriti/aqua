...
<!--                    <th style="background-color:#C7CDC8; color:#000;">Sch. Quantity</th>-->
                    ...
                    <th style="background-color:#C7CDC8; color:#000;border:1px solid black;">Spl Spl Amt.</th>
                ...
                <td width="20%">
                 <?php
                   ...
                   $q = 'SELECT cp.id, CONCAT_WS(" ",cp.name,"[",oc.cp_short_name,"]","[",SUM(s.qty),"]") from catalog_product cp INNER JOIN stock s ON cp.id = s.product_id INNER JOIN other_company oc ON cp.company_id=oc.id where s.dealer_id = '.$_SESSION[SESS.'data']['dealer_id'].' AND s.qty>0 GROUP BY s.product_id ORDER BY cp.id';
                    // h1($q);
...
<!--                 <td>                    
                 ...
                 <td style="width:50px">
                    ...
                    <td style="width:60px">
                     <input style="width:98%" placeholder="Rate" type="text" name="rate[]" id="rate" class="rate" value="<?=$_POST['challan_item'][$keys[$z-1]]['product_rate']?>" readonly/>
                    </td>
                    <td  style="width:50px">
