...
<!--                 <td> &nbsp;Mfg Date.<br><span class="example">(dd/mm/yyyy)</span></td>
  ...
  ?>
                ...
                <!--                 <td>
                  ...
                  <td>
                      <input type="text" style="width:100px" placeholder="MRP" name="base_price[]"  value="<?=$row1['rate']?>" readonly/>
                 </td>
...
<td>  </td>
 ...
 <?php 
                db_pulldown($dbc , 'product_id[]', "SELECT catalog_product.id,catalog_product.name FROM catalog_product INNER JOIN catalog_3 ON catalog_product.catalog_id = catalog_3.id  WHERE catalog_product.company_id = 1 order by name",TRUE,TRUE,' id="pproduct_id'.$count1.'" class="nkget_dealer_rate chosen-select" data-placeholder="Select an item"');
                      ?>
