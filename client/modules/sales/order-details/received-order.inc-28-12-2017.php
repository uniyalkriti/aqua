...
<script>
function showamt(str,id) {
    //alert(str); alert(id);
       var res = id.slice(10);
       //alert(res);
       var nextid = 'dealer_rate'+res;
       var product = Number(document.getElementById('pproduct_id'+res).value);
       var csa = document.getElementById('csa').value;
      //alert(product);
  if (str=="") {
    document.getElementById(nextid).value="0";
    return;
  }
  ...
  xmlhttp.onreadystatechange=function() {
    if (this.readyState==4 && this.status==200) {
      document.getElementById(nextid).value=this.responseText;
    }
  }
  //var a = "../client/sales/order-details/get_amount.php?q="+str;
 var a = "modules/sales/order-details/get_amount.php?q="+str+"&product="+product+"&csa="+csa;
  //alert(a);
...
//                   }
   ...
   <input type="hidden" name="company_id" value="1" id="state_id">
        ...
        db_pulldown($dbc, 'csa_id', $q, TRUE, TRUE,'id = "csa" style="width:200px;height:25px;"','','');
        ?>
                  </td>
<div class="col-xs-2">
            ...
            //pre($_POST['order_item']);
              foreach ($_POST['order_item'] as $inkey=>$invalue) { 
                ...
                <tr class="tdata <?php echo $class ?>">
                 ...
                 <td>
            <?php 
         db_pulldown170($dbc , 'product_id[]', "SELECT id,name FROM catalog_product ",TRUE,TRUE,' id ="pproduct_id'.$inc.'"data-type="2" class="nkget_dealer_rate item_details chosen-select" data-placeholder="Select an item"', '', $invalue['product_id']); 
               ?>

                 <?php 
                    //  db_pulldown170($dbc , 'product_id[]', "SELECT id,name FROM catalog_product ",TRUE,TRUE,' id ="product_id'.$inc.'"  onchange="fetch_location(this.value,\'progress_div\',\'zone_id\',\'get-locality\');" class="nkget_dealer_rate item_details chosen-select" data-placeholder="Select an item"', '', $invalue['product_id']); 
                      ?>
...
<!--                 <td>
                 <input type="text" name="batch_no[]"  value="<?php echo $invalue['batch_no']; ?>"  />
                  ...
                  <td>
                      <?php
                     $where = "id = ".$invalue['product_id'];
                     $division = myrowval('catalog_product', 'division', $where);
                     //echo  $division;
                     $div = $_SESSION[SESS.'data']['csa_margin'][$division]['margin']
                      ?>
  <input type="hidden" style="width:100px" class="nkget_dealer_division_rate" name="division[]" id='division<?=$inc?>'  value="<?=$div?>"  />
    <?php 
     $csa_id = $_SESSION[SESS . 'data']['csa_id'];
        $q = "SELECT mrp as id, mrp as name from ss_margin_per INNER JOIN ss_margin_wise_rate
            ON ss_margin_wise_rate.margin_id = ss_margin_per.margin_id WHERE ss_margin_per.ss_id=$csa_id AND product_id = $invalue[product_id] AND ss_margin_wise_rate.status='1' order by ss_margin_wise_rate.id desc";
    //h1($q);
    db_pulldown170($dbc , 'base_price[]', "SELECT mrp as id, mrp as name from ss_margin_per INNER JOIN ss_margin_wise_rate
            ON ss_margin_wise_rate.margin_id = ss_margin_per.margin_id WHERE ss_margin_per.ss_id=$csa_id AND product_id = $invalue[product_id] AND ss_margin_wise_rate.status='1' order by ss_margin_wise_rate.id desc",TRUE,FALSE,'onchange=showamt(this.value,this.id) id ="base_price'.$inc.'" class="nkget_dealer_mrp_rate" ','', $invalue['mrp']);
         ?>
               ...
               <td>
                    <input type="text" class="nkset_dealer_rate" style="width:100px" placeholder="Landing Price" name="dealer_rate[]"  id="dealer_rate<?=$inc?>"
...
<!--                  <td>
                   ...
                   <td><img  title="more" src="images/more.png" onclick="addnewrow()"/>
                   <img title="less" src="images/less.png" onclick="javascript:addmore22_2('mytable', event);">
                   </td>
...
<!--  <script src="assets/js/jquery-2.1.4.min.js"></script>-->
 <script src="assets/js/jquery.dataTables.min.js"></script>
        ...
        <script type="text/javascript">
                      ...
                      {
                         var table = $('#mytable');
                         var tb = table.find('input[name="quantity[]"]').length;
                         var i = parseInt(tb)+1;
                         ...
                         $c_row.find('.nkget_dealer_mrp_rate').attr('id','base_price'+i);
                         $c_row.find('.nkset_dealer_rate').attr('id','dealer_rate'+i);
                         $c_row.find('.nkget_dealer_division_rate').attr('id','division'+i);
                         
                         $c_row.find('.datepicker').datepicker();
