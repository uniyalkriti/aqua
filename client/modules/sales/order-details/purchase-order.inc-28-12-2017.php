...
<div id="workarea" style="height: 500px;">
    <?php
...
//                 $query = "SELECT focus.product_id,catalog_product.division as division,name,cprl.mrp as rate,cprl.product_id as pid 
//                     FROM `focus` 
//                     INNER JOIN catalog_product ON catalog_product.id = focus.product_id
//                   WHERE product_status ='1' GROUP BY focus.product_id";
                 
                  $query = "SELECT focus.product_id,catalog_product.division as division,name,catalog_product.id as pid 
                     FROM `focus` 
                     INNER JOIN catalog_product ON catalog_product.id = focus.product_id
                   GROUP BY focus.product_id";
                // h1($query);
                 ...
                 while($row1 = mysqli_fetch_assoc($rr))
                 {
                    ...
                    $csa = $_SESSION[SESS . 'data']['csa_id'];
                    $dealer_id = $_SESSION[SESS.'data']['dealer_id'];
                   $q = "SELECT mrp as rate,dealer_rate from ss_margin_per INNER JOIN ss_margin_wise_rate
            ON ss_margin_wise_rate.margin_id = ss_margin_per.margin_id WHERE ss_margin_per.ss_id=$csa AND product_id = $product_id1 order by ss_margin_wise_rate.id desc limit 1";
            $rrow = mysqli_query($dbc,$q);
            $fetch = mysqli_fetch_assoc($rrow);
                   // $reobj = new report();
                   // $getstock = $reobj->get_available_stock($product_id1,$dealer_id);
                    $where = "dealer_id=$dealer_id AND product_id=$product_id1 AND mrp=$fetch[rate]";
                  ...
                  //   $disc = $newmrp*$div/100;
                     $landing_price = $fetch['dealer_rate'];
  ...
  <strong><?=$product_name?></strong>
                  ...
                  <td>
                      <input type="text" style="width:100px" placeholder="MRP" name="base_price[]"  value="<?=$fetch['rate']?>"  />
                 </td>
           
                 <td>
                    
                    <input type="text" class="nkset_dealer_rate" style="width:100px" placeholder="Landing Price" name="dealer_rate[]"  
                     value="<?php echo $landing_price; ?>" readonly/>
<!--                     <input type="text" class="nkset_dealer_rate" style="width:100px" placeholder="Landing Price" name="dealer_rate[]"  
...
<!--                 <input type="text" name="cases[]"  value=""  />-->
 ...
 <?php 
                db_pulldown($dbc , 'product_id[]', "SELECT catalog_product.id,catalog_product.name FROM catalog_product INNER JOIN catalog_2 ON catalog_product.catalog_id = catalog_2.id WHERE catalog_product.company_id = 1 order by name",TRUE,TRUE,' id="pproduct_id'.$count1.'"data-type="1" class="nkget_dealer_rate chosen-select" data-placeholder="Select an item"');
                      ?>
                 ...
                 <td>
                     <input type="text" style="width:120px" id="quantity" placeholder="Quantity" name="quantity[]" onkeypress="javascript:return isNumber(event)" onblur="qty_scheme_calculate();" placeholder="In Pieces"  value=""  />
                 </td>
...
<!--                 <input type="text" name="cases[]"  value=""  />-->
                 ...
                 <input type="hidden" style="width:100px" placeholder="Batch" name="purchase_inv[]"  value=""  />
                  ...
                  <td class="lasttd">
                    <img title="more" src="images/more.png" onclick="addnewrow();"/>
                 </td>
               </tr>
          <?php }else{ 
              $inc2 = 1;
              ...
              $class = ($inc2==1)?'row1':'';
                ...
                <tr class="tdata <?php echo $class ?>">
                 ...
                 <td>
                   <?php 
                      db_pulldown($dbc , 'product_id[]', "SELECT id,name FROM catalog_product ",TRUE,TRUE,' id ="product_id'.$inc2.'" class="nkget_dealer_rate chosen-select" data-placeholder="Select an item"', '', $invalue['product_id']); ?>
                 </td>
                    ...
                    if(!empty($rows['order_item']))
                     {
                        ...
                        <tr class="bordered">
                          ...
                          <td style="border:none;width:120px;text-align:center" >'.my2digit($invalue['rate']).'</td>
...
<!--                              <td style="border:none;width:120px;text-align:center">'.$invalue['scheme_qty'].'</td>-->
                         <td style="border:none;width:120px;text-align:center">'.$invalue['cases'].'</td>
                          <td style="border:none;width:120px;text-align:center">₹ '.my2digit($invalue['rate']*$invalue['quantity']).'</td>
