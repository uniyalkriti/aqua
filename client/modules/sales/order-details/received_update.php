...
$myfilter = 'receive_order.order_id ='; //the main key against which we will fetch data in the get_item_category_function
...
if(isset($_POST['submit']) && $_POST['submit'] == 'Save')
{
  if(valid_token($_POST['hf'])) // checking if post value is same as timestamp stored in session during form load
  {
    ...
    if($checkpass)
    {
      ...
      $funcname = 'update_'.$cls_func_str.'_save'; 
     // echo $funcname;die;
...
{  
  ...
  if(valid_token($_POST['hf'])) // checking if post value is same as timestamp stored in session during form load
  {
    ...
    if($checkpass)
    {
      // triming and stripslashing post data if required and also passing it through mysqli_real_escape_string
      magic_quotes_check($dbc, $check=true);
      $funcname = 'update_'.$cls_func_str.'_edit';
      // echo $funcname;die;
...
$rs = array();
$filterused = '';
$funcname = 'update_'.$cls_func_str.'_list';
   ...
   <input type="hidden" name="company_id" value="1" id="state_id">
         ...
         <td><span class="star">*</span><strong>Challan date</strong><br>
                  ...
                  <td><span class="star">*</span><strong>Challan Number</strong><br>
                      ...
                      <input style="width:200px;height:25px;" type="text" style="width:100px" name="challan_no"  value="<?php  echo $mystat[$id]['challan_no'];  ?>" required="required" />
                 </td>
                 <td>
                   <!-- <span class="star">*</span> --><strong>GRN</strong><br>
                   <input style="width:200px;height:25px;" type="text" style="width:100px" name="grn" value="<?=$mystat[$id]['grn'];  ?>"/>
                 </td>
 ...
 <?php 
                ...
                <!--                 <td>
                 ...
                 <td>
                  ...
                  <input type="text" style="width:100px" placeholder="Rate per unit" name="dealer_rate[]" onkeypress="javascript:return isNumber(event)" value="" readonly/>
...
<!--                  <td>
          ...
          <?php }else{ 
            ...
            //  pre($_POST['order_item']);
              foreach ($mystat[$id]['order_item'] as $inkey=>$invalue) { 
                $class = ($inc==1)?'row1':'';  ?>
                <tr class="tdata <?php echo $class ?>">
                 <td class="myintrow"><?=$inc?></td>
                 <td>
             <?php 
                      db_pulldown170($dbc , 'product_id[]', "SELECT id,name FROM catalog_product where id=$invalue[product_id] ",TRUE,TRUE,' id ="product_id'.$inc.'" class="nkget_dealer_rate item_details chosen-select" data-placeholder="Select an item"', '', $invalue['product_id']); ?>
                 </td>
<!--                 <td>
                 <input type="text" name="batch_no[]"  value="<?php echo $invalue['batch_no']; ?>"  />
                 </td>-->
                  <td>
                      <input type="text" style="width:100px" placeholder="MRP" class="nkget_dealer_mrp_rate" name="base_price[]"  value="<?php echo $invalue['mrp']; ?>"  readonly/>
                 </td>

               <td>
                    <input type="text"  style="width:100px" placeholder="Rate per unit" name="dealer_rate[]" onkeypress="javascript:return isNumber(event)" 
                     value="<?=$invalue['pr_rate']?>"  required/>
                 </td>
  ...
  <td>
      <input type="hidden" style="width:120px" id="quantity"  onkeypress="javascript:return isNumber(event)" placeholder="Quantity" name="quantity[]" lang="quantity" onblur="qty_scheme_calculate();" placeholder="In Pieces"   value="<?php echo $invalue['quantity']; ?>" readonly />
                 </td>                 
                   
                  <td>
                      <input type="text" style="width:100px" placeholder="In Cases" name="cases[]" onkeypress="javascript:return isNumber(event)" value="<?php echo $invalue['cases']; ?>" readonly />
                 </td>
                  <td>
                      <input type="text" style="width:100px" placeholder="Batch Number" name="purchase_inv[]" lang="Batch Number"  value="<?php echo $invalue['batch_no']; ?>" readonly />
                 </td>
