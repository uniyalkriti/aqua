      ...
      <fieldset style="height: 420px; overflow-y: scroll;">
       ...
       <img src="./images/green.jpeg" style="width:25px; height: 25px"/>
                ...
                <td>
                  ...
                  <?php 
                  $bid = (isset($row1['brand_id']))?$row1['brand_id']:1;
                  $bq = "SELECT id,cp_name,cp_short_name FROM other_company WHERE dealer_id=$dealer_id";

                  $bd = mysqli_query($dbc,$bq);
                  $opt = "<select name='brand_id' id='brand_id' style='width:200px;height:25px;'>";
                                      //$opt .= "<option>==Select Brand==</option>";
                ...
                <td><strong>Order date</strong><br>
                 ...
                 <td class="depot"><strong>Depot/C&F Name</strong><br>
                     <?php
...
<!--                 <td> &nbsp;Mfg Date.<br><span class="example">(dd/mm/yyyy)</span></td>
               ...
               //     pre($row);$primary_sale->get_dealer_landing_rate($row1['pid']);
                  /*$dealer_landing_price = $primary_sale->get_dealer_landing_rate($row1['pid']);*/
                   ...
                   // $getstock = $reobj->get_available_stock($product_id1,$dealer_id);
                    ...
                    $where1 = "dealer_id=$dealer_id AND product_id=$product_id1";
                    $f_minlabel = myrowval('threshold','qty',$where1);
                    
                    ?>
                 
                   <tr class="tdata depot" bgcolor="#32c403">
  ...
  <strong><?=$product_name?></strong>
                 ...
                 <td>
                   <center> <strong><?=$f_minlabel?></strong></center>
                 </td>
 ...
 <?php 
               $pdq = "SELECT catalog_product.id,CONCAT_WS(' ',catalog_product.name,'[',other_company.cp_short_name,']') FROM catalog_product INNER JOIN catalog_3 ON catalog_product.catalog_id = catalog_3.id INNER JOIN other_company ON catalog_product.company_id=other_company.id WHERE catalog_product.company_id >= 1 order by catalog_product.name";
                db_pulldown($dbc , 'product_id[]', $pdq,TRUE,TRUE,' id="pproduct_id" class="nkget_dealer_rate chosen-select"');
                      ?>
                 ...
                 <td><input type="text" readonly="" style="width: 150px;" name="minlab" class="minlab"></td>
                 <td><input type="text" readonly="" style="width: 150px;" name="sih" class="sih"></td>
                 <td class="lasttd">
...
$printlink = '<span class="seperator">|</span> <a class="iframef" title="print Invoice ' . $uidname . '" href="index.php?option=' . $formaction . '&showmode=1&mode=1&id=' . $uid . '&actiontype=print"><img src="../icon-system/i16X16/print.png"></a>'; //$rows['postat'] == 1 ? '<span class="seperator">|</span> <a class="iframef" title="print PO '.$uidname.'" href="index.php?option='.$formaction.'&showmode=1&mode=1&id='.$uid.'&actiontype=print"><img src="../icon-system/i16X16/print.png"></a>' : '';
                     ...
                     echo'       <td style=" display: table-cell; vertical-align: top;">'.$rows['csa_name'].'</td>';
                    echo'     <td  colspan="5">';
                        echo'
        ...
        <script>
          $(document).ready(function()
          {
            ...
            $('#mytable').on('click','.addnewrow',function(){
              ...
              var brand = $('#brand_id');
              var bsn = $('option:selected', brand).attr('sht-name');
              var bid = brand.val();
