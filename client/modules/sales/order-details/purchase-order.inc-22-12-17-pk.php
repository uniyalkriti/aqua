  ...
  if(valid_token($_POST['hf'])) // checking if post value is same as timestamp stored in session during form load
  {
    ...
    if($checkpass)
    {
      ...
      if(!empty($_POST['from_date'])){
        ...
        $filter[] = "DATE_FORMAT(`created_date`,'".MYSQL_DATE_SEARCH."') >= '$start'";
        $filterstr[] = '<b>Start : </b>'.$_POST['from_date'];
      }
      if(!empty($_POST['to_date'])){
        $end = get_mysql_date($_POST['to_date'],'/',$time = false, $mysqlsearch = true);
        $filter[] = "DATE_FORMAT(`created_date`,'".MYSQL_DATE_SEARCH."') <= '$end'";
          ...
          <?php if(!isset($heid)){ ?>
               ...
               // pre($_SESSION);
                ...
                $count1 = 0; /*$dealer_landing_price = 0;*/
                 $query = "SELECT focus.product_id,name,cprl.mrp as rate,cprl.product_id as pid,cprl.dealer_rate FROM `focus` 
  ...
  ?>
                ...
                <!--                 <td>
                 ...
                 <td>
                     ...
                     value="<?php echo $row1['dealer_rate'] ?>" readonly/>
                 </td>
                  <td>
