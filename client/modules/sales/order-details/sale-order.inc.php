...
$myobj = new sale();
$dealer = new dealer();
$cls_func_str = 'sale_order'; //The name of the function in the class that will do the job
		...
		if($checkpass)
		{
			...
			if(!empty($_POST['to_date'])){
				...
				$filterstr[] = '<b>Order No  : </b>'.$_POST['order_no'];
                         ...
                         $user_data = $myobj->get_user_wise_sale_data($sesId , $role_id);
                         if(!empty($user_data)){
...
function hideDiv(value)
{
     ...
     <form method="post" action="<?php if(isset($eformaction)) echo  $eformaction; ?>" class="iform" name="genform" onsubmit="return checkuniquearray('genform');" enctype="multipart/form-data">
      <fieldset>
        ...
        <table width="100%" border="0" cellspacing="2" cellpadding="2" class="tableform">
         <tr>
              <td><span class="star">*</span>Dealer<br>
                <?php 
                  
                  $dealer_data = $dealer->get_user_wise_dealer_data($sesId , $role_id);
                  if(!empty($dealer_data)){
                  $dealer_data_str = implode(',' ,$dealer_data);
                  $q = "SELECT id,name FROM dealer WHERE id IN ($dealer_data_str) ORDER BY name ASC";
                  db_pulldown($dbc,'dealer_id',$q,true,true,'lang="dealer" onchange="fetch_location(this.value+\'|\'+\''.$loc_level.'\', \'progress_div\', \'location_id\', \'get-location\');" id="dealer_id"'); 
                  
                  }
                  else {
                      echo '<select><option>==No any dealer assign to this person==</option></select>';
                  }
                
                ?>
              </td>
              <td><span class="star">*</span>Location<br>
                  <select lang="location" name="location_id" id="location_id" onchange="fetch_location(this.value, 'progress_div', 'retailer_id', 'get-retailer');">
                      <option>==Please Select==</option>
                      <?php 
                        if(isset($heid))
                            echo option_builder($dbc, $q= "SELECT location_5.id,location_5.name FROM dealer_location_rate_list INNER JOIN location_3 ON location_3.id = dealer_location_rate_list.location_id INNER JOIN location_4 ON location_3.id = location_4.location_3_id INNER JOIN location_5 ON location_5.location_4_id = location_4.id WHERE dealer_id = '$_POST[dealer_id]'", $_POST['location_id']);
               ...
               <td><span class="star">*</span>Retailer Name<br>
                   <select lang="retailer" name="retailer_id" id="retailer_id">
                      ...
                      if(isset($heid)) {
                          ...
                          echo option_builder($dbc, $q="SELECT id,name FROM retailer WHERE retailer.location_id = '$_POST[location_id]' AND id IN ($retailer_data_str)", $_POST['retailer_id']);
                   ...
                   <?php if(!isset($heid)) { ?>
                    ...
                    <?php } else {
                            ...
                            <tr class="tdata">
                                ...
                                <td>
                                 <?php db_pulldown($dbc , 'product[]', "SELECT id,name FROM catalog_product",TRUE,TRUE,'onchange=getajaxdata(\'get_product_details\',\'mytable\',event);','',$value['product_id']); ?></td>
                                <td><input   type="text" name="base_price[]" onblur="product_calculate();" value="<?php echo $value['rate']; ?>"  /></td>
                                <td><input   type="text" name="quantity[]" onblur="product_calculate();" value="<?php echo $value['quantity']; ?>"  /></td>
                                <td><input   type="text" name="scheme[]"  value="<?php echo $value['scheme_qty']; ?>"  /></td>
                  ...
                  <table width="100%" id="mytable1">
                   ...
                   <?php if(!isset($heid)) { ?>
                    ...
                    <?php } 
                           ...
                           $inc = 1;
                           foreach($_POST['gift_item'] as $key=>$value)
                           {
                        ?>  
                        <tr class="tdata">
                          <td class="myintrow">1</td>
                          <td><?php db_pulldown($dbc , 'gift_id[]', "SELECT id, gift_name FROM _retailer_mkt_gift",TRUE,TRUE,'','',$value['gift_id']); ?></td>
                          
                          <td><input   type="text" name="gift_qty[]"  value="<?php echo $value['quantity']; ?>"  /></td>
    ...
    ?>
		...
		$lastinc = (($inc+PGDISPLAY-1) > $pgoutput['totrecords']) ? $pgoutput['totrecords'] : ($inc+PGDISPLAY-1);
		      ...
		      $callstatus = $rows['call_status'] == '1' ? 'Productive' : 'Non Productive';                                 $lat_lng = $rows['lat_lng'];  
                      $mcc = $rows['mccmnclatcellid'];
                      $location = '';
                      if($lat_lng != '0') 
                          $location = '<a class="iframe" href="indexpop.php?option=user-location&mcc='.$mcc.'&lat_lng='.$lat_lng.'"><img width="18px;" height="18px;" src="./images/green.png"></a>';
		      else if($mcc != '0')  $location = '<a class="iframe" href="indexpop.php?option=user-location&mcc='.$mcc.'&lat_lng='.$lat_lng.'"><img width="18px;" height="18px;" src="./images/red.png"></a>';
                      else $location = '<a class="iframe" href="indexpop.php?option=user-location&mcc='.$mcc.'&lat_lng='.$lat_lng.'"><img width="18px;" height="18px;" src="./images/red.png"></a>';
