...
$forma = 'User Bill Performance '; // to indicate what type of form this is
$formaction = $p;
$myobj = new report();
$cls_func_str = 'user_bill_performance'; //The name of the function in the class that will do the job
...
$rs = array();
$filterused = '';
$funcname = 'get_user_bill_performance_list';
...
if ($checkpass) {
    ...
    if(!empty($_POST['from_date'])){
     ...
     $filter[] = "ch_dealer_id = '{$_SESSION[SESS.'data']['dealer_id']}'";
            ...
            $myresult = $myobj->$funcname($filter, $records = '', $orderby = "GROUP BY co.ch_user_id ASC",$ch_filter); // $myobj->get_item_category_list()
            //pre($myresult);
            if (empty($myresult))
    ...
    $myresult = $myobj->$funcname($filter = "$myfilter'$ajaxshowid'", $records = '', $orderby = 'GROUP BY co.ch_user_id ASC');
} else {
    $filter[] = "ch_dealer_id = '{$_SESSION[SESS.'data']['dealer_id']}'";
    $month=date('Ym');
    $filter[] = "DATE_FORMAT(`ch_date`,'%Y%m') = '$month'";
    $myresult = $myobj->$funcname($filter, $records = '', $orderby = 'GROUP BY co.ch_user_id ASC',$ch_filter);
}
...
<!--                                <th>Total Call</th>
                         ...
                         <tbody>
                            ...
                            foreach ($myresult as $key => $rows) {
                                $tot_value=$tot_value+$rows['sale_value'];
                            ?>
                                <tr>
<!--                                <td class="center">
                                    <label class="pos-rel">
