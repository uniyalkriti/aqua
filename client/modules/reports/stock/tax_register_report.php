...
$forma = 'TAX REGISTER REPORT'; // to indicate what type of form this is
$formaction = $p;
$myobj = new dealer_sale();

$cls_func_str = 'tax_register'; //The name of the function in the class that will do the job
$myorderby = 'GROUP BY dealer_id';
//$groupby='dealer_id';// The orderby clause for fetching of the data
...
if (isset($_POST['filter']) && $_POST['filter'] == 'Filter') {
    if (valid_token($_POST['hf'])) { // checking if post value is same as timestamp stored in session during form load
        ...
        if ($checkpass) {
            ...
            if (!empty($_POST['end']) && !empty($_POST['end'])) {
    ...
    $rs = $myobj->$funcname($filter = "$myfilter'$ajaxshowid'", $records = '', $orderby = '');
     ...
     $filter = "ch_dealer_id = '{$_SESSION[SESS.'data']['dealer_id']}'";
      //$myorderby
    $rs = $myobj->$funcname($filter, $records = '', $orderby = "GROUP BY challan_order.ch_dealer_id");
}
...
<div class="row">
    ...
    color:#000;
                    ...
                    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                        <thead>                            
                            <tr>   
                                <th class="sno" rowspan="2">S.No</th>
<!--                                <th rowspan="2">Distributor Name</th>-->
                                <th colspan="4">SALES ACCOUNT</th>
                                <th colspan="4">PURCHASE ACCOUNT</th>                               
                            </tr>

                            <tr valign="top" class="search1tr">
                                <th colspan="1">VAT(0.00%)</th>
                                <th colspan="1">SALE(0.00%)</th>
                                <th colspan="1">VAT(5.00%)</th>
                                <th colspan="1">SALE(5.00%)</th>
                                <th colspan="1">VAT(0.00%)</th>
                                <th colspan="1">PURCHASE(0.00%)</th>
                                <th colspan="1">VAT(5.00%)</th>
                                <th colspan="1">PURCHASE(5.00%)</th>
                            </tr>                         
                        </thead>
                        ...
                        <tbody>
                         <?php 
                           ...
                           foreach ($rs as $key => $value) {
                             $value['tax'] == 5;
                             $five_amt = $value['five_sale'];
                             // h1($five_amt);
                             $vat_amt_five = ($five_amt * (5 / 100));
                             //h1($vat_amt_five);
                             if ($zero_sale = $value['zero_sale']);
                                 
                             else {
                                 $zero_sale = '0.00';
                             }

                             if (!empty($value['primary_zero_sale']))
                              $primary_zero_sale =  $value['primary_zero_sale'];
                             else {
                                 $primary_zero_sale = '0.00';
                             }
                             if ($primary_five_sale = $value['primary_five_sale']);
                              
                             else {
                                 $primary_five_sale = '0.00';
                             }

                            $primary_vat_amt_five = ($primary_five_sale * (5 / 100));
                             ?>
                        <tr>                               
                            <td>
                                <?=$inc?>
...
<!--                            <td>
                            ...
                            <td>
                             ...
                             <td>
                                <?=$value['primary_zero_vat'];?>
                            </td>
                            <td>
                                <?=$primary_zero_sale;?>
                            </td>
                            <td>
                                <?=$primary_vat_amt_five;?>
                            </td>
                            <td>
                                <?=$primary_five_sale?>
                            </td>
