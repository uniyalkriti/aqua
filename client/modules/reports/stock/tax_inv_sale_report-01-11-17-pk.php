...
<?php
$forma = 'TAX INVOICE SALE'; // to indicate what type of form this is
$formaction = $p;
$myobj = new dealer_sale();
$cls_func_str = 'tax_inv'; //The name of the function in the class that will do the job
...
<!--                                         <td>Batch No.</td>-->
    ...
    if (isset($heid)) {
        ...
        foreach ($_POST['challan_item'] as $key => $value) { //pre($value); 
            ...
            $q = ' SELECT cp.id, cp.name from catalog_product cp INNER JOIN user_primary_sales_order_details upsd ON cp.id = upsd.product_id INNER JOIN user_primary_sales_order ups USING(order_id) where cp.company_id ="'.$_POST[company_id].'" AND dealer_id = '.$_SESSION[SESS.'data']['dealer_id'].'';
                    ...
                    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                        <thead>                            
                            <tr>       
                                ...
                                <th class="sno" rowspan="3">S.No</th>
                                <th rowspan="2">Invoice No.</th>
                                <th rowspan="2">Invoice Date</th>
                                <th rowspan="2">Retailer Name</th>
                                <th rowspan="2">TIN No.</th>
...
//                             $printlink = '<span class="seperator"></span> <a class="iframef" target="_blank" title="Edit Invoice ' . $uidname . '" href="index.php?option=' . $formaction . '&showmode=1&mode=1&id=' . $uid . '&actiontype=print"><img src="./images/b_edit.png"></a>';
                ...
                $total = 0;
                $amt='';
                $challan_item = array_filter($rows['challan_item']);
                if(!empty($challan_item)){
                foreach ($rows['challan_item'] as $inkey => $invalue) {
                   ...
                   $cd_amt = $invalue['total_cd_amt'];
                   //$amt = $invalue['challan_val']; 
                   $amt = $invalue['taxable_amt']; 
                    $vat_amt = $invalue['vat_amt']; 
                    //$vat_amt = (($amt - $cd_amt) * $invalue['tax']) / 100; 
                      $total += ($amt - $cd_amt) + $vat_amt;
                    ...
                    ?>
                         ...
                         ?>
                                ...
                                <?php
                                 ...
                                 $tot_amt=0;
                 $challan_item1 = array_filter($rows['challan_item1']);
                if(!empty($challan_item1)){
                foreach ($rows['challan_item1'] as $inkey => $invalue) {
                     //for tax 5%
                    $amt1='';
                    $cd_amt1 = $invalue['total_cd_amt'];
                   $amt1 = $invalue['challan_val'];
                   //$amt1= $invalue['taxable_amt'];
                   $tot_amt+=$amt1;
                   $vat_amt1= $invalue['vat_amt'];
                   $amt2=$amt1+$vat_amt1;
                                    
                    //$vat_amt1 = (($amt1 - $cd_amt1) * $invalue['tax']) / 100;
                                
                                ?>
                                <td style="border:none;"><?=my2digit($invalue['tax']/2)?></td>
                                <td style="border:none;"><?=my2digit($amt2/2)?></td>
                                <td style="border:none;"><?=my2digit($invalue['tax']/2)?></td>
                                <td style="border:none;"><?=my2digit($amt2/2)?></td>
                                <?php
                                // $total += ($amt1 - $cd_amt1) + $vat_amt1;
                                $total=$amt+$amt2;
                }
                }else{       
                                ?>
                                ...
                                <td style="border:none;"><?=0.00?></td>
                <?php } ?>
                               <td><img src="../icon-system/i16X16/rupee.png"><?=my2digit($total)?></td> 
