...
if (isset($_POST['Update']) && $_POST['Update'] == 'Update') { 
    ...
    $batch_no = $_POST['bno'];// rate
    $mfgd = $_POST['mfgd'];// rate
    ...
    $c_s_mrp = $_POST['current_stock_mrp'];// mrp from stock table
    ...
    foreach($pid as $i=>$val)
    {
        ...
        {
            $stock_q = "SELECT product_id FROM `stock` WHERE `product_id`='$pid[$i]' AND `dealer_id`='$dealer_id' AND `batch_no`='$batch_no[$i]' AND `mfg`='$mfgd[$i]'";
            ...
            if($stock>0)
            {
              $mrp_chunk = ($c_s_mrp[$i]>0) ? "AND `mrp`='$cmrp[$i]'" : '';

              $up_stock="UPDATE `stock` SET `batch_no`='$batch_no[$i]',`mfg`='$mfgd[$i]',`rate`='$r_rate[$i]',`dealer_rate`='$rate[$i]',`mrp`='$mrp[$i]',`qty`='$bstock[$i]',`nonsalable_damage`='$nstock[$i]',`date`=NOW() WHERE `product_id`='$pid[$i]' AND `dealer_id`='$dealer_id' $mrp_chunk";
              ...
              die;*/
               ...
               $up_stock="INSERT INTO `stock` (`product_id`,`batch_no` `rate`, `dealer_rate`, `mrp`, `csa_id`, `dealer_id`, `qty`, `salable_damage`, `nonsalable_damage`, `remaining`, `mfg`, `expire`, `date`, `pr_rate`, `company_id`, `action`) VALUES ('$pid[$i]','$batch_no[$i]','$r_rate[$i]','$rate[$i]','$mrp[$i]','$csa_id','$dealer_id','$bstock[$i]','0','$nss','0','$mfgd[$i]','$expdate',NOW(),'0','$company_id','1')";
            }
          ...
          $nss = ($nstock[$i])?$nstock[$i]:0;
          $qry="INSERT INTO `stock` (`product_id`,`batch_no`,`rate`,`dealer_rate`, `mrp`,`csa_id`, `dealer_id`, `qty`, `salable_damage`, `nonsalable_damage`, `remaining`, `mfg`, `expire`, `date`, `pr_rate`, `company_id`, `action`) VALUES ('$pid[$i]','$batch_no[$i]',$r_rate[$i]','$rate[$i]','$mrp[$i]','$csa_id','$dealer_id','$bstock[$i]','0','$nss','0','$mfgd[$i]','$expdate',NOW(),'0','$company_id','1')";
...
if($condition['edit_stock']==1)
	                 echo'Balance Stock List &nbsp;&nbsp;&nbsp;&nbsp; <input type="submit" name="Update" value="Update" class="btn btn-success"/>';
	                ?>
                ...
                <div>
                    ...
                    <div class="row">
                     ...
                     <div class="col-md-1" style="background-color:#AD0606; color:#fff;">
                      ...
                      <b><?='₹ '.array_sum($value_sale['4'])?></b>
                    </div>
                    <div class="col-md-1" style="background-color:#F67C0F; color:#000;">
                     
                     <!-- For sprinkler -->
                     <b><?='₹ '.array_sum($value_sale['2'])?></b>
                   </div>

                   <div class="col-md-2" style="background-color:#46154D; color:#fff;">
                    
                     <!-- For Straight Premium -->
                     <b><?='₹ '.array_sum($value_sale['1'])?></b>
                   </div>
                   <div class="col-md-2" style="background-color:#0A87B2; color:#fff;">
                    
                    <!-- For Specialities Spices -->
                    <b><?='₹ '.array_sum($value_sale['3'])?></b>
                  </div>

                  <div class="col-md-1" style="background-color:#D2DC13; color:#000;">

                   <!-- For Hing -->
                   <b><?='₹ '.array_sum($value_sale['5'])?></b>
                 </div>
                 <div class="col-md-1" style="background-color:#4D59F0; color:#fff;">        

                   <!-- For Whole -->
                   <b><?='₹ '.array_sum($value_sale['6'])?></b>
                 </div>
                   <!-- <div class="col-md-1" style="background-color:#4D051D; color:#fff;">
                      ...
                      <thead>
                        <tr style="background-color:#AD0606; color:#fff;">
                         <th style="text-align: center; background-color:#000; color:#fff;" colspan="3"><strong>Product Details</th>
                           <th style="text-align: center; background-color:#000; color:#fff;" colspan="7"><strong> Stock Details</th>
                           </tr>
                           <tr style="background-color:<?=$color?>; color:<?=$font?>">
                            ...
                            <th style="background-color:#307ECC; color:#fff;">Stock Value
                            <input type="hidden" id="stateid" value="<?php echo $state_id?>">
                            </th>
                          </tr>
                        </thead>
                          ...
                          foreach ($myresult as $key => $rows)
                          {
                           ...
                           if($rows['mrp'])
                           {
                              ...
                              $d_rate = $rows['dealer_rate'];
                                 ...
                                 /* If states IN ( 'Rajasthan', 'Himanchal Pradesh', 'Bihar', 'Chhattisgarh', 'Madhya Pradesh', 'Chandigarh' ) */
                                  // $d_rate = new_dealer_rate($r_rate,$rows['division']);
                                 $d_rate = $rows['default_dealer_rate'];
                           } 
                           ...
                           <td>
                            <strong><?=$rows['product_name']."[".$rows['cp_short_name']."]"?><?php  //echo' <span style="float:right;"><image src="images/3D packs/'.$rows['pid'].'.png" height="42" width="42"/></span>'; ?>
                            </td>
                           <td><b><?=($rows['cat'])?></b></td>
                            <td>
                                ...
                                <input type="hidden" name="gst" value="<?php echo $rows['product_gst']?>">
                                <input type="hidden" name="current_stock_mrp[]" value="<?php echo $rows['mrp']?>">
                                <input type="hidden" class="division" value="<?php echo $rows['division']?>">
                            </td>
                           <td><strong><input type="text" onkeypress="javascript:return isNumber(event)" class="rate" name="rate[]" value="<?=number_format($d_rate,2)?>"/ readonly=""></td>
                            <td><input type="text" name="bno[]" value="<?php echo $rows['batch_no'] ?>"></td>
                            <td><input type="text" name="mfgd[]" value="<?php echo $rows['mfg']?>"></td>
                            ...
                            <td class="stk_val"><?='₹ ' .my2digit(($rows['qty'])* $d_rate) ?></td>
                              </tr>
...
<script type="text/javascript">
  ...
  $('#dynamic-table').on('keyup','.mrp',function(){
    var mrp = $(this).val();
    ...
    var stateids = [120150112064905,120160523054911,120161129120907,120170403073142,120170403073414,120170403074238];
    var stateid  = parseInt($('#stateid').val());
    var division = parseInt($(this).closest('tr').find('.division').val());
    ...
    $.ajax({
        ...
        success:function(data){
          ...
          /* If states IN ( 'Rajasthan', 'Himanchal Pradesh', 'Bihar', 'Chhattisgarh', 'Madhya Pradesh', 'Chandigarh' ) */
          if(jQuery.inArray(stateid, stateids) !== -1)
          {
            var d_rate = rate-(rate*9/100);
          }else{
  ...
  $('#dynamic-table').on('keyup','.balance_stock',function(){
...
// $('.datepicker').datepicker({ dateFormat: 'M-YY' }).val();
