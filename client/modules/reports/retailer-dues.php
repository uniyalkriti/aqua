...
$forma = 'Retailers Dues'; // to indicate what type of form this is
$formaction = $p;
$myobj = new report();
$cls_func_str = 'retailer_dues'; //The name of the function in the class that will do the job
...
if ($checkpass) {
    ...
    $filter[] = "co.remaining > '0'";
            
            $filterused = implode($filterstr, '<span style="margin: 0 10px;">|</span>');		
            $myresult = $myobj->$funcname($filter, $records = '', $orderby = "GROUP BY co.ch_retailer_id",$ch_filter); // $myobj->get_item_category_list()
            //pre($myresult);
            if (empty($myresult))
    ...
    $filter[] = "co.remaining > '0'";
  //  $month=date('Ym');
   // $filter[] = "DATE_FORMAT(`ch_date`,'Ym') <= '$month'";
    $myresult = $myobj->$funcname($filter, $records = '', $orderby = 'GROUP BY co.ch_retailer_id ',$ch_filter);
}
...
<div class="row">
    ...
    color:#000;
                    ...
                    <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                        <thead>
                        <h3><strong>Retailer Dues Age Analysis-<?=$dname?></strong></h3>
                         ...
                         <tbody>
                            ...
                            $rt+=round($rows['sale_value']);
                            $r30+=round($rows['lt30']);
                            $r60+=round($rows['lt60']);
                            $r90+=round($rows['lt90']);
                            $rg90+=round($rows['lt90plus']);
                            ?>
                                <tr>
                                <td>
                                    <?=$inc?>
                                ...
                                <td>
                                   <?=strtoupper($rows['retailer_name'])?>
                                </td>
                                <td><?=my2digit(round($rows['sale_value'])) ?></td>
                                <td><?=my2digit(round($rows['lt30'])) ?></td>
                                <td><?=my2digit(round($rows['lt60'])) ?></td>
                                <td><?=my2digit(round($rows['lt90'])) ?></td>
                                <td><?=my2digit(round($rows['lt90plus'])) ?></td>
                            </tr>
                        ...
                        <td><b><?=my2digit($rt)?></b></td>
                        <td><b><?=my2digit($r30)?></b></td>
                        <td><b><?=my2digit($r60)?></b></td>
                        <td><b><?=my2digit($r90)?></b></td>
                        <td><b><?=my2digit($rg90)?></b></td>
                        </tr>
                    </table>
                </div>
            </div>
