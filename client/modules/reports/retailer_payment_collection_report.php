...
$formaction = $p;
$myobj = new report();
$cls_func_str = 'retailer_payment'; //The name of the function in the class that will do the job
...
$rs = array();
$filterused = '';
$funcname = 'get_retailer_payment_list';
...
if ($checkpass) {
    ...
    if(!empty($_POST['from_date'])){
        ...
        $filter[] = "DATE_FORMAT(`pay_date_time`,'".MYSQL_DATE_SEARCH."') >= '$start'";
        $filterstr[] = '<b>Start : </b>'.$_POST['from_date'];
      }
      if(!empty($_POST['to_date'])){
        $end = get_mysql_date($_POST['to_date'],'/',$time = false, $mysqlsearch = true);
        $filter[] = "DATE_FORMAT(`pay_date_time`,'".MYSQL_DATE_SEARCH."') <= '$end'";
        $filterstr[] = '<b>End : </b>'.$_POST['to_date'];
                                 
      }
                  if (!empty($_POST['retailer_id'])) {
                $filter[] = "retailer_id = '".$_POST['retailer_id']."'";
                //$filterstr[] = '<b>Retailer : </b>' . $_POST['ch_no'];
            }
     $filter[] = "pc.dealer_id = '{$_SESSION[SESS.'data']['dealer_id']}'";
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <div id="divToPrint">
                    ...
                    ?>
                    <h3><strong>Retailer Payment Collection Report-<?=$dname?> From <?=$start1?> To <?=$end1?></strong></h3>
                        <thead>
                    ...
                    <td><?=$rows['rdate']?></td>
                    <td style="text-align: right;"><?=my2digit($rows['amount'])?></td>
                            </tr> 
                            <?php $inc++; } ?>                    
                         </tbody>
                         <tr>
