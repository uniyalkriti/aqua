...
$myobj = new location();
$cls_func_str = 'beat'; //The name of the function in the class that will do the job
$myorderby = "location_$mtype.id DESC"; // The orderby clause for fetching of the data
$myfilter = "location_$mtype.id="; //the main key against which we will fetch data in the get_item_category_function
...
$(function() {
    ...
    <div id="workarea">
     ...
     <form method="post" action="<?php if(isset($eformaction)) echo  $eformaction; ?>" class="iform" name="genform" onsubmit="return checkForm('genform');">
      <fieldset>
        <legend class="legend" style=""><?php echo $_SESSION[SESS.'constant']["location_title_$mtype"]; ?></legend>
       ...
       <?php
        ...
        for($i = 1; $i<=$loop; $i++)
        {
         ...
         <?php
            ...
            db_pulldown($dbc, "location_".$i."_id", "SELECT id, name FROM location_$i ", true, true, $js_attr);
       ?>
            </td>
             </tr>
       <?php 
    ...
    ?>
           <tr>
               <td><?php echo ucwords($_SESSION[SESS.'constant']["location_title_$mtype"]); ?><br><input type="text" lang="<?php echo ucwords($_SESSION[SESS.'constant']["location_title_$mtype"]); ?>" onChange="this.value = ucwords(trim(this.value));" name="name<?php echo $mtype; ?>" value="<?php if(isset($_POST[$catname])) echo $_POST[$catname]; ?>">
               </td>
         </tr>
         <tr>
       ...
       <table width="100%" border="0" cellspacing="2" cellpadding="2">
         <tr id="mysearchfilter">
           <td>
             ...
             <table>
               <tr>
<!--		<td><?php echo ucwords($_SESSION[SESS.'constant']["location_title_$mtype"]); ?><br />
                    <input type="text" class="location"  name="<?php echo $catname; ?>" id="name" value="<?php if(isset($_POST[$catname])) echo $_POST[$catname];?>"/> 
                </td>-->
               
<!--                <td>
                  <input id="mysave" type="submit" name="filter" value="Filter" class="hidden" />
    ...
    color:#000;
                    ...
                    <tr class="search1tr">
                      ...
                      {
                      ?>
                       <th ><?php echo ucwords($_SESSION[SESS.'constant']["location_title_$i"]); ?></th>
                      <?php
                      }
                      ?>
...
//                        {
//                        
//                            echo'<td><strong>'.$rows["l$k._name"].'</strong></td>';
//                        }
