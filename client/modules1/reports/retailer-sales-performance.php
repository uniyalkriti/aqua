...
$forma = 'Retailers Sales Performance '; // to indicate what type of form this is
$formaction = $p;
$myobj = new report();
$cls_func_str = 'retailer_sales_performance'; //The name of the function in the class that will do the job
...
if ($checkpass) {
    ...
    if(!empty($_POST['from_date'])){
     ...
     $filter[] = "uso.dealer_id = '{$_SESSION[SESS.'data']['dealer_id']}'";
            
            $filterused = implode($filterstr, '<span style="margin: 0 10px;">|</span>');		
            $myresult = $myobj->$funcname($filter, $records = '', $orderby = "GROUP BY uso.retailer_id ASC",$ch_filter); // $myobj->get_item_category_list()
            //pre($myresult);
            if (empty($myresult))
    ...
    $myresult = $myobj->$funcname($filter, $records = '', $orderby = 'GROUP BY uso.retailer_id ASC',$ch_filter);
}
...
<!--                                <td class="center">
                                ...
                                <td><?=($rows['total_sku'])?></td>
                                
                                
                                <td><?=$rupee.' ' .my2digit($rows['sale_value']) ?></td>
                            </tr>
