...
if (isset($_POST['Update']) && $_POST['Update'] == 'Update') { 
    ...
    for($i=1;$i<$size;$i++){
        ...
        $index2="rate".$i; 
        $rate[$i]=$_POST[$index2];
        $index3="purchase_stock".$i;        
        $pstock[$i]=$_POST[$index3];
        $index4="sold_qty".$i;        
        $sqty[$i]=$_POST[$index4];
        $index5="balance_stock".$i;        
        $bstock[$i]=$_POST[$index5];
        $indexn="non_salable_stock".$i;        
        $nstock[$i]=$_POST[$indexn];
        $dealer_id=$_SESSION[SESS . 'data']['dealer_id'];
        $user_id=$_SESSION[SESS . 'data']['id'];
        $csa_id=$_SESSION[SESS . 'data']['csa_id'];
        $company_id=$_SESSION[SESS . 'data']['company_id'];
 ...
 if($stock>0)
 {
   $up_stock="UPDATE `stock` SET `rate`='$rate[$i]',`qty`='$bstock[$i]',`nonsalable_damage`='$nstock[$i]',`date`=NOW() WHERE `product_id`='$pid[$i]' AND `dealer_id`='$dealer_id'";
 ...
 }else
 {
    ...
    $nstock = ($nstock[$i])?$nstock[$i]:0;
    


    $up_stock="INSERT INTO `stock` (`product_id`, `rate`, `csa_id`, `dealer_id`, `qty`, `salable_damage`, `nonsalable_damage`, `remaining`, `mfg`, `expire`, `date`, `pr_rate`, `company_id`, `action`) VALUES ('$pid[$i]','$rate[$i]','$csa_id','$dealer_id','$bstock[$i]','0','$nstock','0','$mfgdate','$expdate',NOW(),'0','$company_id','1')";
 }
    $run_up_stock = mysqli_query($dbc, $up_stock); 
                      ...
                      <thead>
                        ...
                        <tbody>
                          ...
                          foreach ($myresult as $key => $rows)
                          {
                           ...
                           <td><b><?=($rows['cat'])?></b></td>
                           <td><strong><input type="text" onkeypress="javascript:return isNumber(event)" class="rate" name="rate[]"  value="<?=$rows['dealer_rate']?>"/></td>
                            <td>
                                <strong><input type="text" onkeypress="javascript:return isNumber(event)" name="mrp[]" value="<?=$rows['mrp']?>" class="mrp"/>
                                <input type="hidden" class="pid" value="<?php echo $rows['pid']?>">
                                <input type="hidden" name="current_mrp[]" value="<?php echo $rows['mrp']?>">
                                <input type="hidden" name="dealer_rate[]" value="<?php echo $rows['dealer_rate']?>">
                                <input type="hidden" name="retailer_rate[]" class="retailer_rate" value="<?php echo $rows['rate']?>">
                            </td>
                            <td><strong><input type="text" onkeypress="javascript:return isNumber(event)" name="balance_stock[]" value="<?=($rows['qty'])?>"/></td>
                            ...
                            <td><?='₹ ' .my2digit(($rows['qty'])* $rows['dealer_rate']) ?></td>
                              </tr>
