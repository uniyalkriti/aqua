...
<?php 
$forma = 'Claim On sale'; // to indicate what type of form this is
$formaction = $p;
//$myobj = new dispatch();
...
//                          $where = 'id = '.$value['ch_retailer_id'];
                      ...
                      <td><input type="checkbox" name="chk[]" value="'.$value['id'].'"></td>
                        <td>'.$inc.'</td>
                        <td><strong>'.$value['amt'].'</strong></td>';
                      echo'<input type="hidden" value="'.$value[amt].'" name="amt">';
                       
                       $qq = "SELECT value FROM `scheme_value_product_details` WHERE value <='$value[amt]' order by value DESC LIMIT 1";
                     // h1($qq);
                       $rq = mysqli_query($dbc,$qq);
                       $rss = mysqli_fetch_assoc($rq);
                       $val = $rss['value'];
                        echo'<td> <select name="gift">';
                        $qq1 = "SELECT scheme_gift,id FROM `scheme_value_product_details` WHERE value ='$val' ";
                     // h1($qq);
                       $rq1 = mysqli_query($dbc,$qq1);
                      while($rss1 = mysqli_fetch_assoc($rq1))
                      {
                          echo'<option value="'.$rss1['scheme_gift'].'">'.$rss1['scheme_gift'].'</option>';
                      }
                        echo'</td> </select>';
