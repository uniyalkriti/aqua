...
db_pulldownsmall($dbc, 'retailer_id', $qrt, true, true, 'id="retailer" lang="Retailer" onchange="getdata(this.value, \'progress_div\', \'get-retailer-location\', \'location_id\');"','=Please Select=',$_GET['retailer_id']);
?>
            ...
            <table width="100%" id="mytable">
                ...
                <tr class="tdata">
                   ...
                   /* $q = ' SELECT cp.id, cp.name from catalog_product cp INNER JOIN user_primary_sales_order_details upsd ON cp.id = upsd.product_id INNER JOIN user_primary_sales_order ups USING(order_id) where cp.company_id =1 AND dealer_id = '.$_SESSION[SESS.'data']['dealer_id'].' ORDER BY name ASC';
                    ...
                    db_pulldownsmall($dbc , 'product_id[]', 'SELECT cp.id, cp.name from catalog_product cp INNER JOIN stock s ON cp.id = s.product_id where s.dealer_id = '.$_SESSION[SESS.'data']['dealer_id'].' AND s.qty>0  ORDER BY cp.name ASC',TRUE,TRUE,' id="product_id"  class="item_details"');
                    ?>
                    </td>
                   <!--  custum_function(this.id,this.value,event) -->
                    ...
                    <td width="10%">
                        <input style="width:80px" type="text" name="rate[]" class="rate" id="rate" onblur="damage_product_calculate();" value="<?=$_POST['challan_item'][$keys[$z-1]]['product_rate']?>"  />
                    </td>
...
//                    db_pulldown($dbc, 'replace_product_id[]', $q, TRUE, TRUE, 'id="product_id'.$z.'" onchange=getajaxdata(\'get_product_mrp_replace\',\'mytable\',event) ','',$_POST['challan_item'][$keys[$z-1]]['product_id']);
                ...
                $q = "SELECT cp.id, cp.name from catalog_product cp INNER JOIN stock s ON cp.id = s.product_id where s.dealer_id = '".$_SESSION[SESS.'data']['dealer_id']."' ORDER BY cp.name ASC";
                    ...
                    db_pulldownsmall($dbc, 'replace_product_id[]', $q, TRUE, TRUE, 'id="product_id'.$z.'" onchange="custum_function(this.id,this.value,event)" ','=Please Select=',$_POST['challan_item'][$keys[$z-1]]['product_id']);
                    ...
                    <td  width="10%">
                        <input style="width:80px" type="text" name="replace_rate[]" id="replace__rate<?=$z?>" onblur="damage_product_calculate_replace();" value="<?=$_POST['challan_item'][$keys[$z-1]]['product_rate']?>"  />
                    </td>
