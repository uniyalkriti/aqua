...
<?php 
$forma = 'Todays Scheme'; // to indicate what type of form this is
$formaction = $p;
$myobj = new scheme();

$cls_func_str = 'dealer_scheme'; //The name of the function in the class that will do the job
$myorderby = 'scheme_name ASC'; // The orderby clause for fetching of the data
$myfilter = 'dealer_id='; //the main key against which we will fetch data in the get_item_category_function
...
//$tt = $myobj->get_unique_itemname($filter='',  $records = '', $orderby='');
############################# code for checking of submitted form data starts here data starts here ########################
function checkform($mode='add', $id='')
{
	global $dbc;
	if($mode == 'filter') return array(TRUE, '');
	$field_arry = array('utname' => $_POST['utname']);// checking for  duplicate Unit Name
	if($mode == 'add'){
		if(uniqcheck_msg($dbc,$field_arry,'item_unit', false, ""))
			return array(FALSE, '<b>Item Unit</b> already exists, please provide a different value.');
	}elseif($mode == 'edit'){
		if(uniqcheck_msg($dbc,$field_arry,'item_unit', false," utId != '$_GET[id]'"))
			return array(FALSE, '<b>Item Unit</b> already exists, please provide a different value.');
	}
	return array(TRUE, '');
}
...
if(isset($_POST['filter']) && $_POST['filter'] == 'Filter')
{
	if(valid_token($_POST['hf'])) // checking if post value is same as timestamp stored in session during form load
	{
		...
		if($checkpass)
		{
			...
			if(!empty($_POST['utname'])){
				$filter[] = "utname LIKE '%$_POST[utname]%'";
				$filterstr[] = '<b>Item Unit  : </b>'.$_POST['utname'];
			}
	...
	$rs = $myobj->$funcname($filter="$myfilter'$ajaxshowid'",  $records = '', $orderby='');
           ...
           $rs = $myobj->$funcname($filter="dealer_id = '$dealerId'",  $records = '', $orderby='');
}
//pre($rs);
...
$(function() {
	$("#utname").autocomplete({
		source: "./modules/ajax-autocomplete/item/ajax-itemunit.php"
	});
});
</script>
    <div id="workarea">
     ...
     <form method="post" action="<?php if(isset($eformaction)) echo  $eformaction; ?>" class="iform" name="genform" onsubmit="return checkForm_alert('genform');">
      <fieldset>
        ...
        <table width="100%" border="0" cellspacing="2" cellpadding="2" class="tableform">
         <tr>
           <td><span class="star">*</span>Item Unit<br />
             <input type="text" id="utname" name="utname"  value="<?php if(isset($_POST['utname'])) echo $_POST['utname']; ?>" lang="Item Unit" />
            </td> 
         </tr> 
         <tr>
    ...
    ?>
          ...
          <tr>
            <td>            
			  ...
			  $lastinc = (($inc+PGDISPLAY-1) > $totrecords) ? $totrecords : ($inc+PGDISPLAY-1);
			  ?>	 
              
              <div class="searchlistdiv" id="searchlistdiv"> 
                <div><b><?php echo $forma; ?> : <span id="totCounter"><?php echo $totrecords; ?></span></b>
                     ...
                     echo'
                      <tr BGCOLOR="'.$bg.'" id="tr'.$uid.'" class="ihighlight">
                        <td class="myintrow myresultrow">'.$inc.'</td>
                         <td><strong>'.$rows['name'].'</strong><div style="display:none" id="delDiv'.$uid.'"></div></td>
                         <td>'.$rows['buy_quantity'].'</td>
                         <td>'.$rows['scheme_quantity'].'</td>
			 <td>'.($rows['scheme_quantity']*100)/$rows['buy_quantity'].'</td>
