...
if(isset($_POST['filter']) && $_POST['filter'] == 'Filter')
{
	if(valid_token($_POST['hf'])) // checking if post value is same as timestamp stored in session during form load
	{
		...
		if($checkpass)
		{
			...
			$retailer_data = $myobj->get_user_wise_retailer_data($sesId , $role_id);
                        if(!empty($retailer_data) && is_array($retailer_data)) {
                            $retailer_id_str = implode(',', $retailer_data);
                            $filter[] = "id IN ($retailer_id_str)";
                        }
	...
	$rs = $myobj->$funcname($filter="$myfilter'$ajaxshowid'",  $records = '', $orderby='');
        ...
        if(!empty($retailer_data) && is_array($retailer_data)) {
            $retailer_id_str = implode(',', $retailer_data);
            $rs = $myobj->$funcname($filter="id IN ($retailer_id_str) AND company_id = '{$_SESSION[SESS.'data']['company_id']}'",  $records = '', $orderby=" ORDER BY $myorderby");
        }
     ...
     <form method="post" action="<?php if(isset($eformaction)) echo  $eformaction; ?>" class="iform" name="genform" onsubmit="return checkForm('genform');" enctype="multipart/form-data">
      <fieldset>
        ...
        <table width="100%" border="0" cellspacing="2" cellpadding="2" class="tableform">
         <tr>
             <td><span class="star">*</span>Dealer Name <br>
             <input type="hidden" name="dealer_id" value="<?php if(isset($dealer_id)) echo $dealer_id; ?>">
             <input type="text" class="read" readonly="readonly" value="<?php if(isset($dealername[$dealer_id]['name'])) echo $dealername[$dealer_id]['name']; ?>">
              </td>
              <td><span class="star">*</span>Retailer Name <br>
               <input style="width: 220px;" onChange="this.value = ucwords(trim(this.value));" type="text"  lang="Name" name="name" value="<?php if(isset($_POST['name'])) echo $_POST['name']; ?>">
              </td>
              <td>
          ...
          <tr>
            ...
            <td>
              Pin No<Br/>
              <input type="text" name="pin_no" value="<?php if(isset($_POST['pin_no'])) echo $_POST['pin_no']; ?>" maxlength="10" />
             </td>
             <td>
              Tin No<Br/>
            ...
            <?php db_pulldown($dbc, "outlet_type_id", "SELECT id as outlet_type_id, outlet_type FROM _retailer_outlet_type", true,true,'id="outlet_type_id" lang="out"', $_POST['outlet_type_id']); ?>
             </td>                  
           </tr>
           <tr>
           ...
           <tr>
            ...
            <tr>
           <?php
        $loop = $_SESSION[SESS.'constant']['retailer_level'];
        for($i = 1; $i<=$loop; $i++)
        {
            $j = $i+1; //here we getnextplid
            ?>
            
             <td><?php echo ucwords($_SESSION[SESS.'constant']["location_title_$i"]); ?><br>
         <?php
            $js_attr = 'lang="subcat'.$i.'" onchange="fetch_location(this.value+\'|\'+\''.$i.'\'+\'|\'+\''.$j.'\', \'progress_div\', \'location_'.$j.'\', \'location-subcategory\');" id="location_'.$i.'"';
            if($i == $loop) $js_attr = 'lang="locatiion" id="location_'.$i.'"';
            if(isset($heid))
            {
            ...
            db_pulldown($dbc, "location_".$i."_id", "SELECT id, name FROM location_$i WHERE company_id = '{$_SESSION[SESS.'data']['company_id']}'", true, true, $js_attr);
         ...
         <tr id="mysearchfilter">
	      ...
	      <fieldset>
             ...
             <table>
               <tr>
		<td>Name<br />
                <input type="text" class="retailer" name="name" id="name" value="<?php if(isset($_POST['name'])) echo $_POST['name'];?>" /> 
                </td>
                 <td>
    ...
    ?>
	      ...
	      ########################## pagination details fetch starts here ###################################
		     ...
		     $deletelink = '<span class="seperator">|</span> <a href="javascript:void(0);" onclick="do_delete(\'Retailer Delete\', \''.$uid.'\',\'Retailer\',\''.addslashes($uidname).'\');"><img src="./images/b_drop.png"></a>';
                      $path = '../myuploads/retailer/create/'.$rows['image_name'];
                     ...
                     if(!empty($lat_lng)) 
                          $location = '<a class="iframe" href="indexpop.php?option=retailer-location&mcc='.$rows['mccmnclaccellid'].'&lat_lng='.$lat_lng.'"><img width="18px;" height="18px;" src="./images/green.png"></a>';
		      else if(!empty($mcc))  $location = '<a class="iframe" href="indexpop.php?option=retailer-location&mcc='.$rows['mccmnclaccellid'].'&lat_lng='.$lat_lng.'"><img width="18px;" height="18px;" src="./images/red.png"></a>';
		      ...
		      if($auth['del_opt'] !=1) $deletelink = '';
                     echo'
                      <tr BGCOLOR="'.$bg.'" id="tr'.$uid.'" class="ihighlight">
                        ...
                        <td>'.$image.'</td>
                        <td><strong>'.$uidname.'</strong>'.$person_dealer_icon.'<div style="display:none" id="delDiv'.$uid.'"></div></td>						
                        <td>'.$rows['email'].'</td>
                        <td>'.$rows['landline'].'</td>
                        <td>'.$rows['address'].'</td>
                        <td>'.$location.'</td>
