    ...
    <div class="main-content-inner">
        ...
        <div class="breadcrumbs ace-save-state" id="breadcrumbs">
            <ul class="breadcrumb">
                ...
                <li>
                    <a href="{{url('user')}}">{{Lang::get('common.primary_order')}}</a>
                </li>
                <li class="active">{{Lang::get('common.primary_order')}} {{Lang::get('common.dashboard')}}</li>
            </ul><!-- /.breadcrumb -->
        </div>
        @else
        <div class="breadcrumbs ace-save-state" id="breadcrumbs">
                    ...
                    <table id="dynamic-table" class="table table-bordered table-hover">
                        <thead>
                            <tr>
                                ...
                                <th rowspan="2">{{Lang::get('common.order_id')}}</th>
                                <th rowspan="2">{{Lang::get('common.distributor')}} Name</th>
                                <th rowspan="2">{{Lang::get('common.distributor')}} Town</th>
                                {{--<th>Created Date</th>--}}
                                <th rowspan="2">{{Lang::get('common.username')}}</th>
                                <th rowspan="2">Sale {{Lang::get('common.date')}}</th>
                                <th rowspan="2">Receive {{Lang::get('common.date')}}</th>
                                {{--<th>Date Time</th>--}}
                                <th colspan="6">{{Lang::get('common.details')}}</th>
                                {{--<th>Company Id</th>--}}
                            </tr>
                            <tr>
                        ...
                        <tbody>
                           <?php $inc=0; ?>
                            @if(!empty($sale_data))
                                @foreach($sale_data as $k=>$order_data)
                                    <?php 
                                        $encid = Crypt::encryptString($order_data->user_id);
                                        $dencid = Crypt::encryptString($order_data->dealer_id);
                                    ?>
                                    <tr>
                                        <td>{{$k+1}}</td>
                                        <td>{{$order_data->order_id}}</td>
                                        <td><a href="{{url('distributor/'.$dencid)}}">{{$order_data->dealer_name}}</a></td>
                                        <td>{{$order_data->l4_name}}</td>
                                        <td><a href="{{url('user/'.$encid)}}">{{$order_data->user_name}}</a></td>
                                        <td>{{!empty($order_data->sale_date)?date('d-M-Y',strtotime($order_data->sale_date)):'NA'}}</td>
                                        <td>{{!empty($order_data->receive_date)?date('d-M-Y',strtotime($order_data->receive_date)):'NA'}}</td>
