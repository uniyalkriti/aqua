                ...
                <div class="row">
                    <div class="col-xs-12">
                        <form class="form-horizontal open collapse in" action="" method="get" id="user-search" role="form"
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    ...
                                    <div class="col-lg-2">  
                                        <label class="control-label no-padding-right" for="name">{{Lang::get('common.locationFourTown')}}</label>
                                        <select multiple  name="location4_filter[]" id="location_4" class="form-control chosen-select">
                                            <option value="">Select</option>
                                            @if(!empty($location4_filter_array))
                                                @foreach($location4_filter_array as $rk=>$rr)
                                                    <?php if(empty($_GET['location4_filter']))
                                                        $_GET['location4_filter']=array();
                                                    ?>
                                                    <option @if(in_array($rk,$_GET['location4_filter'])){{"selected"}} @endif   value="{{$rk}}"> {{$rr}} 
                                                    </option>                                                        
                                                @endforeach 
                                            @endif
                                        </select>
                        ...
                        <div class="table-header center">
                            {{Lang::get('common.locationFourTown')}}
                            <a href="{{url('locationFourTown/create')}}" class="btn btn-sm btn-success pull-right"><i
                            class="fa fa-plus mg-r-10"></i> Add {{Lang::get('common.locationFourTown')}}</a>
                        </div>
                        ...
                        <div>    
                            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                                ...
                                <tr>
                                    ...
                                    <th>{{Lang::get('common.locationFourTown')}} ID</th>
                                    <th>{{Lang::get('common.locationFourTown')}} Name</th>
                                ...
                                <tbody>
                                @foreach($location4 as $key=>$location4_data)
                                    <?php $encid = Crypt::encryptString($location4_data->id);?>
                                    <tr>
                                        <td class="center">
                                            {{ $key+1 }}
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location4_data->id))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location4_data->name))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location4_data->location_3_id))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location4_data->location_3_name))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location4_data->location_2_id))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location4_data->location_2_name))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location4_data->location_1_id))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location4_data->location_1_name))}}</a>
                                        </td>
                                        <td>{{date('d-M-Y',strtotime($location4_data->created_at))}}</td>

                                        <td class="hidden-480">
                                            @if($location4_data->status==1)
                                                <span class="label label-sm label-success">Active</span>
                                            @elseif($location4_data->status==2)
                                            ...
                                            <div class="hidden-sm hidden-xs btn-group">
                                                @if($location4_data->status==1)
                                                    <button title="Inactive" class="btn btn-xs btn-warning"
                                                            onclick="confirmAction('{{Lang::get('common.locationFourTown')}}','Location4','{{$location4_data->id}}','location_4','inactive');">
                                                        <i class="ace-icon fa fa-ban bigger-120"></i>
                                                    </button>
                                                @else
                                                    <button title="Active" class="btn btn-xs btn-success"
                                                            onclick="confirmAction('{{Lang::get('common.locationFourTown')}}','Location4','{{$location4_data->id}}','location_4','active');">
                                                        <i class="ace-icon fa fa-check bigger-120"></i>
                                                    </button>
                                                @endif

                                                <a title="Edit" class="btn btn-xs btn-info"
                                                   href="{{url('locationFourTown/'.$encid.'/edit')}}">
                                                    <i class="ace-icon fa fa-pencil bigger-120"></i>
                                                </a>

                                                <button title="Delete" class="btn btn-xs btn-danger"
                                                        onclick="confirmAction('{{Lang::get('common.locationFourTown')}}','Location4','{{$location4_data->id}}','location_4','delete');">
