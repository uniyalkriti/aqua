...
<table id="simple-table" class="table table-bordered" >
    <tr><td colspan="{{9+$total_days*2}}"><h3>{{Lang::get('common.ss_monthly')}}</h3></td></tr>
    ...
    @if(!empty($records))
        ...
        <?php // $encid = Crypt::encryptString($data["user_id"]); ?>
                ...
                <tr  class="">
                    ...
                    <td style="background-color:#f5d0a9">{{!empty($ss_district[$data->csa_id])?$ss_district[$data->csa_id]:'N/A'}}</td>
                    <td style="background-color:#f5d0a9">{{!empty($user_name[$data->csa_id])?$user_name[$data->csa_id]:'N/A'}}</td>
                    @if(!empty($datesArr))
                            @foreach($datesArr as $keydd=>$datadd)
                              ...
                              <td style="background-color:#f5d0a9">{{!empty($date_wise_achievement[$data->csa_id.$datadd])?$date_wise_achievement[$data->csa_id.$datadd]:'0'}}</td>
                                                        
                            @endforeach
                    @endif

                    <?php
                    $mtd_target = !empty($mtd_target_amount[$data->csa_id])?$mtd_target_amount[$data->csa_id]:'0';
                    $mtd_achievement = !empty($mtd_achievement_amount[$data->csa_id])?$mtd_achievement_amount[$data->csa_id]:'0';
