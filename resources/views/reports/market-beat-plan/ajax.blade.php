...
<table id="simple-table" class="table table-bordered" style="font-size: 13px;border: 1px black">
    <tr>
        <td colspan="19"><h3>Market Beat Plan</h3></td>
    </tr>
    <tr style="background-color: #7BB0FF !important;">
    <?php
     $dealer_id = Crypt::encryptString($dealer->did); 
    ?>
        <td colspan="7"><strong>DISTRIBUTOR NAME: </strong><a href="{{url('distributor/'.$dealer_id)}}"> {{$dealer->name}}</a></td>
    ...
    @if(!empty($dealer_target) && count($dealer_target)>0)
    <tr style="background-color: #7BB0FF !important;">
        <th colspan="3"> #</th>
        <th> APR (RV LACS)</th>
        <th> MAY (RV LACS)</th>
    ...
    @if(!empty($records))
        @foreach($records as $key=>$d)
            ...
            <tr>
                ...
                <td>{{$d['market']}}</td>
                <td>{{$d['t2']}}</td>
                <td>{{!empty($platinum[$location_id])?$platinum[$location_id]:'0'}}</td>
                <td>{{!empty($diamond[$location_id])?$diamond[$location_id]:'0'}}</td>
                <td>{{!empty($gold[$location_id])?$gold[$location_id]:'0'}}</td>
                <td>{{!empty($silver[$location_id])?$silver[$location_id]:'0'}}</td>
                <td>{{$d['sws']}}</td>
                <td>{{$d['ws']}}</td>
                <td>{{$d['t2']}}</td>
