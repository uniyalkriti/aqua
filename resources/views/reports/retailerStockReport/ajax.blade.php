...
<div class="table-header center">
  {{Lang::get('common.retailer_stock')}}
   ...
   <thead>
    ...
    @foreach($records as $k=> $r)
        <?php    
            ...
            $sencid = Crypt::encryptString($r->person_id_senior);
    ...
    @foreach($dsr[$r->order_id] as $k1=>$data1)
        <tr>
            ...
            <td>{{$r->l5_name}}</td>
            <td>{{$r->l6_name}}</td>
            <td>{{$emp_code[$r->user_id]}}</td>
            <td><a href="{{'user/'.$encid}}"> {{$r->user_name}}</a></td>
            <td>{{$r->role_name}}</td>
            <td>{{$r->mobile}}</td>
            <td><a href="{{'user/'.$sencid}}"> {{!empty($senior_name[$r->person_id_senior])?$senior_name[$r->person_id_senior]:''}}</a></td>
    ...
    <tfoot>
         <tr>
            <th colspan="17"><strong>{{Lang::get('common.grand')}} {{Lang::get('common.total')}}</strong></th>
            <th>{{$gqty}}</th>
