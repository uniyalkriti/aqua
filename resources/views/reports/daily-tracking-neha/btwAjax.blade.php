                        ...
                        @foreach($track_time[$data->user_id][$data->track_date] as $Tkey => $Tvalue)
                            ...
                            @foreach($Tvalue as $T1key => $T1value)
                                @if( $T1value->status == 'Attendance' || $T1value->status == 'CheckOut' || $T1value->track_time <='19:00:00' && $T1value->track_time >='09:00:00' )
                                    ...
                                    <td>
                                        <?php 
                                        echo '<b> Module : </b> '.$T1value->status.'<br><strong>Track Time: </strong>'.$T1value->track_time.'<br><strong>Track Address: </strong>'.$T1value->location.'<br> <strong>Battery Status : '.$T1value->battery_status.'%</strong><br><strong> Latitude,Longitutde : </strong>'.$T1value->lat_lng;
                                        ?>
                                    </td>
                                    <?php  
                                    // $test = array();
