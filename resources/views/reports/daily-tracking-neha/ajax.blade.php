...
<div class="table-header center">
   {{Lang::get('common.daily_tracking')}}
...
<thead>
    ...
    <tbody>
        <?php $h=1; $count=0; $track_count=9; $test = [];
             ...
             <?php
              ...
              ?>
                <tr>
                    ...
                    <td>{{!empty($monthlyTourProgram[$data->track_date.$data->user_id])?$monthlyTourProgram[$data->track_date.$data->user_id]:''}}</td>
                    <td>{{$data->address}}</td>

                    <td>{{!empty($attWorkStatus[$data->track_date.$data->user_id])?$attWorkStatus[$data->track_date.$data->user_id]:''}}</td>
                    <td>{{!empty($attTime[$data->track_date.$data->user_id])?$attTime[$data->track_date.$data->user_id]:''}}</td>

                    <td>{{!empty($attLocation[$data->track_date.$data->user_id])?$attLocation[$data->track_date.$data->user_id]:''}}</td>
                    ...
                    @if(!empty($track_time[$data->user_id][$data->track_date]))
                        @foreach($track_time[$data->user_id][$data->track_date] as $Tkey => $Tvalue)
                            ...
                            @foreach($Tvalue as $T1key => $T1value)
                                @if( $T1value->track_time <='19:00:00' && $T1value->track_time >='10:00:00' )

                                <?php
                                $sim = similar_text($data->address, $T1value->location, $perc);
                                ?>
                                @if($perc < 80 || $data->address == 'NA')
                                    <td>
                                        <?php 
                    ...
                    ?>
                    <td>{{!empty($checkoutTime[$data->track_date.$data->user_id])?$checkoutTime[$data->track_date.$data->user_id]:''}}</td>

                    <td>{{!empty($checkoutLocation[$data->track_date.$data->user_id])?$checkoutLocation[$data->track_date.$data->user_id]:''}}</td>
