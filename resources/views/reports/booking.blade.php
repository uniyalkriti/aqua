        ...
        <div class="main-content-inner">
            ...
            <div class="breadcrumbs ace-save-state" id="breadcrumbs">
                <ul class="breadcrumb">
                    ...
                    <li class="active">{{Lang::get('common.order_booking')}} {{Lang::get('common.dashboard')}}</li>
                </ul><!-- /.breadcrumb -->
            </div>
            @else
            <div class="breadcrumbs ace-save-state" id="breadcrumbs">
                <ul class="breadcrumb">
                    ...
                    <li>
                        <a href="{{url('user')}}">{{Lang::get('common.order_booking')}}</a>
                    </li>
                    <li class="active">{{Lang::get('common.order_booking')}} {{Lang::get('common.dashboard')}}</li>
                </ul><!-- /.breadcrumb -->
            </div>
   ...
   ?>
    @if(!empty($sale_data))
        @foreach($sale_data as $k=>$data)
             ...
             // dd($data);
            //  $retailer_lat_lng = explode(',',$data->retialer_lat_lng);
            //  // dd($retailer_lat_lng);
            //  $order_lat_lng = explode(' ',$data->lat_lng);

            //  $retailer_lat = $retailer_lat_lng[0];
            //  $retailer_lng = $retailer_lat_lng[1];

            //  $order_lat = $order_lat_lng[0];
            //  $order_lng = $order_lat_lng[1];

            //  $replace = str_replace(' ', ',', $data->lat_lng);
            // $diffrence_between_order_booking=App\UserDetail::getDistanceBetweentwopoint($retailer_lat,$retailer_lng,$order_lat,$order_lng); 

             $retailer_lat_lng = explode(',',$data->retialer_lat_lng);
            $order_lat_lng = explode(' ',$data->lat_lng);
            if(empty($retailer_lat_lng))
            {
                $retailer_lat = 0.0;
                $retailer_lng = 0.0;
            }
            if(empty($order_lat_lng))
            {
                $order_lat = 0.0;
                $order_lng = 0.0;
            }

             $retailer_lat = !empty($retailer_lat_lng[0])?$retailer_lat_lng[0]:0;
             $retailer_lng = !empty($retailer_lat_lng[1])?$retailer_lat_lng[1]:0;
             // dd($retailer_lng);
             
             $order_lat = !empty($order_lat_lng[0])?$order_lat_lng[0]:0;
             $order_lng = !empty($order_lat_lng[1])?$order_lat_lng[1]:0;

             $replace = str_replace(' ', ',', $data->lat_lng);
            // $diffrence_between_order_booking=App\UserDetail::getDistanceBetweentwopoint($retailer_lat,$retailer_lng,$order_lat,$order_lng); 
             // if($replace !='0.0,0.0'  || $retailer_lat_lng !='0.0,0.0' ||  $replace !=' '  || $retailer_lat_lng !=' '  ||$replace !=''  || $retailer_lat_lng !='' )
             // {
             //    $diffrence_between_order_booking=App\UserDetail::calculate_distance_matrix($data->retialer_lat_lng,$replace); 

             // }
             // else
             // {
             //    $diffrence_between_order_booking = 0;
             // }
...
?> 
            ...
            <tr>
                ...
                <td>{{!empty($data->non_productive_reason)?$data->non_productive_reason:''}}</td>
                <td>{{!empty($data->total_sale_value_real)?$data->total_sale_value_real:'N/A'}}</td>
                @if($diffrence_between_order_booking <= 0.5)
                    <td>
                        <i class="menu-icon fa fa-map-marker"></i>


                        <a style="color: green;" href="{{url('user_tracking/'.$encid.'?date='.$data->date.'&order_id='.$data->order_id)}}">{{!empty($data->lat_lng)?$data->lat_lng:'N/A'}}</a>
                    </td>
                        @else
                    <td>
                        <i class="menu-icon fa fa-map-marker"></i>
                        ...
                        <a style="color: red;" href="{{url('user_tracking/'.$encid.'?date='.$data->date.'&order_id='.$data->order_id)}}">{{!empty($data->lat_lng)?$data->lat_lng:'N/A'}}</a>
                    </td>
                @endif
            ...
            <div class="modal-dialog" style="width:800px;">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h3 class="modal-title" align="center" >{{Lang::get('common.catalog_4')}} Wise {{Lang::get('common.details')}} </h3>
                    </div>
                      <?php $total=0;
                                $totalqty=0;
                                $totalweight=0; 
                    ...
                    <div class="modal-body"> 
                        <div class="row heading" style="border: 1px solid;">
                            <div class="col-md-5">{{Lang::get('common.catalog_4')}}</div>
                                

                            <div class="col-md-2">Quantity</div>
                            <div class="col-md-2">{{Lang::get('common.rate')}}</div>
                            <div class="col-md-2">{{Lang::get('common.secondary_sale')}}</div>
