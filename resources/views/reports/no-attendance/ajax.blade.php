...
<table id="dynamic-table" class="table table-bordered" style="font-size: 13px;border: 1px black">
    ...
    <tr class="info" style="color: black;">
        ...
        <?php  $null[] = 'null'; ?>
        <th>{{date('j F, Y',strtotime($value))}}</th>
        @endforeach
       
    </tr>
</thead>
    <tbody>
    <?php $count_no_attn = array(); ?>
    @if(!empty($records) && count($records)>0)
        @foreach($records as $key=>$data)
            <?php
             $encid = Crypt::encryptString($data->user_id);
             $sencid = Crypt::encryptString($data->person_id_senior1);
              ?>
                <tr  class="">
                    <td>{{$key+1}}</td>
                    ...
                    @foreach($datearray as $key => $value)
                        <td>
                            @if(!empty($data_no_booking[$value.$data->user_id]))
                                <a style="color: green;">YES <i class="fa fa-check" aria-hidden="true"></i></a>
                                @else
                                <?php $count_no_attn[$value][] = $data->user_id; ?>
                                <a style="color: red;">NO <i class="fa fa-close" aria-hidden="true"></i></a>
    ...
    <tfoot>
        <tr>
            <td colspan="9" style="background-color: #d9edf7;"><b>{{Lang::get('common.no_attendance')}} {{Lang::get('common.grand')}} {{Lang::get('common.total')}}</b></td>
            @foreach($datearray as $key => $value)
                <td style="color: red;">{{(!empty($count_no_attn[$value])?COUNT($count_no_attn[$value]):'')}}</td>
            @endforeach
        </tr>
    </tfoot>
</table>
