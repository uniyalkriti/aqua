...
@if(!empty($person))
        ...
        <button onclick="window.location.href='rds_monthly?{{$query_string}}'" href="javascript:void(0)" class="nav-link">
...
<table id="dynamic-table" class="table table-bordered" style="font-size: 13px;border: 1px black">
    ...
    <tr class="info" style="color: black;">
      ...
      @foreach($catalog_product as $ckey=>$cdata)
        ...
        <th>{{$cdata->name}} Productive Calls</th>
        <th>{{$cdata->name}}</th>
      @endforeach
      <th>Amount</th>
    </tr>
        ...
        @if(!empty($person) && count($person)>0)
            ...
            ?>
                <tr>
                    ...
                    <td>{{($productData[$data->user_id][$data->date]['market'])}}</td>
                    <td>{{$productData[$data->user_id][$data->date]['total_call']}}</td>
                    <td>{{$productData[$data->user_id][$data->date]['productive_call']}}</td>
                    <td>{{!empty($productData[$data->user_id][$data->date]['total_call'])?round(((($productData[$data->user_id][$data->date]['productive_call'])/($productData[$data->user_id][$data->date]['total_call']))*100)).'%':'N/A'}}
                    </td>
                       <?php
                      
                      ?>
                      @foreach($catalog_product as $ckey=>$cdata)
                      <?php //$product_Data = $data->dsrMonthly($cdata->id,$data->user_id,$data->date);
                      $total_qty[$cdata->id][]=!empty($dsr[$data->user_id.$cdata->id.$data->date])?$dsr[$data->user_id.$cdata->id.$data->date]/$cdata->quantiy_per_other_type:'0';
                       ?>
                      <td>{{!empty($prod_retailer_count[$data->user_id.$cdata->id.$data->date])?round($prod_retailer_count[$data->user_id.$cdata->id.$data->date],2):'0'}}</td>
                      <td>{{!empty($dsr[$data->user_id.$cdata->id.$data->date])?round($dsr[$data->user_id.$cdata->id.$data->date]/$cdata->quantiy_per_other_type,2):'0'}}</td>
                    @endforeach
                      <td>{{!empty($productData[$data->user_id][$data->date]['product_amount'])?round($productData[$data->user_id][$data->date]['product_amount']):'0'}}</td>
                      <?php $total_amt[]=!empty($productData[$data->user_id][$data->date]['product_amount'])?$productData[$data->user_id][$data->date]['product_amount']:'0';?>
                </tr>
           @endforeach
    ...
    <tfoot>
        <tr>
          <th colspan="16"> {{Lang::get('common.grand')}} {{Lang::get('common.total')}} </th>
