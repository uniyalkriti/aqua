...
@section('title') 
    <title>{{Lang::get('common.dsr_monthly')}}</title>
        ...
        <div class="main-content-inner">
            <div class="breadcrumbs ace-save-state" id="breadcrumbs">
                <ul class="breadcrumb">
                    ...
                    <li class="active">{{Lang::get('common.dsr_monthly')}}1</li>
                </ul><!-- /.breadcrumb -->
                            ...
                            @include('common_filter.filter_sale')
                                        <div class="col-xs-6 col-sm-6 col-lg-2">
                                            <div class="">
                                                <label class="control-label no-padding-right"
                                                       for="name">{{Lang::get('common.catalog_3')}}</label>
                                                <select multiple name="catalog_2[]" id="catalog_2" class="form-control chosen-select" >
                                                    <option disabled="disabled" value="">select</option>
                                                    @if(!empty($catalog_2))
                                                        @foreach($catalog_2 as $ck=>$cr)
                                                            <option value="{{$ck}}">{{$cr}}</option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                            </div>
                                      ...
                                      <div class="col-xs-6 col-sm-6 col-lg-2">
                                            <div class="">
                                                <label class="control-label no-padding-right"
                                                       for="name">Date</label>
                                                <input value="" autocomplete="off" type="text" placeholder="Date" name="from_date" id="from_date" class="form-control date-picker">
                                            </div>
                                        </div>
...
@section('js')
    ...
    <script src="{{asset('msell/page/report173.js')}}"></script>
     ...
     $(document).on('change', '#catalog_2', function () {
        ...
        if(val != '')
       {
        $.ajaxSetup({
            ...
            $.ajax({
                type: "GET",
                url: domain + '/get_catalog_product',
                dataType: 'json',
                data: "id=" + val,
                success: function (data) {
