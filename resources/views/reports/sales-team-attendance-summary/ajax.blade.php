...
{{Lang::get('common.sales-team-attendance')}}
...
<table id="dynamic-table" class="table table-bordered" >
    ...
    <tr class="info" style="color: black;">
        ...
        <th>{{Lang::get('common.total')}} No. Of Days</th>

        @foreach($working_status_header as $wkey=>$wdata)
                 <?php 
                    $null[] = 'null';
                ?>
        <th>{{Lang::get('common.total')}} {{$wdata}}</th>
        @endforeach
                ...
                <tr  class="">
                    ...
                    @foreach ($datearray as $dkey => $dvalue)
                        @if(isset($working_status[$data->user_id.$dvalue]))
                             <td> <font color="green"> {{$working_status[$data->user_id.$dvalue]}}  </font> <br> <font color="blue"> {{$att_time[$data->user_id.$dvalue]}}  </font> </td> 
                        @else
                             <td> <font color="red"> Not Punched </font> </td>  
                             <?php $absent[] = 1;  ?>   
                        @endif     
                    @endforeach
                    ...
                    @foreach ($working_status_header as $wskey => $wsvalue)
                        @if(isset($working_status_data[$data->user_id.$wskey]))
                             <td>{{$working_status_data[$data->user_id.$wskey]}} <br></td> 
                        @else
                             <td>0</td>     
                        @endif     
                    @endforeach

                    <td>{{array_sum($absent)}}</td>
                            ...
                            jQuery(function ($) {
                                ...
                                var myTable =
                                        ...
                                        .DataTable({
                                            ...
                                            "aoColumns": [
                                                {"bSortable": false},
                                                 null,null,null,null,null,null,null,null,null,null,<?= $implode ?>,
                                                {"bSortable": false}
                                            ],
                                            "aaSorting": [],
                                            "sScrollY": "300px",
