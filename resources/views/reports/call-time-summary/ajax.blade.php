...
{{Lang::get('common.call-time-summary-report')}}
...
<table id="dynamic-table" class="table table-bordered" >
    ...
    @if(!empty($records) && count($records)>0)
        ...
        $person_id = Crypt::encryptString($data->person_id_senior); ?>
                <tr  class="">
                    <td>{{$i+1}}</td>
                    <td>{{$data->state}}</td>
                    <td>{{$data->l4_name}}</td>
                    ...
                    <td>{{$data->rolename}}</td>
                    <td>{{$data->mobile}}</td>
                    <td><a href="{{url('user/'.$person_id)}}">{{!empty($senior_name_data[$data->person_id_senior])?$senior_name_data[$data->person_id_senior]:'N/A'}}</a></td>
                    @foreach ($datearray as $dkey => $dvalue)
                       <?php
                                if(!empty($first_call[$data->user_id.$dvalue])){
                                $f_time = '<font color="blue">'.$first_call[$data->user_id.$dvalue].'</font>';
                                }
                               else{
                                $f_time = '<font color="red">'."00:00:00".'</font>';
                               }

                               if(!empty($last_call[$data->user_id.$dvalue])){
                                $l_time = '<font color="green">'.$last_call[$data->user_id.$dvalue].'</font>';
                                }
                               else{
                                $l_time = '<font color="red">'."00:00:00".'</font>';
                               }
                       ?>

                       <?php
                       echo "<td>".$f_time." / ".$l_time."</td>";
                       ?>
