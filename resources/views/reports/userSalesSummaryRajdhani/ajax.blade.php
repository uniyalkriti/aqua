...
<table id="dynamic-table" class="table table-bordered" style="font-size: 13px;border: 1px black">
   <thead>
    <tr class="info" style="color: black;">
        ...
        <th>Working Days</th>
        <th>Avg. Calls/Day</th>
        <th>Total Sale Weight( SECONDARY)</th>
        <th>Total Sale Weight( PRIMARY)</th>
    ...
    @if(!empty($records) && count($records)>0)
        ...
        $avg_calls = $data['tc']/$data['working_day'];
        ...
        ?>
                <tr  class="">
                    ...
                    <td>0</td>
                    @else
                    <td>{{  round((($data['pc'])/($data['tc']))*100)  }}</td>
                    @endif
                   

                    <td>{{$data['working_day']}}</td>

                    <td>{{ROUND($avg_calls)}}</td>


                    <td>{{$data['secondary_weight']/1000}}</td>
                    <td>{{$data['primary_weight']}}</td>
                ...
                $tc+=$data['tc'];
                $pc+=$data['pc'];
                // $tq+=!empty($total_sale_qty[$data->user_id])?$total_sale_qty[$data->user_id]:0;
                // $tv+=!empty($total_sale_value[$data->user_id])?$total_sale_value[$data->user_id]:0;
                $wt+=$data['secondary_weight']/1000;
                $wtpr+=$data['primary_weight'];
