...
@section('title')
    <title>{{Lang::get('common.location5')}}</title>
                ...
                <div class="row">
                    <div class="col-xs-12">
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    ...
                                    <div class="col-lg-2">  
                                        ...
                                        <select  multiple name="location5_filter[]" id="location_5" class="form-control chosen-select">
                                            <option value="">Select</option>
                                            @if(!empty($location5_filter_array))
                                                @foreach($location5_filter_array as $rk=>$rr)
                                                    <?php if(empty($_GET['location5_filter']))
                                                        $_GET['location5_filter']=array();
                                                    ?>
                                                    <option @if(in_array($rk,$_GET['location5_filter'])){{"selected"}} @endif   value="{{$rk}}"> {{$rr}} 
                                                    </option>                                                       
                                                @endforeach 
                                            @endif
                                        </select>
                        ...
                        <div class="table-header center">
                            {{Lang::get('common.location5')}}
                                <p class="bs-component pull-right">
                                <span id="checkTrialLocation">
                                <a href="{{url('location5/create')}}" class="btn btn-sm btn-success pull-right ActionCreate"><i
                                class="fa fa-plus mg-r-10"></i> Add {{Lang::get('common.location5')}}</a>
                                </span>
                                </p>
                        </div>
                        ...
                        <div>   
                            <table id="dynamic-table" class="table  table-bordered table-hover">
                                ...
                                <tbody>
                                @foreach($location5 as $key=>$location5_data)
                                    <?php $encid = Crypt::encryptString($location5_data->id);?>
                                    <tr>
                                        <td class="center">
                                            {{ $key+1 }}
                                        </td>
                                        ...
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location5_data->name))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location5_data->location_4_name))}}</a>
                                        </td><td>
                                            <a href="#">{{ucwords(strtolower($location5_data->location_3_name))}}</a>
                                        </td><td>
                                            <a href="#">{{ucwords(strtolower($location5_data->location_2_name))}}</a>
                                        </td><td>
                                            <a href="#">{{ucwords(strtolower($location5_data->location_1_name))}}</a>
                                        </td>
                                        <td>{{date('d-M-Y',strtotime($location5_data->created_at))}}</td>

                                        <td class="hidden-480">
                                            @if($location5_data->status==1)
                                                <span class="label label-sm label-success">Active</span>
                                            @elseif($location5_data->status==2)
                                            ...
                                            <div class="hidden-sm hidden-xs btn-group">
                                                @if($location5_data->status==1)
                                                    <button title="Inactive" class="btn btn-xs btn-warning ActionData"
                                                            onclick="confirmAction('{{Lang::get('common.location5')}}','{{Lang::get('common.location5')}}','{{$location5_data->id}}','location_5','inactive');">
                                                        <i class="ace-icon fa fa-ban bigger-120"></i>
                                                    </button>
                                                @else
                                                    <button title="Active" class="btn btn-xs btn-success ActionData"
                                                            onclick="confirmAction('{{Lang::get('common.location5')}}','{{Lang::get('common.location5')}}','{{$location5_data->id}}','location_5','active');">
                                                        <i class="ace-icon fa fa-check bigger-120"></i>
                                                    </button>
                                                @endif

                                                <a title="Edit" class="btn btn-xs btn-info ActionData"
                                                   href="{{url('location5/'.$encid.'/edit')}}">
                                                    <i class="ace-icon fa fa-pencil bigger-120"></i>
                                                </a>

                                                <button title="Delete" class="btn btn-xs btn-danger ActionDelete"
                                                        onclick="confirmAction('{{Lang::get('common.location5')}}','{{Lang::get('common.location5')}}','{{$location5_data->id}}','location_5','delete');">
