        ...
        <div class="main-content-inner">
            <div class="breadcrumbs ace-save-state" id="breadcrumbs">
                <ul class="breadcrumb">
                    ...
                    <li class="active">@Lang('common.location1')</li>
                </ul><!-- /.breadcrumb -->
                ...
                <span id="checkTrialLocation">
                    ...
                    <a href="{{url('location1/create')}}" class="btn btn-sm btn-info ActionCreate"><i class="fa fa-plus mg-r-10"></i> Add @Lang('common.location1')</a>
                </span>
                </p>
                                ...
                                <div class="col-xs-4 form-group">
                                    <div class="input-group">
                                        @if(empty(Request::get('search')))
                                            <strong>{{Lang::get('common.search_by_name')}}</strong>
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                ...
                                <table id="dynamic-table" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                    @foreach($location1 as $key=>$location1_data)
                                        <?php $encid = Crypt::encryptString($location1_data->id);?>
                                        <tr>
                                            <td class="center">
                                                {{ $key+1 }}
                                            </td>
                                            <td>
                                                <a href="#">{{ucwords(strtolower($location1_data->name))}}</a>
                                            </td>
                                            <td>{{date('d-M-Y',strtotime($location1_data->created_at))}}</td>

                                            <td class="hidden-480">
                                                @if($location1_data->status==1)
                                                    <span class="label label-sm label-success">Active</span>
                                                @elseif($location1_data->status==2)
                                                ...
                                                <div class="hidden-sm hidden-xs btn-group">
                                                    @if($location1_data->status==1)
                                                        <button title="Inactive" class="btn btn-xs btn-warning ActionData"
                                                                onclick="confirmAction('{{Lang::get('common.location1')}}','Location1','{{$location1_data->id}}','location_1','inactive');">
                                                            <i class="ace-icon fa fa-ban bigger-120"></i>
                                                        </button>
                                                    @else
                                                        <button title="Active" class="btn btn-xs btn-success ActionData"
                                                                onclick="confirmAction('{{Lang::get('common.location1')}}','Location1','{{$location1_data->id}}','location_1','active');">
                                                            <i class="ace-icon fa fa-check bigger-120"></i>
                                                        </button>
                                                    @endif

                                                    <a title="Edit" class="btn btn-xs btn-info ActionData"
                                                       href="{{url('location1/'.$encid.'/edit')}}">
                                                        <i class="ace-icon fa fa-pencil bigger-120"></i>
                                                    </a>

                                                    <button title="Delete" class="btn btn-xs btn-danger ActionDelete"
                                                            onclick="confirmAction('{{Lang::get('common.location1')}}','Location1','{{$location1_data->id}}','location_1','delete');">
                           ...
                           {{--     <div class="col-xs-6">
                                    <div class="dataTables_info">
                                        Showing {{($location1->currentpage()-1)*$location1->perpage()+1}}
                                        to {{(($location1->currentpage()-1)*$location1->perpage())+$location1->count()}}
                                        of {{$location1->total()}} entries
                                    </div>
                                </div>
                                <div class="col-xs-6">
                                    <div class="dataTables_paginate paging_simple_numbers">
                                        {{--Larvel default pagination with custom view --}}
                                        {{-- $location1->appends(request()->except('page')) --}}
                                    </div>
                                </div>
                            </div>
                        </div>
