...
@section('title')
    <title>{{Lang::get('common.editappmodule')}} - {{config('app.name')}}</title>
                    ...
                    <div class="col-xs-12">
                        ...
                        <div>   
                            <table id="dynamic-table" class="table  table-bordered table-hover">
                                ...
                                <tbody>
                                @foreach($editappmodule as $key=>$editappmodule_data)
                                    <?php $encid = Crypt::encryptString($editappmodule_data->id);?>
                                    <tr>
                                        <td class="center">
                                            {{ $key+1 }}
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($editappmodule_data->cname))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{($editappmodule_data->base_url)}}</a>
                                        </td><td>
                                            <a href="#">{{($editappmodule_data->title_name)}}</a>
                                        </td>

                                        <td> <a href="#">{{date('d-M-Y',strtotime($editappmodule_data->created_at))}}</a></td>

                                        <td class="hidden-480">
                                            @if($editappmodule_data->status==1)
                                                <span class="label label-sm label-success">Active</span>
                                            @elseif($editappmodule_data->status==2)
                                            ...
                                            <div class="hidden-sm hidden-xs btn-group">
                                                @if($editappmodule_data->status==1)
                                                   <!--  <button class="btn btn-xs btn-warning"
                                                            onclick="confirmAction('{{Lang::get('common.editappmodule')}}','{{Lang::get('common.editappmodule')}}','{{$editappmodule_data->id}}','interface_url','inactive');">
                                                        <i class="ace-icon fa fa-ban bigger-120"></i>
                                                    </button> -->
                                                @else
                                                   <!--  <button class="btn btn-xs btn-success"
                                                            onclick="confirmAction('{{Lang::get('common.editappmodule')}}','{{Lang::get('common.editappmodule')}}','{{$editappmodule_data->id}}','interface_url','active');">
                                                        <i class="ace-icon fa fa-check bigger-120"></i>
                                                    </button> -->
                                                @endif

                                                <a class="btn btn-xs btn-info"
                                                   href="{{url('editAppModule/'.$encid.'/edit')}}">
                                                    <i class="ace-icon fa fa-pencil bigger-120"></i>
                                                </a>

                                               <!--  <button class="btn btn-xs btn-danger"
                                                        onclick="confirmAction('{{Lang::get('common.editappmodule')}}','{{Lang::get('common.editappmodule')}}','{{$editappmodule_data->id}}','interface_url','delete');">
