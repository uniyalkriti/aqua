...
@section('title')
    <title>{{Lang::get('common.location3')}}</title>
                    ...
                    <div class="col-xs-12">
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    ...
                                    <div class="col-lg-2">
                                        ...
                                        <select  multiple name="location3_filter[]" id="location_3" class="form-control chosen-select">
                                            <option value="">Select</option>
                                            @if(!empty($location3_filter_array))
                                                @foreach($location3_filter_array as $sk=>$sr) 
                                                    <?php if(empty($_GET['location3_filter']))
                                                        $_GET['location3_filter']=array();
                                                    ?>
                                                  
                                                    <option @if(in_array($sk,$_GET['location3_filter'])){{"selected"}}  @endif value="{{$sk}}" > {{$sr}} 
                                                    </option>
                                                @endforeach 
                                            @endif
                                        </select>
                            ...
                            {{Lang::get('common.location3')}}
                                <p class="bs-component pull-right">
                                 <span id="checkTrialLocation">
                                    <a href="{{url('location3/create')}}" class="btn btn-sm btn-success pull-right ActionCreate"><i
                                    class="fa fa-plus mg-r-10"></i> Add {{Lang::get('common.location3')}}</a>
                                    </span>
                                </p>
                        </div>
                        ...
                        <div>
                            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                                ...
                                <tbody>
                                @foreach($location3 as $key=>$location3_data)
                                    <?php $encid = Crypt::encryptString($location3_data->id);?>
                                    <tr>
                                        <td class="center">
                                            {{ $key+1 }}
                                        </td>
                                        ...
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location3_data->id))}}</a>
                                        </td>
                                          <td>
                                            <a href="#">{{ucwords(strtolower($location3_data->name))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location3_data->location_2_id))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location3_data->location_2_name))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location3_data->location_1_id))}}</a>
                                        </td>
                                        <td>
                                            <a href="#">{{ucwords(strtolower($location3_data->location_1_name))}}</a>
                                        </td>
                                        <td>{{date('d-M-Y',strtotime($location3_data->created_at))}}</td>

                                        <td class="hidden-480">
                                            @if($location3_data->status==1)
                                                <span class="label label-sm label-success">Active</span>
                                            @elseif($location3_data->status==2)
                                            ...
                                            <div class="hidden-sm hidden-xs btn-group">
                                                @if($location3_data->status==1)
                                                    <button title="Inactive" class="btn btn-xs btn-warning ActionData"
                                                            onclick="confirmAction('{{Lang::get('common.location3')}}','{{Lang::get('common.location3')}}','{{$location3_data->id}}','location_3','inactive');">
                                                        <i class="ace-icon fa fa-ban bigger-120"></i>
                                                    </button>
                                                @else
                                                    <button title="Active" class="btn btn-xs btn-success ActionData"
                                                            onclick="confirmAction('{{Lang::get('common.location3')}}','{{Lang::get('common.location3')}}','{{$location3_data->id}}','location_3','active');">
                                                        <i class="ace-icon fa fa-check bigger-120"></i>
                                                    </button>
                                                @endif

                                                <a title="Edit" class="btn btn-xs btn-info ActionData"
                                                   href="{{url('location3/'.$encid.'/edit')}}">
                                                    <i class="ace-icon fa fa-pencil bigger-120"></i>
                                                </a>

                                                <button title="Delete" class="btn btn-xs btn-danger ActionDelete"
                                                        onclick="confirmAction('{{Lang::get('common.location3')}}','{{Lang::get('common.location3')}}','{{$location3_data->id}}','location_3','delete');">
