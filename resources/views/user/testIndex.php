                ...
                <form class="form-horizontal open collapse in" action="" method="get" id="user-search" role="form"
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <div class="col-lg-2 ">
                                <label class="control-label no-padding-right" for="name">Search Anything</label>
                            ...
                            <div class="col-lg-2">
                                ...
                                <select name="state[]" multiple id="state" class="form-control chosen-select">
                                    <option value="">Select</option>
                                    @if(!empty($state))
                            ...
                            <div class="col-lg-2">  
                                ...
                                <select name="head_quater[]" multiple id="head_quater" class="form-control chosen-select">
                                    <option value="">Select</option>
                            ...
                            <div class="col-lg-2">  
                                ...
                                <select name="location_6[]" multiple id="location_6" class="form-control chosen-select">
                                    <option value="">Select</option>
                                    @if(!empty($location_6))
                                ...
                                <table id="dynamic-table" class="table table-bordered table-hover">
                                    ...
                                    @foreach($records as $key=>$data)
                                        ...
                                        <tr class="search_from_this">
                                            ...
                                            <td>{{!empty($data->imei_number)?$data->imei_number:'N/A'}}</td>
                                            ...
                                            <td>
                                                ...
                                                <!-- for clear imei  -->
                                                 <button title="Clear Imei" class="btn btn-xs btn-danger" onclick="confirmAction('Clear IMEI','IMEI','{{$data->id}}','{{$status_table}}','clear');">
                                                        <i class="ace-icon fa fa-mobile-phone bigger-120"></i>
                                                    </button>
                                                    <!-- for active and inactive -->
                                                    @if($data->person_status==1)
                                                        <button title="Inactive" class="btn btn-xs btn-warning" onclick="confirmAction('{{Lang::get('common.'.$current_menu)}}','{{Lang::get('common.'.$current_menu)}}','{{$data->id}}','{{$status_table}}','inactive');">
                                                            <i class="ace-icon fa fa-ban bigger-120"></i>
                                                        </button>
                                                    @else
                                                        <button title="Active" class="btn btn-xs btn-success" onclick="confirmAction('{{Lang::get('common.'.$current_menu)}}','{{Lang::get('common.'.$current_menu)}}','{{$data->id}}','{{$status_table}}','active');">
                                                    ...
                                                    <a title="Delete" user_id="{{ $data->id }}" role_id="{{ $data->role_id }}" data-toggle="modal"  data-target="#immediate_junior_list" class="immediate_junior_list">
                                                        <button class="btn btn-xs btn-danger">
                                                            <i class="ace-icon fa fa-trash-o bigger-120"></i>
                                                        </button>
                                                    </a>

                                                     <!-- for attendance with new process -->
                                                    ...
                                                    <div class="row">
                                                        <div class="col-xs-12 col-md-2">
                                                            <div class="text-center">
                                                                <img height="150" class="thumbnail inline no-margin-bottom" alt="Domain Owner's Avatar" src="{{asset('users-profile/')}}{{'/'.$data->person_image}}" alt=" " />
                                                                <br />
                                                                <div class="width-80 label label-info label-xlg arrowed-in arrowed-in-right">
                                                                    <div class="inline position-relative">
                                                                        <a class="user-title-label" href="#">
                                                                            ...
                                                                            &nbsp;
                                                                            <span class="white">{{$data->first_name.' '.$data->middle_name.' '.$data->last_name}}</span>
                                                                        </a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        ...
                                                        <div class="col-xs-12 col-md-10">
                                                            ...
                                                            <table class="table table-bordered table-detail table-responsive table-hover table-stipped">
                                                                ...
                                                                <tbody>
                                                                    ...
                                                                    @foreach($data->details as $key1=>$data1)
                                                                    <?php 
                                                                        $encid = Crypt::encryptString($data1->dealer_id);
                                                                        

                                                                    ?>
                                                                    <tr>
                                                                    <td>{{$key1+1}}</td>
                                                                    <td><a href="{{url('distributor/'.$encid)}}"> {{$data1->dealer_name}}</a></td>
                                                                    <td>{{$data1->beat_count}}</td>
                                                                    <td>{{$data1->retailer_count}}</td>
                                                                    <td>
                                                                        <button class="btn btn-xs btn-danger"
                                                                                onclick="delete_dealer('{{$data1->dealer_id}}','dealer','delete','{{$data1->user_id}}');">
                                                                            <i class="ace-icon fa fa-trash-o bigger-120"></i>
                                                                        </button>
                                                                    </td>

                                                                    @php 
                                                                    $total_beat +=$data1->beat_count;
                                                                    $total_retailer +=$data1->retailer_count;
                                                                    @endphp 
                                                                    </tr>
                                                                    @endforeach
                                                                    <tr>
...
<div class="modal fade" id="call_from_jquery" role="dialog">
    <div class="modal-dialog" style="width:800px;">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h3 class="modal-title" ><u>Junior User List</u></h3>
                <br>
