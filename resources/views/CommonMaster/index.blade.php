                ...
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <div class="table-header">
                            {{Lang::get('common.'.$table_name)}}
                            <a href="{{url($table_name.'/create')}}" class="btn btn-sm btn-success pull-right"><i
                            class="fa fa-plus mg-r-10"></i> Add {{Lang::get('common.'.$table_name)}}</a>
                        </div>
                                    ...
                                    @foreach($working_type as $key=>$working_type_data)
                                        ...
                                        <tr>
                                                ...
                                                <div class="hidden-sm hidden-xs btn-group">
                                                    @if($working_type_data->status==1)
                                                        <button class="btn btn-xs btn-warning"
                                                                onclick="confirmAction('{{Lang::get('common.'.$table_name)}}','{{Lang::get('common.'.$table_name)}}','{{$working_type_data->id}}','{{$table_name}}','inactive');">
                                                            <i class="ace-icon fa fa-ban bigger-120"></i>
                                                        </button>
                                                    @else
                                                        <button class="btn btn-xs btn-success"
                                                                onclick="confirmAction('{{Lang::get('common.'.$table_name)}}','{{Lang::get('common.'.$table_name)}}','{{$working_type_data->id}}','{{$table_name}}','active');">
                                                            <i class="ace-icon fa fa-check bigger-120"></i>
                                                        </button>
                                                    @endif

                                                    {!! Form::open(array('route'=>[$table_name.'.edit',$encid],'method' => 'get')) !!}
                                                        <input type="hidden" name="table_name" value="{{$table_name}}">
                                                        <button type="submit" class="btn  btn-xs btn-info"> <i class="ace-icon fa fa-pencil xtra-small-120"></i></button>
                                                        
                                                    {!! Form::close() !!}
                                                    
                                                    <button class="btn btn-xs btn-danger"
                                                            onclick="confirmAction('{{Lang::get('common.'.$table_name)}}','{{Lang::get('common.'.$table_name)}}','{{$working_type_data->id}}','{{$table_name}}','delete');">
                                            ...
                                            @if($table_name == '_retailer_filter_master_details')
                                                ...
                                                <td>{{!empty($pluck_value[$working_type_data->retailer_filer_id])?$pluck_value[$working_type_data->retailer_filer_id]:''}}</td>
                                            @endif
                                        </tr>
