...
<div class="main-container ace-save-state" id="main-container">
    <div class="main-content">
        <div class="main-content-inner">
            ...
            <form id="month_form" method="get" enctype="multipart/form-data" enctype="multipart/form-data">
                {{-- <form id="month_form" method="post" enctype="multipart/form-data" enctype="multipart/form-data">
                    {!! csrf_field() !!} --}}
                    <div class="col-md-2">
                        <label>{{Lang::get('common.catalog_1')}}</label>
                           <select multiple  name="catalog_0[]" id="catalog_0" class="form-control  chosen-select">
                                <option value="">Select</option>
                                @if(!empty($catalog_0))
                                    @foreach($catalog_0 as $l1_key=>$l1_data)
                                        <option {{in_array($l1_key,$catalog_0_array)?'selected':''}} value="{{$l1_key}}">{{$l1_data}}</option>
                                    @endforeach
                                @endif
                            </select>
                    </div>
                    <div class="col-md-2">
                        <label>{{Lang::get('common.catalog_2')}}</label>
                           <select multiple  name="catalog_1[]" id="catalog_1" class="form-control  chosen-select">
                                <option value="">Select</option>
                                @if(!empty($catalog_1))
                                    @foreach($catalog_1 as $l1_key=>$l1_data)
                                        <option {{in_array($l1_key,$catalog_1_array)?'selected':''}} value="{{$l1_key}}">{{$l1_data}}</option>
                                    @endforeach
                                @endif
                            </select>
                    </div>
                        ...
                        <div>
                            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                                <thead>
                                    ...
                                    <th>{{Lang::get('common.catalog_3')}} ID</th>
                                    <th>{{Lang::get('common.catalog_3')}} Name</th>
                                    <th>{{Lang::get('common.catalog_3')}} Sequence</th>
                                    <th>{{Lang::get('common.catalog_2')}} ID</th>
...
@section('js')
    ...
    <script>
        $(document).on('change', '#catalog_0', function () {
            ...
            catalog_data(_current_val,1);
        });

        $(document).on('change', '#catalog_1', function () {
            _current_val = $(this).val();
            catalog_data(_current_val,2);
        });

        $(document).on('change', '#catalog_2', function () {
            _current_val = $(this).val();
            catalog_data(_current_val,3);
        });
   

        function catalog_data(val,level) {
            ...
            if (val != '') {
                ...
                $.ajax({
                    type: "POST",
                    url: domain + '/getCatalogForStandaradFilter',
                    dataType: 'json',
