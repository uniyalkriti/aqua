...
$(function () {
    ...
    $('#daily-attendance').validate({
        ...
        submitHandler: function (form) {
            $.ajax({
                type: "GET",
                url: domain + '/distributor-performance',
                // dataType: 'json',
                data: $('form').serialize(),
    ...
    function get_receipt(val) {
        ...
        if (val != '') {
            ...
            $.ajax({
                type: "POST",
                url: domain + '/get_receipt',
                dataType: 'json',
                data: "id=" + val+"&date="+_date,
                success: function (data) {
                    // console.log(data);
                    if (data.code == 401) {
                        //  $('#loading-image').hide();
                        ...
                        $.each(data.result, function (key4, value4) {
                            if (value4.name != '') {
                                template4 += '<option value="' + key4 + '" >' + stripslashes(value4) + '</option>';
                            }
                        });
                        _receipt.empty();
                        _receipt.append(template4).trigger("chosen:updated");
                    }
